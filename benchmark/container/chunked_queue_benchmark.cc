...
// turbo::chunked_queue against another plausable implementation.
template <typename T>
...
template <class T>
using Chunked = turbo::chunked_queue<T>;
template <class T>
using ExpChunked = turbo::chunked_queue<T, 2, 64>;
...
void BM_push_pop(benchmark::State& state) {
  ...
  for (auto s : state) {
    ...
    for (int j = 0; j < num_elements; j++) {
      if (q.empty() || turbo::Bernoulli(rnd, 0.5)) {
