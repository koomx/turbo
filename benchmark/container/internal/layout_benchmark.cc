...
void BM_OffsetConstantStatic(benchmark::State& state) {
  ...
  TURBO_RAW_CHECK(L::Partial().template Offset<3>() == Offset, "Invalid offset");
  for (auto _ : state) {
...
void BM_OffsetConstantIndirect(benchmark::State& state) {
  ...
  TURBO_RAW_CHECK(p.template Offset<3>() == Offset, "Invalid offset");
  for (auto _ : state) {
    DoNotOptimize(p);
...
void BM_OffsetPartialHeadroom(benchmark::State& state) {
  size_t k = 7;
  TURBO_RAW_CHECK(PartialOffset<Ts...>(k) == Offset, "Invalid offset");
  for (auto _ : state) {
    DoNotOptimize(k);
...
void BM_OffsetPartialStatic(benchmark::State& state) {
  ...
  size_t k = 7;
  TURBO_RAW_CHECK(L::Partial(k).template Offset<3>() == Offset,
                 "Invalid offset");
  for (auto _ : state) {
    DoNotOptimize(k);
...
void BM_OffsetPartial(benchmark::State& state) {
  using L = Layout<Ts...>;
  size_t k = 7;
  TURBO_RAW_CHECK(L::Partial(3, 5, k).template Offset<3>() == Offset,
                 "Invalid offset");
  for (auto _ : state) {
    DoNotOptimize(k);
...
void BM_AllocSizeHeadroom(benchmark::State& state) {
  size_t x = 9;
  TURBO_RAW_CHECK(AllocSize<Ts...>(x) == Size, "Invalid size");
  for (auto _ : state) {
    DoNotOptimize(x);
...
void BM_AllocSizeStatic(benchmark::State& state) {
  using L = typename Layout<Ts...>::template WithStaticSizes<3, 5, 7>;
  size_t x = 9;
  TURBO_RAW_CHECK(L(x).AllocSize() == Size, "Invalid offset");
  for (auto _ : state) {
    DoNotOptimize(x);
...
void BM_AllocSize(benchmark::State& state) {
  ...
  size_t k = 7;
  size_t x = 9;
  TURBO_RAW_CHECK(L(n, m, k, x).AllocSize() == Size, "Invalid offset");
  for (auto _ : state) {
    DoNotOptimize(n);
    DoNotOptimize(m);
    DoNotOptimize(k);
...
void BM_AllocSizeIndirect(benchmark::State& state) {
  using L = Layout<Ts...>;
  auto l = L(3, 5, 7, 9);
  TURBO_RAW_CHECK(l.AllocSize() == Size, "Invalid offset");
  for (auto _ : state) {
    DoNotOptimize(l);
