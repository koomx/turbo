...
void BM_CacheInSteadyState(benchmark::State& state) {
  ...
  state.SetLabel(turbo::str_sprintf("load_factor=%.2f", t.load_factor()));
}
...
void BM_EraseIf(benchmark::State& state) {
  ...
  for (auto& k : keys) {
    ...
    for (int64_t i = 0; i < num_elements; i++) {
      ...
      k.push_back(
          turbo::Uniform<int64_t>(rng, 0, std::numeric_limits<int64_t>::max()));
  ...
  while (state.KeepRunningBatch(static_cast<int64_t>(kRepetitions) *
    ...
    for (size_t t_id = 0; t_id < kRepetitions; t_id++) {
      ...
      turbo::container_internal::EraseIf(pred, &table);
    }
    state.PauseTiming();
    for (size_t t_id = 0; t_id < kRepetitions; t_id++) {
      auto& k = keys[t_id];
