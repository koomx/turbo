  ...
  static std::string Make(uint32_t v) {
    switch (size) {
      ...
      case StringSize::kMedium:  // < 32 bytes
        return turbo::str_sprintf(kStringFormat, "/path", v);
      case StringSize::kLarge:  // 33-64 bytes
        return turbo::str_sprintf(kStringFormat, "/path/to/file", v);
      case StringSize::kExtraLarge:  // > 64 bytes
        return turbo::str_sprintf(kStringFormat,
    ...
    case OutputStyle::kBenchmark: {
      ...
      for (const auto& result : results) {
        auto print = [&](std::string_view stat, double Ratios::*val) {
          ...
          turbo::str_printf("    %s{\n", comma);
          turbo::str_printf("      \"cpu_time\": %f,\n", reported_value);
          turbo::str_printf("      \"real_time\": %f,\n", reported_value);
          turbo::str_printf("      \"iterations\": 1,\n");
          turbo::str_printf("      \"name\": \"%s\",\n", name);
          turbo::str_printf("      \"time_unit\": \"ns\"\n");
          turbo::str_printf("    }\n");
          comma = ",";
        };
