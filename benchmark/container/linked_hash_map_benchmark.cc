...
struct BenchmarkData {
  ...
  explicit BenchmarkData(int size,
                         turbo::FunctionRef<std::string(int)> factory) {
    sample.reserve(size);
...
void BenchmarkTryEmplaceStrings(benchmark::State& state,
  ...
  std::vector<turbo::linked_hash_map<std::string, int>> sets(batch_size);
...
void BenchmarkInsertOrAssignStrings(
    benchmark::State& state, turbo::FunctionRef<std::string(int)> factory) {
  BenchmarkData data(state.range(0), factory);
...
constexpr std::string_view kFormatShort = "%10d";
constexpr std::string_view kFormatLong =
