...
// Performance of `update_log_sites` depends upon the number and organization of
  ...
  explicit SyntheticBinary(const size_t num_tus,
    ...
    for (size_t i = 0; i < num_tus; i++) {
      ...
      container_internal::Layout<char, VLogSite, char> layout(
          filename.size() + 1,
          turbo::LogUniform<size_t>(bitgen_, 1, max_sites_per_tu),
          turbo::LogUniform<size_t>(bitgen_, 0,
    ...
    for (int shuffle_num = 0; shuffle_num < num_shuffles; shuffle_num++) {
      ...
      const size_t cut_a = turbo::Uniform(bitgen_, size_t{0}, num_sites_);
      const size_t cut_b = turbo::Uniform(bitgen_, size_t{0}, num_sites_);
      const size_t cut_c = turbo::Uniform(bitgen_, size_t{0}, num_sites_);
    ...
    turbo::log_internal::set_vmodule_list_head_for_test_only(sites[0]);
    sites[num_tus - 1]->next_.store(nullptr, std::memory_order_seq_cst);
  }
  ~SyntheticBinary() {
    static_assert(std::is_trivially_destructible_v<VLogSite>, "");
    turbo::log_internal::set_vmodule_list_head_for_test_only(nullptr);
  }
...
void BM_UpdateVModuleShort(benchmark::State& state) {
  SyntheticBinary bin(static_cast<size_t>(state.range(0)), 10 * 1024 * 1024,
                      256, state.range(1));
  for (auto s : state) {
    turbo::log_internal::update_vmodule("directory2/*=4");
  }
  state.SetItemsProcessed(static_cast<int>(bin.num_new_files()));
}
