...
class PrecompiledSeedSeq {
 public:
  ...
  void generate(OutIterator begin, OutIterator end) {
    ...
    for (; begin != end; begin++) {
      *begin = kSeedData[idx++];
      if (idx >= KUMO_ARRAYSIZE(kSeedData)) {
        idx = 0;
      }
    }
  }

  size_t size() const { return KUMO_ARRAYSIZE(kSeedData); }
...
#define BM_BASIC(Engine)                                                       \
  ...
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     turbo::random_internal::FastUniformBits<uint32_t>);        \
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     turbo::random_internal::FastUniformBits<uint64_t>);        \
  ...
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     turbo::uniform_int_distribution<int32_t>);                 \
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     turbo::uniform_int_distribution<int64_t>);                 \
  ...
  BENCHMARK_TEMPLATE(BM_Dist, Engine, turbo::uniform_real_distribution<float>); \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, turbo::uniform_real_distribution<double>)
...
#define BM_THREAD(Engine)                                           \
  BENCHMARK_TEMPLATE(BM_Thread, Engine,                             \
      ...
      ->ThreadPerCpu();                                             \
  BENCHMARK_TEMPLATE(BM_Thread, Engine,                             \
                     turbo::uniform_real_distribution<double>)       \
      ->ThreadPerCpu();                                             \
...
#define BM_EXTENDED(Engine)                                                    \
  ...
  BENCHMARK_TEMPLATE(BM_Small, Engine,                                         \
                     turbo::uniform_real_distribution<float>);                  \
  BENCHMARK_TEMPLATE(BM_Small, Engine,                                         \
                     turbo::uniform_real_distribution<double>);                 \
  /* -------------- Other -----------------------*/                            \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, std::normal_distribution<double>);       \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, turbo::gaussian_distribution<double>);    \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, std::exponential_distribution<double>);  \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, turbo::exponential_distribution<double>); \
  BENCHMARK_TEMPLATE(BM_Poisson, Engine, std::poisson_distribution<int64_t>,   \
                     100);                                                     \
  BENCHMARK_TEMPLATE(BM_Poisson, Engine, turbo::poisson_distribution<int64_t>,  \
                     100);                                                     \
  BENCHMARK_TEMPLATE(BM_Poisson, Engine, std::poisson_distribution<int64_t>,   \
                     10 * 100);                                                \
  BENCHMARK_TEMPLATE(BM_Poisson, Engine, turbo::poisson_distribution<int64_t>,  \
  ...
  BENCHMARK_TEMPLATE(BM_Poisson, Engine, turbo::poisson_distribution<int64_t>,  \
                     13 * 100);                                                \
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     turbo::log_uniform_int_distribution<int32_t>);             \
  BENCHMARK_TEMPLATE(BM_Dist, Engine,                                          \
                     turbo::log_uniform_int_distribution<int64_t>);             \
  BENCHMARK_TEMPLATE(BM_Dist, Engine, std::geometric_distribution<int64_t>);   \
  BENCHMARK_TEMPLATE(BM_Zipf, Engine, turbo::zipf_distribution<uint64_t>);      \
  BENCHMARK_TEMPLATE(BM_Zipf, Engine, turbo::zipf_distribution<uint64_t>, 2,    \
                     3);                                                       \
  BENCHMARK_TEMPLATE(BM_Bernoulli, Engine, std::bernoulli_distribution,        \
                     257305);                                                  \
  BENCHMARK_TEMPLATE(BM_Bernoulli, Engine, turbo::bernoulli_distribution,       \
                     257305);                                                  \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, turbo::beta_distribution<double>, 65,     \
                     41);                                                      \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, turbo::beta_distribution<double>, 99,     \
                     330);                                                     \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, turbo::beta_distribution<double>, 150,    \
                     150);                                                     \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, turbo::beta_distribution<double>, 410,    \
                     580);                                                     \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, turbo::beta_distribution<float>, 65, 41); \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, turbo::beta_distribution<float>, 99,      \
                     330);                                                     \
  BENCHMARK_TEMPLATE(BM_Beta, Engine, turbo::beta_distribution<float>, 150,     \
                     150);                                                     \
...
BM_EXTENDED(turbo::BitGen);

// Instantiate benchmarks for multiple engines.
using randen_engine_64 = turbo::random_internal::randen_engine<uint64_t>;
using randen_engine_32 = turbo::random_internal::randen_engine<uint32_t>;
