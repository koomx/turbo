...
void BM_FastIntToBuffer(benchmark::State& state) {
  ...
  char buf[turbo::numbers_internal::kFastToBufferSize];
  // Use the unsigned type to increment to take advantage of well-defined
  // modular arithmetic.
  std::make_unsigned_t<T> x = 0;
  for (auto _ : state) {
    turbo::numbers_internal::fast_int_to_buffer(static_cast<T>(x), buf);
    x += inc;
  }
}
...
void BM_FastHexToBufferZeroPad16(benchmark::State& state) {
  ...
  while (state.KeepRunningBatch(nums.size())) {
    for (auto num : nums) {
      auto digits = turbo::numbers_internal::FastHexToBufferZeroPad16(num, buf);
