...
void BM_Mutex(benchmark::State& state) {
  static turbo::NoDestructor<turbo::Mutex> mu;
  for (auto _ : state) {
    turbo::MutexLock lock(*mu.get());
  }
}
...
void BM_ReaderLock(benchmark::State& state) {
  static turbo::NoDestructor<turbo::Mutex> mu;
  for (auto _ : state) {
    turbo::ReaderMutexLock lock(*mu.get());
  }
}
...
static void DelayNs(int64_t ns, int* data) {
  int64_t end = turbo::base_internal::CycleClock::Now() +
                ns * turbo::base_internal::CycleClock::Frequency() / 1e9;
  while (turbo::base_internal::CycleClock::Now() < end) {
    ++(*data);
...
void BM_MutexEnqueue(benchmark::State& state) {
  ...
  static turbo::NoDestructor<Shared> shared;
...
BENCHMARK_TEMPLATE(BM_Contended, turbo::Mutex)
    ->Apply([](benchmark::internal::Benchmark* bm) {
      SetupBenchmarkArgs(bm, /*do_test_priorities=*/true);
    });

BENCHMARK_TEMPLATE(BM_Contended, turbo::base_internal::SpinLock)
    ->Apply([](benchmark::internal::Benchmark* bm) {
...
void BM_ConditionWaiters(benchmark::State& state) {
  ...
  struct Helper {
    static void Waiter(turbo::BlockingCounter* init, turbo::Mutex* m, int* p) {
      init->DecrementCount();
      m->LockWhen(turbo::Condition(
  ...
  turbo::BlockingCounter init(num_waiters);
  turbo::Mutex mu;
  std::vector<int> equivalence_classes(num_classes, 1);

  // Must be declared last to be destroyed first.
  turbo::synchronization_internal::ThreadPool pool(num_waiters);
...
#if defined(__linux__) && !KUMO_HAVE_THREAD_SANITIZER
