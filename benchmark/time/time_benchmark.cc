...
void BM_Time_FromCivil_Turbo(benchmark::State& state) {
  ...
  while (state.KeepRunning()) {
    if ((i & 1) == 0) {
      ...
      benchmark::DoNotOptimize(
          turbo::FromCivil(turbo::CivilSecond(2013, 11, 15, 18, 30, 27), tz));
    }
    ++i;
  }
}
...
void BM_Time_FromCivilDay0_Turbo(benchmark::State& state) {
  ...
  while (state.KeepRunning()) {
    if ((i & 1) == 0) {
      benchmark::DoNotOptimize(
          turbo::FromCivil(turbo::CivilSecond(2014, 12, 0, 20, 16, 18), tz));
    } else {
      benchmark::DoNotOptimize(
          turbo::FromCivil(turbo::CivilSecond(2013, 11, 0, 18, 30, 27), tz));
    }
    ++i;
  }
}
