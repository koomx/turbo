...
TURBO_FLAG(uint64_t, benchmark_container_size, 2000,
...
Container MakeContainer() {
  const size_t container_size = turbo::GetFlag(FLAGS_benchmark_container_size);
  Container c;
...
void BM_Sequential_Span(benchmark::State& state) {
  ...
  turbo::Span<typename Container::value_type> array_span(container);
...
void BM_Sequential_DerefSpan(benchmark::State& state) {
  ...
  turbo::Span<T* const> array_span(container);
  benchmark::DoNotOptimize(array_span);
  for (auto _ : state) {
