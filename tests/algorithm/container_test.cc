...
TEST(Span, IsSpan) {
  static_assert(
      turbo::container_algorithm_internal::IsSpan<turbo::Span<int>>::value);
  static_assert(
      turbo::container_algorithm_internal::IsSpan<const turbo::Span<int>>::value);
  static_assert(turbo::container_algorithm_internal::IsSpan<
                volatile turbo::Span<int>>::value);
  static_assert(turbo::container_algorithm_internal::IsSpan<
                const volatile turbo::Span<int>>::value);
...
#ifdef __cpp_lib_span
  static_assert(
      turbo::container_algorithm_internal::IsSpan<std::span<int>>::value);
  static_assert(
      !turbo::container_algorithm_internal::IsSpan<std::span<int>&>::value);
  static_assert(
      !turbo::container_algorithm_internal::IsSpan<std::span<int>&&>::value);
...
TEST_F(NonMutatingTest, Distance) {
  ...
  EXPECT_EQ(KUMO_ARRAYSIZE(array_),
...
TEST_F(NonMutatingTest, Contains) {
  EXPECT_TRUE(turbo::c_contains(container_, 3));
  EXPECT_FALSE(turbo::c_contains(container_, 4));
}
...
TEST_F(NonMutatingTest, FindFirstOf) {
  ...
  turbo::c_find_first_of(sequence_, std::array<int, 2>{1, 2});
}
...
TEST_F(NonMutatingTest, FindFirstOfWithPredicate) {
  ...
  turbo::c_find_first_of(sequence_, std::array<int, 2>{1, 2}, BinPredicate);
}
...
TEST_F(NonMutatingTest, ContainsSubrange) {
  EXPECT_TRUE(turbo::c_contains_subrange(sequence_, vector_));
  EXPECT_TRUE(turbo::c_contains_subrange(vector_, sequence_));
  EXPECT_TRUE(turbo::c_contains_subrange(array_, sequence_));
}

TEST_F(NonMutatingTest, ContainsSubrangeWithPredicate) {
  EXPECT_TRUE(turbo::c_contains_subrange(sequence_, vector_, Equals));
  EXPECT_TRUE(turbo::c_contains_subrange(vector_, sequence_, Equals));
}
...
TEST(MutatingTest, CopyNWithNegativeN) {
  ...
  turbo::c_copy_n(input, -1, actual.begin());
...
TEST(MutatingTest, CopyToCArrayInvalidSize) {
  ...
  if (IsHardened()) {
    turbo::base_internal::ScopedSetTurboHardeningForTesting hardener(true);
    EXPECT_DEATH(turbo::c_copy(input, actual), "");
  }
#endif
}
...
TEST(MutatingTest, CopyNToCArrayInvalidSize) {
  ...
  if (IsHardened()) {
    turbo::base_internal::ScopedSetTurboHardeningForTesting hardener(true);
    EXPECT_DEATH(turbo::c_copy_n(input, 3, actual), "");
  }
#endif
}
...
TEST(MutatingTest, CopyNToCArrayNGreaterThanInput) {
  ...
  if (IsHardened()) {
    turbo::base_internal::ScopedSetTurboHardeningForTesting hardener(true);
    EXPECT_DEATH(turbo::c_copy_n(input, 4, actual), "");
  }
#endif
}
...
TEST(MutatingTest, MoveToCArrayInvalidSize) {
  ...
  if (IsHardened()) {
    turbo::base_internal::ScopedSetTurboHardeningForTesting hardener(true);
    EXPECT_DEATH(turbo::c_move(input, actual), "");
  }
#endif
}
...
TEST_F(NonMutatingTest, Transform) {
  ...
  *end = 7;
  ...
  turbo::c_transform(x2, y2, out_it, std::plus<int>());
...
TEST(MutatingTest, TransformToContainer) {
  const std::vector<int> input = {1, 2, 3};
  std::vector<int> actual = {0, 0, 0, 4, 5};
  turbo::c_transform(input, actual, [](int x) { return x * 2; });
...
TEST(MutatingTest, BinaryTransformToContainer) {
  {
    ...
    std::vector<int> actual = {0, 0, 0, 4, 5};
    turbo::c_transform(input1, input2, actual, std::plus<int>());
...
TEST(MutatingTest, TransformToCArrayInvalidSize) {
  ...
  if (IsHardened()) {
    turbo::base_internal::ScopedSetTurboHardeningForTesting hardener(true);
    EXPECT_DEATH(turbo::c_transform(input, actual, [](int x) { return x * 2; }),
                 "");
  }
#endif
}
...
TEST(MutatingTest, BinaryTransformToCArrayInvalidSize) {
  ...
  if (IsHardened()) {
    turbo::base_internal::ScopedSetTurboHardeningForTesting hardener(true);
    EXPECT_DEATH(turbo::c_transform(input1, input2, actual, std::plus<int>()),
                 "");
  }
#endif
}
...
TEST(ConstexprTest, Distance) {
  ...
  static_assert(turbo::c_distance(std::array<int, 3>()) == 3);
}

TEST(ConstexprTest, MinElement) {
  constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(*turbo::c_min_element(kArray) == 1);
}

TEST(ConstexprTest, MinElementWithPredicate) {
  constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(*turbo::c_min_element(kArray, std::greater<int>()) == 3);
}

TEST(ConstexprTest, MaxElement) {
  constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(*turbo::c_max_element(kArray) == 3);
}

TEST(ConstexprTest, MaxElementWithPredicate) {
  constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(*turbo::c_max_element(kArray, std::greater<int>()) == 1);
}

TEST(ConstexprTest, MinMaxElement) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  constexpr auto kMinMaxPair = turbo::c_minmax_element(kArray);
...
TEST(ConstexprTest, MinMaxElementWithPredicate) {
  ...
  constexpr auto kMinMaxPair =
      turbo::c_minmax_element(kArray, std::greater<int>());
  static_assert(*kMinMaxPair.first == 3);
  static_assert(*kMinMaxPair.second == 1);
}
#endif  // KUMO_CPLUSPLUS_LANG >= 201703L

#if KUMO_CPLUSPLUS_LANG >= 202002L

TEST(ConstexprTest, LinearSearch) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(turbo::c_linear_search(kArray, 3));
  static_assert(!turbo::c_linear_search(kArray, 4));
}

TEST(ConstexprTest, AllOf) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(!turbo::c_all_of(kArray, [](int x) { return x > 1; }));
  static_assert(turbo::c_all_of(kArray, [](int x) { return x > 0; }));
}

TEST(ConstexprTest, AnyOf) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(turbo::c_any_of(kArray, [](int x) { return x > 2; }));
  static_assert(!turbo::c_any_of(kArray, [](int x) { return x > 5; }));
}

TEST(ConstexprTest, NoneOf) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(!turbo::c_none_of(kArray, [](int x) { return x > 2; }));
  static_assert(turbo::c_none_of(kArray, [](int x) { return x > 5; }));
}

TEST(ConstexprTest, ForEach) {
  static constexpr std::array<int, 3> kArray = [] {
    std::array<int, 3> array = {1, 2, 3};
    turbo::c_for_each(array, [](int& x) { x += 1; });
    return array;
  }();
...
TEST(ConstexprTest, Find) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(turbo::c_find(kArray, 1) == kArray.begin());
  static_assert(turbo::c_find(kArray, 4) == kArray.end());
}

TEST(ConstexprTest, Contains) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(turbo::c_contains(kArray, 1));
  static_assert(!turbo::c_contains(kArray, 4));
}

TEST(ConstexprTest, FindIf) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(turbo::c_find_if(kArray, [](int x) { return x > 2; }) ==
                kArray.begin() + 2);
  static_assert(turbo::c_find_if(kArray, [](int x) { return x > 5; }) ==
                kArray.end());
}

TEST(ConstexprTest, FindIfNot) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(turbo::c_find_if_not(kArray, [](int x) { return x > 1; }) ==
                kArray.begin());
  static_assert(turbo::c_find_if_not(kArray, [](int x) { return x > 0; }) ==
                kArray.end());
}
...
TEST(ConstexprTest, FindEnd) {
  ...
  static_assert(turbo::c_find_end(kHaystack, kNeedle) == kHaystack.begin() + 3);
}

TEST(ConstexprTest, FindFirstOf) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(turbo::c_find_first_of(kArray, kArray) == kArray.begin());
}

TEST(ConstexprTest, AdjacentFind) {
  static constexpr std::array<int, 4> kArray = {1, 2, 2, 3};
  static_assert(turbo::c_adjacent_find(kArray) == kArray.begin() + 1);
}

TEST(ConstexprTest, AdjacentFindWithPredicate) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(turbo::c_adjacent_find(kArray, std::less<int>()) ==
                kArray.begin());
}

TEST(ConstexprTest, Count) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(turbo::c_count(kArray, 1) == 1);
  static_assert(turbo::c_count(kArray, 2) == 1);
  static_assert(turbo::c_count(kArray, 3) == 1);
  static_assert(turbo::c_count(kArray, 4) == 0);
}

TEST(ConstexprTest, CountIf) {
  static constexpr std::array<int, 3> kArray = {1, 2, 3};
  static_assert(turbo::c_count_if(kArray, [](int x) { return x > 0; }) == 3);
  static_assert(turbo::c_count_if(kArray, [](int x) { return x > 1; }) == 2);
}
...
TEST(ConstexprTest, Mismatch) {
  ...
  static_assert(turbo::c_mismatch(kArray1, kArray2) ==
                std::pair{kArray1.end(), kArray2.end()});
  static_assert(turbo::c_mismatch(kArray1, kArray3) ==
...
TEST(ConstexprTest, MismatchWithPredicate) {
  ...
  static constexpr std::array<int, 3> kArray2 = {1, 2, 3};
  static constexpr std::array<int, 3> kArray3 = {2, 3, 4};
  static_assert(turbo::c_mismatch(kArray1, kArray2, std::not_equal_to<int>()) ==
                std::pair{kArray1.begin(), kArray2.begin()});
  static_assert(turbo::c_mismatch(kArray1, kArray3, std::not_equal_to<int>()) ==
...
TEST(ConstexprTest, Equal) {
  ...
  static constexpr std::array<int, 3> kArray2 = {1, 2, 3};
  static constexpr std::array<int, 3> kArray3 = {2, 3, 4};
  static_assert(turbo::c_equal(kArray1, kArray2));
  static_assert(!turbo::c_equal(kArray1, kArray3));
}
...
TEST(ConstexprTest, EqualWithPredicate) {
  ...
  static constexpr std::array<int, 3> kArray2 = {1, 2, 3};
  static constexpr std::array<int, 3> kArray3 = {2, 3, 4};
  static_assert(!turbo::c_equal(kArray1, kArray2, std::not_equal_to<int>()));
  static_assert(turbo::c_equal(kArray1, kArray3, std::not_equal_to<int>()));
}
...
TEST(ConstexprTest, IsPermutation) {
  ...
  static constexpr std::array<int, 3> kArray3 = {2, 3, 4};
  static_assert(turbo::c_is_permutation(kArray1, kArray2));
  static_assert(!turbo::c_is_permutation(kArray1, kArray3));
}
...
TEST(ConstexprTest, IsPermutationWithPredicate) {
  ...
  static constexpr std::array<int, 3> kArray2 = {3, 2, 1};
  static constexpr std::array<int, 3> kArray3 = {2, 3, 4};
  static_assert(turbo::c_is_permutation(kArray1, kArray2, std::equal_to<int>()));
  static_assert(
      !turbo::c_is_permutation(kArray1, kArray3, std::equal_to<int>()));
}
...
TEST(ConstexprTest, Search) {
  ...
  static constexpr std::array<int, 3> kArray2 = {1, 2, 3};
  static constexpr std::array<int, 3> kArray3 = {2, 3, 4};
  static_assert(turbo::c_search(kArray1, kArray2) == kArray1.begin());
  static_assert(turbo::c_search(kArray1, kArray3) == kArray1.end());
}
...
TEST(ConstexprTest, SearchWithPredicate) {
  ...
  static constexpr std::array<int, 3> kArray2 = {1, 2, 3};
  static constexpr std::array<int, 3> kArray3 = {2, 3, 4};
  static_assert(turbo::c_search(kArray1, kArray2, std::not_equal_to<int>()) ==
                kArray1.end());
  static_assert(turbo::c_search(kArray1, kArray3, std::not_equal_to<int>()) ==
                kArray1.begin());
}
...
TEST(ConstexprTest, ContainsSubrange) {
  ...
  static constexpr std::array<int, 3> kArray2 = {1, 2, 3};
  static constexpr std::array<int, 3> kArray3 = {2, 3, 4};
  static_assert(turbo::c_contains_subrange(kArray1, kArray2));
  static_assert(!turbo::c_contains_subrange(kArray1, kArray3));
}
...
TEST(ConstexprTest, ContainsSubrangeWithPredicate) {
  ...
  static constexpr std::array<int, 3> kArray3 = {2, 3, 4};
  static_assert(
      !turbo::c_contains_subrange(kArray1, kArray2, std::not_equal_to<>()));
  static_assert(
      turbo::c_contains_subrange(kArray1, kArray3, std::not_equal_to<>()));
}

TEST(ConstexprTest, SearchN) {
  static constexpr std::array<int, 4> kArray = {1, 2, 2, 3};
  static_assert(turbo::c_search_n(kArray, 1, 1) == kArray.begin());
  static_assert(turbo::c_search_n(kArray, 2, 2) == kArray.begin() + 1);
  static_assert(turbo::c_search_n(kArray, 1, 4) == kArray.end());
}

TEST(ConstexprTest, SearchNWithPredicate) {
  static constexpr std::array<int, 4> kArray = {1, 2, 2, 3};
  static_assert(turbo::c_search_n(kArray, 1, 1, std::not_equal_to<int>()) ==
                kArray.begin() + 1);
  static_assert(turbo::c_search_n(kArray, 2, 2, std::not_equal_to<int>()) ==
                kArray.end());
  static_assert(turbo::c_search_n(kArray, 1, 4, std::not_equal_to<int>()) ==
                kArray.begin());
}
...
TEST(ConstexprTest, Copy) {
  ...
  static constexpr auto kArrayCopy = [] {
    ...
    turbo::c_copy(kArray, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, CopyN) {
  ...
  static constexpr auto kArrayCopy = [] {
    ...
    turbo::c_copy_n(kArray, 2, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, CopyIf) {
  ...
  static constexpr auto kArrayCopy = [] {
    std::array<int, 3> array;
    turbo::c_copy_if(kArray, array.begin(), [](int x) { return x > 1; });
    return array;
  }();
...
TEST(ConstexprTest, CopyBackward) {
  ...
  static constexpr auto kArrayCopy = [] {
    std::array<int, 3> array;
    turbo::c_copy_backward(kArray, array.end());
    return array;
  }();
  static_assert(kArrayCopy == kArray);
}
...
TEST(ConstexprTest, Move) {
  ...
  static constexpr auto kArrayMove = [] {
    std::array<int, 3> array;
    turbo::c_move(kArray, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, MoveBackward) {
  ...
  static constexpr auto kArrayMove = [] {
    std::array<int, 3> array;
    turbo::c_move_backward(kArray, array.end());
    return array;
  }();
  static_assert(kArrayMove == kArray);
}
...
TEST(ConstexprTest, Transform) {
  ...
  static constexpr auto kArrayTransform = [] {
    std::array<int, 3> array;
    turbo::c_transform(kArray, array.begin(), [](int x) { return x + 1; });
    return array;
  }();
...
TEST(ConstexprTest, ReplaceIf) {
  ...
  static constexpr auto kArrayReplaceIf = [] {
    std::array array = kArray;
    turbo::c_replace_if(array, [](int x) { return x == 1; }, 4);
    return array;
  }();
...
TEST(ConstexprTest, ReplaceCopy) {
  ...
  static constexpr auto kArrayReplaceCopy = [] {
    std::array<int, 3> array;
    turbo::c_replace_copy(kArray, array.begin(), 1, 4);
    return array;
  }();
...
TEST(ConstexprTest, Generate) {
  static constexpr auto kArrayGenerate = [] {
    std::array<int, 3> array;
    turbo::c_generate(array, []() { return 4; });
    return array;
  }();
...
TEST(ConstexprTest, GenerateN) {
  static constexpr auto kArrayGenerateN = [] {
    std::array array = {0, 0, 0};
    turbo::c_generate_n(array, 2, []() { return 4; });
    return array;
  }();
...
TEST(ConstexprTest, RemoveCopy) {
  ...
  static constexpr auto kArrayRemoveCopy = [] {
    std::array<int, 2> array;
    turbo::c_remove_copy(kArray, array.begin(), 1);
    return array;
  }();
...
TEST(ConstexprTest, RemoveCopyIf) {
  ...
  static constexpr auto kArrayRemoveCopyIf = [] {
    std::array<int, 2> array;
    turbo::c_remove_copy_if(kArray, array.begin(), [](int x) { return x == 1; });
    return array;
  }();
...
TEST(ConstexprTest, UniqueCopy) {
  ...
  static constexpr auto kArrayUniqueCopy = [] {
    std::array<int, 3> array;
    turbo::c_unique_copy(kArray, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, UniqueCopyWithPredicate) {
  ...
  static constexpr auto kArrayUniqueCopy = [] {
    std::array<int, 3> array;
    turbo::c_unique_copy(kArray, array.begin(), std::equal_to<>());
    return array;
  }();
  static_assert(kArrayUniqueCopy == std::array{1, 2, 3});
}
...
TEST(ConstexprTest, ReverseCopy) {
  ...
  static constexpr auto kArrayReverseCopy = [] {
    std::array<int, 3> array;
    turbo::c_reverse_copy(kArray, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, Rotate) {
  ...
  static constexpr auto kArrayRotate = [] {
    std::array array = kArray;
    turbo::c_rotate(array, array.begin() + 1);
    return array;
  }();
...
TEST(ConstexprTest, RotateCopy) {
  ...
  static constexpr auto kArrayRotateCopy = [] {
    std::array<int, 3> array;
    turbo::c_rotate_copy(kArray, kArray.begin() + 1, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, IsPartitioned) {
  static constexpr std::array kArray = {1, 2, 3};
  static_assert(!turbo::c_is_partitioned(kArray, [](int x) { return x > 1; }));

  static constexpr std::array kPartitionedArray = {2, 3, 1};
  static_assert(
      turbo::c_is_partitioned(kPartitionedArray, [](int x) { return x > 1; }));
}
...
TEST(ConstexprTest, Partition) {
  ...
  static constexpr auto kArrayPartition = [] {
    std::array array = kArray;
    turbo::c_partition(array, [](int x) { return x > 1; });
    return array;
  }();
  static_assert(
      turbo::c_is_partitioned(kArrayPartition, [](int x) { return x > 1; }));
}
...
TEST(ConstexprTest, PartitionCopy) {
  ...
  static constexpr auto kPartitioned = [] {
    ...
    turbo::c_partition_copy(kArray, true_part.begin(), false_part.begin(),
                           [](int x) { return x > 1; });
...
TEST(ConstexprTest, PartitionPoint) {
  ...
  static constexpr auto kPartitionPoint =
      turbo::c_partition_point(kArray, [](int x) { return x > 1; });
...
TEST(ConstexprTest, SortWithPredicate) {
  ...
  static constexpr auto kArraySort = [] {
    std::array array = kArray;
    turbo::c_sort(array, std::greater<>());
    return array;
  }();
...
TEST(ConstexprTest, IsSorted) {
  ...
  static_assert(turbo::c_is_sorted(kSortedArray));
  static constexpr std::array kUnsortedArray = {1, 3, 2};
  static_assert(!turbo::c_is_sorted(kUnsortedArray));
}

TEST(ConstexprTest, IsSortedWithPredicate) {
  static constexpr std::array kSortedArray = {3, 2, 1};
  static_assert(turbo::c_is_sorted(kSortedArray, std::greater<>()));
  static constexpr std::array kUnsortedArray = {1, 3, 2};
  static_assert(!turbo::c_is_sorted(kUnsortedArray, std::greater<>()));
}
...
TEST(ConstexprTest, PartialSort) {
  ...
  static constexpr auto kArrayPartialSort = [] {
    std::array array = kArray;
    turbo::c_partial_sort(array, array.begin() + 2);
    return array;
  }();
...
TEST(ConstexprTest, PartialSortWithPredicate) {
  ...
  static constexpr auto kArrayPartialSort = [] {
    std::array array = kArray;
    turbo::c_partial_sort(array, array.begin() + 2, std::greater<>());
    return array;
  }();
...
TEST(ConstexprTest, PartialSortCopy) {
  ...
  static constexpr auto kArrayPartialSort = [] {
    ...
    turbo::c_partial_sort_copy(kArray, array);
    return array;
  }();
  static_assert(kArrayPartialSort[0] == 1);
  static_assert(kArrayPartialSort[1] == 2);
...
TEST(ConstexprTest, PartialSortCopyWithPredicate) {
  ...
  static constexpr auto kArrayPartialSort = [] {
    std::array<int, 4> array;
    turbo::c_partial_sort_copy(kArray, array, std::greater<>());
    return array;
  }();
  static_assert(kArrayPartialSort[0] == 4);
  static_assert(kArrayPartialSort[1] == 3);
...
TEST(ConstexprTest, IsSortedUntil) {
  static constexpr std::array kSortedArray = {1, 2, 3};
  static_assert(turbo::c_is_sorted_until(kSortedArray) == kSortedArray.end());
  static constexpr std::array kUnsortedArray = {1, 3, 2};
  static_assert(turbo::c_is_sorted_until(kUnsortedArray) ==
                kUnsortedArray.begin() + 2);
}

TEST(ConstexprTest, IsSortedUntilWithPredicate) {
  static constexpr std::array kSortedArray = {3, 2, 1};
  static_assert(turbo::c_is_sorted_until(kSortedArray, std::greater<>()) ==
                kSortedArray.end());
  static constexpr std::array kUnsortedArray = {1, 3, 2};
  static_assert(turbo::c_is_sorted_until(kUnsortedArray, std::greater<>()) ==
...
TEST(ConstexprTest, NthElement) {
  ...
  static constexpr auto kArrayNthElement = [] {
    std::array array = kArray;
    turbo::c_nth_element(array, array.begin() + 2);
    return array;
  }();
...
TEST(ConstexprTest, NthElementWithPredicate) {
  ...
  static constexpr auto kArrayNthElement = [] {
    std::array array = kArray;
    turbo::c_nth_element(array, array.begin() + 2, std::greater<>());
    return array;
  }();
...
TEST(ConstexprTest, LowerBound) {
  static constexpr std::array kArray = {1, 2, 3, 4};
  static constexpr auto kLowerBound = turbo::c_lower_bound(kArray, 2);
...
TEST(ConstexprTest, LowerBoundWithPredicate) {
  ...
  static constexpr auto kLowerBound =
      turbo::c_lower_bound(kArray, 2, std::greater<>());
  static_assert(kLowerBound == kArray.begin() + 2);
}

TEST(ConstexprTest, UpperBound) {
  static constexpr std::array kArray = {1, 2, 3, 4};
  static constexpr auto kUpperBound = turbo::c_upper_bound(kArray, 2);
...
TEST(ConstexprTest, UpperBoundWithPredicate) {
  ...
  static constexpr auto kUpperBound =
      turbo::c_upper_bound(kArray, 2, std::greater<>());
  static_assert(kUpperBound == kArray.begin() + 3);
}

TEST(ConstexprTest, EqualRange) {
  static constexpr std::array kArray = {1, 2, 3, 4};
  static constexpr auto kEqualRange = turbo::c_equal_range(kArray, 2);
...
TEST(ConstexprTest, EqualRangeWithPredicate) {
  ...
  static constexpr auto kEqualRange =
      turbo::c_equal_range(kArray, 2, std::greater<>());
...
TEST(ConstexprTest, BinarySearch) {
  static constexpr std::array kArray = {1, 2, 3, 4};
  static constexpr bool kBinarySearch = turbo::c_binary_search(kArray, 2);
...
TEST(ConstexprTest, BinarySearchWithPredicate) {
  ...
  static constexpr bool kBinarySearch =
      turbo::c_binary_search(kArray, 2, std::greater<>());
  static_assert(kBinarySearch);
}
...
TEST(ConstexprTest, Merge) {
  ...
  static constexpr auto kArrayMerge = [] {
    ...
    turbo::c_merge(kArray1, kArray2, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, MergeWithPredicate) {
  ...
  static constexpr auto kArrayMerge = [] {
    std::array<int, 6> array;
    turbo::c_merge(kArray1, kArray2, array.begin(), std::greater<>());
    return array;
  }();
...
TEST(ConstexprTest, Includes) {
  ...
  static constexpr bool kIncludes = turbo::c_includes(kArray1, kArray2);
...
TEST(ConstexprTest, IncludesWithPredicate) {
  ...
  static constexpr bool kIncludes =
      turbo::c_includes(kArray1, kArray2, std::greater<>());
  static_assert(kIncludes);
}
...
TEST(ConstexprTest, SetUnion) {
  ...
  static constexpr auto kArraySetUnion = [] {
    std::array<int, 4> array;
    turbo::c_set_union(kArray1, kArray2, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, SetUnionWithPredicate) {
  ...
  static constexpr auto kArraySetUnion = [] {
    std::array<int, 4> array;
    turbo::c_set_union(kArray1, kArray2, array.begin(), std::greater<>());
    return array;
  }();
...
TEST(ConstexprTest, SetIntersection) {
  ...
  static constexpr auto kArraySetIntersection = [] {
    std::array<int, 2> array;
    turbo::c_set_intersection(kArray1, kArray2, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, SetIntersectionWithPredicate) {
  ...
  static constexpr auto kArraySetIntersection = [] {
    std::array<int, 2> array;
    turbo::c_set_intersection(kArray1, kArray2, array.begin(), std::greater<>());
    return array;
  }();
...
TEST(ConstexprTest, SetDifference) {
  ...
  static constexpr auto kArraySetDifference = [] {
    ...
    turbo::c_set_difference(kArray1, kArray2, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, SetDifferenceWithPredicate) {
  ...
  static constexpr auto kArraySetDifference = [] {
    std::array<int, 1> array;
    turbo::c_set_difference(kArray1, kArray2, array.begin(), std::greater<>());
    return array;
  }();
  static_assert(kArraySetDifference == std::array{2});
}
...
TEST(ConstexprTest, SetSymmetricDifference) {
  ...
  static constexpr auto kArraySetSymmetricDifference = [] {
    std::array<int, 2> array;
    turbo::c_set_symmetric_difference(kArray1, kArray2, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, PushHeapWithPredicate) {
  static constexpr auto kArray = [] {
    ...
    turbo::c_push_heap(array, std::greater<>());
    return array;
  }();
...
TEST(ConstexprTest, PopHeapWithPredicate) {
  static constexpr auto kArray = [] {
    std::array array = {1, 2, 3, 4};
    turbo::c_pop_heap(array, std::greater<>());
    return array;
  }();
...
TEST(ConstexprTest, MakeHeap) {
  static constexpr auto kArray = [] {
    std::array array = {1, 2, 3, 4};
    turbo::c_make_heap(array);
    return array;
  }();
  static_assert(turbo::c_is_heap(kArray));
}

TEST(ConstexprTest, MakeHeapWithPredicate) {
  static constexpr auto kArray = [] {
    std::array array = {4, 3, 2, 1};
    turbo::c_make_heap(array, std::greater<>());
    return array;
  }();
  static_assert(turbo::c_is_heap(kArray, std::greater<>()));
}
...
TEST(ConstexprTest, SortHeapWithPredicate) {
  static constexpr auto kArray = [] {
    std::array array = {4, 3, 2, 1};
    turbo::c_make_heap(array, std::greater<>());
    turbo::c_sort_heap(array, std::greater<>());
    return array;
  }();
...
TEST(ConstexprTest, IsHeap) {
  ...
  static_assert(turbo::c_is_heap(kHeap));
  static constexpr std::array kNotHeap = {1, 2, 3, 4};
  static_assert(!turbo::c_is_heap(kNotHeap));
}

TEST(ConstexprTest, IsHeapWithPredicate) {
  static constexpr std::array kHeap = {1, 2, 3, 4};
  static_assert(turbo::c_is_heap(kHeap, std::greater<>()));
  static constexpr std::array kNotHeap = {4, 3, 2, 1};
  static_assert(!turbo::c_is_heap(kNotHeap, std::greater<>()));
}

TEST(ConstexprTest, IsHeapUntil) {
  static constexpr std::array kHeap = {4, 2, 3, 1};
  static_assert(turbo::c_is_heap_until(kHeap) == kHeap.end());
  static constexpr std::array kNotHeap = {4, 2, 3, 5};
  static_assert(turbo::c_is_heap_until(kNotHeap) == kNotHeap.begin() + 3);
}

TEST(ConstexprTest, IsHeapUntilWithPredicate) {
  static constexpr std::array kHeap = {1, 2, 3, 4};
  static_assert(turbo::c_is_heap_until(kHeap, std::greater<>()) == kHeap.end());
  static constexpr std::array kNotHeap = {1, 2, 3, 0};
  static_assert(turbo::c_is_heap_until(kNotHeap, std::greater<>()) ==
                kNotHeap.begin() + 3);
}
...
TEST(ConstexprTest, LexicographicalCompare) {
  ...
  static_assert(turbo::c_lexicographical_compare(kArray1, kArray2));
  static_assert(!turbo::c_lexicographical_compare(kArray2, kArray1));
  static_assert(!turbo::c_lexicographical_compare(kArray1, kArray3));
}
...
TEST(ConstexprTest, LexicographicalCompareWithPredicate) {
  ...
  static constexpr std::array kArray3 = {1, 2, 3};
  static_assert(
      !turbo::c_lexicographical_compare(kArray1, kArray2, std::greater<>()));
  static_assert(
      turbo::c_lexicographical_compare(kArray2, kArray1, std::greater<>()));
  static_assert(
      !turbo::c_lexicographical_compare(kArray1, kArray3, std::greater<>()));
}

TEST(ConstexprTest, NextPermutation) {
  static constexpr auto kArray = [] {
    std::array array = {1, 2, 3};
    turbo::c_next_permutation(array);
    return array;
  }();
...
TEST(ConstexprTest, NextPermutationWithPredicate) {
  static constexpr auto kArray = [] {
    ...
    turbo::c_next_permutation(array, std::greater<>());
    return array;
  }();
...
TEST(ConstexprTest, PrevPermutation) {
  static constexpr auto kArray = [] {
    ...
    turbo::c_prev_permutation(array);
    return array;
  }();
...
TEST(ConstexprTest, PrevPermutationWithPredicate) {
  static constexpr auto kArray = [] {
    std::array array = {1, 2, 3};
    turbo::c_prev_permutation(array, std::greater<>());
    return array;
  }();
  static_assert(kArray == std::array{1, 3, 2});
}
...
TEST(ConstexprTest, Accumulate) {
  static constexpr std::array kArray = {1, 2, 3};
  static_assert(turbo::c_accumulate(kArray, 0) == 6);
}

TEST(ConstexprTest, AccumulateWithPredicate) {
  static constexpr std::array kArray = {1, 2, 3};
  static_assert(turbo::c_accumulate(kArray, 1, std::multiplies<>()) == 6);
}

TEST(ConstexprTest, InnerProduct) {
  static constexpr std::array kArray1 = {1, 2, 3};
  static constexpr std::array kArray2 = {4, 5, 6};
  static_assert(turbo::c_inner_product(kArray1, kArray2, 0) == 32);
}

TEST(ConstexprTest, InnerProductWithPredicate) {
  static constexpr std::array kArray1 = {1, 2, 3};
  static constexpr std::array kArray2 = {4, 5, 6};
  static_assert(turbo::c_inner_product(kArray1, kArray2, 1, std::multiplies<>(),
                                      std::plus<>()) == 315);
}
...
TEST(ConstexprTest, AdjacentDifference) {
  ...
  static constexpr auto kArrayAdjacentDifference = [] {
    std::array<int, 3> array;
    turbo::c_adjacent_difference(kArray, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, AdjacentDifferenceWithPredicate) {
  ...
  static constexpr auto kArrayAdjacentDifference = [] {
    std::array<int, 3> array;
    turbo::c_adjacent_difference(kArray, array.begin(), std::multiplies<>());
    return array;
  }();
...
TEST(ConstexprTest, PartialSum) {
  ...
  static constexpr auto kArrayPartialSum = [] {
    std::array<int, 3> array;
    turbo::c_partial_sum(kArray, array.begin());
    return array;
  }();
...
TEST(ConstexprTest, PartialSumWithPredicate) {
  ...
  static constexpr auto kArrayPartialSum = [] {
    std::array<int, 3> array;
    turbo::c_partial_sum(kArray, array.begin(), std::multiplies<>());
    return array;
  }();
...
struct CanCopy<Container, Output,
               std::void_t<decltype(turbo::c_copy(std::declval<Container>(),
                                                 std::declval<Output>()))>>
    : std::true_type {};
...
struct CanCopyN<Container, Output,
                std::void_t<decltype(turbo::c_copy_n(
...
struct CanMove<Container, Output,
               std::void_t<decltype(turbo::c_move(std::declval<Container>(),
                                                 std::declval<Output>()))>>
    : std::true_type {};
...
struct CanTransformUnary<Container, Output, UnaryOp,
                         std::void_t<decltype(turbo::c_transform(
...
TEST(CanMoveTest, MoveToRValue) {
  ...
  using Span = turbo::Span<std::unique_ptr<int>>;
...
struct CanFill<C, T,
               std::void_t<decltype(turbo::c_fill(std::declval<C (*)()>()(),
                                                 std::declval<T (*)()>()()))>>
    : std::true_type {};
...
TEST(CanFillTest, NonSpans) {
  ...
  struct TurboSpanSubclass : turbo::Span<T> {};
  static_assert(!CanFill<std::vector<T>, T>::value,
                "non-spans must not be allowed");
  static_assert(!CanFill<TurboSpanSubclass, T>::value,
