...
class ThrowingBool {
 public:
  ...
  operator bool() const {                   // NOLINT
    exceptions_internal::MaybeThrow(KUMO_PRETTY_FUNCTION);
    return b_;
  }
...
class ThrowingValue : private exceptions_internal::TrackedObject {
 ...
 public:
  ...
  // turbo expects nothrow destructors
  ...
  static void* operator new(size_t s) noexcept(
    ...
    if (!IsSpecified(TypeSpec::kNoThrowNew)) {
      exceptions_internal::MaybeThrow(KUMO_PRETTY_FUNCTION, true);
    }
 ...
 * throwing, or nothing throws, in line with the TURBO_ALLOCATOR_THROWS
 * configuration macro.
 */
...
class ThrowingAllocator : private exceptions_internal::TrackedObject {
 ...
 public:
  ...
  pointer allocate(size_type n) noexcept(
    ...
    ReadStateAndMaybeThrow(KUMO_PRETTY_FUNCTION);
...
#endif  // TURBO_BASE_INTERNAL_EXCEPTION_SAFETY_TESTING_H_
