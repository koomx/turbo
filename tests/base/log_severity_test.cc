...
TEST(StreamTest, Works) {
  ...
  EXPECT_THAT(StreamHelper(turbo::LogSeverity::kTrace), Eq("TRACE"));
  EXPECT_THAT(StreamHelper(turbo::LogSeverity::kDebug), Eq("DEBUG"));
  ...
  EXPECT_THAT(StreamHelper(static_cast<turbo::LogSeverity>(6)),
              Eq("turbo::LogSeverity(6)"));
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(std::make_tuple("0", turbo::LogSeverity::kTrace),
           std::make_tuple(" 0", turbo::LogSeverity::kTrace),
           std::make_tuple("-0", turbo::LogSeverity::kTrace),
           std::make_tuple("+0", turbo::LogSeverity::kTrace),
           std::make_tuple("00", turbo::LogSeverity::kTrace),
           std::make_tuple("0 ", turbo::LogSeverity::kTrace),
           std::make_tuple("0x0", turbo::LogSeverity::kTrace),
           std::make_tuple("1", turbo::LogSeverity::kDebug),
           std::make_tuple("+1", turbo::LogSeverity::kDebug),
           std::make_tuple("2", turbo::LogSeverity::kInfo),
           std::make_tuple("3", turbo::LogSeverity::kWarning),
           std::make_tuple("4", turbo::LogSeverity::kError),
           std::make_tuple("5", turbo::LogSeverity::kFatal)));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(std::make_tuple("TRACE", turbo::LogSeverity::kTrace),
           std::make_tuple("trace", turbo::LogSeverity::kTrace),
           std::make_tuple("kTrace", turbo::LogSeverity::kTrace),
           std::make_tuple("DEBUG", turbo::LogSeverity::kDebug),
           std::make_tuple("debug", turbo::LogSeverity::kDebug),
           std::make_tuple("kDebug", turbo::LogSeverity::kDebug),
           ...
           std::make_tuple("FaTaL", turbo::LogSeverity::kFatal),
           std::make_tuple("KfAtAl", turbo::LogSeverity::kFatal),
           std::make_tuple("DFATAL", turbo::kLogDebugFatal),
           std::make_tuple("dfatal", turbo::kLogDebugFatal),
           std::make_tuple("kLogDebugFatal", turbo::kLogDebugFatal),
           std::make_tuple("dFaTaL", turbo::kLogDebugFatal),
           std::make_tuple("kLoGdEbUgFaTaL", turbo::kLogDebugFatal)));
...
INSTANTIATE_TEST_SUITE_P(
    ...
    Values(std::make_tuple(turbo::LogSeverity::kTrace, "TRACE"),
           std::make_tuple(turbo::LogSeverity::kDebug, "DEBUG"),
           std::make_tuple(turbo::LogSeverity::kInfo, "INFO"),
...
INSTANTIATE_TEST_SUITE_P(Instantiation, UnparseFlagToOtherIntegerTest,
                         Values(std::numeric_limits<int>::min(), -1, 6,
                                std::numeric_limits<int>::max()));
...
TEST(LogThresholdTest, Output) {
  EXPECT_THAT(StreamHelper(turbo::LogSeverityAtLeast::kTrace), Eq(">=TRACE"));
  EXPECT_THAT(StreamHelper(turbo::LogSeverityAtLeast::kDebug), Eq(">=DEBUG"));
  ...
  EXPECT_THAT(StreamHelper(turbo::LogSeverityAtMost::kTrace), Eq("<=TRACE"));
  EXPECT_THAT(StreamHelper(turbo::LogSeverityAtMost::kDebug), Eq("<=DEBUG"));
...
TEST(NormalizeLogSeverityTest, ClampsOutOfRange) {
  EXPECT_EQ(turbo::NormalizeLogSeverity(static_cast<turbo::LogSeverity>(-1)),
            turbo::LogSeverity::kTrace);
  EXPECT_EQ(turbo::NormalizeLogSeverity(turbo::LogSeverity::kTrace),
            turbo::LogSeverity::kTrace);
  EXPECT_EQ(turbo::NormalizeLogSeverity(turbo::LogSeverity::kFatal),
            turbo::LogSeverity::kFatal);
  EXPECT_EQ(turbo::NormalizeLogSeverity(static_cast<turbo::LogSeverity>(6)),
            turbo::LogSeverity::kError);
}

TEST(LogSeveritiesTest, OrderedSixLevels) {
  const auto levels = turbo::LogSeverities();
  ASSERT_EQ(levels.size(), 6u);
  EXPECT_EQ(levels[0], turbo::LogSeverity::kTrace);
  EXPECT_EQ(levels[1], turbo::LogSeverity::kDebug);
  EXPECT_EQ(levels[2], turbo::LogSeverity::kInfo);
  EXPECT_EQ(levels[3], turbo::LogSeverity::kWarning);
  EXPECT_EQ(levels[4], turbo::LogSeverity::kError);
  EXPECT_EQ(levels[5], turbo::LogSeverity::kFatal);
}
