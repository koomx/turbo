...
struct Blob {
  ...
  ~Blob() { TURBO_INTERNAL_CHECK(moved_out, "~Blob"); }
...
TEST(NoDestructorTest, DestructorNeverCalled) {
  turbo::NoDestructor<TypeWithDeletedDestructor> a;
  (void)a;
}

TEST(NoDestructorTest, Noncopyable) {
  using T = turbo::NoDestructor<int>;
...
TEST(NoDestructorTest, Interface) {
  EXPECT_TRUE(std::is_trivially_destructible_v<turbo::NoDestructor<Blob>>);
  EXPECT_TRUE(std::is_trivially_destructible_v<turbo::NoDestructor<const Blob>>);
  {
    turbo::NoDestructor<Blob> b;  // default c-tor
    // access: *, ->, get()
    EXPECT_EQ(42, (*b).val);
    (*b).val = 55;
    EXPECT_EQ(55, b->val);
  ...
  {
    turbo::NoDestructor<const Blob> b(70, 7);  // regular c-tor, const
    EXPECT_EQ(77, (*b).val);
    EXPECT_EQ(77, b->val);
    EXPECT_EQ(77, b.get()->val);
  }
  {
    const turbo::NoDestructor<Blob> b{
        {20, 28, 40}};  // init-list c-tor, deep const
    // This only works in clang, not in gcc:
    // const turbo::NoDestructor<Blob> b({20, 28, 40});
    EXPECT_EQ(88, (*b).val);
    EXPECT_EQ(88, b->val);
...
TEST(NoDestructorTest, SfinaeRegressionAbstractArg) {
  ...
  Concrete input;
  turbo::NoDestructor<UsesAbstractInConstructor> foo1(input);
  EXPECT_EQ(foo1->i, 17);
  turbo::NoDestructor<UsesAbstractInConstructor> foo2(
...
std::string* Str0() {
  static turbo::NoDestructor<std::string> x;
  return x.get();
}
...
const char* Str1() {
  static turbo::NoDestructor<std::string> x(Str2() + "_Str1");
  return x->c_str();
}

const std::string& Str2() {
  static turbo::NoDestructor<std::string> x("Str2");
  return *x;
}
...
const MyArray& Array() {
  static turbo::NoDestructor<MyArray> x{{{"foo", "bar", "baz"}}};
  // This only works in clang, not in gcc:
  // static turbo::NoDestructor<MyArray> x({{"foo", "bar", "baz"}});
  return *x;
}
...
const MyVector& Vector() {
  static turbo::NoDestructor<MyVector> x{{1, 2, 3}};
  return *x;
}

const int& Int() {
  static turbo::NoDestructor<int> x;
  return *x;
}

TEST(NoDestructorTest, StaticPattern) {
  EXPECT_TRUE(
      std::is_trivially_destructible_v<turbo::NoDestructor<std::string>>);
  EXPECT_TRUE(std::is_trivially_destructible_v<turbo::NoDestructor<MyArray>>);
  EXPECT_TRUE(std::is_trivially_destructible_v<turbo::NoDestructor<MyVector>>);
  EXPECT_TRUE(std::is_trivially_destructible_v<turbo::NoDestructor<int>>);
...
TEST(NoDestructorTest, ClassTemplateArgumentDeduction) {
  turbo::NoDestructor i(1);
  static_assert(std::is_same_v<decltype(i), turbo::NoDestructor<int>>,
