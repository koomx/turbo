...
namespace {
void funcWithNonnullArg(int* turbo_nonnull /*arg*/) {}
template <typename T>
void funcWithDeducedNonnullArg(T* turbo_nonnull /*arg*/) {}
...
int* turbo_nonnull funcWithNonnullReturn() {
  static int var = 0;
  return &var;
}
...
TEST(PassThroughTest, PassesThroughRawPointerToInt) {
  EXPECT_TRUE((std::is_same_v<int* turbo_nonnull, int*>));
  EXPECT_TRUE((std::is_same_v<int* turbo_nullable, int*>));
  EXPECT_TRUE((std::is_same_v<int* turbo_nullability_unknown, int*>));
}

TEST(PassThroughTest, PassesThroughRawPointerToVoid) {
  EXPECT_TRUE((std::is_same_v<void* turbo_nonnull, void*>));
  EXPECT_TRUE((std::is_same_v<void* turbo_nullable, void*>));
  EXPECT_TRUE((std::is_same_v<void* turbo_nullability_unknown, void*>));
}

TEST(PassThroughTest, PassesThroughUniquePointerToInt) {
  using T = std::unique_ptr<int>;
  EXPECT_TRUE((std::is_same_v<turbo_nonnull T, T>));
  EXPECT_TRUE((std::is_same_v<turbo_nullable T, T>));
  EXPECT_TRUE((std::is_same_v<turbo_nullability_unknown T, T>));
}
