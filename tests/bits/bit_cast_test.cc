...
namespace turbo {
    namespace {
        ...
        TEST(BitCast, Bool) {
            ...
            TestMarshall<bool>(bool_list, KUMO_ARRAYSIZE(bool_list));
        }

        TEST(BitCast, Int32) {
            static const int32_t int_list[] =
                    {0, 1, 100, 2147483647, -1, -100, -2147483647, -2147483647 - 1};
            TestMarshall<int32_t>(int_list, KUMO_ARRAYSIZE(int_list));
        }

        TEST(BitCast, Int64) {
            static const int64_t int64_list[] =
                    {0, 1, 1LL << 40, -1, -(1LL << 40)};
            TestMarshall<int64_t>(int64_list, KUMO_ARRAYSIZE(int64_list));
        }

        TEST(BitCast, Uint64) {
            static const uint64_t uint64_list[] =
                    {0, 1, 1LLU << 40, 1LLU << 63};
            TestMarshall<uint64_t>(uint64_list, KUMO_ARRAYSIZE(uint64_list));
        }
        ...
        TEST(BitCast, Float) {
            ...
            TestMarshall<float>(float_list, KUMO_ARRAYSIZE(float_list));
            TestIntegral<float, int>(float_list, KUMO_ARRAYSIZE(float_list));
            TestIntegral<float, unsigned>(float_list, KUMO_ARRAYSIZE(float_list));
        }
        ...
        TEST(BitCast, Double) {
            ...
            TestMarshall<double>(double_list, KUMO_ARRAYSIZE(double_list));
            TestIntegral<double, int64_t>(double_list, KUMO_ARRAYSIZE(double_list));
            TestIntegral<double, uint64_t>(double_list, KUMO_ARRAYSIZE(double_list));
        }
    } // namespace
} // namespace turbo
