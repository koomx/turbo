...
namespace {
    ...
    static_assert(std::is_same_v<decltype(turbo::implicit_cast<BaseForImplicitCast &>(
            std::declval<DerivedForImplicitCast &>())),
        BaseForImplicitCast &>);
    static_assert(
        std::is_same_v<decltype(turbo::implicit_cast<const BaseForImplicitCast &>(
                std::declval<DerivedForImplicitCast>())),
            const BaseForImplicitCast &>);

    TEST(ImplicitCastTest, LValueReference) {
        DerivedForImplicitCast derived(5);
        EXPECT_EQ(&turbo::implicit_cast<BaseForImplicitCast&>(derived), &derived);
        EXPECT_EQ(&turbo::implicit_cast<const BaseForImplicitCast&>(derived),
                  &derived);
    }
    ...
    TEST(ImplicitCastTest, RValueReference) {
        ...
        BaseForImplicitCast &&base =
                turbo::implicit_cast<BaseForImplicitCast &&>(std::move(derived));
        EXPECT_EQ(&base, &derived);

        const DerivedForImplicitCast cderived(6);
        const BaseForImplicitCast &&cbase =
                turbo::implicit_cast<const BaseForImplicitCast &&>(std::move(cderived));
    ...
    TEST(DownCastTest, Pointer) {
        ...
        EXPECT_EQ(&derived, turbo::down_cast<DerivedForDownCast*>(base_ptr));

        // Tests casting a const BaseForDownCast* to a const DerivedForDownCast*.
        const BaseForDownCast *const_base_ptr = base_ptr;
        EXPECT_EQ(&derived,
                  turbo::down_cast<const DerivedForDownCast*>(const_base_ptr));

        // Tests casting a BaseForDownCast* to a const DerivedForDownCast*.
        EXPECT_EQ(&derived, turbo::down_cast<const DerivedForDownCast*>(base_ptr));

        // Tests casting a BaseForDownCast* to a BaseForDownCast* (an identity cast).
        EXPECT_EQ(base_ptr, turbo::down_cast<BaseForDownCast*>(base_ptr));

        // Tests down casting NULL.
        EXPECT_EQ(nullptr,
                  (turbo::down_cast<DerivedForDownCast*, BaseForDownCast>(nullptr)));
...
#if GTEST_HAS_DEATH_TEST && (!defined(NDEBUG) || (KUMO_OPTION_HARDENED == 1))
        EXPECT_DEATH(static_cast<void>(turbo::down_cast<DerivedForDownCast*>(base2)),
    ...
    TEST(DownCastTest, Reference) {
        ...
        EXPECT_EQ(&derived, &turbo::down_cast<DerivedForDownCast&>(base_ref));

        // Tests casting a const BaseForDownCast& to a const DerivedForDownCast&.
        const BaseForDownCast &const_base_ref = base_ref;
        // NOLINTNEXTLINE(runtime/casting)
        EXPECT_EQ(&derived,
                  &turbo::down_cast<const DerivedForDownCast&>(const_base_ref));

        // Tests casting a BaseForDownCast& to a const DerivedForDownCast&.
        // NOLINTNEXTLINE(runtime/casting)
        EXPECT_EQ(&derived, &turbo::down_cast<const DerivedForDownCast&>(base_ref));

        // Tests casting a BaseForDownCast& to a BaseForDownCast& (an identity cast).
        // NOLINTNEXTLINE(runtime/casting)
        EXPECT_EQ(&base_ref, &turbo::down_cast<BaseForDownCast&>(base_ref));
...
#if GTEST_HAS_DEATH_TEST && (!defined(NDEBUG) || (KUMO_OPTION_HARDENED == 1))
        EXPECT_DEATH(static_cast<void>(turbo::down_cast<Derived2ForDownCast&>(base)),
