...
template <typename T>
std::string str_printf_to(const T& t) {
  std::stringstream ss;
  ss << t;
  return ss.str();
}
...
TEST(CivilTime, DefaultConstruction) {
  civil_second ss;
  EXPECT_EQ("1970-01-01T00:00:00", str_printf_to(ss));

  civil_minute mm;
  EXPECT_EQ("1970-01-01T00:00", str_printf_to(mm));

  civil_hour hh;
  EXPECT_EQ("1970-01-01T00", str_printf_to(hh));

  civil_day d;
  EXPECT_EQ("1970-01-01", str_printf_to(d));

  civil_month m;
  EXPECT_EQ("1970-01", str_printf_to(m));

  civil_year y;
  EXPECT_EQ("1970", str_printf_to(y));
}
...
TEST(CivilTime, FieldsConstruction) {
  EXPECT_EQ("2015-01-02T03:04:05", str_printf_to(civil_second(2015, 1, 2, 3, 4, 5)));
  EXPECT_EQ("2015-01-02T03:04:00", str_printf_to(civil_second(2015, 1, 2, 3, 4)));
  EXPECT_EQ("2015-01-02T03:00:00", str_printf_to(civil_second(2015, 1, 2, 3)));
  EXPECT_EQ("2015-01-02T00:00:00", str_printf_to(civil_second(2015, 1, 2)));
  EXPECT_EQ("2015-01-01T00:00:00", str_printf_to(civil_second(2015, 1)));
  EXPECT_EQ("2015-01-01T00:00:00", str_printf_to(civil_second(2015)));

  EXPECT_EQ("2015-01-02T03:04", str_printf_to(civil_minute(2015, 1, 2, 3, 4, 5)));
  EXPECT_EQ("2015-01-02T03:04", str_printf_to(civil_minute(2015, 1, 2, 3, 4)));
  EXPECT_EQ("2015-01-02T03:00", str_printf_to(civil_minute(2015, 1, 2, 3)));
  EXPECT_EQ("2015-01-02T00:00", str_printf_to(civil_minute(2015, 1, 2)));
  EXPECT_EQ("2015-01-01T00:00", str_printf_to(civil_minute(2015, 1)));
  EXPECT_EQ("2015-01-01T00:00", str_printf_to(civil_minute(2015)));

  EXPECT_EQ("2015-01-02T03", str_printf_to(civil_hour(2015, 1, 2, 3, 4, 5)));
  EXPECT_EQ("2015-01-02T03", str_printf_to(civil_hour(2015, 1, 2, 3, 4)));
  EXPECT_EQ("2015-01-02T03", str_printf_to(civil_hour(2015, 1, 2, 3)));
  EXPECT_EQ("2015-01-02T00", str_printf_to(civil_hour(2015, 1, 2)));
  EXPECT_EQ("2015-01-01T00", str_printf_to(civil_hour(2015, 1)));
  EXPECT_EQ("2015-01-01T00", str_printf_to(civil_hour(2015)));

  EXPECT_EQ("2015-01-02", str_printf_to(civil_day(2015, 1, 2, 3, 4, 5)));
  EXPECT_EQ("2015-01-02", str_printf_to(civil_day(2015, 1, 2, 3, 4)));
  EXPECT_EQ("2015-01-02", str_printf_to(civil_day(2015, 1, 2, 3)));
  EXPECT_EQ("2015-01-02", str_printf_to(civil_day(2015, 1, 2)));
  EXPECT_EQ("2015-01-01", str_printf_to(civil_day(2015, 1)));
  EXPECT_EQ("2015-01-01", str_printf_to(civil_day(2015)));

  EXPECT_EQ("2015-01", str_printf_to(civil_month(2015, 1, 2, 3, 4, 5)));
  EXPECT_EQ("2015-01", str_printf_to(civil_month(2015, 1, 2, 3, 4)));
  EXPECT_EQ("2015-01", str_printf_to(civil_month(2015, 1, 2, 3)));
  EXPECT_EQ("2015-01", str_printf_to(civil_month(2015, 1, 2)));
  EXPECT_EQ("2015-01", str_printf_to(civil_month(2015, 1)));
  EXPECT_EQ("2015-01", str_printf_to(civil_month(2015)));

  EXPECT_EQ("2015", str_printf_to(civil_year(2015, 1, 2, 3, 4, 5)));
  EXPECT_EQ("2015", str_printf_to(civil_year(2015, 1, 2, 3, 4)));
  EXPECT_EQ("2015", str_printf_to(civil_year(2015, 1, 2, 3)));
  EXPECT_EQ("2015", str_printf_to(civil_year(2015, 1, 2)));
  EXPECT_EQ("2015", str_printf_to(civil_year(2015, 1)));
  EXPECT_EQ("2015", str_printf_to(civil_year(2015)));
}

TEST(CivilTime, FieldsConstructionLimits) {
  const int kIntMax = std::numeric_limits<int>::max();
  EXPECT_EQ("2038-01-19T03:14:07",
            str_printf_to(civil_second(1970, 1, 1, 0, 0, kIntMax)));
  EXPECT_EQ("6121-02-11T05:21:07",
            str_printf_to(civil_second(1970, 1, 1, 0, kIntMax, kIntMax)));
  EXPECT_EQ("251104-11-20T12:21:07",
            str_printf_to(civil_second(1970, 1, 1, kIntMax, kIntMax, kIntMax)));
  EXPECT_EQ("6130715-05-30T12:21:07",
            str_printf_to(civil_second(1970, 1, kIntMax, kIntMax, kIntMax, kIntMax)));
  EXPECT_EQ(
      "185087685-11-26T12:21:07",
      str_printf_to(civil_second(1970, kIntMax, kIntMax, kIntMax, kIntMax, kIntMax)));

  const int kIntMin = std::numeric_limits<int>::min();
  EXPECT_EQ("1901-12-13T20:45:52",
            str_printf_to(civil_second(1970, 1, 1, 0, 0, kIntMin)));
  EXPECT_EQ("-2182-11-20T18:37:52",
            str_printf_to(civil_second(1970, 1, 1, 0, kIntMin, kIntMin)));
  EXPECT_EQ("-247165-02-11T10:37:52",
            str_printf_to(civil_second(1970, 1, 1, kIntMin, kIntMin, kIntMin)));
  EXPECT_EQ("-6126776-08-01T10:37:52",
            str_printf_to(civil_second(1970, 1, kIntMin, kIntMin, kIntMin, kIntMin)));
  EXPECT_EQ(
      "-185083747-10-31T10:37:52",
      str_printf_to(civil_second(1970, kIntMin, kIntMin, kIntMin, kIntMin, kIntMin)));
}
...
TEST(CivilTime, ExplicitCrossAlignment) {
  ...
  EXPECT_EQ("2015-01-02T03:04:05", str_printf_to(second));

  civil_minute minute(second);
  EXPECT_EQ("2015-01-02T03:04", str_printf_to(minute));

  civil_hour hour(minute);
  EXPECT_EQ("2015-01-02T03", str_printf_to(hour));

  civil_day day(hour);
  EXPECT_EQ("2015-01-02", str_printf_to(day));

  civil_month month(day);
  EXPECT_EQ("2015-01", str_printf_to(month));

  civil_year year(month);
  EXPECT_EQ("2015", str_printf_to(year));
  ...
  day = civil_day(month);
  EXPECT_EQ("2015-01-01", str_printf_to(day));

  hour = civil_hour(day);
  EXPECT_EQ("2015-01-01T00", str_printf_to(hour));

  minute = civil_minute(hour);
  EXPECT_EQ("2015-01-01T00:00", str_printf_to(minute));

  second = civil_second(minute);
  EXPECT_EQ("2015-01-01T00:00:00", str_printf_to(second));
}
...
TEST(CivilTime, ValueSemantics) {
  ...
  civil_hour d;
  d = c;
  EXPECT_EQ("2015-01-02T03", str_printf_to(d));
}
...
TEST(CivilTime, Arithmetic) {
  civil_second second(2015, 1, 2, 3, 4, 5);
  EXPECT_EQ("2015-01-02T03:04:06", str_printf_to(second += 1));
  EXPECT_EQ("2015-01-02T03:04:07", str_printf_to(second + 1));
  EXPECT_EQ("2015-01-02T03:04:08", str_printf_to(2 + second));
  EXPECT_EQ("2015-01-02T03:04:05", str_printf_to(second - 1));
  EXPECT_EQ("2015-01-02T03:04:05", str_printf_to(second -= 1));
  EXPECT_EQ("2015-01-02T03:04:05", str_printf_to(second++));
  EXPECT_EQ("2015-01-02T03:04:07", str_printf_to(++second));
  EXPECT_EQ("2015-01-02T03:04:07", str_printf_to(second--));
  EXPECT_EQ("2015-01-02T03:04:05", str_printf_to(--second));

  civil_minute minute(2015, 1, 2, 3, 4);
  EXPECT_EQ("2015-01-02T03:05", str_printf_to(minute += 1));
  EXPECT_EQ("2015-01-02T03:06", str_printf_to(minute + 1));
  EXPECT_EQ("2015-01-02T03:07", str_printf_to(2 + minute));
  EXPECT_EQ("2015-01-02T03:04", str_printf_to(minute - 1));
  EXPECT_EQ("2015-01-02T03:04", str_printf_to(minute -= 1));
  EXPECT_EQ("2015-01-02T03:04", str_printf_to(minute++));
  EXPECT_EQ("2015-01-02T03:06", str_printf_to(++minute));
  EXPECT_EQ("2015-01-02T03:06", str_printf_to(minute--));
  EXPECT_EQ("2015-01-02T03:04", str_printf_to(--minute));

  civil_hour hour(2015, 1, 2, 3);
  EXPECT_EQ("2015-01-02T04", str_printf_to(hour += 1));
  EXPECT_EQ("2015-01-02T05", str_printf_to(hour + 1));
  EXPECT_EQ("2015-01-02T06", str_printf_to(2 + hour));
  EXPECT_EQ("2015-01-02T03", str_printf_to(hour - 1));
  EXPECT_EQ("2015-01-02T03", str_printf_to(hour -= 1));
  EXPECT_EQ("2015-01-02T03", str_printf_to(hour++));
  EXPECT_EQ("2015-01-02T05", str_printf_to(++hour));
  EXPECT_EQ("2015-01-02T05", str_printf_to(hour--));
  EXPECT_EQ("2015-01-02T03", str_printf_to(--hour));

  civil_day day(2015, 1, 2);
  EXPECT_EQ("2015-01-03", str_printf_to(day += 1));
  EXPECT_EQ("2015-01-04", str_printf_to(day + 1));
  EXPECT_EQ("2015-01-05", str_printf_to(2 + day));
  EXPECT_EQ("2015-01-02", str_printf_to(day - 1));
  EXPECT_EQ("2015-01-02", str_printf_to(day -= 1));
  EXPECT_EQ("2015-01-02", str_printf_to(day++));
  EXPECT_EQ("2015-01-04", str_printf_to(++day));
  EXPECT_EQ("2015-01-04", str_printf_to(day--));
  EXPECT_EQ("2015-01-02", str_printf_to(--day));

  civil_month month(2015, 1);
  EXPECT_EQ("2015-02", str_printf_to(month += 1));
  EXPECT_EQ("2015-03", str_printf_to(month + 1));
  EXPECT_EQ("2015-04", str_printf_to(2 + month));
  EXPECT_EQ("2015-01", str_printf_to(month - 1));
  EXPECT_EQ("2015-01", str_printf_to(month -= 1));
  EXPECT_EQ("2015-01", str_printf_to(month++));
  EXPECT_EQ("2015-03", str_printf_to(++month));
  EXPECT_EQ("2015-03", str_printf_to(month--));
  EXPECT_EQ("2015-01", str_printf_to(--month));

  civil_year year(2015);
  EXPECT_EQ("2016", str_printf_to(year += 1));
  EXPECT_EQ("2017", str_printf_to(year + 1));
  EXPECT_EQ("2018", str_printf_to(2 + year));
  EXPECT_EQ("2015", str_printf_to(year - 1));
  EXPECT_EQ("2015", str_printf_to(year -= 1));
  EXPECT_EQ("2015", str_printf_to(year++));
  EXPECT_EQ("2017", str_printf_to(++year));
  EXPECT_EQ("2017", str_printf_to(year--));
  EXPECT_EQ("2015", str_printf_to(--year));
}
...
TEST(CivilTime, ArithmeticLimits) {
  ...
  second += kIntMax;
  EXPECT_EQ("2038-01-19T03:14:07", str_printf_to(second));
  second -= kIntMax;
  EXPECT_EQ("1970-01-01T00:00:00", str_printf_to(second));
  second += kIntMin;
  EXPECT_EQ("1901-12-13T20:45:52", str_printf_to(second));
  second -= kIntMin;
  EXPECT_EQ("1970-01-01T00:00:00", str_printf_to(second));

  civil_minute minute(1970, 1, 1, 0, 0);
  minute += kIntMax;
  EXPECT_EQ("6053-01-23T02:07", str_printf_to(minute));
  minute -= kIntMax;
  EXPECT_EQ("1970-01-01T00:00", str_printf_to(minute));
  minute += kIntMin;
  EXPECT_EQ("-2114-12-08T21:52", str_printf_to(minute));
  minute -= kIntMin;
  EXPECT_EQ("1970-01-01T00:00", str_printf_to(minute));

  civil_hour hour(1970, 1, 1, 0);
  hour += kIntMax;
  EXPECT_EQ("246953-10-09T07", str_printf_to(hour));
  hour -= kIntMax;
  EXPECT_EQ("1970-01-01T00", str_printf_to(hour));
  hour += kIntMin;
  EXPECT_EQ("-243014-03-24T16", str_printf_to(hour));
  hour -= kIntMin;
  EXPECT_EQ("1970-01-01T00", str_printf_to(hour));

  civil_day day(1970, 1, 1);
  day += kIntMax;
  EXPECT_EQ("5881580-07-11", str_printf_to(day));
  day -= kIntMax;
  EXPECT_EQ("1970-01-01", str_printf_to(day));
  day += kIntMin;
  EXPECT_EQ("-5877641-06-23", str_printf_to(day));
  day -= kIntMin;
  EXPECT_EQ("1970-01-01", str_printf_to(day));

  civil_month month(1970, 1);
  month += kIntMax;
  EXPECT_EQ("178958940-08", str_printf_to(month));
  month -= kIntMax;
  EXPECT_EQ("1970-01", str_printf_to(month));
  month += kIntMin;
  EXPECT_EQ("-178955001-05", str_printf_to(month));
  month -= kIntMin;
  EXPECT_EQ("1970-01", str_printf_to(month));

  civil_year year(0);
  year += kIntMax;
  EXPECT_EQ("2147483647", str_printf_to(year));
  year -= kIntMax;
  EXPECT_EQ("0", str_printf_to(year));
  year += kIntMin;
  EXPECT_EQ("-2147483648", str_printf_to(year));
  year -= kIntMin;
  EXPECT_EQ("0", str_printf_to(year));
}
...
TEST(CivilTime, OutputStream) {
  ...
  EXPECT_EQ("2016", str_printf_to(civil_year(2016)));
  EXPECT_EQ("123", str_printf_to(civil_year(123)));
  EXPECT_EQ("0", str_printf_to(civil_year(0)));
  EXPECT_EQ("-1", str_printf_to(civil_year(-1)));

  // Tests formatting of sub-year types, which pad to 2 digits
  EXPECT_EQ("2016-02", str_printf_to(civil_month(2016, 2)));
  EXPECT_EQ("2016-02-03", str_printf_to(civil_day(2016, 2, 3)));
  EXPECT_EQ("2016-02-03T04", str_printf_to(civil_hour(2016, 2, 3, 4)));
  EXPECT_EQ("2016-02-03T04:05", str_printf_to(civil_minute(2016, 2, 3, 4, 5)));
  EXPECT_EQ("2016-02-03T04:05:06", str_printf_to(civil_second(2016, 2, 3, 4, 5, 6)));

  // Tests formatting of weekday.
  EXPECT_EQ("Monday", str_printf_to(weekday::monday));
  EXPECT_EQ("Tuesday", str_printf_to(weekday::tuesday));
  EXPECT_EQ("Wednesday", str_printf_to(weekday::wednesday));
  EXPECT_EQ("Thursday", str_printf_to(weekday::thursday));
  EXPECT_EQ("Friday", str_printf_to(weekday::friday));
  EXPECT_EQ("Saturday", str_printf_to(weekday::saturday));
  EXPECT_EQ("Sunday", str_printf_to(weekday::sunday));
}
...
TEST(CivilTime, NormalizeWithHugeYear) {
  ...
  EXPECT_EQ("9223372036854775807-01", str_printf_to(c));
  c = c - 1;  // Causes normalization
  EXPECT_EQ("9223372036854775806-12", str_printf_to(c));

  c = civil_month(-9223372036854775807 - 1, 1);
  EXPECT_EQ("-9223372036854775808-01", str_printf_to(c));
...
TEST(CivilTime, FirstThursdayInMonth) {
  ...
  EXPECT_EQ("2014-11-06", str_printf_to(thursday));

  // Bonus: Date of Thanksgiving in the United States
  // Rule: Fourth Thursday of November
  const civil_day thanksgiving = thursday + 7 * 3;
  EXPECT_EQ("2014-11-27", str_printf_to(thanksgiving));
}
