...
xcli::Option *
add_option(xcli::App &app, std::string name, cx &variable, std::string description = "", bool defaulted = false) {
    xcli::callback_t fun = [&variable](xcli::results_t res) {
        double x = 0, y = 0;
        bool worked = xcli::detail::lexical_cast(res[0], x) && xcli::detail::lexical_cast(res[1], y);
        if(worked)
            variable = cx(x, y);
        return worked;
    };

    xcli::Option *opt = app.add_option(name, fun, description, defaulted);
...
template <> bool lexical_cast<std::complex<double>>(const std::string &input, std::complex<double> &output) {
    ...
    if(m.size() == 9) {
        worked = xcli::detail::lexical_cast(m[1], x) && xcli::detail::lexical_cast(m[6], y);
        if(worked) {
            if(*m[5].first == '-') {
                y = -y;
            }
...
TEST_CASE_METHOD(TApp, "AddingComplexParserDetail", "[complex]") {
    ...
    static_assert(xcli::detail::is_complex<cx>::value, "complex should register as complex in this situation");
    if(!skip_tests) {
        cx comp{0, 0};
...
TEST_CASE_METHOD(TApp, "newComplex", "[complex]") {
    complex_new cval;
    static_assert(xcli::detail::is_complex<complex_new>::value, "complex new does not register as a complex type");
    static_assert(xcli::detail::classify_object<complex_new>::value == xcli::detail::object_category::complex_number,
