...
TEST_CASE_METHOD(TApp, "AddingExistingShortLong", "[creation]") {
    app.add_flag("-c");
    CHECK_THROWS_AS(app.add_flag("--c"), xcli::OptionAlreadyAdded);
}

TEST_CASE_METHOD(TApp, "AddingExistingLongShort", "[creation]") {
    app.add_flag("--c");
    CHECK_THROWS_AS(app.add_option("-c"), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "AddingExistingWithCaseAfter", "[creation]") {
    ...
    CHECK_THROWS_AS(count->ignore_case(), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "AddingExistingWithCaseAfter2", "[creation]") {
    ...
    CHECK_THROWS_AS(cat->ignore_case(), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "AddingExistingWithUnderscoreAfter", "[creation]") {
    ...
    CHECK_THROWS_AS(count->ignore_underscore(), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "matchPositional", "[creation]") {
    ...
    CHECK_THROWS_AS(app.add_option("--firstoption"), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "matchPositionalInOptionGroup2", "[creation]") {
    ...
    app.add_option("firstoption");
    auto *g1 = app.add_option_group("group_b");
    CHECK_THROWS_AS(g1->add_option("--firstoption"), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "matchPositionalInOptionGroup3", "[creation]") {
    ...
    app.add_option("f");
    auto *g1 = app.add_option_group("group_b");
    CHECK_THROWS_AS(g1->add_option("-f"), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "AddingMultipleInfPositionals", "[creation]") {
    ...
    CHECK_THROWS_AS(run(), xcli::InvalidError);
}

TEST_CASE_METHOD(TApp, "AddingMultipleInfPositionalsSubcom", "[creation]") {
    std::vector<std::string> one, two;
    xcli::App *below = app.add_subcommand("below");
...
TEST_CASE_METHOD(TApp, "MultipleSubcomMatching", "[creation]") {
    ...
    app.add_subcommand("Second");
    CHECK_THROWS_AS(app.add_subcommand("first"), xcli::OptionAlreadyAdded);
}

TEST_CASE_METHOD(TApp, "RecoverSubcommands", "[creation]") {
    xcli::App *app1 = app.add_subcommand("app1");
    xcli::App *app2 = app.add_subcommand("app2");
    xcli::App *app3 = app.add_subcommand("app3");
    xcli::App *app4 = app.add_subcommand("app4");

    CHECK(std::vector<xcli::App *>({app1, app2, app3, app4}) == app.get_subcommands({}));
}
...
TEST_CASE_METHOD(TApp, "MultipleSubcomMatchingWithUnderscore", "[creation]") {
    ...
    CHECK_THROWS_AS(app.add_subcommand("firstoption"), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "MultipleSubcomMatchingWithCaseInplace", "[creation]") {
    ...
    CHECK_THROWS_AS(first->ignore_case(), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "MultipleSubcomMatchingWithUnderscoreInplace", "[creation]") {
    ...
    CHECK_THROWS_AS(first->ignore_underscore(), xcli::OptionAlreadyAdded);
}
...
TEST_CASE_METHOD(TApp, "IncorrectConstructionFlagPositional1", "[creation]") {
    ...
    CHECK_THROWS_AS(app.add_flag("cat"), xcli::IncorrectConstruction);
}

TEST_CASE_METHOD(TApp, "IncorrectConstructionFlagPositional2", "[creation]") {
    int x{0};
    CHECK_THROWS_AS(app.add_flag("cat", x), xcli::IncorrectConstruction);
}
...
TEST_CASE_METHOD(TApp, "IncorrectConstructionNeedsCannotFind", "[creation]") {
    ...
    CHECK_THROWS_AS(cat->needs("--nothing"), xcli::IncorrectConstruction);
}

TEST_CASE_METHOD(TApp, "IncorrectConstructionExcludesCannotFind", "[creation]") {
    auto *cat = app.add_flag("--cat");
    CHECK_THROWS_AS(cat->excludes("--nothing"), xcli::IncorrectConstruction);
}
...
TEST_CASE_METHOD(TApp, "OptionFromDefaultsSubcommands", "[creation]") {
    ...
    CHECK(xcli::MultiOptionPolicy::Throw == app.option_defaults()->get_multi_option_policy());
    ...
    app.option_defaults()
        ->required()
        ->multi_option_policy(xcli::MultiOptionPolicy::TakeLast)
        ->ignore_case()
        ->ignore_underscore()
        ->configurable(false)
    ...
    CHECK(xcli::MultiOptionPolicy::TakeLast == app2->option_defaults()->get_multi_option_policy());
...
TEST_CASE_METHOD(TApp, "GetOptionList", "[creation]") {
    ...
    const xcli::App &const_app = app;  // const alias to force use of const-methods
    std::vector<const xcli::Option *> opt_list = const_app.get_options();

    REQUIRE(static_cast<std::size_t>(3) == opt_list.size());
    CHECK(flag == opt_list.at(1));
    CHECK(opt == opt_list.at(2));

    std::vector<xcli::Option *> nonconst_opt_list = app.get_options();
...
TEST_CASE_METHOD(TApp, "GetOptionListFilter", "[creation]") {
    ...
    const xcli::App &const_app = app;  // const alias to force use of const-methods
    std::vector<const xcli::Option *> opt_listc =
        const_app.get_options([](const xcli::Option *opt) { return opt->get_name() == "--one"; });

    REQUIRE(static_cast<std::size_t>(1) == opt_listc.size());
    CHECK(flag == opt_listc.at(0));

    std::vector<xcli::Option *> opt_list =
        app.get_options([](const xcli::Option *opt) { return opt->get_name() == "--one"; });
...
TEST_CASE("ValidatorTests: TestValidatorOps", "[creation]") {
    ...
    xcli::Validator V1(op1, "SIZE >= 5");

    xcli::Validator V2(op2, "SIZE >= 9");
    xcli::Validator V3(op3, "SIZE < 3");
    xcli::Validator V4(op4, "SIZE <= 9");
...
TEST_CASE("ValidatorTests: TestValidatorNegation", "[creation]") {
    ...
    xcli::Validator V1(op1, "SIZE >= 5", "size");
...
static_assert(!xcli::detail::is_istreamable<Unstreamable, std::istream>::value, "Unstreamable type is streamable");
...
static_assert(xcli::detail::is_istreamable<Unstreamable>::value,
