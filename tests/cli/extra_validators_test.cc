...
TEST_CASE("Validators: IPValidate1", "[helpers]") {
    ...
    CHECK(xcli::ValidIPV4(ip).empty());
    ip = "224.255.0.1";
    CHECK(xcli::ValidIPV4(ip).empty());
    // check that it doesn't work with a trailing .
    ip = "224.255.0.1.";
    CHECK_FALSE(xcli::ValidIPV4(ip).empty());
    ip = "-1.255.0.1";
    CHECK_FALSE(xcli::ValidIPV4(ip).empty());
    ip = "1.256.0.1";
    CHECK_FALSE(xcli::ValidIPV4(ip).empty());
...
TEST_CASE("Validators: NumberValidator", "[helpers]") {
    ...
    CHECK_FALSE(xcli::Number(num).empty());
    num = "1.7";
    CHECK(xcli::Number(num).empty());
    num = "10000";
    CHECK(xcli::Number(num).empty());
    num = "-0.000";
    CHECK(xcli::Number(num).empty());
...
TEST_CASE_METHOD(TApp, "PositionalValidation", "[app]") {
    ...
    app.add_option("bar", options)->check(xcli::Number.name("valbar"));
    // disable the check on foo
    app.add_option("foo", foo)->check(xcli::Number.active(false));
...
TEST_CASE_METHOD(TApp, "BoundTests", "[transform]") {
    double value = NAN;
    app.add_option("-s", value)->transform(xcli::Bound(3.4, 5.9));
    args = {"-s", "15"};
    run();
    CHECK(5.9 == value);
...
TEST_CASE_METHOD(TApp, "typeCheck", "[app]") {
    ...
    app.add_option("--one")->check(xcli::TypeValidator<unsigned int>());
...
TEST_CASE_METHOD(TApp, "NumberWithUnitCorrectlySplitNumber", "[transform]") {
    ...
    int value = 0;
    app.add_option("-n", value)->transform(xcli::AsNumberWithUnit(mapping));
...
TEST_CASE_METHOD(TApp, "NumberWithUnitCaseSensitive", "[transform]") {
    ...
    int value{0};
    app.add_option("-n", value)->transform(xcli::AsNumberWithUnit(mapping, xcli::AsNumberWithUnit::CASE_SENSITIVE));
...
TEST_CASE_METHOD(TApp, "NumberWithUnitCaseInsensitive", "[transform]") {
    ...
    int value{0};
    app.add_option("-n", value)->transform(xcli::AsNumberWithUnit(mapping, xcli::AsNumberWithUnit::CASE_INSENSITIVE));
...
TEST_CASE_METHOD(TApp, "NumberWithUnitMandatoryUnit", "[transform]") {
    ...
    app.add_option("-n", value)
        ->transform(xcli::AsNumberWithUnit(
            mapping, xcli::AsNumberWithUnit::UNIT_REQUIRED | xcli::AsNumberWithUnit::CASE_SENSITIVE));
...
TEST_CASE_METHOD(TApp, "NumberWithUnitMandatoryUnit2", "[transform]") {
    ...
    app.add_option("-n", value)
        ->transform(xcli::AsNumberWithUnit(
            mapping, xcli::AsNumberWithUnit::UNIT_REQUIRED | xcli::AsNumberWithUnit::CASE_INSENSITIVE));
...
TEST_CASE_METHOD(TApp, "NumberWithUnitBadMapping", "[transform]") {
    CHECK_THROWS_AS(xcli::AsNumberWithUnit(std::map<std::string, int>{{"a", 10}, {"A", 100}},
                                          xcli::AsNumberWithUnit::CASE_INSENSITIVE),
                    xcli::ValidationError);
    CHECK_THROWS_AS(xcli::AsNumberWithUnit(std::map<std::string, int>{{"a", 10}, {"9", 100}}), xcli::ValidationError);
    CHECK_THROWS_AS(xcli::AsNumberWithUnit(std::map<std::string, int>{{"a", 10}, {"AA A", 100}}), xcli::ValidationError);
    CHECK_THROWS_AS(xcli::AsNumberWithUnit(std::map<std::string, int>{{"a", 10}, {"", 100}}), xcli::ValidationError);
}
...
TEST_CASE_METHOD(TApp, "AsSizeValue1000_1024", "[transform]") {
    ...
    app.add_option("-s", value)->transform(xcli::AsSizeValue(true));
...
TEST_CASE_METHOD(TApp, "AsSizeValue1024", "[transform]") {
    std::uint64_t value{0};
    app.add_option("-s", value)->transform(xcli::AsSizeValue(false));
...
TEST_CASE_METHOD(TApp, "FileExistsForRead", "[validate]") {
    ...
    CHECK(!xcli::ReadPermissions(myfile).empty());

    bool ok = static_cast<bool>(std::ofstream(myfile.c_str()).put('a'));  // create file
    CHECK(ok);

    std::string filename = "Failed";
    app.add_option("--file", filename)->check(xcli::ReadPermissions);
    args = {"--file", myfile};
...
TEST_CASE_METHOD(TApp, "FileExistsForWrite", "[validate]") {
    ...
    CHECK(!xcli::WritePermissions(myfile).empty());

    bool ok = static_cast<bool>(std::ofstream(myfile.c_str()).put('a'));  // create file
    CHECK(ok);

    std::string filename = "Failed";
    app.add_option("--file", filename)->check(xcli::WritePermissions);
    args = {"--file", myfile};
...
TEST_CASE_METHOD(TApp, "FileExistsForExec", "[validate]") {
    ...
    CHECK(!xcli::ExecPermissions(myfile).empty());

    bool ok = static_cast<bool>(std::ofstream(myfile.c_str()).put('a'));  // create file
    CHECK(ok);

    std::string filename = "Failed";
    app.add_option("--file", filename)->check(xcli::ExecPermissions);
    args = {"--file", myfile};
...
TEST_CASE_METHOD(TApp, "noPermissionCheck", "[validate]") {
    ...
    CHECK(!xcli::detail::PermissionValidator(xcli::detail::Permission::none)(myfile).empty());

    bool ok = static_cast<bool>(std::ofstream(myfile.c_str()).put('a'));  // create file
    CHECK(ok);

    std::string filename = "Failed";
    app.add_option("--file", filename)->check(xcli::detail::PermissionValidator(xcli::detail::Permission::none));
    args = {"--file", myfile};
...
TEST_CASE_METHOD(TApp, "FileSizeValidatorMinOnly", "[validate]") {
    ...
    xcli::FileSizeValidator minValidator(50);
...
TEST_CASE_METHOD(TApp, "FileSizeValidatorRange", "[validate]") {
    ...
    xcli::FileSizeValidator rangeValidator(10, 200);
...
TEST_CASE_METHOD(TApp, "NonEmptyFile", "[validate]") {
    ...
    // Test NonEmptyFile
    CHECK(!xcli::NonEmptyFile(emptyfile).empty());    // empty file fails
    CHECK(xcli::NonEmptyFile(nonemptyfile).empty());  // non-empty file passes

    // Test with app
    std::string filename = "Failed";
    app.add_option("--file", filename)->check(xcli::NonEmptyFile);
    ...
    CHECK(!xcli::NonEmptyFile(nonExistingFile).empty());
}
