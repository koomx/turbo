...
class SimpleFormatter : public xcli::FormatterBase {
  public:
    SimpleFormatter() : FormatterBase() {}

    std::string make_help(const xcli::App *, std::string, xcli::AppFormatMode) const override {
...
TEST_CASE("Formatter: NothingLambda", "[formatter]") {
    ...
    app.formatter_fn(
        [](const xcli::App *, std::string, xcli::AppFormatMode) { return std::string("This is really simple"); });
...
TEST_CASE("Formatter: OptCustomize", "[formatter]") {
    ...
    auto optfmt = std::make_shared<xcli::Formatter>();
...
TEST_CASE("Formatter: AppCustomize", "[formatter]") {
    ...
    auto appfmt = std::make_shared<xcli::Formatter>();
...
TEST_CASE("Formatter: AllSub", "[formatter]") {
    xcli::App app{"My prog"};
    xcli::App *sub = app.add_subcommand("subcom", "This");
...
TEST_CASE("Formatter: NamelessSub", "[formatter]") {
    xcli::App app{"My prog"};
    xcli::App *sub = app.add_subcommand("", "This subcommand");
    sub->add_flag("--insub", "MyFlag");

    std::string help = app.help("", xcli::AppFormatMode::Normal);
    CHECK_THAT(help, Contains("--insub"));
...
TEST_CASE("Formatter: NamelessSubInGroup", "[formatter]") {
    xcli::App app{"My prog"};
    xcli::App *sub = app.add_subcommand("", "This subcommand");
    xcli::App *sub2 = app.add_subcommand("sub2", "subcommand2");
    sub->add_flag("--insub", "MyFlag");
    int val{0};
...
TEST_CASE("Formatter: Footer", "[formatter]") {
    ...
    help = app.help("", xcli::AppFormatMode::Sub);
    CHECK_THAT(help, Contains("is       a"));
    CHECK_THAT(help, Contains("to  Pr e  s  e  r  v  e  SPA C ES"));
    CHECK_THAT(help, Contains(footer_string));
    ...
    CHECK(app.get_formatter()->is_footer_paragraph_formatting_enabled());
    help = app.help("", xcli::AppFormatMode::Normal);
    CHECK_THAT(help, !Contains("is       a"));
    CHECK_THAT(help, !Contains("to  Pr e  s  e  r  v  e  SPA C ES"));
    CHECK_THAT(help, !Contains(footer_string));
