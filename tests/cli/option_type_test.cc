...
TEST_CASE_METHOD(TApp, "atomic_bool_flags", "[optiontype]") {
    ...
    app.add_flag("-b", bflag);
    app.add_flag("-i,--int", iflag)->multi_option_policy(xcli::MultiOptionPolicy::Sum);
...
TEST_CASE_METHOD(TApp, "CallbackBoolFlags", "[optiontype]") {
    ...
    CHECK_THROWS_AS(app.add_flag_callback("hi", func), xcli::IncorrectConstruction);
    cback->multi_option_policy(xcli::MultiOptionPolicy::Throw);
    args = {"--val", "--val=false"};
...
TEST_CASE_METHOD(TApp, "pair_check", "[optiontype]") {
    ...
    auto v0 = xcli::ExistingFile;
    v0.application_index(0);
    auto v1 = xcli::PositiveNumber;
...
TEST_CASE_METHOD(TApp, "pair_check_take_first", "[optiontype]") {
    ...
    auto *opt = app.add_option("--file", findex)->check(xcli::ExistingFile)->check(xcli::PositiveNumber);
    CHECK_THROWS_AS(opt->get_validator(3), xcli::OptionNotFound);
    opt->get_validator(0)->application_index(0);
    opt->get_validator(1)->application_index(1);
    opt->multi_option_policy(xcli::MultiOptionPolicy::TakeLast);
    ...
    opt->multi_option_policy(xcli::MultiOptionPolicy::TakeFirst);
...
TEST_CASE_METHOD(TApp, "VectorFixedString", "[optiontype]") {
    ...
    xcli::Option *opt = app.add_option("-s,--string", strvec)->expected(3);
...
TEST_CASE_METHOD(TApp, "VectorDefaultedFixedString", "[optiontype]") {
    ...
    xcli::Option *opt = app.add_option("-s,--string", strvec, "")->expected(3)->capture_default_str();
    CHECK(opt->get_expected() == 3);
...
TEST_CASE_METHOD(TApp, "VectorIndexedValidator", "[optiontype]") {
    ...
    xcli::Option *opt = app.add_option("-v", vvec);

    args = {"-v", "1", "-1", "-v", "3", "-v", "-976"};
    run();
    CHECK(app.count("-v") == 4u);
    CHECK(vvec.size() == 4u);
    opt->check(xcli::PositiveNumber.application_index(0));
    opt->check((!xcli::PositiveNumber).application_index(1));
    CHECK_NOTHROW(run());
    CHECK(vvec.size() == 4u);
    // v[3] would be negative
    opt->check(xcli::PositiveNumber.application_index(3));
    CHECK_THROWS_AS(run(), xcli::ValidationError);
}
...
TEST_CASE_METHOD(TApp, "VectorUnlimString", "[optiontype]") {
    ...
    xcli::Option *opt = app.add_option("-s,--string", strvec);
    CHECK(opt->get_expected() == 1);
    CHECK(opt->get_expected_max() == xcli::detail::expected_max_vector_size);
...
TEST_CASE_METHOD(TApp, "AsStringView", "[app]") {
    app.add_option("--input", "input option")->default_val("optA")->check(xcli::IsMember({"optA", "optB", "optC"}));
...
TEST_CASE_METHOD(TApp, "VectorExpectedRange", "[optiontype]") {
    ...
    xcli::Option *opt = app.add_option("--string", strvec);
    opt->expected(2, 4)->multi_option_policy(xcli::MultiOptionPolicy::Throw);
...
TEST_CASE_METHOD(TApp, "VectorFancyOpts", "[optiontype]") {
    ...
    xcli::Option *opt = app.add_option("-s,--string", strvec)->required()->expected(3);
    CHECK(opt->get_expected() == 3);
    ...
    CHECK_THROWS_AS(run(), xcli::ParseError);
}
...
TEST_CASE_METHOD(TApp, "ArrayTriple", "[optiontype]") {
    ...
    CHECK(!xcli::detail::is_ostreamable<TY>::value);
    auto ts = std::tuple_size<typename std::decay<TY>::type>::value;
    CHECK(ts == 3);

    auto vb = xcli::detail::type_count_base<TY>::value;
    CHECK(vb >= 2);
    CHECK(!xcli::detail::is_complex<TY>::value);
    CHECK(xcli::detail::is_tuple_like<TY>::value);
}
...
TEST_CASE_METHOD(TApp, "vectorVector", "[optiontype]") {
    ...
    opt->check(xcli::PositiveNumber.application_index(9));
...
TEST_CASE_METHOD(TApp, "vectorVectorFixedSize", "[optiontype]") {
    ...
    opt->type_size(xcli::detail::expected_max_vector_size);
    opt->type_size(1, xcli::detail::expected_max_vector_size);
    CHECK_NOTHROW(run());
    REQUIRE(2U == custom_opt.size());
}
...
static_assert(xcli::detail::is_mutable_container<std::set<std::string>>::value, "set should be a container");
static_assert(xcli::detail::is_mutable_container<std::map<std::string, std::string>>::value,
              "map should be a container");
static_assert(xcli::detail::is_mutable_container<std::unordered_map<std::string, double>>::value,
              "unordered_map should be a container");

static_assert(xcli::detail::is_mutable_container<std::list<std::pair<int, std::string>>>::value,
              "list should be a container");

static_assert(xcli::detail::type_count<std::set<std::string>>::value == 1, "set should have a type size of 1");
static_assert(xcli::detail::type_count<std::set<std::tuple<std::string, int, int>>>::value == 3,
              "tuple set should have size of 3");
static_assert(xcli::detail::type_count<std::map<std::string, std::string>>::value == 2,
              "map should have a type size of 2");
static_assert(xcli::detail::type_count<std::unordered_map<std::string, double>>::value == 2,
              "unordered_map should have a type size of 2");

static_assert(xcli::detail::type_count<std::list<std::pair<int, std::string>>>::value == 2,
              "list<int,string> should have a type size of 2");
static_assert(xcli::detail::type_count<std::map<std::string, std::pair<int, std::string>>>::value == 3,
...
TEMPLATE_TEST_CASE("Container int single",
    ...
    xcli::Option *opt = tapp.app.add_option("-v", cv);
    ...
    // v[3] would be negative
    opt->check(xcli::PositiveNumber.application_index(3));
    CHECK_THROWS_AS(tapp.run(), xcli::ValidationError);
}
...
TEST_CASE_METHOD(TApp, "JoinDefaultStrResults", "[optiontype]") {
    ...
    auto *opt = app.add_option("--str", strs)
                    ->multi_option_policy(xcli::MultiOptionPolicy::Join)
                    ->expected(2)
                    ->default_str("[a,b]");
...
TEST_CASE_METHOD(TApp, "SumLargeInteger", "[optiontype]") {
    ...
    std::int64_t val{0};
    app.add_option("--s", val)->multi_option_policy(xcli::MultiOptionPolicy::Sum);
