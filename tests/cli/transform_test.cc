...
TEST_CASE_METHOD(TApp, "SimpleTransform", "[transform]") {
    int value{0};
    auto *opt = app.add_option("-s", value)->transform(xcli::Transformer({{"one", std::string("1")}}));
    args = {"-s", "one"};
    run();
...
TEST_CASE_METHOD(TApp, "SimpleTransformInitList", "[transform]") {
    int value{0};
    auto *opt = app.add_option("-s", value)->transform(xcli::Transformer({{"one", "1"}}));
    args = {"-s", "one"};
    run();
    CHECK(app.count("-s") == 1u);
    CHECK(opt->count() == 1u);
...
TEST_CASE_METHOD(TApp, "SimpleNumericalTransform", "[transform]") {
    int value{0};
    auto *opt = app.add_option("-s", value)->transform(xcli::Transformer(xcli::TransformPairs<int>{{"one", 1}}));
    args = {"-s", "one"};
    run();
    CHECK(app.count("-s") == 1u);
    CHECK(opt->count() == 1u);
...
TEST_CASE_METHOD(TApp, "EnumTransform", "[transform]") {
    ...
    auto *opt = app.add_option("-s", value)
                    ->transform(xcli::Transformer(xcli::TransformPairs<test_cli>{
...
TEST_CASE_METHOD(TApp, "EnumCheckedTransform", "[transform]") {
    ...
    auto *opt = app.add_option("-s", value)
                    ->transform(xcli::CheckedTransformer(xcli::TransformPairs<test_cli>{
                        {"val1", test_cli::val1}, {"val2", test_cli::val2}, {"val3", test_cli::val3}}));
    args = {"-s", "val1"};
    run();
    CHECK(app.count("-s") == 1u);
...
TEST_CASE_METHOD(TApp, "EnumCheckedTransformUint8", "[transform]") {
    ...
    app.add_option("-f,--foo", type, "FooType")
        ->transform(xcli::CheckedTransformer(foo_map, xcli::ignore_case))
...
TEST_CASE_METHOD(TApp, "EnumCheckedDefaultTransform", "[transform]") {
    ...
    app.add_option("--existing", "What to do if file already exists in the destination")
        ->transform(
            xcli::CheckedTransformer(std::unordered_map<std::string, existing>{{"abort", existing::abort},
...
TEST_CASE_METHOD(TApp, "EnumCheckedDefaultTransformCallback", "[transform]") {
    enum class existing : std::int16_t { abort, overwrite, remove };
    auto cmd = std::make_shared<xcli::App>("deploys the repository somewhere", "deploy");
...
TEST_CASE_METHOD(TApp, "SimpleTransformFn", "[transform]") {
    int value{0};
    auto *opt = app.add_option("-s", value)->transform(xcli::Transformer({{"one", "1"}}, xcli::ignore_case));
    args = {"-s", "ONE"};
    run();
    CHECK(app.count("-s") == 1u);
    CHECK(opt->count() == 1u);
...
TEST_CASE_METHOD(TApp, "StringViewTransformFn", "[transform]") {
    ...
    app.add_option("-s", value)->transform(xcli::CheckedTransformer(map));
...
TEST_CASE_METHOD(TApp, "SimpleNumericalTransformFn", "[transform]") {
    ...
    auto *opt =
        app.add_option("-s", value)
            ->transform(xcli::Transformer(std::vector<std::pair<std::string, int>>{{"one", 1}}, xcli::ignore_case));
    args = {"-s", "ONe"};
    run();
    CHECK(app.count("-s") == 1u);
    CHECK(opt->count() == 1u);
...
TEST_CASE_METHOD(TApp, "SimpleNumericalTransformFnVector", "[transform]") {
    ...
    int value{0};
    auto *opt = app.add_option("-s", value)->transform(xcli::Transformer(conversions, xcli::ignore_case));
    args = {"-s", "ONe"};
    run();
    CHECK(app.count("-s") == 1u);
    CHECK(opt->count() == 1u);
...
TEST_CASE_METHOD(TApp, "SimpleNumericalTransformFnconstexprArray", "[transform]") {
    ...
    int value{0};
    auto *opt = app.add_option("-s", value)->transform(xcli::Transformer(&conversions_c, xcli::ignore_case));
    args = {"-s", "ONe"};
    run();
    CHECK(app.count("-s") == 1u);
    CHECK(opt->count() == 1u);
...
TEST_CASE_METHOD(TApp, "EnumTransformFn", "[transform]") {
    ...
    test_cli value{test_cli::val2};
    auto *opt = app.add_option("-s", value)
                    ->transform(xcli::Transformer(xcli::TransformPairs<test_cli>{{"val1", test_cli::val1},
                                                                               {"val2", test_cli::val2},
                                                                               {"val3", test_cli::val3}},
                                                 xcli::ignore_case,
                                                 xcli::ignore_underscore));
    args = {"-s", "val_1"};
    run();
    CHECK(app.count("-s") == 1u);
    CHECK(opt->count() == 1u);
...
TEST_CASE_METHOD(TApp, "EnumTransformFnMap", "[transform]") {
    ...
    auto *opt = app.add_option("-s", value)->transform(xcli::Transformer(map, xcli::ignore_case, xcli::ignore_underscore));
    args = {"-s", "val_1"};
    run();
    CHECK(app.count("-s") == 1u);
    CHECK(opt->count() == 1u);
...
TEST_CASE_METHOD(TApp, "EnumTransformFnPtrMap", "[transform]") {
    ...
    test_cli value{test_cli::val2};
    auto *opt =
        app.add_option("-s", value)->transform(xcli::Transformer(&map, xcli::ignore_case, xcli::ignore_underscore));
    args = {"-s", "val_1"};
    run();
    CHECK(app.count("-s") == 1u);
    CHECK(opt->count() == 1u);
...
TEST_CASE_METHOD(TApp, "TransformCascade", "[transform]") {
    ...
    opt->transform(xcli::Transformer({{"abc", "abcd"}, {"bbc", "bbcd"}, {"cbc", "cbcd"}}, xcli::ignore_case));
    opt->transform(
        xcli::Transformer({{"ab", "abc"}, {"bc", "bbc"}, {"cb", "cbc"}}, xcli::ignore_case, xcli::ignore_underscore));
    opt->transform(xcli::Transformer({{"a", "ab"}, {"b", "bb"}, {"c", "cb"}}, xcli::ignore_case));
    opt->check(xcli::IsMember({"abcd", "bbcd", "cbcd"}));
    args = {"-s", "abcd"};
    run();
    CHECK("abcd" == output);
...
TEST_CASE_METHOD(TApp, "TransformCascadeDeactivate", "[transform]") {
    ...
    auto *opt = app.add_option("-s", output);
    opt->transform(
        xcli::Transformer({{"abc", "abcd"}, {"bbc", "bbcd"}, {"cbc", "cbcd"}}, xcli::ignore_case).name("tform1"));
    opt->transform(
        xcli::Transformer({{"ab", "abc"}, {"bc", "bbc"}, {"cb", "cbc"}}, xcli::ignore_case, xcli::ignore_underscore)
            .name("tform2")
            .active(false));
    opt->transform(xcli::Transformer({{"a", "ab"}, {"b", "bb"}, {"c", "cb"}}, xcli::ignore_case).name("tform3"));
    opt->check(xcli::IsMember({"abcd", "bbcd", "cbcd"}).name("check"));
    args = {"-s", "abcd"};
    run();
    CHECK("abcd" == output);
    ...
    CHECK_THROWS_AS(opt->get_validator("sdfsdf"), xcli::OptionNotFound);
}
...
TEST_CASE_METHOD(TApp, "TransformEmptyNameAndDescription", "[transform]") {
    ...
    auto *opt = app.add_option("-n,--number", result_number, "My number")
                    ->transform(xcli::Transformer(numbers_type_map), "", "");
...
TEST_CASE_METHOD(TApp, "TransformEmptyDescription", "[transform]") {
    ...
    auto *opt = app.add_option("-n,--number", result_number, "My number")
                    ->transform(xcli::Transformer(numbers_type_map), "", "my number transform");
...
TEST_CASE_METHOD(TApp, "IntTransformFn", "[transform]") {
    ...
    app.add_option("-s", value)
        ->transform(
            xcli::CheckedTransformer(std::map<int, int>{{15, 5}, {18, 6}, {21, 7}}, [](int in) { return in - 10; }));
    args = {"-s", "25"};
    run();
    CHECK("5" == value);
...
TEST_CASE_METHOD(TApp, "IntTransformNonConvertible", "[transform]") {
    std::string value;
    app.add_option("-s", value)->transform(xcli::Transformer(std::map<int, int>{{15, 5}, {18, 6}, {21, 7}}));
    args = {"-s", "15"};
    run();
    CHECK("5" == value);
...
TEST_CASE_METHOD(TApp, "IntTransformNonMerge", "[transform]") {
    std::string value;
    app.add_option("-s", value)
        ->transform(xcli::Transformer(std::map<int, int>{{15, 5}, {18, 6}, {21, 7}}) &
                        xcli::Transformer(std::map<int, int>{{25, 5}, {28, 6}, {31, 7}}),
                    "merge");
    args = {"-s", "15"};
    run();
    CHECK("5" == value);
...
TEST_CASE_METHOD(TApp, "IntTransformMergeWithCustomSharedValidator", "[transform]") {
    ...
    xcli::Validator_p ctrans = std::make_shared<xcli::Validator>(
        xcli::Transformer(std::map<int, int>{{15, 5}, {18, 6}, {21, 7}}) | xcli::Validator(
                                                                              [](std::string &element) {
                                                                                  if(element == "frog") {
                                                                                      element = "hops";
                                                                                  }
...
TEST_CASE_METHOD(TApp, "StringEscapeValid", "[transform]") {
    ...
    app.add_option("-n", value)->transform(xcli::EscapedString);
