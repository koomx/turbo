...
void BtreeTest() {
  ...
  const std::vector<V> random_values = GenerateValuesWithSeed<V>(
      turbo::GetFlag(FLAGS_test_values), 4 * turbo::GetFlag(FLAGS_test_values),
...
TEST(Btree, set_cord) { SetTest<turbo::Cord>(); }
TEST(Btree, map_int32) { MapTest<int32_t>(); }
TEST(Btree, map_string) { MapTest<std::string>(); }
TEST(Btree, map_cord) { MapTest<turbo::Cord>(); }
...
TEST(Btree, multiset_cord) { MultiSetTest<turbo::Cord>(); }
TEST(Btree, multimap_int32) { MultiMapTest<int32_t>(); }
TEST(Btree, multimap_string) { MultiMapTest<std::string>(); }
TEST(Btree, multimap_cord) { MultiMapTest<turbo::Cord>(); }
...
class BtreeMapTest : public ::testing::Test {
 public:
  ...
  static bool IsLinear() {
    return BtreeNodePeer::UsesLinearNodeSearch<turbo::btree_map<K, int, C>>();
  }
};
...
TEST(Btree, InitializerListConstructor) {
  ...
  turbo::btree_map<int, int> map({{1, 5}, {2, 10}});
  EXPECT_EQ(map[1], 5);
  EXPECT_EQ(map[2], 10);

  turbo::btree_multimap<int, int> mmap({{1, 5}, {1, 10}});
...
TEST(Btree, MovesComparisonsCopiesSwapsTracking) {
  ...
  if (sizeof(void *) == 8) {
    EXPECT_EQ(BtreeNodePeer::GetNumSlotsPerNode<turbo::btree_set<int32_t>>(),
...
TEST(Btree, MapAt) {
  turbo::btree_map<int, int> map = {{1, 2}, {2, 4}};
...
TEST(Btree, BtreeMultimapEmplace) {
  ...
  const char value0[] = "a";
  turbo::btree_multimap<int, std::string> m;
...
TEST(Btree, MapValueCompProtected) {
  ...
  using value_compare = turbo::btree_map<int, int, key_compare>::value_compare;
...
TEST(Btree, ExtractAndInsertNodeHandleMap) {
  turbo::btree_map<int, int> src1 = {{1, 2}, {3, 4}, {5, 6}};
  auto nh = src1.extract(src1.find(3));
...
TEST(Btree, ExtractAndInsertNodeHandleMultiMap) {
  turbo::btree_multimap<int, int> src1 = {{1, 2}, {3, 4}, {5, 6}};
  auto nh = src1.extract(src1.find(3));
  EXPECT_THAT(src1, ElementsAre(Pair(1, 2), Pair(5, 6)));
...
TEST(Btree, ExtractAndGetNextMultiSet) {
  turbo::btree_multiset<int> src = {1, 2, 3, 4, 5};
  auto it = src.find(3);
  auto extracted_and_next = src.extract_and_get_next(it);
  EXPECT_THAT(src, ElementsAre(1, 2, 4, 5));
  EXPECT_EQ(extracted_and_next.node.value(), 3);
...
TEST(Btree, ExtractAndGetNextMap) {
  turbo::btree_map<int, int> src = {{1, 2}, {3, 4}, {5, 6}};
  auto it = src.find(3);
  auto extracted_and_next = src.extract_and_get_next(it);
...
TEST(Btree, ExtractAndGetNextMultiMap) {
  turbo::btree_multimap<int, int> src = {{1, 2}, {3, 4}, {5, 6}};
  auto it = src.find(3);
  auto extracted_and_next = src.extract_and_get_next(it);
  EXPECT_THAT(src, ElementsAre(Pair(1, 2), Pair(5, 6)));
  EXPECT_EQ(extracted_and_next.node.key(), 3);
...
TEST(Btree, ExtractDoesntCauseExtraMoves) {
  ...
  using Set = turbo::btree_set<MovableOnlyInstance>;
...
TEST(Btree, MergeIntoMultiMapsWithDifferentComparators) {
  turbo::btree_map<int, int, IntCompareToCmp> src1 = {{1, 1}, {2, 2}, {3, 3}};
...
TEST(Btree, MergeIntoSetMovableOnly) {
  turbo::btree_set<MovableOnlyInstance> src;
  src.insert(MovableOnlyInstance(1));
  turbo::btree_multiset<MovableOnlyInstance> dst1;
  dst1.insert(MovableOnlyInstance(2));
  turbo::btree_set<MovableOnlyInstance> dst2;
...
TEST(Btree, TryEmplaceWithHintWorks) {
  ...
  turbo::btree_map<int, int, CheckedCompareOptedOutCmp<Cmp>> m(cmp);
  for (int i = 0; i < 128; ++i) {
    m.emplace(i, i);
  }
...
TEST(Btree, TryEmplaceWithBadHint) {
  turbo::btree_map<int, int> m = {{1, 1}, {9, 9}};
...
using BtreeSetAlloc = turbo::btree_set<int, std::less<int>, Alloc>;
...
TEST(Btree, MinimumAlignmentAllocator) {
  turbo::btree_set<int8_t, std::less<int8_t>, MinimumAlignmentAlloc<int8_t>> set;
...
TEST(Btree, EraseIf) {
  ...
  {
    turbo::btree_map<int, int> m = {{1, 1}, {3, 3}, {6, 6}, {100, 100}};
    EXPECT_EQ(
  ...
  {
    turbo::btree_multimap<int, int> m = {{1, 1}, {3, 3}, {3, 6},
                                        {6, 6}, {6, 7}, {100, 6}};
    EXPECT_EQ(
        erase_if(m,
...
TEST(Btree, InsertOrAssign) {
  turbo::btree_map<int, int> m = {{1, 1}, {3, 3}};
...
TEST(Btree, InsertOrAssignMovableOnly) {
  turbo::btree_map<int, MovableOnlyInstance> m;
  using value_type = typename decltype(m)::value_type;
...
TEST(Btree, SetRangeConstructorAndInsertSupportExplicitConversionComparable) {
  ...
  turbo::btree_set<std::string> name_set2;
...
TEST(Btree,
  ...
  turbo::btree_set<ConstructorCounted, ConstructorCountedCompare> set{
...
TEST(Btree,
  ...
  turbo::btree_set<std::vector<void *>> s1{std::begin(i), std::end(i)};
  EXPECT_THAT(s1, ElementsAre(IsEmpty(), ElementsAre(IsNull())));

  turbo::btree_set<std::vector<void *>> s2;
...
TEST(Btree, MapRangeConstructorAndInsertSupportExplicitConversionComparable) {
  ...
  turbo::btree_map<std::string, int> name_map2;
...
TEST(Btree,
  ...
  turbo::btree_map<ConstructorCounted, int, ConstructorCountedCompare> map{
      std::begin(i), std::end(i)};
...
TEST(Btree,
  ...
  turbo::btree_map<std::vector<void *>, int> m1{std::begin(i), std::end(i)};
  EXPECT_THAT(m1,
              ElementsAre(Pair(IsEmpty(), 1), Pair(ElementsAre(IsNull()), 2)));

  turbo::btree_map<std::vector<void *>, int> m2;
...
TEST(Btree, NodeHandleMutableKeyAccess) {
  {
    turbo::btree_map<std::string, std::string> map;
  ...
  // Also for multimap.
  {
    turbo::btree_multimap<std::string, std::string> map;
...
struct MultiKeyThreeWayComp {
  using is_transparent = void;
  turbo::weak_ordering operator()(const MultiKey a, const MultiKey b) const {
    if (a.i1 < b.i1) return turbo::weak_ordering::less;
    if (a.i1 > b.i1) return turbo::weak_ordering::greater;
    if (a.i2 < b.i2) return turbo::weak_ordering::less;
    if (a.i2 > b.i2) return turbo::weak_ordering::greater;
    return turbo::weak_ordering::equivalent;
  }
  turbo::weak_ordering operator()(const int a, const MultiKey b) const {
    if (a < b.i1) return turbo::weak_ordering::less;
    if (a > b.i1) return turbo::weak_ordering::greater;
    return turbo::weak_ordering::equivalent;
  }
  turbo::weak_ordering operator()(const MultiKey a, const int b) const {
    if (a.i1 < b) return turbo::weak_ordering::less;
    if (a.i1 > b) return turbo::weak_ordering::greater;
    return turbo::weak_ordering::equivalent;
  }
};
...
TYPED_TEST(BtreeMultiKeyTest, EqualRange) {
  turbo::btree_set<MultiKey, TypeParam> set;
  for (int i = 0; i < 100; ++i) {
    for (int j = 0; j < 100; ++j) {
      set.insert({i, j});
    }
...
TYPED_TEST(BtreeMultiKeyTest, Count) {
  const turbo::btree_set<MultiKey, TypeParam> set = {
      {1, 1}, {2, 1}, {2, 2}, {3, 1}};
...
TEST(Btree, AllocConstructor) {
  ...
  using Set = turbo::btree_set<int, std::less<int>, Alloc>;
...
TEST(Btree, SupportsFunctionPtrComparator) {
  turbo::btree_set<int, decltype(IntCmp) *> set(IntCmp);
  set.insert({1, 2, 3});
  EXPECT_THAT(set, ElementsAre(1, 2, 3));
  EXPECT_TRUE(set.key_comp()(1, 2));
  EXPECT_TRUE(set.value_comp()(1, 2));

  turbo::btree_map<int, int, decltype(IntCmp) *> map(&IntCmp);
  map[1] = 1;
...
TEST(Btree,
  ...
  turbo::btree_set<MultiKey, TransparentPassThroughComp<MultiKeyComp>> set;
  set.insert(MultiKey{1, 2});
  EXPECT_TRUE(set.contains(1));
}

TEST(Btree, ConstructImplicitlyWithUnadaptedComparator) {
  turbo::btree_set<MultiKey, MultiKeyComp> set = {{}, MultiKeyComp{}};
...
TEST(Btree, InvalidComparatorsCaught) {
...

  {
    ...
    turbo::btree_set<int, ZeroAlwaysLessCmp> set;
    EXPECT_DEATH(set.insert({0, 1, 2}), "is_self_equivalent");
  }
  {
    struct ThreeWayAlwaysLessCmp {
      turbo::weak_ordering operator()(int, int) const {
        return turbo::weak_ordering::less;
      }
    };
    turbo::btree_set<int, ThreeWayAlwaysLessCmp> set;
    EXPECT_DEATH(set.insert({0, 1, 2}), "is_self_equivalent");
  }
  {
    ...
    turbo::btree_set<int, SumGreaterZeroCmp> set;
  ...
  }
  {
    struct ThreeWaySumGreaterZeroCmp {
      turbo::weak_ordering operator()(int lhs, int rhs) const {
        // First, do equivalence correctly - so we can test later condition.
        if (lhs == rhs) return turbo::weak_ordering::equivalent;

        if (lhs + rhs > 0) return turbo::weak_ordering::less;
        if (lhs + rhs == 0) return turbo::weak_ordering::equivalent;
        return turbo::weak_ordering::greater;
      }
    };
    turbo::btree_set<int, ThreeWaySumGreaterZeroCmp> set;
  ...
  ClockTime c = {6, 0};
  {
    ...
    turbo::btree_set<ClockTime, NonTransitiveTimeCmp> set;
    EXPECT_DEATH(set.insert({a, b, c}), "is_ordered_correctly");
    turbo::btree_multiset<ClockTime, NonTransitiveTimeCmp> mset;
    EXPECT_DEATH(mset.insert({a, a, b, b, c, c}), "is_ordered_correctly");
  }
  {
    struct ThreeWayNonTransitiveTimeCmp {
      turbo::weak_ordering operator()(ClockTime lhs, ClockTime rhs) const {
        if (lhs.hour.has_value() && rhs.hour.has_value() &&
            *lhs.hour != *rhs.hour) {
          return *lhs.hour < *rhs.hour ? turbo::weak_ordering::less
                                       : turbo::weak_ordering::greater;
        }
        return lhs.minute < rhs.minute    ? turbo::weak_ordering::less
               : lhs.minute == rhs.minute ? turbo::weak_ordering::equivalent
                                          : turbo::weak_ordering::greater;
      }
    };
    ThreeWayNonTransitiveTimeCmp cmp;
    ASSERT_TRUE(cmp(a, b) < 0 && cmp(b, c) < 0 && cmp(a, c) > 0);
    turbo::btree_set<ClockTime, ThreeWayNonTransitiveTimeCmp> set;
    EXPECT_DEATH(set.insert({a, b, c}), "is_ordered_correctly");
    turbo::btree_multiset<ClockTime, ThreeWayNonTransitiveTimeCmp> mset;
    EXPECT_DEATH(mset.insert({a, a, b, b, c, c}), "is_ordered_correctly");
  }
}
...
TEST(Btree, MutatedKeysCaught) {
  ...
  {
    turbo::btree_set<int *, IntPtrCmp> set;
    int arr[] = {0, 1, 2};
  ...
  {
    turbo::btree_multiset<int *, IntPtrCmp> set;
    int arr[] = {0, 1, 2};
...
TEST(Btree, OnlyConstructibleByAllocatorType) {
  ...
  {
    turbo::btree_set<OnlyConstructibleByAllocator,
  ...
  {
    turbo::btree_multiset<OnlyConstructibleByAllocator,
                         OnlyConstructibleByAllocatorComp,
                         OnlyConstructibleAllocator<>>
        set;
    set.emplace(1);
  ...
  {
    turbo::btree_map<OnlyConstructibleByAllocator, int,
                    OnlyConstructibleByAllocatorComp,
                    OnlyConstructibleAllocator<>>
        map;
    map.emplace(1);
  ...
  {
    turbo::btree_multimap<OnlyConstructibleByAllocator, int,
                         OnlyConstructibleByAllocatorComp,
                         OnlyConstructibleAllocator<>>
        map;
    map.emplace(1);
...
TEST(Btree, NotAssignableType) {
  {
    turbo::btree_set<NotAssignable> set;
    set.emplace(1);
    set.emplace_hint(set.end(), 2);
  ...
  {
    turbo::btree_multiset<NotAssignable> set;
    set.emplace(1);
    set.emplace_hint(set.end(), 2);
  ...
  {
    turbo::btree_map<NotAssignable, int> map;
  ...
  {
    turbo::btree_multimap<NotAssignable, int> map;
    map.emplace(NotAssignable(1), 1);
    map.emplace_hint(map.end(), NotAssignable(2), 2);
...
TEST(Btree, ReusePoisonMemory) {
  ...
  using Set = turbo::btree_set<int64_t, std::less<int64_t>, Alloc>;
  ArenaLike arena;
  Alloc alloc(&arena);
  Set set(alloc);
...
TEST(Btree, IteratorAddition) {
  ...
  for (int i = 0; i < 1000; ++i) {
    int begin = turbo::Uniform(bitgen, 0, kSetSize);
    int end = turbo::Uniform(bitgen, begin, kSetSize);
    ASSERT_LE(begin, end);
...
TEST(Btree, IteratorAdditionOutOfBounds) {
  const turbo::btree_set<int> set({5});
...
TEST(Btree, InvalidIteratorComparison) {
  ...
  turbo::btree_set<int> set1, set2;
  for (int i = 0; i < 1000; ++i) {
    set1.insert(i);
    set2.insert(i);
  }

  typename turbo::btree_set<int>::iterator iter1, iter2;
...
TEST(Btree, FieldTypeEqualsSlotType) {
  ...
  using set_type = turbo::btree_set<uint8_t>;
