...
#include <tests/container/internal/hash_generator_testing.h>
#include <tests/container/internal/hash_policy_testing.h>
#include <tests/container/internal/test_allocator.h>
#include <tests/container/internal/unordered_map_constructor_test.h>
#include <tests/container/internal/unordered_map_lookup_test.h>
#include <tests/container/internal/unordered_map_members_test.h>
#include <tests/container/internal/unordered_map_modifiers_test.h>
...
// Check that turbo::flat_hash_map works in a global constructor.
struct BeforeMain {
  BeforeMain() {
    turbo::flat_hash_map<int, int> x;
    x.insert({1, 1});
    KCHECK(x.find(0) == x.end()) << "x should not contain 0";
    auto it = x.find(1);
    KCHECK(it != x.end()) << "x should contain 1";
    KCHECK(it->second) << "1 should map to 1";
  }
};
...
TEST(FlatHashMap, Relocatability) {
  ...
  static_assert(
      std::is_same_v<
          decltype(turbo::container_internal::FlatHashMapPolicy<
  ...
  EXPECT_TRUE(
      (std::is_same_v<decltype(turbo::container_internal::FlatHashMapPolicy<
                               int, NonRelocatable>::
...
TEST(FlatHashMap, IteratesMsan) {
  ...
  std::vector<turbo::flat_hash_map<int, balast>> garbage;
  for (int i = 0; i < 100; ++i) {
    turbo::flat_hash_map<int, balast> t;
    for (int j = 0; j < 100; ++j) {
      t[j];
...
TEST(FlatHashMap, MergeExtractInsert) {
  ...
  turbo::flat_hash_map<int, int> m = {{1, 7}, {2, 9}};
  ...
  for (int i = 0; i < 100; ++i) {
    {
      ...
      turbo::container_internal::c_for_each_fast(
...
TEST(Iterator, InconsistentHashEqFunctorsValidation) {
  ...
  turbo::flat_hash_map<InconsistentHashEqType, int> m;
...
TEST(FlatHashMap, FromRangeWithHasherAndAllocator) {
  ...
  turbo::flat_hash_map<int, int, TestingHash,
                      turbo::container_internal::hash_default_eq<int>,
                      Alloc<std::pair<const int, int>>>
