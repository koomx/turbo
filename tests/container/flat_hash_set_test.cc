...
struct BeforeMain {
  BeforeMain() {
    ...
    x.insert(1);
    KCHECK(!x.contains(0)) << "x should not contain 0";
    KCHECK(x.contains(1)) << "x should contain 1";
  }
};
...
class PoisonSoo {
 ...
 public:
  ...
  template <typename H>
  friend H TurboHashValue(H h, const PoisonSoo& pi) {
...
class MoveOnlyInt {
 ...
 private:
  template <typename H>
  friend H TurboHashValue(H h, const MoveOnlyInt& m) {
...
TEST(FlatHashSet, FromRange) {
  ...
  turbo::flat_hash_set<int> s(std::from_range, v);
  EXPECT_THAT(s, UnorderedElementsAre(1, 2, 3, 4, 5));
}

TEST(FlatHashSet, FromRangeWithAllocator) {
  std::vector<int> v = {1, 2, 3, 4, 5};
  turbo::flat_hash_set<int, turbo::container_internal::hash_default_hash<int>,
...
TEST(FlatHashSet, FromRangeWithHasherAndAllocator) {
  ...
  turbo::flat_hash_set<int, TestingHash,
                      turbo::container_internal::hash_default_eq<int>,
                      Alloc<int>>
