...
#include <tests/container/internal/hash_policy_testing.h>
...
// Convenience function to static cast to ctrl_t.
            ...
            TYPED_TEST(HashtableDataTest, HashtableInlineDataSeed) {
                constexpr HashtableCapacityStorageMode kMode = TypeParam::value;
                using InlineData = HashtableInlineDataImpl<kMode>;
                using Capacity = HashtableCapacityImpl<kMode>;
                ...
                data.generate_new_seed();
                // Seed is non-deterministic; just ensure generate_new_seed is callable.
                (void) data.seed().seed();
            }
...
#pragma GCC diagnostic pop
                ...
                template<typename H>
                friend H TurboHashValue(H h, SizedValue sv) {
...
// Value is aligned as type T and contains N copies of it.
            ...
            public:
                ...
                template<typename H>
                friend H TurboHashValue(H h, AlignedValue sv) {
                    return H::combine(std::move(h), *sv);
                }
            ...
            class StringPolicy {
                template<class F, class K, class V,
                        class = std::enable_if_t<
                                std::is_convertible_v<const K &, std::string_view>>>
            ...
            struct ChangingSizeAndTrackingTypeAlloc : std::allocator<T> {
                ...
                static size_t ComputeTypeId() { return turbo::HashOf(typeid(T).name()); }
...
// Allows for freezing the allocator to expect no further allocations.
            ...
            TEST(Table, EmptyFunctorOptimization) {
                static_assert(std::is_empty_v<std::equal_to<std::string_view>>, "");
...
// many groups with deleted elements
            ...
            TEST(Table, ReservedTableRehashWithoutGrowthWorksWell) {
                ...
                for (size_t capacity = 31; capacity < 256;
                     capacity = NextCapacity(capacity)) {
                    SCOPED_TRACE(turbo::StrCat("capacity: ", capacity));
            ...
            TYPED_TEST(ExtendedSooTest, InsertWithinCapacity) {
                ...
                for (size_t fn_i = 0; fn_i < reserve_and_insert_fns.size(); ++fn_i) {
                    ...
                    for (int size: {3, 5, 7, 11, 27, 36}) {
                        SCOPED_TRACE(turbo::StrCat("fn_i: ", fn_i, ", size: ", size));
                        TableType t;
                ...
                for (size_t size = 0; size < 32; ++size) {
                    for (bool reserve: {false, true}) {
                        for (bool clear_via_erase: {false, true}) {
                            SCOPED_TRACE(turbo::StrCat("size: ", size, ", reserve: ", reserve,
            ...
            TYPED_TEST(ExtendedSooTest, ReserveTwice) {
                for (int reserve_size = 0; reserve_size < 32; ++reserve_size) {
                    for (int reserve_size2 = reserve_size; reserve_size2 < 32;
                         ++reserve_size2) {
                        SCOPED_TRACE(turbo::StrCat("reserve_size: ", reserve_size,
...
// Timeout if copy is quadratic as it was in Rust. See b/34756399.
            ...
            TYPED_TEST(SooTest, CopyAssignment) {
                ...
                for (size_t source_size: sizes) {
                    for (size_t target_size: sizes) {
                        SCOPED_TRACE(turbo::StrCat("source_size: ", source_size,
            ...
            TYPED_TEST(ExtendedSooTest, CopyDifferentSizesWithReserve) {
                for (size_t size = 0; size < 153; ++size) {
                    SCOPED_TRACE(turbo::StrCat("size: ", size));
                    TypeParam t;
                    t.reserve(size);
                    for (size_t i = 0; i < size; ++i) {
                        ASSERT_TRUE(t.insert(static_cast<int>(i)).second) << i;
            ...
            TYPED_TEST(SooTest, IterationOrderChangesByInstance) {
                for (bool do_reserve: {false, true}) {
                    for (size_t size: {2u, 6u, 12u, 20u}) {
                        SCOPED_TRACE(turbo::StrCat("size: ", size, " do_reserve: ", do_reserve));
                ...
                for (bool do_reserve: {false, true}) {
                    for (size_t size: {2u, 3u, 6u, 7u, 12u, 15u, 20u, 50u}) {
                        for (size_t rehash_size: {
                            ...
                            SCOPED_TRACE(turbo::StrCat("size: ", size, " rehash_size: ", rehash_size,
...
// This prevents dependency on pointer stability on small tables.
            ...
            TYPED_TEST(SooTest, EraseIfAll) {
                ...
                for (int size = 0; size < 100; ++size) {
                    TypeParam t;
                    for (int i = 0; i < size; ++i) t.insert(i);
                    turbo::container_internal::EraseIf(pred, &t);
                    ASSERT_EQ(t.size(), 0);
                }
            }
            ...
            TYPED_TEST(SooTest, EraseIfPartial) {
                for (int mod: {0, 1}) {
                    ...
                    for (int size = 0; size < 100; ++size) {
                        SCOPED_TRACE(turbo::StrCat(mod, " ", size));
                        TypeParam t;
            ...
            TYPED_TEST(SooTest, ForEach) {
                ...
                for (int size = 0; size < 100; ++size) {
                    SCOPED_TRACE(size);
                    {
                        ...
                        turbo::container_internal::ForEach(f, &t);
                    ...
                    }
                    {
                        ...
                        const auto &ct = t;
                        turbo::container_internal::ForEach(f, &ct);
                        ASSERT_THAT(actual, testing::UnorderedElementsAreArray(expected));
                    }
                    t.insert(size);
...
// 3. Collect ProbeStats from final table.
            ...
            ExpectedStats XorSeedExpectedStats() {
                ...
                KLOG(FATAL) << "Unknown Group width";
                return {};
            }
...
// Following two tests use non-SOO table because they test for 0 capacity.
            ...
            TEST(Table, NoThrowMoveConstruct) {
                ASSERT_TRUE(
                        std::is_nothrow_copy_constructible_v<turbo::Hash<std::string_view>>);
                ASSERT_TRUE(
                        std::is_nothrow_copy_constructible_v<std::equal_to<std::string_view>>);
            ...
            TEST(Table, NoThrowMoveAssign) {
                ASSERT_TRUE(std::is_nothrow_move_assignable_v<turbo::Hash<std::string_view>>);
                ASSERT_TRUE(
                        std::is_nothrow_move_assignable_v<std::equal_to<std::string_view>>);
            ...
            TEST(Table, NoThrowSwappable) {
                ASSERT_TRUE(std::is_nothrow_swappable<turbo::Hash<std::string_view>>());
                ASSERT_TRUE(std::is_nothrow_swappable<std::equal_to<std::string_view>>());
            ...
            TEST(Table, HeterogeneousLookupOverloads) {
                ...
                using TransparentTable =
                        raw_hash_set<StringPolicy, hash_default_hash<std::string_view>,
                                hash_default_eq<std::string_view>, std::allocator<int>>;
            ...
            TEST(Table, IteratorEmplaceConstructibleRequirement) {
                ...
                struct H {
                    size_t operator()(const Value &v) const {
                        return turbo::Hash<std::string>{}(v.value);
                    }
                };
  ...
  EXPECT_TRUE(__asan_address_is_poisoned(&v));
            ...
            struct InstanceTrackerHash {
                size_t operator()(const CopyableMovableInstance &t) const {
                    return turbo::HashOf(t.value());
                }
            };

            using InstanceTrackerTableTypes = ::testing::Types<
                    turbo::node_hash_set<CopyableMovableInstance, InstanceTrackerHash>,
                    turbo::flat_hash_set<CopyableMovableInstance, InstanceTrackerHash>>;
            ...
            TYPED_TEST(InstanceTrackerTest, EraseIfAll) {
                ...
                for (int size = 0; size < 100; ++size) {
                    ...
                    turbo::erase_if(t, [](const auto &) { return true; });
                    ASSERT_EQ(t.size(), 0);
                }
            ...
            TYPED_TEST(InstanceTrackerTest, EraseIfNone) {
                ...
                InstanceTracker tracker;
                {
                    Table t;
                    for (size_t size = 0; size < 100; ++size) {
                        turbo::erase_if(t, [](const auto &) { return false; });
                        ASSERT_EQ(t.size(), size);
                        t.emplace(size);
                    }
                }
            ...
            TYPED_TEST(InstanceTrackerTest, EraseIfPartial) {
                ...
                for (int mod: {0, 1}) {
                    for (int size = 0; size < 100; ++size) {
                        ...
                        turbo::erase_if(t, [mod](const auto &x) { return x.value() % 2 == mod; });
                        ASSERT_THAT(t, testing::UnorderedElementsAreArray(expected));
                    }
                }
                EXPECT_EQ(tracker.live_instances(), 0);
...
// IterateOverFullSlots doesn't support SOO.
            ...
            TEST(SooTable, Basic) {
                ...
                if (t.capacity() != SooCapacity()) {
                    KCHECK_LT(sizeof(void *), 8) << "missing SOO coverage";
            ...
            struct InconsistentHashEqType {
                ...
                template<typename H>
                friend H TurboHashValue(H h, InconsistentHashEqType t) {
            ...
            struct ConstructCaller {
                ...
                ConstructCaller(int v, turbo::FunctionRef<void()> func) : val(v) { func(); }

                template<typename H>
                friend H TurboHashValue(H h, const ConstructCaller &d) {
            ...
            struct DestroyCaller {
                ...
                DestroyCaller(int v, turbo::FunctionRef<void()> func)
                ...
                template<typename H>
                friend H TurboHashValue(H h, const DestroyCaller &d) {
                    return H::combine(std::move(h), d.val);
                }
                ...
                int val;
                std::optional<turbo::FunctionRef<void()>> destroy_func;
            };
...
#elif KUMO_HAVE_THREAD_SANITIZER
  ...
  // Note: we use else rather than endif here to avoid unreachable code errors.
            ...
            TEST(Table, DestroyedCallsFailDuringDestruction) {
                ...
                GTEST_SKIP() << "Flaky on GCC.";
#endif
#if KUMO_HAVE_ADDRESS_SANITIZER && defined(NDEBUG)
  ...
  GTEST_SKIP() << "Fails to die in opt ASAN.";
                ...
                using Table = turbo::flat_hash_map<int, std::shared_ptr<int>>;
            ...
            TEST(Table, MovedFromCallsFail) {
                ...
                {
                    KUMO_ATTRIBUTE_UNUSED IntTable t1, t2, t3;
                    t1.insert(1);
                    t2 = std::move(t1);
                    // NOLINTNEXTLINE(bugprone-use-after-move)
                ...
                {
                    KUMO_ATTRIBUTE_UNUSED IntTable t1;
                    t1.insert(1);
                    KUMO_ATTRIBUTE_UNUSED IntTable t2(std::move(t1));
                    // NOLINTNEXTLINE(bugprone-use-after-move)
                    EXPECT_DEATH_IF_SUPPORTED(t1.contains(1), "moved-from");
            ...
            TEST(Table, MaxValidSize) {
                ...
                if constexpr (sizeof(size_t) == 8) {
                    for (size_t i = 0; i < 35; ++i) {
                        ...
                        ASSERT_LT(turbo::uint128(max_size) *slot_size, uint64_t{1} << 63);
                    }
                }
...
// doesn't crash and we end up with kMaxValidCapacity.
            TEST(Table, MaxSizeOverflowUniqueKeys) {
                turbo::flat_hash_set<uint8_t> t8;
                t8.reserve(1 << 9);
                EXPECT_EQ(t8.capacity(), SizeToCapacity(t8.max_size()));
                turbo::flat_hash_set<uint16_t> t16;
                t16.reserve(1 << 17);
...
// TODO(b/397453582): Remove support for const hasher and remove this test.
            TEST(Table, ConstLambdaHash) {
                ...
                turbo::flat_hash_set<SizedValue<64>, decltype(hash)> t(0, hash);
                t.insert(1);
                EXPECT_EQ(t.size(), 1);
...
// 5. Finally we will catch up and go to overflow codepath.
    ...
    defined(_MSC_VER) || KUMO_HAVE_THREAD_SANITIZER || \
    ...
    (!defined(__clang__) && defined(__GNUC__))
                ...
                turbo::flat_hash_set<uint32_t, ZeroHash> t(63);
