...
#include <turbo/container/linked_hash_map.h>
...
#include <tests/container/internal/heterogeneous_lookup_testing.h>
...
TEST(LinkedHashMapTest, At) {
  ...
  EXPECT_EQ(2, m.at(1));
  TURBO_BASE_INTERNAL_EXPECT_FAIL(m.at(3), std::out_of_range,
                                 "linked_hash_map::at");

  const linked_hash_map<int, int> cm = {{1, 2}};
  EXPECT_EQ(2, cm.at(1));
  TURBO_BASE_INTERNAL_EXPECT_FAIL(cm.at(3), std::out_of_range,
                                 "linked_hash_map::at");
}
...
class CopyableMovableType
    : public turbo::test_internal::CopyableMovableInstance {
 public:
  ...
  template <typename H>
  friend H TurboHashValue(H h, const CopyableMovableType& c) {
...
TEST(LinkedHashMapTest, HeterogeneousStringViewLookup) {
  ...
  // Not found.
  {
    std::string_view lookup("foobar");
    EXPECT_EQ(map.end(), map.find(lookup));
  }

  {
    std::string_view lookup("blah");
    auto itr = map.find(lookup);
    ASSERT_NE(itr, map.end());
...
TEST(LinkedHashMap, ExtractAndEmplaceUseSameStatefulAllocator) {
  using Alloc =
      turbo::container_internal::CountingAllocator<std::pair<const int, int>>;
