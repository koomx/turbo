...
using MapTypes = ::testing::Types<
    turbo::node_hash_map<int, int, StatefulTestingHash, StatefulTestingEqual,
                        Alloc<std::pair<const int, int>>>,
    turbo::node_hash_map<std::string, std::string, StatefulTestingHash,
...
using M = turbo::node_hash_map<std::string, Tracked<int>>;
...
TEST(NodeHashMap, AssignRecursive) {
  struct Tree {
    ...
    turbo::node_hash_map<int, Tree> children;
  };
  Tree root;
...
TEST(FlatHashMap, MoveOnlyKey) {
  ...
  turbo::node_hash_map<Key, int, Hash, Eq> m;
  m[Key()];
}
...
TEST(NodeHashMap, MergeExtractInsert) {
  turbo::node_hash_map<NonMovableKey, int, NonMovableKeyHash, NonMovableKeyEq>
      set1, set2;
...
TEST(NodeHashMap, FromRangeWithHasherAndAllocator) {
  ...
  turbo::node_hash_map<int, int, TestingHash,
                      turbo::container_internal::hash_default_eq<int>,
                      Alloc<std::pair<const int, int>>>
