...
#include <tests/cord/internal/cord_rep_test_util.h>
#include <string_view>
...
class CordBufferTestPeer {
 public:
  static cord_internal::CordRep* consume_value(CordBuffer& buffer,
                                              std::string_view& short_value) {
    return buffer.consume_value(short_value);
  }
};
...
TEST(CordBufferTest, CreateSsoWithDefaultLimit) {
  ...
  std::string_view short_value;
  EXPECT_THAT(CordBufferTestPeer::consume_value(buffer, short_value),
              Eq(nullptr));
...
TEST_P(CordBufferTest, consume_value) {
  const size_t requested = GetParam();
  CordBuffer buffer = CordBuffer::create_with_default_limit(requested);
  ...
  if (cord_internal::CordRep* rep =
          CordBufferTestPeer::consume_value(buffer, short_value)) {
