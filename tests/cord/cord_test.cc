...
#include <turbo/types/fixed_array.h>
...
TEST_P(CordTest, GigabyteCordFromExternal) {
    ...
    if (sizeof(max_size) > 4)
        max_size = 128 * one_gig;
    ...
    KLOG(INFO) << "Made a Cord with " << c.size() << " bytes!";
...
TEST_P(CordTest, Assignment) {
    ...
    }
    {
        ...
        turbo::Cord* my_small_alias = my_unique_true_boolean ? &my_small_cord : &my_big_cord;
        turbo::Cord* my_big_alias = !my_unique_true_boolean ? &my_small_cord : &my_big_cord;
...
static void VerifyAppendCordToString(const turbo::Cord& cord) {
    ...
    std::string expected_after_append = turbo::str_cat(kInitialContents, std::string(cord));
...
static void VerifyCopyToSpan(const turbo::Cord& cord) {
    ...
    // Test with span larger than the cord.
    {
        ...
        if (cord.size() < dst.size()) {
            std::string_view tail = std::string_view(dst).substr(copied);
...
TEST_P(CordTest, CopyToSpan) {
    VerifyCopyToSpan(turbo::Cord()); // Empty cords cannot be hardened.
    VerifyCopyToSpan(MaybeHardened(turbo::Cord("small cord")));
...
public:
    ...
    turbo::CordBuffer GetAppendBuffer(turbo::Cord& cord, size_t capacity,
...
TEST_P(CordAppendBufferTest, GetAppendBufferOnEmptyCord) {
    turbo::Cord cord;
    turbo::CordBuffer buffer = GetAppendBuffer(cord, 1000);
...
TEST_P(CordAppendBufferTest, GetAppendBufferOnTreeWithoutMinCapacity) {
    ...
    turbo::CordBuffer buffer = GetAppendBuffer(cord, 1000, 900);
    EXPECT_GE(buffer.capacity(), 1000);
    EXPECT_EQ(buffer.length(), 0);
...
namespace {
    ...
    turbo::Cord SpliceCord(const turbo::Cord& blob, int64_t offset,
        ...
        KCHECK_GE(offset, 0);
        KCHECK_LE(static_cast<size_t>(offset) + block.size(), blob.size());
        ...
        KCHECK_EQ(blob.size(), result.size());
        return result;
    }
    ...
    TEST_P(CordTest, CompareRandomComparisons) {
        ...
        for (int i = 0; i < kIters; i++) {
            ...
            for (int j = 0; j < (i % 7) + 1; j++) {
                c.Append(a[GetUniformRandomUpTo(&rng, KUMO_ARRAYSIZE(a))]);
                d.Append(a[GetUniformRandomUpTo(&rng, KUMO_ARRAYSIZE(a))]);
            }
            std::bernoulli_distribution coin_flip(0.5);
            MaybeHarden(c);
            MaybeHarden(d);
    ...
    TEST_P(CordTest, ConstructFromExternalCompareContents) {
        ...
        for (int length = 1; length <= 2048; length *= 2) {
            ...
            auto cord = turbo::MakeCordFromExternal(*external, [external](std::string_view sv) {
    ...
    TEST_P(CordTest, ConstructFromExternalFunctionPointerReleaser) {
        ...
        static bool invoked;
        auto* releaser = static_cast<void (*)(std::string_view)>([](std::string_view sv) {
            EXPECT_EQ(data, sv);
            invoked = true;
        });
        invoked = false;
    ...
    TEST_P(CordTest, ConstructFromExternalMoveOnlyReleaser) {
        struct Releaser {
            explicit Releaser(bool* invoked)
                : invoked(invoked) { }
    ...
    TEST_P(CordTest, ConstructFromExternalNonConstReleaser) {
        struct Releaser {
            explicit Releaser(bool* invoked)
                : invoked(invoked) { }
            // Non const method.
            void operator()(std::string_view) { *invoked = true; }
    ...
    TEST_P(CordTest, ConstructFromExternalNonTrivialReleaserDestructor) {
        struct Releaser {
            explicit Releaser(bool* destroyed)
                : destroyed(destroyed) { }
    ...
    TEST_P(CordTest, ConstructFromExternalReferenceQualifierOverloads) {
        ...
        class Releaser {
        public:
            explicit Releaser(Tracker* tracker)
                : tr_(tracker) { *tracker = Tracker(); }
            Releaser(Releaser&& rhs)
                : tr_(rhs.tr_) { tr_->Record(kMove); }
            Releaser(const Releaser& rhs)
                : tr_(rhs.tr_) { tr_->Record(kCopy); }
    ...
    constexpr auto kTotalMorePrecise = turbo::CordMemoryAccounting::kTotalMorePrecise;
    ...
    TEST(CordTest, CordMemoryUsageEmpty) {
        ...
        EXPECT_EQ(sizeof(turbo::Cord), cord.EstimatedMemoryUsage(kTotalMorePrecise));
    }
    ...
    TEST(CordTest, CordMemoryUsageInlined) {
        ...
        EXPECT_EQ(a.EstimatedMemoryUsage(kTotalMorePrecise), sizeof(turbo::Cord));
    }

    TEST(CordTest, CordMemoryUsageExternalMemory) {
        turbo::Cord cord;
        AddExternalMemory(std::string(1000, 'x'), &cord);
        const size_t expected = sizeof(turbo::Cord) + 1000 + sizeof(CordRepExternal) + sizeof(intptr_t);
    ...
    TEST(CordTest, CordMemoryUsageSubStringSharedFlat) {
        ...
        const size_t flat_size = turbo::CordTestPeer::Tree(flat)->flat()->AllocatedSize();
    ...
    TEST(CordTest, CordMemoryUsageBTree) {
        ...
        if (!turbo::CordTestPeer::Tree(cord1)->IsBtree()) {
            KLOG(WARNING) << "Cord library code not respecting btree flag";
            return;
        }
        ...
        EXPECT_EQ(cord.EstimatedMemoryUsage(kTotalMorePrecise),
            sizeof(turbo::Cord) + sizeof(CordRepBtree) + rep1_size + rep2_size);
        EXPECT_EQ(cord.EstimatedMemoryUsage(kFairShare),
            sizeof(turbo::Cord) + sizeof(CordRepBtree) + rep1_shared_size / 2 + rep2_size);
    }
    ...
    TEST(CordTest, TestHashFragmentation) {
        ...
        EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly({
            turbo::Cord(),
            turbo::MakeFragmentedCord({ std::string(600, 'a'), std::string(600, 'a') }),
            turbo::MakeFragmentedCord({ std::string(1200, 'a') }),
            turbo::MakeFragmentedCord({ std::string(900, 'b'), std::string(900, 'b') }),
            turbo::MakeFragmentedCord({ std::string(1800, 'b') }),
            turbo::MakeFragmentedCord(
                { std::string(2000, 'c'), std::string(2000, 'c') }),
            turbo::MakeFragmentedCord({ std::string(4000, 'c') }),
            turbo::MakeFragmentedCord({ std::string(1024, 'd') }),
            turbo::MakeFragmentedCord({ std::string(1023, 'd'), "d" }),
            turbo::MakeFragmentedCord({ std::string(1025, 'e') }),
            turbo::MakeFragmentedCord({ std::string(1024, 'e'), "e" }),
            turbo::MakeFragmentedCord({ std::string(1023, 'e'), "e", "e" }),
        }));
    }
    ...
    TEST_P(CordTest, CordMemoryUsageTotalMorePreciseMode) {
        ...
        const size_t flat_internal_usage = flat.EstimatedMemoryUsage() - sizeof(turbo::Cord);
        ...
        EXPECT_EQ(fragmented.EstimatedMemoryUsage(kTotalMorePrecise),
            sizeof(turbo::Cord) + sizeof(CordRepBtree) + flat_internal_usage);

        // This is a case where kTotal produces an overestimate:
        EXPECT_EQ(fragmented.EstimatedMemoryUsage(),
            sizeof(turbo::Cord) + sizeof(CordRepBtree) + 2 * flat_internal_usage);
    }
    ...
    TEST_P(CordTest, CordMemoryUsageTotalMorePreciseModeWithSubstring) {
        ...
        EXPECT_EQ(fragmented.EstimatedMemoryUsage(kTotalMorePrecise),
            sizeof(turbo::Cord) + sizeof(CordRepBtree) + 2 * sizeof(CordRepSubstring) + flat_internal_usage);

        // This is a case where kTotal produces an overestimate:
        EXPECT_EQ(fragmented.EstimatedMemoryUsage(),
            sizeof(turbo::Cord) + sizeof(CordRepBtree) + 2 * sizeof(CordRepSubstring) + 2 * flat_internal_usage);
    }
} // namespace
...
TEST_P(CordTest, DiabolicalGrowth) {
    ...
    KLOG(INFO) << "Diabolical size allocated = " << cord.EstimatedMemoryUsage();
}
...
static turbo::Cord MakeHuge(std::string_view prefix) {
    ...
    if (sizeof(size_t) > 4) {
        ...
        const size_t size = static_cast<size_t>(std::numeric_limits<uint32_t>::max()) + 314;
        cord.Append(turbo::MakeCordFromExternal(
...
TEST_P(CordTest, HugeCord) {
    ...
    const size_t acceptable_delta = 100 + (UseCrc() ? sizeof(turbo::cord_internal::CordRepCrc) : 0);
...
TEST_P(CordTest, MakeFragmentedCordFromInitializerList) {
    turbo::Cord fragmented = turbo::MakeFragmentedCord({ "A ", "fragmented ", "Cord" });
...
TEST_P(CordTest, CordChunkIteratorTraits) {
    static_assert(std::is_copy_constructible_v<turbo::Cord::ChunkIterator>, "");
    static_assert(std::is_copy_assignable_v<turbo::Cord::ChunkIterator>, "");

    // Move semantics to satisfy swappable via std::swap
    static_assert(std::is_move_constructible_v<turbo::Cord::ChunkIterator>, "");
    static_assert(std::is_move_assignable_v<turbo::Cord::ChunkIterator>, "");
    ...
    static_assert(std::is_same_v<
                      std::iterator_traits<turbo::Cord::ChunkIterator>::value_type,
                      std::string_view>,
        "");
    static_assert(
        std::is_same_v<
        ...
        "");
    static_assert(
        std::is_same_v<std::iterator_traits<turbo::Cord::ChunkIterator>::pointer,
            const std::string_view*>,
        "");
    static_assert(
        std::is_same_v<std::iterator_traits<turbo::Cord::ChunkIterator>::reference,
            std::string_view>,
        "");
}
...
TEST_P(CordTest, CordChunkIteratorOperations) {
    ...
    turbo::Cord subcords;
    for (int i = 0; i < 128; ++i)
        subcords.Prepend(flat_cord.Subcord(i, 128));
...
TEST_P(CordTest, AdvanceAndReadOnDataEdge) {
    ...
    for (bool as_flat : { true, false }) {
        ...
        it = cord.Chars().begin();
        turbo::Cord frag = cord.AdvanceAndRead(&it, 2000);
        EXPECT_EQ(frag, data);
...
TEST_P(CordTest, CharIteratorTraits) {
    static_assert(std::is_copy_constructible_v<turbo::Cord::CharIterator>, "");
    static_assert(std::is_copy_assignable_v<turbo::Cord::CharIterator>, "");

    // Move semantics to satisfy swappable via std::swap
    static_assert(std::is_move_constructible_v<turbo::Cord::CharIterator>, "");
    static_assert(std::is_move_assignable_v<turbo::Cord::CharIterator>, "");
    ...
    static_assert(
        ...
        "");
    static_assert(
        std::is_same_v<std::iterator_traits<turbo::Cord::CharIterator>::value_type,
            char>,
        "");
    static_assert(
        std::is_same_v<
        ...
        "");
    static_assert(
        std::is_same_v<std::iterator_traits<turbo::Cord::CharIterator>::pointer,
            const char*>,
        "");
    static_assert(
        std::is_same_v<std::iterator_traits<turbo::Cord::CharIterator>::reference,
            const char&>,
        "");
}
...
static void VerifyCharIterator(const turbo::Cord& cord) {
    ...
    EXPECT_EQ(turbo::Cord::Distance(range.end(), range.begin()),
        -static_cast<ptrdiff_t>(cord.size()));
    ...
    while (pre_iter != cord.char_end() && post_iter != cord.char_end()) {
        ...
        EXPECT_EQ(turbo::Cord::Distance(range.begin(), advance_iter),
            static_cast<ptrdiff_t>(i));

        advance_iter = range.begin();
        EXPECT_EQ(turbo::Cord::AdvanceAndRead(&advance_iter, i), cord.Subcord(0, i));
        EXPECT_EQ(pre_iter, advance_iter);
        EXPECT_EQ(turbo::Cord::Distance(range.begin(), advance_iter),
            static_cast<ptrdiff_t>(i));
        ...
        EXPECT_EQ(turbo::Cord::Distance(range.begin(), advance_iter),
            static_cast<ptrdiff_t>(cord.size()));
        EXPECT_EQ(turbo::Cord::Distance(advance_iter, range.end()), 0);

        advance_iter = pre_iter;
        EXPECT_EQ(turbo::Cord::AdvanceAndRead(&advance_iter, cord.size() - i),
...
TEST_P(CordTest, CharIteratorAdvanceAndRead) {
    ...
    for (size_t chunk_size :
        ...
        while (offset < data.length()) {
            EXPECT_EQ(turbo::Cord::Distance(it, cord.char_end()), it_remaining);
            EXPECT_EQ(turbo::Cord::Distance(cord.char_begin(), it), it_advanced);
            const size_t n = std::min<size_t>(data.length() - offset, chunk_size);
            turbo::Cord chunk = cord.AdvanceAndRead(&it, n);
...
TEST_P(CordTest, StreamingOutput) {
    turbo::Cord c = turbo::MakeFragmentedCord({ "A ", "small ", "fragmented ", "Cord", "." });
    MaybeHarden(c);
...
TEST_P(CordTest, str_printf_to) {
    turbo::Cord c;
    turbo::str_printf_to(&c, "There were %04d little %s.", 3, "pigs");
    EXPECT_EQ(c, "There were 0003 little pigs.");
    MaybeHarden(c);
    turbo::str_printf_to(&c, "And %-3llx bad wolf!", 1);
    MaybeHarden(c);
...
TEST_P(CordTest, Hardening) {
    ...
    KUMO_HARDENING_ASSERT([&]() {
        // This only runs when KUMO_HARDENING_ASSERT is active.
        test_hardening = true;
        return true;
    }());
    if (!test_hardening)
...
CordMutator cord_mutators[] = {
  ...
  {"overwrite", [](turbo::Cord& c) { c = "overwritten"; }},
  {
    ...
    [](turbo::Cord& c) { c.RemoveSuffix(10); }
  },
  {
    "append cord",
    [](turbo::Cord& c) {
  ...
  },
  {
    ...
    [](turbo::Cord& c) { c.RemoveSuffix(c.size() / 2); }
  },
  {
    "append empty string",
  ...
  },
  {
    ...
    [](turbo::Cord& c) { c.RemovePrefix(10); }
  },
  {
    "prepend cord",
    [](turbo::Cord& c) {
  ...
  },
  {
    ...
    [](turbo::Cord& c) { c.RemovePrefix(c.size() / 2); }
  },
  {"remove prefix", [](turbo::Cord& c) { c.RemovePrefix(c.size() / 2); }},
  {"remove suffix", [](turbo::Cord& c) { c.RemoveSuffix(c.size() / 2); }},
  {"remove 0-prefix", [](turbo::Cord& c) { c.RemovePrefix(0); }},
  {"remove 0-suffix", [](turbo::Cord& c) { c.RemoveSuffix(0); }},
...
TEST_P(CordTest, ExpectedChecksum) {
    for (const PopulatedCordFactory& factory : cord_factories) {
        ...
        for (bool shared : { false, true }) {
            ...
            EXPECT_EQ(turbo::Cord::AdvanceAndRead(&char_it, 2), "cd");
            EXPECT_EQ(*char_it, 'e');
...
KUMO_ATTRIBUTE_WEAK
size_t FalseReport(const turbo::Cord& a, bool f);

KUMO_ATTRIBUTE_NOINLINE
size_t FalseReport(const turbo::Cord& a, bool f) {
    turbo::Cord b;
    const turbo::Cord& ref = f ? b : a;
...
#else
    EXPECT_EQ(turbo::Cord("a") <=> turbo::Cord("a"), std::strong_ordering::equal);
    EXPECT_EQ(turbo::Cord("aaaa") <=> turbo::Cord("aaab"),
        std::strong_ordering::less);
    EXPECT_EQ(turbo::Cord("baaa") <=> turbo::Cord("a"),
...
TEST(CordThreeWayComparisonTest, CompareCordsAndStringViews) {
    ...
    GTEST_SKIP() << "C++20 three-way <=> comparison not supported";
#else
    EXPECT_EQ(std::string_view("a") <=> turbo::Cord("a"),
        std::strong_ordering::equal);
    EXPECT_EQ(turbo::Cord("a") <=> std::string_view("b"),
        std::strong_ordering::less);
    EXPECT_EQ(std::string_view("b") <=> turbo::Cord("a"),
        std::strong_ordering::greater);
#endif
}
