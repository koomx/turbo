...
TEST(CRC32C, ExtendByZeroesAllLengthBits) {
  turbo::crc32c_t base_crc = turbo::crc32c_t{0xc99465aa};
  const std::array<std::tuple<uint64_t, turbo::crc32c_t>, 5> kTestCases = {{
      {0, turbo::crc32c_t(0xc99465aa)},
      {std::numeric_limits<uint32_t>::max(), turbo::crc32c_t(0x9b1d5aaa)},
      {0x12345678, turbo::crc32c_t(0xcf0e9553)},
      {std::numeric_limits<uint64_t>::max(), turbo::crc32c_t(0xf5bff489)},
      {0x12345678abcdefff, turbo::crc32c_t(0xaa1ffb0b)},
  }};
  for (const auto &test_case : kTestCases) {
    uint64_t length = std::get<0>(test_case);
    turbo::crc32c_t expected_value = std::get<1>(test_case);
    SCOPED_TRACE(length);
    if (length > std::numeric_limits<size_t>::max()) {
      // On 32-bit platforms, 64-bit lengths cannot be used or tested.
      continue;
    }
    EXPECT_EQ(turbo::ExtendCrc32cByZeroes(base_crc, static_cast<size_t>(length)),
              expected_value);
  }
}
...
TEST(CRC32C, turbo_stringify) {
  // str_sprintf
  EXPECT_EQ(turbo::str_sprintf("%v", turbo::crc32c_t{0xc99465aa}), "c99465aa");
  EXPECT_EQ(turbo::str_sprintf("%v", turbo::crc32c_t{0}), "00000000");
  EXPECT_EQ(turbo::str_sprintf("%v", turbo::crc32c_t{17}), "00000011");
