...
#else
#define TURBO_SYMBOLIZE_TEST_NOINLINE KUMO_ATTRIBUTE_NOINLINE
#endif
...
int KUMO_ATTRIBUTE_SECTION_VARIABLE(.text.unlikely) unlikely_func() {
  return 0;
}

int KUMO_ATTRIBUTE_SECTION_VARIABLE(.text.hot) hot_func() { return 0; }

int KUMO_ATTRIBUTE_SECTION_VARIABLE(.text.startup) startup_func() { return 0; }

int KUMO_ATTRIBUTE_SECTION_VARIABLE(.text.exit) exit_func() { return 0; }

int /*KUMO_ATTRIBUTE_SECTION_VARIABLE(.text)*/ regular_func() { return 0; }

// Thread-local data may confuse the symbolizer, ensure that it does not.
// Variable sizes and order are important.
#if KUMO_PER_THREAD_TLS
static KUMO_PER_THREAD_TLS_KEYWORD char symbolize_test_thread_small[1];
static KUMO_PER_THREAD_TLS_KEYWORD char
...
const char kHpageTextPadding[kHpageSize * 4] KUMO_ATTRIBUTE_SECTION_VARIABLE(
        .text) = "";
...
static const char *TrySymbolizeWithLimit(void *pc, int limit) {
  KCHECK_LE(limit, sizeof(try_symbolize_buffer))
...
void KUMO_ATTRIBUTE_NOINLINE TestWithReturnAddress() {
...
static int GetStackConsumptionUpperLimit() {
  ...
  int stack_consumption_upper_limit = 2048;
    ...
    KUMO_HAVE_MEMORY_SANITIZER || KUMO_HAVE_THREAD_SANITIZER
...
const char kPadding0[kPageSize * 4] KUMO_ATTRIBUTE_SECTION_VARIABLE(.text) = "";
const char kPadding1[kPageSize * 4] KUMO_ATTRIBUTE_SECTION_VARIABLE(.text) = "";
...
class TestSymbolDecorator final
    : public turbo::debugging_internal::SymbolDecorator {
 public:
  static turbo::debugging_internal::SymbolDecoratorPtr Factory(int /*fd*/) {
    void* ptr = turbo::base_internal::LowLevelAlloc::AllocWithArena(
        sizeof(TestSymbolDecorator), turbo::base_internal::SigSafeArena());
    return turbo::debugging_internal::SymbolDecoratorPtr(
  ...
  void Decorate(const void* /*pc*/, ptrdiff_t /*relocation*/, char* symbol_buf,
    ...
    turbo::str_snprintf(symbol_buf + len, symbol_buf_size - len, " hello %c", C);
  }
};

TEST(Symbolize, SetSymbolDecorator) {
  turbo::Cleanup cleanup =
      [old_decorator = turbo::debugging_internal::SetSymbolDecoratorFactory(
           &TestSymbolDecorator<'a'>::Factory)] {
        turbo::debugging_internal::SetSymbolDecoratorFactory(old_decorator);
      };

  EXPECT_STREQ("nonstatic_func hello a",
               TrySymbolize(reinterpret_cast<void*>(&nonstatic_func)));

  EXPECT_EQ(turbo::debugging_internal::SetSymbolDecoratorFactory(
...
void *KUMO_ATTRIBUTE_NOINLINE non_inline_func() {
  void *pc = nullptr;
#if defined(__i386__)
  __asm__ __volatile__("call 1f;\n 1: pop %[PC]" : [PC] "=r"(pc));
#elif defined(__x86_64__)
...
void KUMO_ATTRIBUTE_NOINLINE TestWithPCInsideNonInlineFunction() {
#if defined(TURBO_HAVE_ATTRIBUTE_NOINLINE) && \
...
void KUMO_ATTRIBUTE_NOINLINE TestWithPCInsideInlineFunction() {
...
#if defined(__arm__) && KUMO_HAVE_ATTRIBUTE(target) && \
...
void KUMO_ATTRIBUTE_NOINLINE TestArmThumbOverlap() {
#if defined(TURBO_HAVE_ATTRIBUTE_NOINLINE)
...
#endif  // defined(__arm__) && KUMO_HAVE_ATTRIBUTE(target) && ((__ARM_ARCH >= 7)
