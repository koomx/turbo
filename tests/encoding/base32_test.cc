...
TYPED_TEST(BaseCoderTest, Null)
{
    ...
    uint8_t output[64] = {0};
    {
        size_t const result = coder_t::base32_encode(nullptr, 0, output);
        EXPECT_EQ(result, 0) << " for empty input encode";
    }
    {
        auto const result = coder_t::base32_decode(nullptr, 0, output);
...
TYPED_TEST(BaseCoderTest, EncodeDecode)
{
    ...
    for (const auto & test : test_data_t::encode_tests)
    {
        ...
        uint8_t encode_output[128] = {0};

        size_t const encoded_result_len
            = coder_t::base32_encode(reinterpret_cast<const uint8_t *>(test.input.data()), input_len, encode_output);
        ...
        {
            uint8_t decode_output[128] = {0};
            auto const decode_result = coder_t::base32_decode(reinterpret_cast<const uint8_t *>(what.data()), what.size(), decode_output);
...
TYPED_TEST(BaseCoderTest, DecodeInvalid)
{
    ...
    for (const auto & test : test_data_t::bad_decode_tests)
    {
        ...
        auto const result = coder_t::base32_decode(reinterpret_cast<const uint8_t *>(test.input.data()), input_len, output);
...
TYPED_TEST(BaseCoderTest, LargeTest)
{
    ...
    std::vector<uint8_t> large_input(large_size);
    for (size_t i = 0; i < large_size; ++i)
    {
        large_input[i] = static_cast<uint8_t>(i % 256);
    }

    size_t const encoded_len = ((large_size + 4) / 5) * 8;
    std::vector<uint8_t> encoded(encoded_len + 1);
    auto const actual_encoded_len = coder_t::base32_encode(large_input.data(), large_size, encoded.data());
    ASSERT_EQ(encoded_len, actual_encoded_len) << " for input (as hex) " << to_hex(large_input.data(), large_size);

    std::vector<uint8_t> decoded(large_size);
    auto const decode_result = coder_t::base32_decode(encoded.data(), encoded_len, decoded.data());
