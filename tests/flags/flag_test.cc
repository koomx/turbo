...
#define DEFINE_CONSTRUCTED_FLAG(T, dflt, dflt_kind)                        \
  ...
  constexpr turbo::Flag<T> f1##T{"f1", #T, "file", help_arg, f1default##T}; \
  KUMO_CONST_INIT turbo::Flag<T> f2##T {                                    \
    "f2", #T, "file",                                                      \
...
TEST_F(FlagTest, ConcurrentSetAndGet) {
  ...
  while (turbo::Now() < end_time) {
    turbo::SetFlag(&FLAGS_test_flag_12,
                  kValidDurations[i++ % KUMO_ARRAYSIZE(kValidDurations)]);
  }
...
bool initialization_order_fiasco_test KUMO_ATTRIBUTE_UNUSED = [] {
...
TURBO_FLAG(NonTriviallyCopyableUDT<1>, test_flag_ntc_udt1, {}, "help");
TURBO_FLAG(NonTriviallyCopyableUDT<2>, test_flag_ntc_udt2, {}, "help");
TURBO_FLAG(NonTriviallyCopyableUDT<3>, test_flag_ntc_udt3, {}, "help");
TURBO_FLAG(NonTriviallyCopyableUDT<4>, test_flag_ntc_udt4, {}, "help");
TURBO_FLAG(NonTriviallyCopyableUDT<5>, test_flag_ntc_udt5, {}, "help");
...
TEST_F(FlagTest, TestNonTriviallyCopyableGetSetSet) {
  ...
  TestExpectedLeaks<1>(
      [&] {
        NonTriviallyCopyableUDT<1> value =
            turbo::GetFlag(FLAGS_test_flag_ntc_udt1);
        EXPECT_EQ(value.c, 'A');
      },
      0);
  ...
  TestExpectedLeaks<1>(
      [&] {
        NonTriviallyCopyableUDT<1> value;
        value.c = 'B';
        turbo::SetFlag(&FLAGS_test_flag_ntc_udt1, value);
        EXPECT_EQ(value.c, 'B');
      },
      1);
...
TEST_F(FlagTest, TestNonTriviallyCopyableParseSet) {
  ...
  TestExpectedLeaks<2>(
      [&] {
        ...
        value.c = 'B';
        turbo::SetFlag(&FLAGS_test_flag_ntc_udt2, value);
        EXPECT_EQ(value.c, 'B');
      },
      0);
}
...
TEST_F(FlagTest, TestNonTriviallyCopyableSet) {
  TestExpectedLeaks<3>(
      [&] {
        ...
        value.c = 'B';
        turbo::SetFlag(&FLAGS_test_flag_ntc_udt3, value);
        EXPECT_EQ(value.c, 'B');
      },
      0);
}
...
auto premain_utd4_get =
    (TestExpectedLeaks<4>([] { (void)turbo::GetFlag(FLAGS_test_flag_ntc_udt4); },
                          0, 1),
     false);
  ...
  TestExpectedLeaks<4>(
      [&] {
        NonTriviallyCopyableUDT<4> value =
            turbo::GetFlag(FLAGS_test_flag_ntc_udt4);
        EXPECT_EQ(value.c, 'C');
      },
      0);
}
...
auto premain_utd5_set = (TestExpectedLeaks<5>(
                             [] {
                               ...
                               value.c = 'B';
                               turbo::SetFlag(&FLAGS_test_flag_ntc_udt5, value);
                             },
                             0, 1),
                         false);
...
TEST_F(FlagTest, TestNonTriviallyCopyableSetParseGet) {
  ...
  TestExpectedLeaks<5>(
      [&] {
        NonTriviallyCopyableUDT<5> value =
            turbo::GetFlag(FLAGS_test_flag_ntc_udt5);
        EXPECT_EQ(value.c, 'C');
      },
      0);
}
...
TEST_F(FlagTest, TestOptionalBool) {
  ...
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_bool), std::nullopt);
  ...
  turbo::SetFlag(&FLAGS_optional_bool, std::nullopt);
  EXPECT_FALSE(turbo::GetFlag(FLAGS_optional_bool).has_value());
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_bool), std::nullopt);
}
...
TEST_F(FlagTest, TestOptionalInt) {
  ...
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_int), std::nullopt);
  ...
  turbo::SetFlag(&FLAGS_optional_int, std::nullopt);
  EXPECT_FALSE(turbo::GetFlag(FLAGS_optional_int).has_value());
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_int), std::nullopt);
}
...
TEST_F(FlagTest, TestOptionalDouble) {
  ...
  turbo::SetFlag(&FLAGS_optional_double, std::nullopt);
  EXPECT_FALSE(turbo::GetFlag(FLAGS_optional_double).has_value());
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_double), std::nullopt);
}
...
TEST_F(FlagTest, TestOptionalString) {
  ...
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_string), std::nullopt);
  ...
  turbo::SetFlag(&FLAGS_optional_string, std::nullopt);
  EXPECT_FALSE(turbo::GetFlag(FLAGS_optional_string).has_value());
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_string), std::nullopt);
}
...
TEST_F(FlagTest, TestOptionalDuration) {
  ...
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_duration), std::nullopt);
  ...
  turbo::SetFlag(&FLAGS_optional_duration, std::nullopt);
  EXPECT_FALSE(turbo::GetFlag(FLAGS_optional_duration).has_value());
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_duration), std::nullopt);
}
...
TEST_F(FlagTest, TestOptionalOptional) {
  ...
  turbo::SetFlag(&FLAGS_optional_optional_int, std::optional<int>{0});
  EXPECT_TRUE(turbo::GetFlag(FLAGS_optional_optional_int).has_value());
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_optional_int), 0);
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_optional_int), std::optional<int>{0});

  turbo::SetFlag(&FLAGS_optional_optional_int, std::nullopt);
  EXPECT_FALSE(turbo::GetFlag(FLAGS_optional_optional_int).has_value());
  EXPECT_EQ(turbo::GetFlag(FLAGS_optional_optional_int), std::nullopt);
}
