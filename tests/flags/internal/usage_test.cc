...
static bool TurboParseFlag(std::string_view, UDT*, std::string*) {
  return true;
}
...
TEST_F(UsageReportingTest, TestFlagsHelpHRF) {
  ...
  Flags from turbo/tests/flags/internal/usage_test.cc:
...
path.
  ...
  EXPECT_TRUE(turbo::StrContains(
      test_out_str, "Flags from turbo/tests/flags/internal/usage_test.cc:"));
  EXPECT_TRUE(
