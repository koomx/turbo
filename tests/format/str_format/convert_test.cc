...
#include <turbo/format/str_format/bind.h>
...
std::string EscCharImpl(int v) {
  char buf[64];
  int n = turbo::ascii_isprint(static_cast<unsigned char>(v))
...
std::string StrPrint(const char *, ...) KUMO_PRINTF_ATTRIBUTE(1, 2);
...
MATCHER_P(MatchesPointerString, ptr, "") {
  ...
  if (sscanf(arg.c_str(), "%p", &parsed) != 1) {
    KLOG(FATAL) << "Could not parse " << arg;
  }
  return ptr == parsed;
}
...
std::optional<std::string> StrPrintChar(wchar_t c) {
  ...
  // Force a UTF-8 locale to match the expected `str_sprintf()` behavior.
...
TEST_F(FormatConvertTest, UnicodeWideString) {
  // str_sprintf() should be able to convert wide strings containing Unicode
  // characters (to UTF-8).
...
TEST_F(FormatConvertTest, Int128) {
  turbo::int128 positive = static_cast<turbo::int128>(0x1234567890abcdef) * 1979;
  turbo::int128 negative = -positive;
  turbo::int128 max = turbo::Int128Max(), min = turbo::Int128Min();
...
TEST_F(FormatConvertTest, Uint128) {
  turbo::uint128 v = static_cast<turbo::uint128>(0x1234567890abcdef) * 1979;
  turbo::uint128 max = turbo::Uint128Max();
...
TEST_F(FormatConvertTest, GlibcHasCorrectTraits) {
  ...
  // be revisited and/or possibly the str_sprintf implementation.
