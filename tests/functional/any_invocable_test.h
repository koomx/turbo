...
#ifndef TURBO_FUNCTIONAL_ANY_INVOCABLE_TEST_H_
#define TURBO_FUNCTIONAL_ANY_INVOCABLE_TEST_H_
...
static_assert(std::is_constructible_v<Wrapper<turbo::AnyInvocable<void()>>,
                                      Wrapper<turbo::AnyInvocable<void()>>>,
              "");
...
struct TypeErasedPadding<ObjSize::large> {
  char dummy_data[turbo::internal_any_invocable::kStorageSize + 1] = {};
};
...
enum class ObjAlign : std::size_t {
  normal = turbo::internal_any_invocable::kAlignment,
  large = turbo::internal_any_invocable::kAlignment * 2,
};
...
template <class Sig>
using AnyInvocable = turbo::AnyInvocable<Sig>;
...
TYPED_TEST_P(AnyInvTestBasic, MoveConstructionFromEmpty) {
  ...
  turbo_nullable AnyInvType source_fun;
  turbo_nullable AnyInvType fun(std::move(source_fun));
...
TYPED_TEST_P(AnyInvTestCombinatoric, SwapEmptyLhsEmptyRhs) {
  ...
  // Swap idiom
  {
    ...
    EXPECT_TRUE(
        turbo::type_traits_internal::IsNothrowSwappable<AnyInvType>::value);
  }
...
// build binaries for.
static_assert(std::is_convertible_v<void (*)(), turbo::AnyInvocable<void() &&>>,
              "");
static_assert(!std::is_convertible_v<void*, turbo::AnyInvocable<void() &&>>, "");

}  // namespace turbo_any_invocable_test

#endif  // TURBO_FUNCTIONAL_ANY_INVOCABLE_TEST_H_
