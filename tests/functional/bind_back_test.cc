...
#include <turbo/functional/bind_back.h>
...
TEST(BindTest, Basics) {
  EXPECT_EQ('C', turbo::bind_back(CharAt)("ABC", 2));
  EXPECT_EQ('C', turbo::bind_back(CharAt, 2)("ABC"));
  EXPECT_EQ('C', turbo::bind_back(CharAt, "ABC", 2)());
}

TEST(BindTest, Lambda) {
  auto lambda = [](int x, int y, int z) { return x + y + z; };
  EXPECT_EQ(6, turbo::bind_back(lambda)(1, 2, 3));
  EXPECT_EQ(6, turbo::bind_back(lambda, 3)(1, 2));
  EXPECT_EQ(6, turbo::bind_back(lambda, 2, 3)(1));
  EXPECT_EQ(6, turbo::bind_back(lambda, 1, 2, 3)());
}
...
TEST(BindTest, PerfectForwardingOfBoundArgs) {
  auto f = turbo::bind_back(Functor());
  const auto& cf = f;
  EXPECT_EQ("&", f());
  EXPECT_EQ("const&", cf());
...
TEST(BindTest, PerfectForwardingOfFreeArgs) {
  ArgDescribe f;
  int i;
  EXPECT_EQ("&", turbo::bind_back(f)(static_cast<int&>(i)));
  EXPECT_EQ("const&", turbo::bind_back(f)(static_cast<const int&>(i)));
  EXPECT_EQ("&&", turbo::bind_back(f)(static_cast<int&&>(i)));
  EXPECT_EQ("const&&", turbo::bind_back(f)(static_cast<const int&&>(i)));
}
...
TEST(BindTest, RefToFunctor) {
  ...
  auto bound_ncf = turbo::bind_back(std::ref(ncf));
...
TEST(BindTest, StoreByCopy) {
  Struct s = {"hello"};
  auto f = turbo::bind_back(&Struct::value, s);
  auto g = f;
  EXPECT_EQ("hello", f());
  EXPECT_EQ("hello", g());
  EXPECT_NE(&s.value, &f());
...
TEST(BindTest, StoreByRef) {
  NonCopyable s("hello");
  auto f = turbo::bind_back(&GetNonCopyableValue, std::ref(s));
  EXPECT_EQ("hello", f());
  EXPECT_EQ(&s.value, &f());
...
TEST(BindTest, StoreByCRef) {
  NonCopyable s("hello");
  auto f = turbo::bind_back(&GetNonCopyableValue, std::cref(s));
  EXPECT_EQ("hello", f());
  EXPECT_EQ(&s.value, &f());
  auto g = std::move(f);  // NOLINT
  EXPECT_EQ("hello", g());
...
TEST(BindTest, StoreByRefInvokeByWrapper) {
  NonCopyable s("hello");
  auto f = turbo::bind_back(GetNonCopyableValueByWrapper, std::ref(s));
  EXPECT_EQ("hello", f());
  EXPECT_EQ(&s.value, &f());
  auto g = std::move(f);
  EXPECT_EQ("hello", g());
...
TEST(BindTest, StoreByPointer) {
  NonCopyable s("hello");
  auto f = turbo::bind_back(&NonCopyable::value, &s);
  EXPECT_EQ("hello", f());
  EXPECT_EQ(&s.value, &f());
  auto g = std::move(f);
  EXPECT_EQ("hello", g());
...
TEST(BindTest, MemberFunctionFreeInstance) {
  MyStruct s{10};
  auto f = turbo::bind_back(&MyStruct::Add, 5);
  EXPECT_EQ(f(s), 15);
}
...
TEST(BindTest, NonCopyableArg) {
  EXPECT_EQ(42, turbo::bind_back(Sink)(std::make_unique<int>(42)));
  EXPECT_EQ(42, turbo::bind_back(Sink, std::make_unique<int>(42))());
}

TEST(BindTest, NonCopyableResult) {
  EXPECT_THAT(turbo::bind_back(Factory)(42), ::testing::Pointee(42));
  EXPECT_THAT(turbo::bind_back(Factory, 42)(), ::testing::Pointee(42));
}
...
TEST(BindTest, WrappedMoveOnly) {
  ...
  auto f = turbo::bind_back(&GetMember, std::move(x));
...
TEST(BindTest, ConstExpr) {
  constexpr auto f = turbo::bind_back(CharAt);
  EXPECT_EQ(f("ABC", 1), 'B');
  static constexpr int five = 5;
  constexpr auto plus5 = turbo::bind_back(Plus, five);
  EXPECT_EQ(plus5(1), 6);

  static constexpr char data[] = "DEF";
  constexpr auto g = turbo::bind_back(CharAt, 1);
  EXPECT_EQ(g(data), 'E');
}
...
TEST(BindTest, Mangling) {
  ...
  turbo::bind_back(ManglingCall{}, 3.3, "A")(1);
}
