...
TYPED_TEST_P(HashValueIntTest, FastPath) {
  ...
  TypeParam n = 42;
  EXPECT_EQ(turbo::Hash<TypeParam>{}(n),
            turbo::Hash<std::tuple<TypeParam>>{}(std::tuple<TypeParam>(n)));
}
...
TEST(HashValueTest, HashConsistentAcrossIntTypes){
  std::vector<size_t> hashes = {
      turbo::Hash<int8_t>{}(1),  turbo::Hash<uint8_t>{}(1),
      turbo::Hash<int16_t>{}(1), turbo::Hash<uint16_t>{}(1),
      turbo::Hash<int32_t>{}(1), turbo::Hash<uint32_t>{}(1),
      turbo::Hash<int64_t>{}(1), turbo::Hash<uint64_t>{}(1)};
  EXPECT_THAT(hashes, testing::Each(turbo::Hash<int>{}(1)));
}
...
TEST(HashValueTest, PointerAlignment) {
  ...
  for (size_t align = 1; align < kTotalSize / kNumValues;
    ...
    for (size_t i = 0; i < kNumValues; ++i) {
      size_t hash = turbo::Hash<void*>()(data.get() + i * align);
      bits_or |= hash;
      bits_and &= hash;
    }
    ...
    int stuck_bit_count = turbo::popcount(stuck_bits);
...
TEST(HashValueTest, TestIntrinsicInt128) {
  ...
  for (int i = 0; i < 128; ++i) {
    ...
    auto [it, inserted] =
        hash_to_index.insert({turbo::Hash<__uint128_t>{}(value), i});
    ASSERT_TRUE(inserted) << "Duplicate hash: " << i << " vs " << it->second;

    // Verify that the fast-path for MixingHashState does not break the hash.
    EXPECT_EQ(turbo::HashOf(value), turbo::Hash<__uint128_t>{}(value));
    EXPECT_EQ(turbo::HashOf(as_signed), turbo::Hash<__int128_t>{}(as_signed));
  }
...
TEST(HashValueTest, Strings) {
  ...
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly(std::make_tuple(
      std::string(), std::string_view(), turbo::Cord(), std::string(""),
      std::string_view(""), turbo::Cord(""), std::string(small),
      std::string_view(small), turbo::Cord(small), FragmentedCord(small),
  ...
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly(std::make_tuple(
      ...
      t(std::string("")), t(std::string_view("")), t(turbo::Cord("")),
      t(std::string(small)), t(std::string_view(small)), t(turbo::Cord(small)),
      t(FragmentedCord(small)), t(std::string(dup)), t(std::string_view(dup)),
      t(turbo::Cord(dup)), t(std::string(large)), t(std::string_view(large)),
      t(turbo::Cord(large)), t(std::string(huge)), t(std::string_view(huge)),
...
TEST(HashValueTest, WStringView) {
  ...
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly(std::make_tuple(
      ...
      std::wstring_view(L"Some other different std::string_view"),
...
TEST(HashValueTest, U8StringView) {
  ...
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly(
      std::make_tuple(std::u8string_view(), std::u8string_view(u8"ABC"),
                      ...
                      std::u8string_view(u8"Some other different std::string_view"),
...
TEST(HashValueTest, U16StringView) {
  ...
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly(
      std::make_tuple(std::u16string_view(), std::u16string_view(u"ABC"),
                      ...
                      std::u16string_view(u"Some other different std::string_view"),
...
TEST(HashValueTest, U32StringView) {
  ...
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly(
      std::make_tuple(std::u32string_view(), std::u32string_view(U"ABC"),
                      ...
                      std::u32string_view(U"Some other different std::string_view"),
...
class PiecewiseHashTester {
 public:
  ...
  friend H TurboHashValue(H h, const PiecewiseHashTester& p) {
    ...
    turbo::hash_internal::PiecewiseCombiner combiner;
...
TEST(HashValueTest, CombinePiecewiseBuffer) {
  turbo::Hash<PiecewiseHashTester> hash;
  ...
  EXPECT_NE(hash(PiecewiseHashTester("foobar", {3})),
            turbo::Hash<DummyFooBar>()(DummyFooBar{}));
  ...
  // that isn't.
  const size_t kChunkSize = turbo::hash_internal::PiecewiseChunkSize();
...
TEST(IsHashableTest, ValidHash) {
  EXPECT_TRUE((is_hashable<int>::value));
  EXPECT_TRUE(std::is_default_constructible_v<turbo::Hash<int>>);
  EXPECT_TRUE(std::is_copy_constructible_v<turbo::Hash<int>>);
  EXPECT_TRUE(std::is_move_constructible_v<turbo::Hash<int>>);
  EXPECT_TRUE(std::is_copy_assignable_v<turbo::Hash<int>>);
  EXPECT_TRUE(std::is_move_assignable_v<turbo::Hash<int>>);
...
TEST(IsHashableTest, PoisonHash) {
  ...
  EXPECT_FALSE(std::is_default_constructible_v<turbo::Hash<X>>);
  EXPECT_FALSE(std::is_copy_constructible_v<turbo::Hash<X>>);
  EXPECT_FALSE(std::is_move_constructible_v<turbo::Hash<X>>);
  EXPECT_FALSE(std::is_copy_assignable_v<turbo::Hash<X>>);
  EXPECT_FALSE(std::is_move_assignable_v<turbo::Hash<X>>);
...
TEST(IsUniquelyRepresentedTest, SanityTest) {
  using turbo::hash_internal::is_uniquely_represented;
...
TEST(HashTest, DoesNotUseImplicitConversionsToBool) {
  EXPECT_NE(turbo::Hash<ValueWithBoolConversion>()(ValueWithBoolConversion{0}),
...
struct TypeErasedHashStateUser {
  ...
  template <typename H>
  friend H TurboHashValue(H state, const TypeErasedHashStateUser& value) {
    turbo::HashState type_erased_state = turbo::HashState::Create(&state);
    turbo::HashState::combine(std::move(type_erased_state), value.a, value.b);
    return state;
  }
};
...
TEST(HashOf, MatchesTypeErasedHashState) {
  std::string s = "s";
  EXPECT_EQ(turbo::HashOf(1, s), turbo::Hash<TypeErasedHashStateUser>{}(
...
struct AutoReturnTypeUser {
  ...
  template <typename H>
  friend auto TurboHashValue(H state, const AutoReturnTypeUser& value) {
...
TEST(HashOf, AutoReturnTypeUser) {
  std::string s = "s";
  EXPECT_EQ(turbo::HashOf(1, s),
            turbo::Hash<AutoReturnTypeUser>{}(AutoReturnTypeUser{1, s}));
}

TEST(HashOf, DoubleSignCollision) {
  // These values differ only in their most significant bit.
  EXPECT_NE(turbo::HashOf(-1.0), turbo::HashOf(1.0));
}
      ...
      for (s2[0] = kMinChar; s2[0] < kMaxChar; ++s2[0]) {
        for (s2[1] = kMinChar; s2[1] < kMaxChar; ++s2[1]) {
          for (s2[2] = kMinChar; s2[2] < kMaxChar; ++s2[2]) {
            ASSERT_NE(turbo::HashOf(s1), turbo::HashOf(s2))
