...
#include <turbo/log/internal/generic_printer.h>
...
auto HasExactlyNInstancesOf(int n, std::string_view me) {
#ifdef GTEST_USES_SIMPLE_RE
  (void)n;
  return HasSubstr(me);
#else
...
TEST(GenericPrinterTest, TurboStringifiable) {
  EXPECT_EQ("TurboStringifiable!", GenericPrintToString(TurboStringifiable{}));
}
...
TEST(GenericPrinterTest, Strings) {
  ...
  EXPECT_EQ(expected_nonprintable,
            GenericPrintToString(std::string_view("\0\315\n\xAB", 4)));
}
...
TEST(GenericPrinterTest, PreciseFPUnderCommaRadixLocale) {
  // The values are formatted with locale-independent turbo::str_sprintf (a '.'
...
struct CustomContainer : public std::array<int, 4> {
  ...
  friend void turbo_stringify(Sink& sink, const CustomContainer& c) {
    turbo::str_printf_to(&sink, "%d %d", c[0], c[1]);
  }
};

// Checks that turbo_stringify (go/totw/215) is respected for container-like
// types.
...
TEST(GenericPrinterTest, SmartPointerPrintsAddressOfPointee) {
  ...
  EXPECT_THAT(GenericPrintToString(i),
              AnyOf(Eq(turbo::str_sprintf("<%0*" PRIXPTR " pointing to 5>",
                                       sizeof(void*) * 2,
                                       reinterpret_cast<uintptr_t>(&*i))),
                    Eq(turbo::str_sprintf("<%#x pointing to 5>",
                                       reinterpret_cast<uintptr_t>(&*i)))));

  EXPECT_THAT(
      GenericPrintToString(c),
      AnyOf(HasSubstr(turbo::str_sprintf("<%0*" PRIXPTR " pointing to 'z'",
                                      sizeof(void*) * 2,
                                      reinterpret_cast<uintptr_t>(&*c))),
            HasSubstr(turbo::str_sprintf("<%#x pointing to 'z'",
                                      reinterpret_cast<uintptr_t>(&*c)))));

  EXPECT_THAT(GenericPrintToString(cp),
              AnyOf(Eq(turbo::str_sprintf("<%0*" PRIXPTR " pointing to abcdefg>",
                                       sizeof(void*) * 2,
                                       reinterpret_cast<uintptr_t>(&*cp))),
                    Eq(turbo::str_sprintf("<%#x pointing to abcdefg>",
                                       reinterpret_cast<uintptr_t>(&*cp)))));
}
...
TEST(GenericPrinterTest, SmartPointerToArrayOnlyPrintsAddressAndHelpText) {
  ...
  EXPECT_THAT(
      ...
      AllOf(AnyOf(HasSubstr(turbo::str_sprintf(
                      "%0*" PRIXPTR, sizeof(void*) * 2,
                      reinterpret_cast<uintptr_t>(nonempty.get()))),
                  HasSubstr(turbo::str_sprintf(
