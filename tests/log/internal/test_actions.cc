...
void WriteEntryToStderr::operator()(const turbo::LogEntry& entry) const {
  ...
  const std::string source_filename = turbo::c_hex_escape(entry.source_filename());
  const std::string source_basename = turbo::c_hex_escape(entry.source_basename());
  const std::string text_message = turbo::c_hex_escape(entry.text_message());
  const std::string encoded_message = turbo::c_hex_escape(entry.encoded_message());
...
void WriteEntryToStderr::operator()(turbo::LogSeverity severity,
  ...
  if (!message.empty()) std::cerr << message << "\n";
  const std::string source_filename = turbo::c_hex_escape(filename);
  const std::string text_message = turbo::c_hex_escape(log_message);
  std::cerr << "LogEntry{\n"                                         //
            << "  source_filename: \"" << source_filename << "\"\n"  //
