...
Matcher<turbo::Time> InMatchWindow() {
  return AllOf(Ge(turbo::Now()),
               Truly([](turbo::Time arg) { return arg <= turbo::Now(); }));
}
...
// `FATAL`, `QFATAL`, `KCHECK`, etc., but assertions inside a death test
