...
class LogEntryTestPeer {
 public:
  LogEntryTestPeer(std::string_view base_filename, int line, bool prefix,
    ...
    log_internal::append_truncated(text_message, view);
  ...
  std::string format_log_message() const {
    return log_internal::format_log_message(
  ...
  std::string FormatPrefixIntoSizedBuffer(size_t sz) {
    ...
    const size_t prefix_size = log_internal::format_log_prefix(
        entry_.log_severity(), entry_.timestamp(), entry_.tid(),
...
TEST(LogEntryTest, Baseline) {
  LogEntryTestPeer entry("foo.cc", 1234, kUsePrefix, turbo::LogSeverity::kInfo,
                         ...
                         "hello world");
  EXPECT_THAT(entry.format_log_message(),
...
TEST(LogEntryTest, EmptyFields) {
  LogEntryTestPeer entry("", 0, kUsePrefix, turbo::LogSeverity::kInfo,
                         ...
                         turbo::log_internal::PrefixFormat::kNotRaw, "");
  const std::string format_message = entry.format_log_message();
...
TEST(LogEntryTest, NegativeFields) {
  ...
  if (std::is_signed_v<turbo::LogEntry::tid_t>) {
