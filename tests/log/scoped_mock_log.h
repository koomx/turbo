...
// ScopedMockLog
//
// ScopedMockLog is a LogSink that intercepts KLOG() messages issued by all
// threads when active.
//
// Using this together with GoogleTest, it's easy to test how a piece of code
// calls KLOG(). The typical usage, noting the distinction between
...
//    }
//
// Note that in a multi-threaded environment, all KLOG() messages from a single
...
class ScopedMockLog final {
 public:
  ...
  // `KLOG(...).to_sink_only()`) and marks the `ScopedMockLog` as having been used
