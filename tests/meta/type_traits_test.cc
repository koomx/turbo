...
using IsViewAndNotOwner =
    std::conjunction<turbo::type_traits_internal::IsView<T>,
                     std::negation<turbo::type_traits_internal::IsOwner<T>>>;

template <typename T>
using IsOwnerAndNotView =
    std::conjunction<turbo::type_traits_internal::IsOwner<T>,
                      std::negation<turbo::type_traits_internal::IsView<T>>>;
...
TEST(VoidTTest, BasicUsage) {
  StaticAssertTypeEq<void, std::void_t<Dummy>>();
  StaticAssertTypeEq<void, std::void_t<Dummy, Dummy, Dummy>>();
}

TEST(TypeTraitsTest, TestRemoveCVRef) {
  EXPECT_TRUE((std::is_same_v<typename turbo::remove_cvref<int>::type, int>));
  EXPECT_TRUE((std::is_same_v<typename turbo::remove_cvref<int&>::type, int>));
  EXPECT_TRUE((std::is_same_v<typename turbo::remove_cvref<int&&>::type, int>));
  EXPECT_TRUE(
      (std::is_same_v<typename turbo::remove_cvref<const int&>::type, int>));
  EXPECT_TRUE((std::is_same_v<typename turbo::remove_cvref<int*>::type, int*>));
  // Does not remove const in this case.
  EXPECT_TRUE((std::is_same_v<typename turbo::remove_cvref<const int*>::type,
                              const int*>));
  EXPECT_TRUE(
      (std::is_same_v<typename turbo::remove_cvref<int[2]>::type, int[2]>));
  EXPECT_TRUE(
      (std::is_same_v<typename turbo::remove_cvref<int (&)[2]>::type, int[2]>));
  EXPECT_TRUE(
      (std::is_same_v<typename turbo::remove_cvref<int (&&)[2]>::type, int[2]>));
  EXPECT_TRUE((
      std::is_same_v<typename turbo::remove_cvref<const int[2]>::type, int[2]>));
  EXPECT_TRUE(
      (std::is_same_v<typename turbo::remove_cvref<const int (&)[2]>::type,
                      int[2]>));
  EXPECT_TRUE(
      (std::is_same_v<typename turbo::remove_cvref<const int (&&)[2]>::type,
                      int[2]>));
...
TEST(TypeTraitsTest, TestTypeIdentity) {
  EXPECT_TRUE((std::is_same_v<typename turbo::type_identity<int>::type, int>));
  EXPECT_TRUE((std::is_same_v<turbo::type_identity_t<int>, int>));
  EXPECT_TRUE((std::is_same_v<typename turbo::type_identity<int&>::type, int&>));
  EXPECT_TRUE((std::is_same_v<turbo::type_identity_t<int&>, int&>));

  EXPECT_FALSE(
      (std::is_same_v<typename turbo::type_identity<int64_t>::type, int32_t>));
  EXPECT_FALSE((std::is_same_v<turbo::type_identity_t<int64_t>, int32_t>));
}
