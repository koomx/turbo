...
TYPED_TEST(Uint128IntegerTraitsTest, ConstructAssignTest) {
  static_assert(std::is_constructible_v<turbo::uint128, TypeParam>,
                "turbo::uint128 must be constructible from TypeParam");
  static_assert(std::is_assignable_v<turbo::uint128&, TypeParam>,
                "turbo::uint128 must be assignable from TypeParam");
  static_assert(!std::is_assignable_v<TypeParam&, turbo::uint128>,
...
TYPED_TEST(Uint128FloatTraitsTest, ConstructAssignTest) {
  static_assert(std::is_constructible_v<turbo::uint128, TypeParam>,
                "turbo::uint128 must be constructible from TypeParam");
  static_assert(!std::is_assignable_v<turbo::uint128&, TypeParam>,
                "turbo::uint128 must not be assignable from TypeParam");
  static_assert(!std::is_assignable_v<TypeParam&, turbo::uint128>,
                "TypeParam must not be assignable from turbo::uint128");
}

#if KUMO_HAVE_INTRINSIC_INT128
...
TEST(Uint128, IntrinsicTypeTraitsTest) {
  static_assert(std::is_constructible_v<turbo::uint128, __int128>,
                "turbo::uint128 must be constructible from __int128");
  static_assert(std::is_assignable_v<turbo::uint128&, __int128>,
                "turbo::uint128 must be assignable from __int128");
  static_assert(!std::is_assignable_v<__int128&, turbo::uint128>,
                "__int128 must not be assignable from turbo::uint128");

  static_assert(std::is_constructible_v<turbo::uint128, unsigned __int128>,
                "turbo::uint128 must be constructible from unsigned __int128");
  static_assert(std::is_assignable_v<turbo::uint128&, unsigned __int128>,
                "turbo::uint128 must be assignable from unsigned __int128");
  static_assert(!std::is_assignable_v<unsigned __int128&, turbo::uint128>,
...
TEST(Uint128, TrivialTraitsTest) {
  static_assert(std::is_trivially_default_constructible_v<turbo::uint128>, "");
  static_assert(std::is_trivially_copy_constructible_v<turbo::uint128>, "");
  static_assert(std::is_trivially_copy_assignable_v<turbo::uint128>, "");
  static_assert(std::is_trivially_destructible_v<turbo::uint128>, "");
}

TEST(Uint128, AllTests) {
  turbo::uint128 zero = 0;
  turbo::uint128 one = 1;
  turbo::uint128 one_2arg = turbo::MakeUint128(0, 1);
  turbo::uint128 two = 2;
  turbo::uint128 three = 3;
  turbo::uint128 big = turbo::MakeUint128(2000, 2);
  turbo::uint128 big_minus_one = turbo::MakeUint128(2000, 1);
  turbo::uint128 bigger = turbo::MakeUint128(2001, 1);
  turbo::uint128 biggest = turbo::Uint128Max();
  turbo::uint128 high_low = turbo::MakeUint128(1, 0);
  turbo::uint128 low_high =
      turbo::MakeUint128(0, std::numeric_limits<uint64_t>::max());
  EXPECT_LT(one, two);
  EXPECT_GT(two, one);
  EXPECT_LT(one, big);
  EXPECT_LT(one, big);
  ...
  EXPECT_EQ(turbo::Uint128High64(biggest), std::numeric_limits<uint64_t>::max());
  EXPECT_EQ(turbo::Uint128Low64(biggest), std::numeric_limits<uint64_t>::max());
  EXPECT_EQ(zero + one, one);
  ...
  EXPECT_EQ(turbo::Uint128High64((turbo::uint128(1) << 64) - 1), 0);
  EXPECT_EQ(turbo::Uint128Low64((turbo::uint128(1) << 64) - 1),
  ...
  EXPECT_EQ(big, +big);
  EXPECT_EQ(two, +two);
  EXPECT_EQ(turbo::Uint128Max(), +turbo::Uint128Max());
  EXPECT_EQ(zero, +zero);

  EXPECT_EQ(big, -(-big));
  EXPECT_EQ(two, -((-one) - 1));
  EXPECT_EQ(turbo::Uint128Max(), -one);
  EXPECT_EQ(zero, -zero);
}
...
TEST(Int128, RightShiftOfNegativeNumbers) {
  ...
  if ((-6 >> 1) == -3) {
    ...
    EXPECT_EQ(minus_six >> 1, turbo::int128(turbo::uint128(minus_six) >> 1));
    EXPECT_EQ(minus_six >> 2, turbo::int128(turbo::uint128(minus_six) >> 2));
    EXPECT_EQ(minus_six >> 65, turbo::int128(turbo::uint128(minus_six) >> 65));
  }
}
...
TEST(Uint128, ConversionTests) {
  ...
  turbo::uint128 custom =
      turbo::MakeUint128(0x3a5b76c209de76f6, 0x1f25e1d63a2b46c5);

  EXPECT_EQ(custom, turbo::uint128(intrinsic));
  ...
  turbo::uint128 from_precise_ints =
      turbo::MakeUint128(0x530e, 0xda74000000000000);
  ...
  EXPECT_EQ(static_cast<turbo::uint128>(round_to_zero), 0);
  EXPECT_EQ(static_cast<turbo::uint128>(round_to_five), 5);
  EXPECT_EQ(static_cast<turbo::uint128>(round_to_nine), 9);
  ...
  const turbo::uint128 arbitrary_mask =
      turbo::MakeUint128(0xa29f622677ded751, 0xf8ca66add076f468);
  ...
  EXPECT_EQ(static_cast<turbo::uint128>(-0.1L), 0);
}
...
TEST(Uint128, Multiply) {
  ...
  for (int i = 0; i < 64; ++i) {
    for (int j = 0; j < 64; ++j) {
      ...
      b = turbo::uint128(1) << j;
      c = a * b;
      EXPECT_EQ(turbo::uint128(1) << (i + j), c);
    }
  }

  // Verified with dc.
  a = turbo::MakeUint128(0xffffeeeeddddcccc, 0xbbbbaaaa99998888);
  b = turbo::MakeUint128(0x7777666655554444, 0x3333222211110000);
  c = a * b;
  EXPECT_EQ(turbo::MakeUint128(0x530EDA741C71D4C3, 0xBF25975319080000), c);
  EXPECT_EQ(0, c - b * a);
  EXPECT_EQ(a * a - b * b, (a + b) * (a - b));

  // Verified with dc.
  a = turbo::MakeUint128(0x0123456789abcdef, 0xfedcba9876543210);
  b = turbo::MakeUint128(0x02468ace13579bdf, 0xfdb97531eca86420);
  c = a * b;
  EXPECT_EQ(turbo::MakeUint128(0x97a87f4f261ba3f2, 0x342d0bbf48948200), c);
  EXPECT_EQ(0, c - b * a);
  EXPECT_EQ(a * a - b * b, (a + b) * (a - b));
}

TEST(Uint128, AliasTests) {
  turbo::uint128 x1 = turbo::MakeUint128(1, 2);
  turbo::uint128 x2 = turbo::MakeUint128(2, 4);
  x1 += x1;
  EXPECT_EQ(x2, x1);

  turbo::uint128 x3 = turbo::MakeUint128(1, static_cast<uint64_t>(1) << 63);
  turbo::uint128 x4 = turbo::MakeUint128(3, 0);
  x3 += x3;
  EXPECT_EQ(x4, x3);
}
...
TEST(Uint128, DivideAndMod) {
  ...
  a = turbo::MakeUint128(0x530eda741c71d4c3, 0xbf25975319080000);
  q = turbo::MakeUint128(0x4de2cab081, 0x14c34ab4676e4bab);
  ...
  turbo::uint128 expected_r =
      turbo::MakeUint128(0x29876d3a0e38ea61, 0xdf92cba98c83ffff);
  // Sanity checks.
...
TEST(Uint128, ConstexprTest) {
  ...
  EXPECT_EQ(zero, turbo::uint128(0));
  EXPECT_EQ(one, turbo::uint128(1));
  EXPECT_EQ(minus_two, turbo::MakeUint128(-1, -2));

  constexpr double f = static_cast<float>(turbo::uint128(123));
  EXPECT_EQ(f, 123.0f);

  constexpr double d = static_cast<double>(turbo::uint128(123));
  EXPECT_EQ(d, 123.0);

  constexpr long double ld = static_cast<long double>(turbo::uint128(123));
  EXPECT_EQ(ld, 123.0);

#if KUMO_HAVE_INTRINSIC_INT128
  constexpr turbo::uint128 division = turbo::uint128(10) / turbo::uint128(2);
  EXPECT_EQ(division, turbo::uint128(5));

  constexpr turbo::uint128 modulus = turbo::int128(10) % turbo::int128(3);
  EXPECT_EQ(modulus, turbo::uint128(1));

  constexpr turbo::uint128 multiplication = turbo::uint128(10) * turbo::uint128(3);
  EXPECT_EQ(multiplication, turbo::uint128(30));
#endif  // KUMO_HAVE_INTRINSIC_INT128
}
...
TEST(Uint128, NumericLimitsTest) {
  ...
  EXPECT_EQ(0, std::numeric_limits<turbo::uint128>::min());
  EXPECT_EQ(0, std::numeric_limits<turbo::uint128>::lowest());
  EXPECT_EQ(turbo::Uint128Max(), std::numeric_limits<turbo::uint128>::max());
}

// Some arbitrary constant to test hashing. The first hex digits of pi.
constexpr turbo::uint128 kPi = (turbo::uint128(0x3243f6a8885a308d) << 64) |
                              turbo::uint128(0x313198a2e0370734);
...
TEST(Uint128, Hash) {
  ...
  const auto values = std::make_tuple(
      // Some simple values
      turbo::uint128{0}, turbo::uint128{1}, ~turbo::uint128{},
      // 64 bit limits
      turbo::uint128{std::numeric_limits<int64_t>::max()},
      turbo::uint128{std::numeric_limits<uint64_t>::max()} + 0,
      turbo::uint128{std::numeric_limits<uint64_t>::max()} + 1,
      turbo::uint128{std::numeric_limits<uint64_t>::max()} + 2,
      // Keeping high same
      turbo::uint128{1} << 62, turbo::uint128{1} << 63,
      // Keeping low same
      turbo::uint128{1} << 64, turbo::uint128{1} << 65,
      // 128 bit limits
      std::numeric_limits<turbo::uint128>::max(),
      std::numeric_limits<turbo::uint128>::max() - 1,
      std::numeric_limits<turbo::uint128>::min() + 1,
      std::numeric_limits<turbo::uint128>::min(),
      // arbitrary constant
      kPi
#if KUMO_HAVE_INTRINSIC_INT128
  ...
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly(values));
}
...
TEST(Int128Uint128, ConversionTest) {
  turbo::int128 nonnegative_signed_values[] = {
      ...
      1,
      0xffeeddccbbaa9988,
      turbo::MakeInt128(0x7766554433221100, 0),
      turbo::MakeInt128(0x1234567890abcdef, 0xfedcba0987654321),
      turbo::Int128Max()};
  for (turbo::int128 value : nonnegative_signed_values) {
    EXPECT_EQ(value, turbo::int128(turbo::uint128(value)));

    turbo::uint128 assigned_value;
    assigned_value = value;
    EXPECT_EQ(value, turbo::int128(assigned_value));
  }

  turbo::int128 negative_values[] = {
      -1, -0x1234567890abcdef, turbo::MakeInt128(-0x5544332211ffeedd, 0),
      -turbo::MakeInt128(0x76543210fedcba98, 0xabcdef0123456789)};
  for (turbo::int128 value : negative_values) {
    EXPECT_EQ(turbo::uint128(-value), -turbo::uint128(value));

    turbo::uint128 assigned_value;
    assigned_value = value;
    EXPECT_EQ(turbo::uint128(-value), -assigned_value);
  }
}
...
TYPED_TEST(Int128IntegerTraitsTest, ConstructAssignTest) {
  static_assert(std::is_constructible_v<turbo::int128, TypeParam>,
                "turbo::int128 must be constructible from TypeParam");
  static_assert(std::is_assignable_v<turbo::int128&, TypeParam>,
                "turbo::int128 must be assignable from TypeParam");
  static_assert(!std::is_assignable_v<TypeParam&, turbo::int128>,
...
TYPED_TEST(Int128FloatTraitsTest, ConstructAssignTest) {
  static_assert(std::is_constructible_v<turbo::int128, TypeParam>,
                "turbo::int128 must be constructible from TypeParam");
  static_assert(!std::is_assignable_v<turbo::int128&, TypeParam>,
                "turbo::int128 must not be assignable from TypeParam");
  static_assert(!std::is_assignable_v<TypeParam&, turbo::int128>,
                "TypeParam must not be assignable from turbo::int128");
}
...
TEST(Int128, IntrinsicTypeTraitsTest) {
  static_assert(std::is_constructible_v<turbo::int128, __int128>,
                "turbo::int128 must be constructible from __int128");
  static_assert(std::is_assignable_v<turbo::int128&, __int128>,
                "turbo::int128 must be assignable from __int128");
  static_assert(!std::is_assignable_v<__int128&, turbo::int128>,
                "__int128 must not be assignable from turbo::int128");

  static_assert(std::is_constructible_v<turbo::int128, unsigned __int128>,
                "turbo::int128 must be constructible from unsigned __int128");
  static_assert(!std::is_assignable_v<turbo::int128&, unsigned __int128>,
                "turbo::int128 must be assignable from unsigned __int128");
  static_assert(!std::is_assignable_v<unsigned __int128&, turbo::int128>,
                "unsigned __int128 must not be assignable from turbo::int128");
}
#endif  // KUMO_HAVE_INTRINSIC_INT128

TEST(Int128, TrivialTraitsTest) {
  static_assert(std::is_trivially_default_constructible_v<turbo::int128>, "");
  static_assert(std::is_trivially_copy_constructible_v<turbo::int128>, "");
  static_assert(std::is_trivially_copy_assignable_v<turbo::int128>, "");
  static_assert(std::is_trivially_destructible_v<turbo::int128>, "");
}

TEST(Int128, BoolConversionTest) {
  EXPECT_FALSE(turbo::int128(0));
  for (int i = 0; i < 64; ++i) {
    EXPECT_TRUE(turbo::MakeInt128(0, uint64_t{1} << i));
  }
  for (int i = 0; i < 63; ++i) {
    EXPECT_TRUE(turbo::MakeInt128(int64_t{1} << i, 0));
  }
  EXPECT_TRUE(turbo::Int128Min());

  EXPECT_EQ(turbo::int128(1), turbo::int128(true));
  EXPECT_EQ(turbo::int128(0), turbo::int128(false));
}
...
TYPED_TEST(Int128IntegerConversionTest, RoundTripTest) {
  EXPECT_EQ(TypeParam{0}, static_cast<TypeParam>(turbo::int128(0)));
  EXPECT_EQ(std::numeric_limits<TypeParam>::min(),
            static_cast<TypeParam>(
                turbo::int128(std::numeric_limits<TypeParam>::min())));
  EXPECT_EQ(std::numeric_limits<TypeParam>::max(),
            static_cast<TypeParam>(
                turbo::int128(std::numeric_limits<TypeParam>::max())));
}
...
TYPED_TEST(Int128FloatConversionTest, ConstructAndCastTest) {
  ...
  for (int i = 0; i < 110; ++i) {  // 110 = 126 - #bits in 0x9f5b
    ...
    EXPECT_EQ(-float_value, static_cast<TypeParam>(-int_value));
    EXPECT_EQ(int_value, turbo::int128(float_value));
    EXPECT_EQ(-int_value, turbo::int128(-float_value));
  }
  ...
  for (uint64_t value : values) {
    for (int i = 0; i <= 64; ++i) {
      ...
      EXPECT_DOUBLE_EQ(fvalue, static_cast<TypeParam>(turbo::int128(fvalue)));
      EXPECT_DOUBLE_EQ(-fvalue, static_cast<TypeParam>(-turbo::int128(fvalue)));
      EXPECT_DOUBLE_EQ(-fvalue, static_cast<TypeParam>(turbo::int128(-fvalue)));
      EXPECT_DOUBLE_EQ(fvalue, static_cast<TypeParam>(-turbo::int128(-fvalue)));
    }
  ...
  turbo::int128 large_values[] = {
      turbo::MakeInt128(0x5b0640d96c7b3d9f, 0xb7a7189e51d18622),
      turbo::MakeInt128(0x34bed042c6f65270, 0x73b236570669a089),
      turbo::MakeInt128(0x43deba9e6da12724, 0xf7f0f83da686797d),
      turbo::MakeInt128(0x71e8d383be4e5589, 0x75c3f96fb00752b6)};
  for (turbo::int128 value : large_values) {
    ...
    value |= turbo::int128(1) << (std::numeric_limits<TypeParam>::digits - 1);
    value |= 1;
    for (int i = 0; i < 127 - std::numeric_limits<TypeParam>::digits; ++i) {
      turbo::int128 int_value = value << i;
      EXPECT_EQ(int_value,
                static_cast<turbo::int128>(static_cast<TypeParam>(int_value)));
      EXPECT_EQ(-int_value,
                static_cast<turbo::int128>(static_cast<TypeParam>(-int_value)));
    }
  }
  ...
  EXPECT_EQ(0, turbo::int128(TypeParam(0.1)));
  EXPECT_EQ(17, turbo::int128(TypeParam(17.8)));
  EXPECT_EQ(0, turbo::int128(TypeParam(-0.8)));
  EXPECT_EQ(-53, turbo::int128(TypeParam(-53.1)));
  EXPECT_EQ(0, turbo::int128(TypeParam(0.5)));
  EXPECT_EQ(0, turbo::int128(TypeParam(-0.5)));
  TypeParam just_lt_one = std::nexttoward(TypeParam(1), TypeParam(0));
  EXPECT_EQ(0, turbo::int128(just_lt_one));
  TypeParam just_gt_minus_one = std::nexttoward(TypeParam(-1), TypeParam(0));
  EXPECT_EQ(0, turbo::int128(just_gt_minus_one));

  // Check limits
  EXPECT_DOUBLE_EQ(std::ldexp(static_cast<TypeParam>(1), 127),
                   static_cast<TypeParam>(turbo::Int128Max()));
  EXPECT_DOUBLE_EQ(-std::ldexp(static_cast<TypeParam>(1), 127),
                   static_cast<TypeParam>(turbo::Int128Min()));
}

TEST(Int128, FactoryTest) {
  EXPECT_EQ(turbo::int128(-1), turbo::MakeInt128(-1, -1));
  EXPECT_EQ(turbo::int128(-31), turbo::MakeInt128(-1, -31));
  EXPECT_EQ(turbo::int128(std::numeric_limits<int64_t>::min()),
            turbo::MakeInt128(-1, std::numeric_limits<int64_t>::min()));
  EXPECT_EQ(turbo::int128(0), turbo::MakeInt128(0, 0));
  EXPECT_EQ(turbo::int128(1), turbo::MakeInt128(0, 1));
  EXPECT_EQ(turbo::int128(std::numeric_limits<int64_t>::max()),
            turbo::MakeInt128(0, std::numeric_limits<int64_t>::max()));
}
...
TEST(Int128, HighLowTest) {
  ...
  for (const HighLowPair& pair : values) {
    turbo::int128 value = turbo::MakeInt128(pair.high, pair.low);
    EXPECT_EQ(pair.low, turbo::Int128Low64(value));
    EXPECT_EQ(pair.high, turbo::Int128High64(value));
  }
}

TEST(Int128, LimitsTest) {
  EXPECT_EQ(turbo::MakeInt128(0x7fffffffffffffff, 0xffffffffffffffff),
            turbo::Int128Max());
...
TEST(Int128, IntrinsicConversionTest) {
  ...
  turbo::int128 custom =
      turbo::MakeInt128(0x3a5b76c209de76f6, 0x1f25e1d63a2b46c5);
...
TEST(Int128, ConstexprTest) {
  ...
  constexpr turbo::int128 min = turbo::Int128Min();
  constexpr turbo::int128 max = turbo::Int128Max();
  EXPECT_EQ(zero, turbo::int128(0));
  EXPECT_EQ(one, turbo::int128(1));
  EXPECT_EQ(minus_two, turbo::MakeInt128(-1, -2));
  EXPECT_GT(max, one);
  EXPECT_LT(min, minus_two);

  constexpr double f = static_cast<float>(turbo::int128(123));
  EXPECT_EQ(f, 123.0f);

  constexpr double d = static_cast<double>(turbo::int128(123));
  EXPECT_EQ(d, 123.0);

  constexpr long double ld = static_cast<long double>(turbo::int128(123));
  EXPECT_EQ(ld, 123.0);

#if KUMO_HAVE_INTRINSIC_INT128
  constexpr turbo::int128 f_int128(static_cast<float>(123.0));
  EXPECT_EQ(f_int128, turbo::int128(123));

  constexpr turbo::int128 d_int128(static_cast<double>(123.0));
  EXPECT_EQ(d_int128, turbo::int128(123));

  constexpr turbo::int128 ld_int128(static_cast<long double>(123.0));
  EXPECT_EQ(ld_int128, turbo::int128(123));

  constexpr turbo::int128 division = turbo::int128(10) / turbo::int128(2);
  EXPECT_EQ(division, turbo::int128(5));

  constexpr turbo::int128 modulus = turbo::int128(10) % turbo::int128(3);
  EXPECT_EQ(modulus, turbo::int128(1));

  constexpr turbo::int128 multiplication = turbo::int128(10) * turbo::int128(3);
  EXPECT_EQ(multiplication, turbo::int128(30));
#endif  // KUMO_HAVE_INTRINSIC_INT128
}
...
TEST(Int128, ComparisonTest) {
  ...
  TestCase cases[] = {
      ...
      {turbo::MakeInt128(-12, 34), turbo::MakeInt128(12, 34)},
      {turbo::MakeInt128(1, 1000), turbo::MakeInt128(1000, 1)},
      {turbo::MakeInt128(-1000, 1000), turbo::MakeInt128(-1, 1)},
  };
  for (const TestCase& pair : cases) {
    ...
    EXPECT_EQ(pair.smaller <=> pair.larger, turbo::strong_ordering::less);
    EXPECT_EQ(pair.larger <=> pair.smaller, turbo::strong_ordering::greater);
    EXPECT_EQ(pair.smaller <=> pair.smaller, turbo::strong_ordering::equal);
    EXPECT_EQ(pair.larger <=> pair.larger, turbo::strong_ordering::equal);
#endif
  }
}
...
TEST(Int128, UnaryPlusTest) {
  ...
  for (int64_t value : values64) {
    ...
    EXPECT_EQ(turbo::int128(value), +turbo::int128(value));
    EXPECT_EQ(turbo::int128(-value), +turbo::int128(-value));
    EXPECT_EQ(turbo::MakeInt128(value, 0), +turbo::MakeInt128(value, 0));
    EXPECT_EQ(turbo::MakeInt128(-value, 0), +turbo::MakeInt128(-value, 0));
  }
}
...
TEST(Int128, LogicalNotTest) {
  EXPECT_TRUE(!turbo::int128(0));
  for (int i = 0; i < 64; ++i) {
    EXPECT_FALSE(!turbo::MakeInt128(0, uint64_t{1} << i));
  }
  for (int i = 0; i < 63; ++i) {
    EXPECT_FALSE(!turbo::MakeInt128(int64_t{1} << i, 0));
  }
}
...
TEST(Int128, AdditionSubtractionTest) {
  ...
  for (const auto& pair : cases) {
    ...
    EXPECT_EQ(turbo::int128(pair.first + pair.second),
              turbo::int128(pair.first) + turbo::int128(pair.second));
    EXPECT_EQ(turbo::int128(pair.second + pair.first),
    ...
    EXPECT_EQ(
        turbo::MakeInt128(pair.second + pair.first, 0),
        turbo::MakeInt128(pair.second, 0) + turbo::MakeInt128(pair.first, 0));
    EXPECT_EQ(
        turbo::MakeInt128(pair.first + pair.second, 0),
        turbo::MakeInt128(pair.first, 0) += turbo::MakeInt128(pair.second, 0));
  ...
  EXPECT_EQ(turbo::MakeInt128(31, 0),
            turbo::MakeInt128(20, 1) +
                turbo::MakeInt128(10, std::numeric_limits<uint64_t>::max()));
}
...
TEST(Int128, MultiplicationTest) {
  ...
  for (const std::pair<int64_t, int64_t>& pair : small_values) {
    ...
    EXPECT_EQ(turbo::MakeInt128(pair.first * pair.second, 0),
              turbo::MakeInt128(pair.first, 0) * turbo::int128(pair.second));
    EXPECT_EQ(turbo::MakeInt128(pair.first * pair.second, 0),
              turbo::MakeInt128(pair.first, 0) *= turbo::int128(pair.second));
  }
  ...
  for (const std::pair<int64_t, int64_t>& pair : small_values2) {
    ...
    turbo::int128 c = turbo::MakeInt128(pair.first * pair.second, 0);
  ...
  turbo::int128 large_values[] = {
      {turbo::MakeInt128(0xd66f061af02d0408, 0x727d2846cb475b53)},
      {turbo::MakeInt128(0x27b8d5ed6104452d, 0x03f8a33b0ee1df4f)},
      {-turbo::MakeInt128(0x621b6626b9e8d042, 0x27311ac99df00938)},
      {-turbo::MakeInt128(0x34e0656f1e95fb60, 0x4281cfd731257a47)},
  };
  for (turbo::int128 value : large_values) {
    EXPECT_EQ(0, 0 * value);
    EXPECT_EQ(0, value * 0);
    EXPECT_EQ(0, turbo::int128(0) *= value);
    EXPECT_EQ(0, value *= 0);

    EXPECT_EQ(value, 1 * value);
    EXPECT_EQ(value, value * 1);
    EXPECT_EQ(value, turbo::int128(1) *= value);
    EXPECT_EQ(value, value *= 1);
  ...
  EXPECT_EQ(turbo::MakeInt128(0xcd0efd3442219bb, 0xde47c05bcd9df6e1),
            turbo::MakeInt128(0x7c6448, 0x3bc4285c47a9d253) * 0x1a6037537b);
  EXPECT_EQ(-turbo::MakeInt128(0x1f8f149850b1e5e6, 0x1e50d6b52d272c3e),
            -turbo::MakeInt128(0x23, 0x2e68a513ca1b8859) * 0xe5a434cd14866e);
  EXPECT_EQ(-turbo::MakeInt128(0x55cae732029d1fce, 0xca6474b6423263e4),
            0xa9b98a8ddf66bc * -turbo::MakeInt128(0x81, 0x672e58231e2469d7));
  EXPECT_EQ(turbo::MakeInt128(0x19c8b7620b507dc4, 0xfec042b71a5f29a4),
            -0x3e39341147 * -turbo::MakeInt128(0x6a14b2, 0x5ed34cca42327b3c));
  ...
  EXPECT_EQ(-turbo::MakeInt128(0x55cae732029d1fce, 0xca6474b6423263e4),
            ...
            -turbo::MakeInt128(0x81, 0x672e58231e2469d7));
  EXPECT_EQ(turbo::MakeInt128(0x19c8b7620b507dc4, 0xfec042b71a5f29a4),
            turbo::int128(-0x3e39341147) *=
            -turbo::MakeInt128(0x6a14b2, 0x5ed34cca42327b3c));
}
...
TEST(Int128, DivisionAndModuloTest) {
  ...
  for (const std::pair<int64_t, int64_t>& pair : small_pairs) {
    ...
    EXPECT_EQ(quotient, turbo::int128(dividend) /= divisor);
    EXPECT_EQ(remainder, dividend % divisor);
    EXPECT_EQ(remainder, turbo::int128(dividend) %= divisor);
  }

  // Test behavior with 0, 1, and -1 with a sample of randomly generated large
  // values.
  turbo::int128 values[] = {
      turbo::MakeInt128(0x63d26ee688a962b2, 0x9e1411abda5c1d70),
      turbo::MakeInt128(0x152f385159d6f986, 0xbf8d48ef63da395d),
      -turbo::MakeInt128(0x3098d7567030038c, 0x14e7a8a098dc2164),
      -turbo::MakeInt128(0x49a037aca35c809f, 0xa6a87525480ef330),
  };
  for (turbo::int128 value : values) {
    ...
    EXPECT_EQ(value, value / 1);
    EXPECT_EQ(value, turbo::int128(value) /= 1);
    EXPECT_EQ(0, value % 1);
    EXPECT_EQ(0, turbo::int128(value) %= 1);
  ...
  // Min and max values
  EXPECT_EQ(0, turbo::Int128Max() / turbo::Int128Min());
  EXPECT_EQ(turbo::Int128Max(), turbo::Int128Max() % turbo::Int128Min());
  EXPECT_EQ(-1, turbo::Int128Min() / turbo::Int128Max());
  EXPECT_EQ(-1, turbo::Int128Min() % turbo::Int128Max());

  // Power of two division and modulo of random large dividends
  turbo::int128 positive_values[] = {
      turbo::MakeInt128(0x21e1a1cc69574620, 0xe7ac447fab2fc869),
      turbo::MakeInt128(0x32c2ff3ab89e66e8, 0x03379a613fd1ce74),
      turbo::MakeInt128(0x6f32ca786184dcaf, 0x046f9c9ecb3a9ce1),
      turbo::MakeInt128(0x1aeb469dd990e0ee, 0xda2740f243cd37eb),
  };
  for (turbo::int128 value : positive_values) {
    for (int i = 0; i < 127; ++i) {
      ...
      EXPECT_EQ(value >> i, turbo::int128(value) /= power_of_two);
  ...
  DivisionModCase manual_cases[] = {
      {turbo::MakeInt128(0x6ada48d489007966, 0x3c9c5c98150d5d69),
       turbo::MakeInt128(0x8bc308fb, 0x8cb9cc9a3b803344), 0xc3b87e08,
       turbo::MakeInt128(0x1b7db5e1, 0xd9eca34b7af04b49)},
      {turbo::MakeInt128(0xd6946511b5b, 0x4886c5c96546bf5f),
       -turbo::MakeInt128(0x263b, 0xfd516279efcfe2dc), -0x59cbabf0,
       turbo::MakeInt128(0x622, 0xf462909155651d1f)},
      {-turbo::MakeInt128(0x33db734f9e8d1399, 0x8447ac92482bca4d), 0x37495078240,
       -turbo::MakeInt128(0xf01f1, 0xbc0368bf9a77eae8), -0x21a508f404d},
      {-turbo::MakeInt128(0x13f837b409a07e7d, 0x7fc8e248a7d73560), -0x1b9f,
       turbo::MakeInt128(0xb9157556d724, 0xb14f635714d7563e), -0x1ade},
  };
...
TEST(Int128, BitwiseLogicTest) {
  EXPECT_EQ(turbo::int128(-1), ~turbo::int128(0));

  turbo::int128 values[]{
      0, -1, 0xde400bee05c3ff6b, turbo::MakeInt128(0x7f32178dd81d634a, 0),
      turbo::MakeInt128(0xaf539057055613a9, 0x7d104d7d946c2e4d)};
  for (turbo::int128 value : values) {
    ...
    EXPECT_EQ(value, turbo::int128(value) |= value);
    EXPECT_EQ(value, turbo::int128(value) &= value);
    EXPECT_EQ(0, turbo::int128(value) ^= value);

    EXPECT_EQ(value, value | 0);
    EXPECT_EQ(0, value & 0);
    EXPECT_EQ(value, value ^ 0);

    EXPECT_EQ(turbo::int128(-1), value | turbo::int128(-1));
    EXPECT_EQ(value, value & turbo::int128(-1));
    EXPECT_EQ(~value, value ^ turbo::int128(-1));
  }
  ...
  for (const std::pair<int64_t, int64_t>& pair : pairs64) {
    ...
    EXPECT_EQ(turbo::MakeInt128(~pair.first, ~pair.second),
              ~turbo::MakeInt128(pair.first, pair.second));
...
TEST(Int128, BitwiseShiftTest) {
  for (int i = 0; i < 64; ++i) {
    for (int j = 0; j <= i; ++j) {
      ...
      SCOPED_TRACE(::testing::Message() << "i = " << i << "; j = " << j);
      EXPECT_EQ(uint64_t{1} << i, turbo::int128(uint64_t{1} << j) << (i - j));
      EXPECT_EQ(uint64_t{1} << i, turbo::int128(uint64_t{1} << j) <<= (i - j));
    }
  }
  for (int i = 0; i < 63; ++i) {
    for (int j = 0; j < 64; ++j) {
      ...
      SCOPED_TRACE(::testing::Message() << "i = " << i << "; j = " << j);
      EXPECT_EQ(turbo::MakeInt128(uint64_t{1} << i, 0),
    ...
    for (int j = 0; j <= i; ++j) {
      ...
      SCOPED_TRACE(::testing::Message() << "i = " << i << "; j = " << j);
      EXPECT_EQ(turbo::MakeInt128(uint64_t{1} << i, 0),
                turbo::MakeInt128(uint64_t{1} << j, 0) << (i - j));
      EXPECT_EQ(turbo::MakeInt128(uint64_t{1} << i, 0),
                turbo::MakeInt128(uint64_t{1} << j, 0) <<= (i - j));
    }
  }
  ...
  for (int i = 0; i < 64; ++i) {
    for (int j = i; j < 64; ++j) {
      ...
      SCOPED_TRACE(::testing::Message() << "i = " << i << "; j = " << j);
      EXPECT_EQ(uint64_t{1} << i, turbo::int128(uint64_t{1} << j) >> (j - i));
      EXPECT_EQ(uint64_t{1} << i, turbo::int128(uint64_t{1} << j) >>= (j - i));
    }
    for (int j = 0; j < 63; ++j) {
      ...
      EXPECT_EQ(uint64_t{1} << i,
                turbo::MakeInt128(uint64_t{1} << j, 0) >> (j + 64 - i));
      EXPECT_EQ(uint64_t{1} << i,
                turbo::MakeInt128(uint64_t{1} << j, 0) >>= (j + 64 - i));
    }
  }
  for (int i = 0; i < 63; ++i) {
    for (int j = i; j < 63; ++j) {
      ...
      SCOPED_TRACE(::testing::Message() << "i = " << i << "; j = " << j);
      EXPECT_EQ(turbo::MakeInt128(uint64_t{1} << i, 0),
                turbo::MakeInt128(uint64_t{1} << j, 0) >> (j - i));
      EXPECT_EQ(turbo::MakeInt128(uint64_t{1} << i, 0),
                turbo::MakeInt128(uint64_t{1} << j, 0) >>= (j - i));
    }
  }
  ...
  // (val2) values
  turbo::int128 val1 = MAKE_INT128(0x123456789abcdef0, 0x123456789abcdef0);
  turbo::int128 val2 = MAKE_INT128(0xfedcba0987654321, 0xfedcba0987654321);
...
TEST(Int128, NumericLimitsTest) {
  ...
  EXPECT_EQ(turbo::Int128Min(), std::numeric_limits<turbo::int128>::min());
  EXPECT_EQ(turbo::Int128Min(), std::numeric_limits<turbo::int128>::lowest());
  EXPECT_EQ(turbo::Int128Max(), std::numeric_limits<turbo::int128>::max());
}
...
TEST(Int128, BitCastable) {
  ...
  // `std::bit_cast` when converting between `turbo::int128` and `turbo::uint128`,
  ...
  EXPECT_EQ(turbo::bit_cast<turbo::uint128>(turbo::int128(-1)),
            std::numeric_limits<turbo::uint128>::max());
  EXPECT_EQ(
      turbo::bit_cast<turbo::int128>(std::numeric_limits<turbo::uint128>::max()),
      turbo::int128(-1));
  EXPECT_EQ(
      turbo::bit_cast<turbo::uint128>(std::numeric_limits<turbo::int128>::min()),
      turbo::uint128(1) << 127);
  EXPECT_EQ(turbo::bit_cast<turbo::int128>(turbo::uint128(1) << 127),
            std::numeric_limits<turbo::int128>::min());
  EXPECT_EQ(
      turbo::bit_cast<turbo::uint128>(std::numeric_limits<turbo::int128>::max()),
      (turbo::uint128(1) << 127) - 1);
  EXPECT_EQ(turbo::bit_cast<turbo::int128>((turbo::uint128(1) << 127) - 1),
            std::numeric_limits<turbo::int128>::max());

  // Also verify static casting has the same behavior as bit casting.
  EXPECT_EQ(static_cast<turbo::uint128>(turbo::int128(-1)),
            std::numeric_limits<turbo::uint128>::max());
  EXPECT_EQ(
      static_cast<turbo::int128>(std::numeric_limits<turbo::uint128>::max()),
      turbo::int128(-1));
  EXPECT_EQ(
      static_cast<turbo::uint128>(std::numeric_limits<turbo::int128>::min()),
      turbo::uint128(1) << 127);
  EXPECT_EQ(static_cast<turbo::int128>(turbo::uint128(1) << 127),
            std::numeric_limits<turbo::int128>::min());
  EXPECT_EQ(
      static_cast<turbo::uint128>(std::numeric_limits<turbo::int128>::max()),
      (turbo::uint128(1) << 127) - 1);
  EXPECT_EQ(static_cast<turbo::int128>((turbo::uint128(1) << 127) - 1),
            std::numeric_limits<turbo::int128>::max());
}
