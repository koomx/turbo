...
namespace {
    ...
    TEST(StrErrorTest, ValidErrorCode) {
        errno = ERANGE;
        EXPECT_THAT(turbo::str_error(EDOM), Eq(strerror(EDOM)));
        EXPECT_THAT(errno, Eq(ERANGE));
    }

    TEST(StrErrorTest, InvalidErrorCode) {
        errno = ERANGE;
        EXPECT_THAT(turbo::str_error(-1),
    ...
    TEST(StrErrorTest, MultipleThreads) {
        ...
        // str_error 1000 times, each time with a different errnum.  We
        // expect that str_error(errnum) will return a string equal to the
        ...
        auto thread_fun = [&]() {
            for (int i = 0; i < kNumCodes; ++i) {
                ++counter;
                errno = ERANGE;
                const std::string value = turbo::str_error(i);
