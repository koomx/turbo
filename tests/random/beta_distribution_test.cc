...
constexpr bool ShouldExerciseLongDoubleTests() {
  ...
  return !turbo::numeric_internal::IsDoubleDouble();
#else
  return false;
#endif
}
...
TYPED_TEST(BetaDistributionInterfaceTest, SerializeTest) {
  ...
  using param_type = typename turbo::beta_distribution<TypeParam>::param_type;
  ...
  for (TypeParam alpha : kValues) {
    for (TypeParam beta : kValues) {
      KLOG(INFO) << turbo::stream_printf("Smoke test for Beta(%a, %a)", alpha,
                                      beta);
      ...
      {
        turbo::beta_distribution<TypeParam> via_param(param);
      ...
      std::stringstream ss;
      ss << before;
      turbo::beta_distribution<TypeParam> after(3.8f, 1.43f);
...
TYPED_TEST(BetaDistributionInterfaceTest, DegenerateCases) {
  ...
  };
  {
    ...
    for (TypeParam alpha : kSmallValues) {
      for (TypeParam beta : kSmallValues) {
        int zeros = 0;
        int ones = 0;
        turbo::beta_distribution<TypeParam> d(alpha, beta);
        for (int i = 0; i < kCount; ++i) {
          TypeParam x = d(rng);
          if (x == 0.0) {
            zeros++;
  ...
  }
  {
    ...
    turbo::beta_distribution<TypeParam> d(std::numeric_limits<TypeParam>::max(),
                                         std::numeric_limits<TypeParam>::max());
    for (int i = 0; i < kCount; ++i) {
      EXPECT_EQ(d(rng), 0.5);
    }
  }
  {
    ...
    turbo::beta_distribution<TypeParam> d(
        std::numeric_limits<TypeParam>::max(),
...
bool BetaDistributionTest::SingleChiSquaredTest(double p, size_t samples,
  ...
  for (; i < buckets; ++i) {
    ...
    const double boundary =
        turbo::random_internal::BetaIncompleteInv(alpha_, beta_, p);
...
TEST_P(BetaDistributionTest, TestSampleStatistics) {
  ...
  const double p =
      turbo::random_internal::RequiredSuccessProbability(kPFail, kRuns);
  static constexpr int kSampleCount = 10000;
  static constexpr int kBucketCount = 100;
  int failed = 0;
  for (int i = 0; i < kRuns; ++i) {
    if (!SingleZTestOnMeanAndVariance<turbo::beta_distribution<double>>(
            p, kSampleCount)) {
      failed++;
    }
    if (!SingleChiSquaredTest<turbo::beta_distribution<double>>(
...
// NOTE: turbo::beta_distribution is not guaranteed to be stable.
TEST(BetaDistributionTest, StabilityTest) {
  // turbo::beta_distribution stability relies on the stability of
  // turbo::random_interna::RandU64ToDouble, std::exp, std::log, std::pow,
  // and std::sqrt.
  //
  ...
  std::vector<uint64_t> output(20);
  {
    ...
    turbo::beta_distribution<float> dist(0.1f, 0.2f);
  ...
  urbg.reset();
  {
    ...
    turbo::beta_distribution<double> dist(0.1, 0.2);
    std::generate(std::begin(output), std::end(output),
  ...
  {
    // Algorithm Cheng 1
    turbo::beta_distribution<double> dist(0.9, 2.0);
    std::generate(std::begin(output), std::end(output),
                  [&] { return double_to_u64(dist(urbg)); });
  ...
  {
    // Algorithm Cheng 2
    turbo::beta_distribution<double> dist(1.5, 2.5);
    std::generate(std::begin(output), std::end(output),
                  [&] { return double_to_u64(dist(urbg)); });
...
TEST(BetaDistributionTest, AlgorithmBounds) {
...

  {
    ...
    // u=0.499, v=0.5
    turbo::beta_distribution<double> dist(1e-4, 1e-4);
    double a = dist(urbg);
  ...
  {
    // 1/alpha = 1/beta = 2.
    turbo::beta_distribution<float> dist(0.5, 0.5);
    ...
    {
      double y = turbo::beta_distribution<double>(0.5, 0.5)(urbg);
    ...
    urbg.reset();
    {
      float x = turbo::beta_distribution<float>(0.5, 0.5)(urbg);
      EXPECT_EQ(4, urbg.invocations());
