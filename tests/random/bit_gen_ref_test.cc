...
struct MinStdRand {
  ...
  // turbo::BitGenRef as well as explicitly constructing an turbo::BitGenRef from
  // a MinStdRand.
  operator turbo::BitGenRef() const {
    conversion_count++;
    return turbo::BitGenRef(minstd_gen);
  }
...
TEST(BitGenRefTest, IsConvertibleTest) {
  // Verify that MinStdRandBitGen is convertible to turbo::BitGenRef.
  EXPECT_TRUE((std::is_convertible_v<MinStdRand, turbo::BitGenRef>));

  // Explicit construction should trigger the conversion.
  {
    MinStdRand minstd;
    turbo::BitGenRef gen_ref(minstd);
    EXPECT_EQ(minstd.conversion_count, 1);
    (void)gen_ref;
  }

  // Calling a function that accepts turbo::BitGenRef should trigger the
