...
TYPED_TEST(DiscreteDistributionTypeTest, ParamSerializeTest) {
  using param_type =
      typename turbo::discrete_distribution<TypeParam>::param_type;

  turbo::discrete_distribution<TypeParam> empty;
  EXPECT_THAT(empty.probabilities(), testing::ElementsAre(1.0));

  turbo::discrete_distribution<TypeParam> before({1.0, 2.0, 1.0});
  ...
  // Validate the same data via an initializer list.
  {
    ...
    turbo::discrete_distribution<TypeParam> via_param{
  ...
  std::stringstream ss;
  ss << before;
  turbo::discrete_distribution<TypeParam> after;
...
TYPED_TEST(DiscreteDistributionTypeTest, Constructor) {
  ...
  {
    turbo::discrete_distribution<int> unary(0, 1.0, 9.0, fn);
    EXPECT_THAT(unary.probabilities(), testing::ElementsAre(1.0));
  }

  {
    turbo::discrete_distribution<int> unary(2, 1.0, 9.0, fn);
    // => fn(1.0 + 0 * 4 + 2) => 3
    // => fn(1.0 + 1 * 4 + 2) => 7
...
TEST(DiscreteDistributionTest, InitDiscreteDistribution) {
...

  {
    ...
    std::vector<std::pair<double, size_t>> q =
        turbo::random_internal::InitDiscreteDistribution(&p);
...
TEST(DiscreteDistributionTest, ChiSquaredTest50) {
  ...
  const int kThreshold =
      turbo::random_internal::ChiSquareValue(kBuckets, 0.99999);

  std::vector<double> weights(kBuckets, 0);
  std::iota(std::begin(weights), std::end(weights), 1);
  turbo::discrete_distribution<int> dist(std::begin(weights), std::end(weights));
  ...
  double chi_square =
      turbo::random_internal::ChiSquare(std::begin(counts), std::end(counts),
...
TEST(DiscreteDistributionTest, StabilityTest) {
  // turbo::discrete_distribution stability relies on
  // turbo::uniform_int_distribution and turbo::bernoulli_distribution.
  ...
  {
    turbo::discrete_distribution<int32_t> dist({1.0, 2.0, 3.0, 5.0, 2.0});
  ...
  {
    urbg.reset();
    turbo::discrete_distribution<int64_t> dist({1.0, 2.0, 3.0, 5.0, 2.0});
    EXPECT_EQ(0, dist.min());
    EXPECT_EQ(4, dist.max());
    for (auto& v : output) {
      v = dist(urbg);
