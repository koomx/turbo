...
#include <tests/random/internal/chi_square.h>
#include <tests/random/internal/distribution_test_util.h>
...
using RealTypes =
    std::conditional_t<turbo::numeric_internal::IsDoubleDouble(),
...
TYPED_TEST(ExponentialDistributionTypedTest, SerializeTest) {
  using param_type =
      typename turbo::exponential_distribution<TypeParam>::param_type;
  ...
  for (const TypeParam lambda : kParams) {
    ...
    KUMO_ASSERT(lambda > 0);

    const param_type param(lambda);

    turbo::exponential_distribution<TypeParam> before(lambda);
    EXPECT_EQ(before.lambda(), param.lambda());

    {
      turbo::exponential_distribution<TypeParam> via_param(param);
    ...
    if (!std::is_same_v<TypeParam, long double>) {
      KLOG(INFO) << "Range {" << lambda << "}: " << sample_min << ", "
    ...
    turbo::exponential_distribution<TypeParam> after(34.56f);
...
bool ExponentialDistributionTests::SingleZTest(const double p,
  ...
  const auto m = turbo::random_internal::ComputeDistributionMoments(data);
  const double max_err = turbo::random_internal::MaxErrorTolerance(p);
  const double z = turbo::random_internal::ZScore(mean(), m);
...
double ExponentialDistributionTests::SingleChiSquaredTest() {
  ...
  double p = turbo::random_internal::ChiSquarePValue(chi_square, dof);

  if (chi_square > threshold) {
    for (size_t i = 0; i < cutoffs.size(); i++) {
      KLOG(INFO) << i << " : (" << cutoffs[i] << ") = " << counts[i];
    }

    // clang-format off
    KLOG(INFO) << "lambda " << lambda() << "\n"
                 " expected " << expected << "\n"
...
TEST_P(ExponentialDistributionTests, ZTest) {
  ...
  const double p = turbo::random_internal::RequiredSuccessProbability(
      param.p_fail, param.trials);

  int failures = 0;
  for (int i = 0; i < param.trials; i++) {
    failures += SingleZTest<turbo::exponential_distribution<double>>(p, kSamples)
                    ? 0
                    : 1;
  }
...
TEST_P(ExponentialDistributionTests, ChiSquaredTest) {
  ...
  for (int i = 0; i < kTrials; i++) {
    double p_value =
        SingleChiSquaredTest<turbo::exponential_distribution<double>>();
...
// NOTE: turbo::exponential_distribution is not guaranteed to be stable.
TEST(ExponentialDistributionTest, StabilityTest) {
  // turbo::exponential_distribution stability relies on std::log1p and
  // turbo::uniform_real_distribution.
  ...
  {
    turbo::exponential_distribution<double> dist;
  ...
  urbg.reset();
  {
    turbo::exponential_distribution<float> dist;
    std::generate(std::begin(output), std::end(output),
...
TEST(ExponentialDistributionTest, AlgorithmBounds) {
  // Relies on turbo::uniform_real_distribution, so some of these comments
  // reference that.
  ...
  {
    // This returns the smallest value >0 from turbo::uniform_real_distribution.
    turbo::random_internal::sequence_urbg urbg({0x0000000000000001ull});
    double a = dist(urbg);
    EXPECT_EQ(a, 5.42101086242752217004e-20);
  }

  {
    // This returns a value very near 0.5 from turbo::uniform_real_distribution.
    turbo::random_internal::sequence_urbg urbg({0x7fffffffffffffefull});
    double a = dist(urbg);
    EXPECT_EQ(a, 0.693147180559945175204);
  }

  {
    // This returns the largest value <1 from turbo::uniform_real_distribution.
    // WolframAlpha: ~39.1439465808987766283058547296341915292187253
    turbo::random_internal::sequence_urbg urbg({0xFFFFFFFFFFFFFFeFull});
    double a = dist(urbg);
    EXPECT_EQ(a, 36.7368005696771007251);
  }
  {
    // This *ALSO* returns the largest value <1.
    turbo::random_internal::sequence_urbg urbg({0xFFFFFFFFFFFFFFFFull});
    double a = dist(urbg);
    EXPECT_EQ(a, 36.7368005696771007251);
  }
}
