...
TYPED_TEST(GaussianDistributionInterfaceTest, SerializeTest) {
  using param_type =
      typename turbo::gaussian_distribution<TypeParam>::param_type;
  ...
  for (const auto mod : {0, 1, 2, 3}) {
    for (const auto x : kParams) {
      ...
      for (const auto y : kParams) {
        ...
        turbo::gaussian_distribution<TypeParam> before(mean, stddev);
        EXPECT_EQ(before.mean(), param.mean());
        EXPECT_EQ(before.stddev(), param.stddev());

        {
          turbo::gaussian_distribution<TypeParam> via_param(param);
        ...
        if (!std::is_same_v<TypeParam, long double>) {
          KLOG(INFO) << "Range{" << mean << ", " << stddev << "}: " << sample_min
                    << ", " << sample_max;
        }
        ...
        turbo::gaussian_distribution<TypeParam> after(-0.53f, 2.3456f);
...
double GaussianDistributionTests::SingleChiSquaredTest() {
  ...
  if (chi_square > threshold) {
    ...
    // clang-format off
    KLOG(INFO) << "mean=" << mean() << " stddev=" << stddev() << "\n"
                 " expected " << expected << "\n"
...
TEST_P(GaussianDistributionTests, ZTest) {
  // TODO(turbo-team): Run these tests against std::normal_distribution<double>
  ...
  for (int i = 0; i < param.trials; i++) {
    failures +=
        SingleZTest<turbo::gaussian_distribution<double>>(p, kSamples) ? 0 : 1;
  }
...
TEST_P(GaussianDistributionTests, ChiSquaredTest) {
  ...
  for (int i = 0; i < kTrials; i++) {
    double p_value =
        SingleChiSquaredTest<turbo::gaussian_distribution<double>>();
...
// NOTE: turbo::gaussian_distribution is not guaranteed to be stable.
TEST(GaussianDistributionTest, StabilityTest) {
  // turbo::gaussian_distribution stability relies on the underlying zignor
  // data, turbo::random_interna::RandU64ToDouble, std::exp, std::log, and
  // std::abs.
  ...
  {
    turbo::gaussian_distribution<double> dist;
  ...
  urbg.reset();
  {
    turbo::gaussian_distribution<float> dist;
    std::generate(std::begin(output), std::end(output),
