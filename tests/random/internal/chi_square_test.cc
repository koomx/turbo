...
using turbo::random_internal::ChiSquareValue;
using turbo::random_internal::ChiSquareWithExpected;
...
TEST(ChiSquareTest, CalcChiSquareInt64) {
  ...
  // 4.25410123524
  double sum = turbo::c_accumulate(data, double{0});
  size_t n = turbo::c_distance(data);
...
TEST(ChiSquareTest, TableData) {
  ...
  //    0.90      0.95     0.975      0.99     0.999
  for (int i = 0; i < KUMO_ARRAYSIZE(data); i++) {
...
TEST(ChiSquareTest, DiceRolls) {
  ...
  double sum = turbo::c_accumulate(rolls, double{0});
  size_t n = turbo::c_distance(rolls);
