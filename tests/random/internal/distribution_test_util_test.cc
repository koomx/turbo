...
TEST(TestUtil, InverseErf) {
  ...
  for (const auto& data : kErfInvTable) {
    auto value = turbo::random_internal::erfinv(data.z);
...
TEST(BetaTest, BetaIncomplete) {
  for (const auto& data : kBetaTable) {
    auto value = turbo::random_internal::BetaIncomplete(data.x, data.p, data.q);
...
TEST(BetaTest, BetaIncompleteInv) {
  for (const auto& data : kBetaTable) {
    auto value =
        turbo::random_internal::BetaIncompleteInv(data.p, data.q, data.alpha);
...
TEST(MaxErrorTolerance, MaxErrorTolerance) {
  ...
  for (auto entry : cases) {
    EXPECT_NEAR(turbo::random_internal::MaxErrorTolerance(entry.first),
                entry.second, 1e-8);
  }
}

TEST(ZScore, WithSameMean) {
  turbo::random_internal::DistributionMoments m;
  m.n = 100;
  m.mean = 5;
  m.variance = 1;
  EXPECT_NEAR(turbo::random_internal::ZScore(5, m), 0, 1e-12);

  m.n = 1;
  m.mean = 0;
  m.variance = 1;
  EXPECT_NEAR(turbo::random_internal::ZScore(0, m), 0, 1e-12);
...
TEST(ZScore, DifferentMean) {
  ...
  m.mean = 5;
  m.variance = 1;
  EXPECT_NEAR(turbo::random_internal::ZScore(4, m), 10, 1e-12);

  m.n = 1;
  m.mean = 0;
  m.variance = 1;
  EXPECT_NEAR(turbo::random_internal::ZScore(-1, m), 1, 1e-12);
