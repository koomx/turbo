...
using ::turbo::random_internal::ExplicitSeedSeq;
...
bool ConformsToInterface() {
  ...
  // Check that param() and size() return state provided to constructor.
  {
    ...
    Sseq seq(init_array, &init_array[KUMO_ARRAYSIZE(init_array)]);
    EXPECT_EQ(seq.size(), KUMO_ARRAYSIZE(init_array));

    uint32_t state_array[KUMO_ARRAYSIZE(init_array)];
    seq.param(state_array);

    for (int i = 0; i < KUMO_ARRAYSIZE(state_array); i++) {
  ...
  // Check for presence of generate() method.
  {
    ...
    seq.generate(seeds, &seeds[KUMO_ARRAYSIZE(seeds)]);
  }
  return true;
}
}  // namespace
...
#pragma GCC diagnostic pop
  ...
  // Move constructor.
  {
    ...
    turbo::random_internal::ExplicitSeedSeq moved_seq(
