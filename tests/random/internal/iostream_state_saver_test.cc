...
using turbo::random_internal::make_istream_state_saver;
using turbo::random_internal::make_ostream_state_saver;
...
StreamRoundTrip(T t) {
  ...
  {
    auto saver = make_istream_state_saver(ss);
    result = turbo::random_internal::read_floating_point<T>(ss);
  }
  EXPECT_FALSE(ss.fail())            //
      << ss.str() << " "             //
      << (ss.good() ? "good " : "")  //
