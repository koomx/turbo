...
void MeasureDiv(const FuncInput (&inputs)[N]) {
  ...
  for (size_t i = 0; i < num_results; ++i) {
    KLOG(INFO) << turbo::stream_printf("%5u: %6.2f ticks; MAD=%4.2f%%\n",
                                    results[i].input, results[i].ticks,
                                    results[i].variability * 100.0);
    KCHECK_NE(results[i].ticks, 0.0f) << "Zero duration";
  }
}
...
void RunAll(const int argc, char* argv[]) {
  ...
  if (argc == 2) {
    if (!turbo::SimpleAtoi(argv[1], &cpu)) {
      KLOG(FATAL) << "The optional argument must be a CPU number >= 0.";
    }
  }
  PinThreadToCPU(cpu);
...
int main(int argc, char* argv[]) {
  turbo::random_internal_nanobenchmark::RunAll(argc, argv);
  return 0;
}
