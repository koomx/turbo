...
using ExampleNonsecureURBG =
    turbo::random_internal::NonsecureURBGBase<std::mt19937>;
...
TEST(RecommendedTemplates, CanBeConstructed) {
  turbo::BitGen default_generator;
  turbo::InsecureBitGen insecure_generator;
}
...
TEST(NonsecureURBGBase, StandardInterface) {
  ...
  using E = turbo::random_internal::NonsecureURBGBase<std::minstd_rand>;
...
TEST(NonsecureURBGBase, DistinctSequencesPerThread) {
  ...
  turbo::flat_hash_set<result_type> seen;
...
TEST(RandenPoolSeedSeqTest, SeederWorksForU32) {
  turbo::random_internal::RandenPoolSeedSeq seeder;
