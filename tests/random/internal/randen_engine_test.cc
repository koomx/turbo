...
using randen_u64 = turbo::random_internal::randen_engine<uint64_t>;
using randen_u32 = turbo::random_internal::randen_engine<uint32_t>;
...
TYPED_TEST(RandenEngineTypedTest, VerifyReseedChangesAllValues) {
  using randen = typename turbo::random_internal::randen_engine<TypeParam>;
...
TEST(RandenTest, IsFastOrSlow) {
  ...
  KLOG(INFO) << static_cast<double>(duration) / static_cast<double>(kCount)
            << "ns";
