...
using turbo::random_internal::GetSaltMaterial;
using turbo::random_internal::MakeSaltedSeedSeq;
using turbo::random_internal::SaltedSeedSeq;
using testing::Eq;
...
void ConformsToInterface() {
  ...
  // Check that param() and size() return state provided to constructor.
  {
    ...
    EXPECT_EQ(state_vector.size(), KUMO_ARRAYSIZE(init_array));
