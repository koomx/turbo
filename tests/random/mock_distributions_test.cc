...
#include <tests/random/internal/mock_distributions.h>
...
#include <tests/random/internal/mocking_bit_gen.h>
...
TEST(MockDistributions, Examples) {
  ...
  EXPECT_NE(turbo::LogUniform<int>(gen, 0, kHigh, 2), kHigh);
  EXPECT_CALL(turbo::MockLogUniform<int>(), Call(gen, 0, kHigh, 2))
      .WillOnce(Return(kHigh));
  EXPECT_EQ(turbo::LogUniform<int>(gen, 0, kHigh, 2), kHigh);
}
