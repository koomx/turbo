...
// turbo::poisson_distribution conforms to the interface and serialization
...
TYPED_TEST(PoissonDistributionInterfaceTest, SerializeTest) {
  using param_type = typename turbo::poisson_distribution<TypeParam>::param_type;
  const double kMax =
  ...
  for (const double m : kParams) {
    ...
    // Validate parameters.
    turbo::poisson_distribution<TypeParam> before(mean);
    EXPECT_EQ(before.mean(), param.mean());

    {
      turbo::poisson_distribution<TypeParam> via_param(param);
    ...
    KLOG(INFO) << "Range {" << param.mean() << "}: " << sample_min << ", "
              << sample_max;
    ...
    turbo::poisson_distribution<TypeParam> after(3.8);
...
class PoissonModel {
 public:
  ...
  void LogCDF() {
    KLOG(INFO) << "CDF (mean = " << mean_ << ")";
    for (const auto c : cdf_) {
      KLOG(INFO) << c.index << ": pmf=" << c.pmf << " cdf=" << c.cdf;
    }
  }
...
bool PoissonDistributionZTest::SingleZTest(const double p,
  ...
  turbo::flat_hash_map<int32_t, int> buckets;
...
TEST_P(PoissonDistributionZTest, TurboPoissonDistribution) {
  ...
  for (int i = 0; i < param.trials; i++) {
    failures +=
        SingleZTest<turbo::poisson_distribution<int32_t>>(p, param.samples) ? 0
                                                                           : 1;
  }
...
double PoissonDistributionChiSquaredTest::ChiSquaredTestImpl() {
  ...
  if (chi_square > threshold) {
    ...
    KLOG(INFO) << "VALUES  buckets=" << counts.size()
              << "  samples=" << kSamples;
    for (size_t i = 0; i < counts.size(); i++) {
      KLOG(INFO) << cutoffs_[i] << ": " << counts[i] << " vs. E=" << e[i];
    }

    KLOG(INFO) << kChiSquared << "(data, dof=" << dof << ") = " << chi_square
              << " (" << p << ")\n"
              << " vs.\n"
...
TEST_P(PoissonDistributionChiSquaredTest, TurboPoissonDistribution) {
  ...
  for (int i = 0; i < kTrials; i++) {
    double p_value = ChiSquaredTestImpl<turbo::poisson_distribution<int32_t>>();
    if (p_value < 0.005) {
      failures++;
    }
  }
...
// NOTE: turbo::poisson_distribution is not guaranteed to be stable.
TEST(PoissonDistributionTest, StabilityTest) {
  using testing::ElementsAre;
  // turbo::poisson_distribution stability relies on stability of
  // std::exp, std::log, std::sqrt, std::ceil, std::floor, and
  // turbo::FastUniformBits, turbo::StirlingLogFactorial, turbo::RandU64ToDouble.
  ...
  // Method 1.
  {
    turbo::poisson_distribution<int> dist(5);
  ...
  {
    urbg.reset();
    turbo::poisson_distribution<int> dist(25);
    std::generate(std::begin(output), std::end(output),
                  [&] { return dist(urbg); });
  }
  ...
  {
    urbg.reset();
    turbo::poisson_distribution<int> dist(121);
    std::generate(std::begin(output), std::end(output),
                  [&] { return dist(urbg); });
  }
...
TEST(PoissonDistributionTest, AlgorithmExpectedValue_1) {
  ...
  turbo::random_internal::sequence_urbg urbg({0x8000000000000001ull});
  turbo::poisson_distribution<int> dist(5);
