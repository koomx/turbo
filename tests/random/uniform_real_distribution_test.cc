...
//
// * turbo::uniform_real_distribution<float> generates between 2^28 and 2^29
//   distinct floating point values in the range [0, 1).
//
// * turbo::uniform_real_distribution<float> generates at least 2^23 distinct
...
//
// * turbo::uniform_real_distribution<double> generates more than 2^52 distinct
...
TYPED_TEST(UniformRealDistributionTest, ParamSerializeTest) {
...
#endif
  using DistributionType = turbo::uniform_real_distribution<TypeParam>;
  ...
  for (const auto& param : {
    ...
    if (!std::is_same_v<real_type, long double>) {
      ...
      KLOG(INFO) << "Range: " << static_cast<double>(sample_min) << ", "
...
TYPED_TEST(UniformRealDistributionTest, ChiSquaredTest50) {
  ...
  const int kThreshold =
      turbo::random_internal::ChiSquareValue(kBuckets - 1, 0.999999);
...
TYPED_TEST(UniformRealDistributionTest, StabilityTest) {
  ...
  // turbo::uniform_real_distribution stability relies only on
...
TEST(UniformRealDistributionTest, AlgorithmBounds) {
  turbo::uniform_real_distribution<double> dist;
  ...
  {
    // This returns a value very near 0.5 from turbo::uniform_real_distribution.
    turbo::random_internal::sequence_urbg urbg({0x8000000000000000ull});
    double a = dist(urbg);
    EXPECT_EQ(a, 0.5);
  }
