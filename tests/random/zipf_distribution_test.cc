...
TYPED_TEST(ZipfDistributionTypedTest, SerializeTest) {
  using param_type = typename turbo::zipf_distribution<TypeParam>::param_type;
  ...
  for (const auto& param : {
    ...
    turbo::zipf_distribution<TypeParam> before(k, q, v);
    EXPECT_EQ(before.k(), param.k());
    EXPECT_EQ(before.q(), param.q());
    EXPECT_EQ(before.v(), param.v());

    {
      turbo::zipf_distribution<TypeParam> via_param(param);
    ...
    std::stringstream ss;
    ss << before;
    turbo::zipf_distribution<TypeParam> after(4, 5.5, 4.4);
    ...
    KLOG(INFO) << "Range: " << sample_min << ", " << sample_max;
  }
}
...
using zipf_u64 = turbo::zipf_distribution<uint64_t>;
...
TEST_P(ZipfTest, ChiSquaredTest) {
  ...
  const double threshold = turbo::random_internal::ChiSquareValue(dof, 0.9999);
  ...
  if (chi_square > threshold) {
    ...
    for (size_t i = 0; i < expected.size(); i++) {
      KLOG(INFO) << points[i] << ": " << buckets[i] << " vs. E=" << expected[i];
    }
    KLOG(INFO) << "trials " << trials;
    KLOG(INFO) << "mean " << avg << " vs. expected " << mean();
    KLOG(INFO) << kChiSquared << "(data, " << dof << ") = " << chi_square << " ("
              << p_actual << ")";
    KLOG(INFO) << kChiSquared << " @ 0.9995 = " << threshold;
...
// NOTE: turbo::zipf_distribution is not guaranteed to be stable.
TEST(ZipfDistributionTest, StabilityTest) {
  // turbo::zipf_distribution stability relies on
  // turbo::uniform_real_distribution, std::log, std::exp, std::log1p
  ...
  {
    turbo::zipf_distribution<int32_t> dist;
  ...
  urbg.reset();
  {
    turbo::zipf_distribution<int32_t> dist(std::numeric_limits<int32_t>::max(),
                                          3.3);
    std::generate(std::begin(output), std::end(output),
                  [&] { return dist(urbg); });
...
TEST(ZipfDistributionTest, AlgorithmBounds) {
  ...
  // Small values from turbo::uniform_real_distribution map to larger Zipf
  // distribution values.
  ...
  for (const auto& instance : kInputs) {
    turbo::random_internal::sequence_urbg urbg({instance.first});
