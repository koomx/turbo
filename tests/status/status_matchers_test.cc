...
namespace {
    ...
    TEST(StatusMatcherTest, StatusIsOk) {
        EXPECT_THAT(turbo::ok_status(), IsOk());
    }
    ...
    TEST(StatusMatcherTest, ResultIs) {
        ...
        turbo::Result<std::string_view> invalid = turbo::invalid_argument_error("ungueltig");
        ...
        EXPECT_THAT(Explain(m, turbo::Result<int>(turbo::invalid_argument_error("internal error"))),
            Eq("whose status code is wrong"));
        EXPECT_THAT(
