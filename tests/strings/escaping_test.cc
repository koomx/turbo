...
#include <string_view>
#include <tests/strings/internal/escaping_test_common.h>
...
namespace {
    ...
    void TestEscapeAndUnescape() {
        ...
        for (const auto& tc : base64_tests) {
            ...
            encoded = turbo::base64_escape(tc.plaintext);
            EXPECT_EQ(encoded, tc.cyphertext);
            EXPECT_EQ(turbo::base64_escape(tc.plaintext), tc.cyphertext);

            StringType decoded("this junk should be ignored");
            EXPECT_TRUE(turbo::base64_unescape(encoded, &decoded));
            EXPECT_EQ(decoded, tc.plaintext);

            StringType websafe_with_padding(tc.cyphertext);
            for (unsigned int c = 0; c < websafe_with_padding.size(); ++c) {
                if ('+' == websafe_with_padding[c])
                    websafe_with_padding[c] = '-';
                if ('/' == websafe_with_padding[c])
                    websafe_with_padding[c] = '_';
            ...
            encoded = turbo::web_safe_base64_escape(tc.plaintext);
            EXPECT_EQ(encoded, websafe);
            EXPECT_EQ(turbo::web_safe_base64_escape(tc.plaintext), websafe);

            decoded = "this junk should be ignored";
            EXPECT_TRUE(turbo::web_safe_base64_unescape(websafe, &decoded));
            EXPECT_EQ(decoded, tc.plaintext);
        }
        ...
        for (const auto& tc : turbo::strings_internal::base64_strings()) {
            StringType buffer;
            buffer = turbo::web_safe_base64_escape(tc.plaintext);
            EXPECT_EQ(tc.cyphertext, buffer);
            EXPECT_EQ(turbo::web_safe_base64_escape(tc.plaintext), tc.cyphertext);
        }
        ...
        // Verify the behavior when decoding bad data
        {
            ...
            for (std::string_view bad_data : data_set) {
                StringType buf;
                EXPECT_FALSE(turbo::base64_unescape(bad_data, &buf));
                EXPECT_FALSE(turbo::web_safe_base64_unescape(bad_data, &buf));
    ...
    TEST(Base64, Padding) {
        ...
        for (std::string_view b64 : good_padding) {
            std::string decoded;
            EXPECT_TRUE(turbo::base64_unescape(b64, &decoded));
            EXPECT_EQ(decoded, "a");
            std::string websafe_decoded;
            EXPECT_TRUE(turbo::web_safe_base64_unescape(b64, &websafe_decoded));
        ...
        for (std::string_view b64 : bad_padding) {
            std::string decoded;
            EXPECT_FALSE(turbo::base64_unescape(b64, &decoded));
            std::string websafe_decoded;
            EXPECT_FALSE(turbo::web_safe_base64_unescape(b64, &websafe_decoded));
        }
    }
    ...
    TEST(Base64, DISABLED_HugeData) {
        ...
        std::string escaped = turbo::base64_escape(huge);
        ...
        std::string unescaped;
        EXPECT_TRUE(turbo::base64_unescape(escaped, &unescaped));
    ...
    TEST(UrlEscape, Basics) {
        EXPECT_EQ(turbo::UrlEscape(""), "");
        EXPECT_THAT(turbo::UrlUnescape(""), Optional(Eq("")));

        EXPECT_EQ(turbo::UrlEscape("abc"), "abc");
        EXPECT_THAT(turbo::UrlUnescape("abc"), Optional(Eq("abc")));

        EXPECT_EQ(turbo::UrlEscape("a/b"), "a%2Fb");
        EXPECT_THAT(turbo::UrlUnescape("a%2Fb"), Optional(Eq("a/b")));

        EXPECT_EQ(turbo::UrlEscape("one two"), "one%20two");
        EXPECT_THAT(turbo::UrlUnescape("one%20two"), Optional(Eq("one two")));

        EXPECT_EQ(turbo::UrlEscape("10%"), "10%25");
        EXPECT_THAT(turbo::UrlUnescape("10%25"), Optional(Eq("10%")));

        EXPECT_EQ(turbo::UrlEscape(" ?&=#+%!<>#\"{}|\\^[]`☺\t:/@$'()*,;"),
            "%20%3F%26%3D%23%2B%25%21%3C%3E%23%22%7B%7D%7C%5C%5E%5B%5D%60%E2%"
            "98%BA%09%3A%2F%40%24%27%28%29%2A%2C%3B");
        EXPECT_THAT(turbo::UrlUnescape("%20%3F%26%3D%23%2B%25%21%3C%3E%23%22%7B%7D%7C%"
        ...
        for (int i = 0; i < 256; ++i) {
            ...
            EXPECT_EQ(turbo::UrlEscape(std::string_view(&c, 1)), expected);
            EXPECT_EQ(turbo::UrlUnescape(expected), std::string_view(&c, 1));
        }
    }
    ...
    TEST(UrlUnescape, SuccessCases) {
        EXPECT_THAT(turbo::UrlUnescape(""), Optional(Eq("")));
        EXPECT_THAT(turbo::UrlUnescape("abc"), Optional(Eq("abc")));
        EXPECT_THAT(turbo::UrlUnescape("1%41"), Optional(Eq("1A")));
        EXPECT_THAT(turbo::UrlUnescape("1%41%42%43"), Optional(Eq("1ABC")));
        EXPECT_THAT(turbo::UrlUnescape("%4a"), Optional(Eq("J")));
        EXPECT_THAT(turbo::UrlUnescape("%6F"), Optional(Eq("o")));
        EXPECT_THAT(turbo::UrlUnescape("a%20b"), Optional(Eq("a b")));
        EXPECT_THAT(turbo::UrlUnescape("a+b"), Optional(Eq("a+b")));
    }

    TEST(UrlUnescape, NotEnoughCharsAfterPercent) {
        EXPECT_EQ(turbo::UrlUnescape("%"), std::nullopt);
        EXPECT_EQ(turbo::UrlUnescape("%a"), std::nullopt);
        EXPECT_EQ(turbo::UrlUnescape("%1"), std::nullopt);
        EXPECT_EQ(turbo::UrlUnescape("123%45%6"), std::nullopt);
    }

    TEST(UrlUnescape, InvalidHexDigits) {
        EXPECT_EQ(turbo::UrlUnescape("%zzzzz"), std::nullopt);
    }

    TEST(UrlUnescape, NoErrorWithNoEscapeSequence) {
        // Any string that does not contain '%' should not produce an error, even if
        // turbo::UrlEscape() would never produce a string with certain characters.
        ...
        EXPECT_THAT(turbo::UrlUnescape(no_percent), Optional(no_percent));
    }

    TEST(UrlEscapePlus, Basics) {
        EXPECT_EQ(turbo::UrlEscapePlus(""), "");
        EXPECT_THAT(turbo::UrlUnescapePlus(""), Optional(Eq("")));

        EXPECT_EQ(turbo::UrlEscapePlus("abc"), "abc");
        EXPECT_THAT(turbo::UrlUnescapePlus("abc"), Optional(Eq("abc")));

        EXPECT_EQ(turbo::UrlEscapePlus("one two"), "one+two");
        EXPECT_THAT(turbo::UrlUnescapePlus("one+two"), Optional(Eq("one two")));

        EXPECT_EQ(turbo::UrlEscapePlus("gift for mom & dad"), "gift+for+mom+%26+dad");
        EXPECT_THAT(turbo::UrlUnescapePlus("gift+for+mom+%26+dad"),
            Optional(Eq("gift for mom & dad")));

        EXPECT_EQ(turbo::UrlEscapePlus("10%"), "10%25");
        EXPECT_THAT(turbo::UrlUnescapePlus("10%25"), Optional(Eq("10%")));

        EXPECT_EQ(turbo::UrlEscapePlus(" ?&=#+%!<>#\"{}|\\^[]`☺\t:/@$'()*,;"),
            "+%3F%26%3D%23%2B%25%21%3C%3E%23%22%7B%7D%7C%5C%5E%5B%5D%60%E2%"
            "98%BA%09%3A%2F%40%24%27%28%29%2A%2C%3B");
        EXPECT_THAT(turbo::UrlUnescapePlus("+%3F%26%3D%23%2B%25%21%3C%3E%23%22%7B%7D%"
        ...
        for (int i = 0; i < 256; ++i) {
            ...
            if (c == ' ')
                expected = '+';
            EXPECT_EQ(turbo::UrlEscapePlus(std::string_view(&c, 1)), expected);
            EXPECT_EQ(turbo::UrlUnescapePlus(expected), std::string_view(&c, 1));
        }
    }
    ...
    TEST(UrlUnescapePlus, SuccessCases) {
        EXPECT_THAT(turbo::UrlUnescapePlus(""), Optional(Eq("")));
        EXPECT_THAT(turbo::UrlUnescapePlus("abc"), Optional(Eq("abc")));
        EXPECT_THAT(turbo::UrlUnescapePlus("1%41"), Optional(Eq("1A")));
        EXPECT_THAT(turbo::UrlUnescapePlus("1%41%42%43"), Optional(Eq("1ABC")));
        EXPECT_THAT(turbo::UrlUnescapePlus("%4a"), Optional(Eq("J")));
        EXPECT_THAT(turbo::UrlUnescapePlus("%6F"), Optional(Eq("o")));
        EXPECT_THAT(turbo::UrlUnescapePlus("a%20b"), Optional(Eq("a b")));
        EXPECT_THAT(turbo::UrlUnescapePlus("a+b"), Optional(Eq("a b")));
    }

    TEST(UrlUnescapePlus, NotEnoughCharsAfterPercent) {
        EXPECT_EQ(turbo::UrlUnescapePlus("%"), std::nullopt);
        EXPECT_EQ(turbo::UrlUnescapePlus("%a"), std::nullopt);
        EXPECT_EQ(turbo::UrlUnescapePlus("%1"), std::nullopt);
        EXPECT_EQ(turbo::UrlUnescapePlus("123%45%6"), std::nullopt);
    }

    TEST(UrlUnescapePlus, InvalidHexDigits) {
        EXPECT_EQ(turbo::UrlUnescapePlus("%zzzzz"), std::nullopt);
    }

    TEST(UrlUnescapePlus, NoErrorWithNoEscapeSequence) {
        // Any string that does not contain '%' should not produce an error, even if
        // turbo::UrlEscapePlus() would never produce a string with certain
        // characters.
        std::string no_percent;
        ...
        EXPECT_THAT(turbo::UrlUnescapePlus(no_percent), Optional(no_percent_expected));
    }
