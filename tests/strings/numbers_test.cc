...
using turbo::format_internal::kSixDigitsToBufferSize;
using turbo::numbers_internal::safe_strto16_base;
using turbo::numbers_internal::safe_strto32_base;
using turbo::numbers_internal::safe_strto64_base;
using turbo::numbers_internal::safe_strto8_base;
using turbo::numbers_internal::safe_strtou16_base;
using turbo::numbers_internal::safe_strtou32_base;
using turbo::numbers_internal::safe_strtou64_base;
using turbo::numbers_internal::safe_strtou8_base;
using turbo::format_internal::six_digits_to_buffer;
...
typedef MyInteger<turbo::uint128> MyUInt128;
typedef MyInteger<turbo::int128> MyInt128;

void CheckInt32(int32_t x) {
  char buffer[turbo::format_internal::kFastToBufferSize];
  char* actual = turbo::format_internal::fast_int_to_buffer(x, buffer);
  std::string expected = std::to_string(x);
  EXPECT_EQ(expected, std::string(buffer, actual)) << " Input " << x;

  char* generic_actual = turbo::format_internal::fast_int_to_buffer(x, buffer);
  EXPECT_EQ(expected, std::string(buffer, generic_actual)) << " Input " << x;
}

void CheckInt64(int64_t x) {
  char buffer[turbo::format_internal::kFastToBufferSize + 3];
  buffer[0] = '*';
  buffer[23] = '*';
  buffer[24] = '*';
  char* actual = turbo::format_internal::fast_int_to_buffer(x, &buffer[1]);
  std::string expected = std::to_string(x);
  ...
  char* my_actual =
      turbo::format_internal::fast_int_to_buffer(MyInt64(x), &buffer[1]);
...
void CheckUInt64(uint64_t x) {
  char buffer[turbo::format_internal::kFastToBufferSize + 1];
  char* actual = turbo::format_internal::fast_int_to_buffer(x, &buffer[1]);
  std::string expected = std::to_string(x);
  EXPECT_EQ(expected, std::string(&buffer[1], actual)) << " Input " << x;

  char* generic_actual = turbo::format_internal::fast_int_to_buffer(x, &buffer[1]);
  EXPECT_EQ(expected, std::string(&buffer[1], generic_actual))
      << " Input " << x;

  char* my_actual =
      turbo::format_internal::fast_int_to_buffer(MyUInt64(x), &buffer[1]);
  EXPECT_EQ(expected, std::string(&buffer[1], my_actual)) << " Input " << x;
}

void CheckUInt128(turbo::uint128 x) {
  char buffer[turbo::format_internal::kFastToBuffer128Size];
  char* actual = turbo::format_internal::fast_int_to_buffer(x, buffer);
  std::string s;
  ...
  char* my_actual =
      turbo::format_internal::fast_int_to_buffer(MyUInt128(x), buffer);
  EXPECT_EQ(s, std::string(buffer, my_actual)) << " Input " << s;
}

void CheckInt128(turbo::int128 x) {
  char buffer[turbo::format_internal::kFastToBuffer128Size];
  char* actual = turbo::format_internal::fast_int_to_buffer(x, buffer);
  std::string s;
  turbo::strings_internal::OStringStream strm(&s);
  ...
  char* my_actual =
      turbo::format_internal::fast_int_to_buffer(MyInt128(x), buffer);
  EXPECT_EQ(s, std::string(buffer, my_actual)) << " Input " << s;
}
...
TEST(Numbers, TestFastPrints) {
  ...
  CheckUInt128(turbo::uint128(std::numeric_limits<uint64_t>::max()) + 1);
  CheckUInt128(turbo::MakeUint128(1, 0));
  turbo::uint128 k1e16 = 10000000000000000ULL;
  ...
  CheckUInt128(k1e16 * k1e16 + 1);
  CheckUInt128(turbo::Uint128Max() - 1);
  CheckUInt128(turbo::Uint128Max());
  ...
  CheckInt128(10);
  CheckInt128(-10);
  CheckInt128(turbo::Int128Max());
  CheckInt128(turbo::Int128Min());
  CheckInt128(turbo::MakeInt128(-1, 1));
  CheckInt128(turbo::MakeInt128(-1, std::numeric_limits<uint64_t>::max()));
...
TEST(NumbersTest, Atoi) {
  // SimpleAtoi(std::string_view, int8_t)
  ...
  // SimpleAtoi(std::string_view, uint8_t)
  ...
  // SimpleAtoi(std::string_view, uint16_t)
  ...
  VerifySimpleAtoiGood<turbo::int128>(std::numeric_limits<int32_t>::min(),
                                      std::numeric_limits<int32_t>::min());
  VerifySimpleAtoiGood<turbo::int128>(std::numeric_limits<int32_t>::max(),
                                      std::numeric_limits<int32_t>::max());
  VerifySimpleAtoiGood<turbo::int128>(std::numeric_limits<uint32_t>::max(),
                                      std::numeric_limits<uint32_t>::max());
  VerifySimpleAtoiGood<turbo::int128>(std::numeric_limits<int64_t>::min(),
                                      std::numeric_limits<int64_t>::min());
  VerifySimpleAtoiGood<turbo::int128>(std::numeric_limits<int64_t>::max(),
                                      std::numeric_limits<int64_t>::max());
  VerifySimpleAtoiGood<turbo::int128>(std::numeric_limits<uint64_t>::max(),
                                      std::numeric_limits<uint64_t>::max());
  VerifySimpleAtoiGood<turbo::int128>(
      std::numeric_limits<turbo::int128>::min(),
      std::numeric_limits<turbo::int128>::min());
  VerifySimpleAtoiGood<turbo::int128>(
      std::numeric_limits<turbo::int128>::max(),
      std::numeric_limits<turbo::int128>::max());
  VerifySimpleAtoiBad<turbo::int128>(std::numeric_limits<turbo::uint128>::max());
...
TEST(NumbersTest, AtodEmpty) {
  double d;
  EXPECT_FALSE(turbo::SimpleAtod("", &d));
  // Empty std::string_view takes a different code path from "".
  EXPECT_FALSE(turbo::SimpleAtod({}, &d));
}

TEST(NumbersTest, AtofEmpty) {
  float f;
  EXPECT_FALSE(turbo::SimpleAtof("", &f));
  // Empty std::string_view takes a different code path from "".
  EXPECT_FALSE(turbo::SimpleAtof({}, &f));
}
...
TEST(NumbersTest, HexAtoi) {
  // SimpleHexAtoi(std::string_view, int8_t)
  ...
  // SimpleHexAtoi(std::string_view, uint8_t)
  ...
  // SimpleHexAtoi(std::string_view, int16_t)
  ...
  // SimpleHexAtoi(std::string_view, uint16_t)
  ...
  VerifySimpleHexAtoiBad<turbo::uint128>(std::numeric_limits<int32_t>::min());
  VerifySimpleHexAtoiGood<turbo::uint128>(std::numeric_limits<int32_t>::max(),
                                         std::numeric_limits<int32_t>::max());
  VerifySimpleHexAtoiGood<turbo::uint128>(std::numeric_limits<uint32_t>::max(),
                                         std::numeric_limits<uint32_t>::max());
  VerifySimpleHexAtoiBad<turbo::uint128>(std::numeric_limits<int64_t>::min());
  VerifySimpleHexAtoiGood<turbo::uint128>(std::numeric_limits<int64_t>::max(),
                                         std::numeric_limits<int64_t>::max());
  VerifySimpleHexAtoiGood<turbo::uint128>(std::numeric_limits<uint64_t>::max(),
                                         std::numeric_limits<uint64_t>::max());
  VerifySimpleHexAtoiGood<turbo::uint128>(
      std::numeric_limits<turbo::uint128>::max(),
      std::numeric_limits<turbo::uint128>::max());
...
TEST(stringtest, safe_strto128_random) {
  ...
  // test_random_integer_parse_base<turbo::int128>(
  //     &turbo::numbers_internal::safe_strto128_base);
...
TEST(stringtest, safe_strtou8_exhaustive) {
  ...
  constexpr auto parse_func = &turbo::numbers_internal::safe_strtou8_base;
...
TEST(stringtest, safe_strto8_exhaustive) {
  ...
  constexpr auto parse_func = &turbo::numbers_internal::safe_strto8_base;
...
TEST(SimpleDtoa, HighPrecisionIsLocaleIndependent) {
  // turbo::HighPrecision(double) routes through round_trip_double_to_buffer(), which
  ...
  auto restore_locale =
      turbo::MakeCleanup([&] { setlocale(LC_NUMERIC, old_locale.c_str()); });
...
TEST_F(SimpleDtoaTest, ExhaustiveFloatToBuffer) {
  ...
  ExhaustiveFloat(kFloatNumCases, [&](float f) {
    ...
    ++test_count;
    char fastbuf[turbo::format_internal::kFastToBufferSize];
    turbo::format_internal::round_trip_float_to_buffer(f, fastbuf);
    float round_trip = strtof(fastbuf, nullptr);
    if (f != round_trip) {
      mismatches.push_back(f);
      if (mismatches.size() < 10) {
        KLOG(ERROR) << "Round-trip failure with float.  f=" << f << "="
  ...
  if (!mismatches.empty()) {
    ...
    for (size_t i = 0; i < mismatches.size(); ++i) {
      ...
      char buf[turbo::format_internal::kFastToBufferSize];
      float rt = strtof(buf, nullptr);
      KLOG(ERROR) << "Mismatch #" << i << "  f=" << f << " (" << ToNineDigits(f)
                 << ") fast='"
                 << turbo::format_internal::round_trip_float_to_buffer(f, buf)
                 << "' rt=" << ToNineDigits(rt);
    }
  }
}
...
TEST_F(SimpleDtoaTest, ExhaustiveDoubleToSixDigits) {
  ...
  auto checker = [&](double d) {
    ...
    if (strcmp(sixdigitsbuf, snprintfbuf) != 0) {
      ...
      if (mismatches.size() < 10) {
        KLOG(ERROR) << "Six-digit failure with double.  d=" << d
  ...
  if (mismatches.empty()) {
    ...
    for (int exponent = -324; exponent <= 308; ++exponent) {
      double powten = turbo::strings_internal::Pow10(exponent);
      if (powten == 0) powten = 5e-324;
      if (kFloatNumCases >= 1e9) {
        // The exhaustive test takes a very long time, so log progress.
        char buf[kSixDigitsToBufferSize];
        KLOG(INFO) << "Exp " << exponent << " powten=" << powten << "(" << powten
                  << ") ("
                  << std::string_view(buf, six_digits_to_buffer(powten, buf))
                  << ")";
      }
...
TEST(StrToInt8, Partial) {
  ...
  for (const Int8TestLine& test_line : int8_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = -2;
    status = safe_strto8_base(std::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToUint8, Partial) {
  ...
  for (const Uint8TestLine& test_line : uint8_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = 2;
    status = safe_strtou8_base(std::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToInt16, Partial) {
  ...
  for (const Int16TestLine& test_line : int16_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = -2;
    status = safe_strto16_base(std::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
TEST(StrToUint16, Partial) {
  ...
  for (const Uint16TestLine& test_line : uint16_test_line) {
    ...
    EXPECT_EQ(test_line.value, value) << test_line.input;
    value = 2;
    status = safe_strtou16_base(std::string_view(test_line.input), &value, 10);
    EXPECT_EQ(test_line.status, status) << test_line.input;
    EXPECT_EQ(test_line.value, value) << test_line.input;
  }
}
...
void ExpectWritesNull() {
  {
    ...
    EXPECT_THAT(turbo::format_internal::fast_int_to_buffer(x, buf), Pointee('\0'));
  }
  {
    char buf[turbo::format_internal::kFastToBufferSize];
...
TEST(fast_int_to_buffer, WritesNull) {
