...
TEST(StrJoin, APIExamples) {
...

  {
    // Array of ints
    const int a[] = {1, 2, 3, -4};
    EXPECT_EQ("1-2-3--4", turbo::StrJoin(a, a + KUMO_ARRAYSIZE(a), "-"));
  }
  ...
  {
    // Shows turbo::str_split and turbo::StrJoin working together. This example is
    // equivalent to s/=/-/g.
    const std::string s = "a=b=c=d";
    EXPECT_EQ("a-b-c-d", turbo::StrJoin(turbo::str_split(s, "="), "-"));
  }
...
TEST(StrJoin, StringViewInitializerList) {
  ...
  };
  {
    ...
    // for T that is implicitly convertible to std::string_view
    EXPECT_EQ("a-b-c",
