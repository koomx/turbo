...
TEST(Split, APIExamples) {
  {
    ...
    v = turbo::str_split("a,b,c", ByString(","));
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));

    // Equivalent to...
    EXPECT_THAT(turbo::str_split("a,b,c", ByString(",")),
                ElementsAre("a", "b", "c"));
  }

  {
    ...
    // Equivalent to...
    using turbo::ByChar;
    v = turbo::str_split("a,b,c", ByChar(','));
    EXPECT_THAT(v, ElementsAre("a", "b", "c"));
  }
...

  {
    ...
    std::vector<std::string> v =
        turbo::str_split(" a , ,,b,", ',', SkipWhitespace());
...

  {
    ...
    std::vector<std::string> v = turbo::str_split("abcdefg", ByLength(3));
...

  {
    ...
    for (std::string_view sp : turbo::str_split(s, ',', SkipWhitespace())) {
      EXPECT_EQ("x", sp);
    }
  }
...
TEST(Split, EmptyAndNull) {
  ...
  EXPECT_THAT(turbo::str_split(std::string_view(""), '-'), ElementsAre(""));
  EXPECT_THAT(turbo::str_split(std::string_view(), '-'), ElementsAre());
}
...
TEST(Splitter, ConversionOperator) {
  ...
  TestMapConversionOperator<std::unordered_map<
      std::string_view, std::string_view, turbo::Hash<std::string_view>>>(
      splitter);
  TestMapConversionOperator<std::unordered_map<std::string_view, std::string,
  ...
  TestArrayConversionOperator<std::string_view>(splitter);
}
...
TEST(Splitter, ToArray) {
  {
    // Empty string
    std::array<std::string, 2> p = turbo::str_split("", ',');
    EXPECT_THAT(p, ElementsAre("", ""));
  }

  {
    // Only first
    std::array<std::string, 2> p = turbo::str_split("a", ',');
    EXPECT_THAT(p, ElementsAre("a", ""));
  }

  {
    // Only second
    std::array<std::string, 2> p = turbo::str_split(",b", ',');
    EXPECT_THAT(p, ElementsAre("", "b"));
  }

  {
    // First and second.
    std::array<std::string, 2> p = turbo::str_split("a,b", ',');
    EXPECT_THAT(p, ElementsAre("a", "b"));
  }

  {
    // First and second and then more stuff that will be ignored.
    std::array<std::string, 2> p = turbo::str_split("a,b,c", ',');
    EXPECT_THAT(p, ElementsAre("a", "b"));
    // "c" is omitted.
  }
}
...
TEST(Splitter, Predicates) {
...

  {
    ...
    auto splitter = turbo::str_split(kTestChars, ',', AllowEmpty());
  ...
  {
    // Skips empty strings.
    auto splitter = turbo::str_split(kTestChars, ',', SkipEmpty());
    std::vector<std::string> v = splitter;
    EXPECT_THAT(v, ElementsAre("a", " ", "b"));
  }

  {
    ...
    auto splitter = turbo::str_split(kTestChars, ',', SkipWhitespace());
    std::vector<std::string> v = splitter;
    EXPECT_THAT(v, ElementsAre("a", "b"));
  }
}
...
TEST(Split, Temporary) {
  ...
  EXPECT_LT(sizeof(std::string), KUMO_ARRAYSIZE(input))
...
TEST(Split, UTF8) {
...

  {
    ...
    std::vector<std::string_view> v =
        turbo::str_split(u8"Foo h\u00E4llo th\u4E1Ere", turbo::ByAnyChar(" \t"));
...
TEST(Split, EmptyResults) {
  ...
  results = turbo::str_split("", '#', turbo::SkipEmpty());
  EXPECT_THAT(results, ElementsAre());
}
...
TEST(Split, ByAsciiWhitespace) {
  using turbo::ByAsciiWhitespace;
  using turbo::SkipEmpty;
  std::vector<std::string_view> results;

  results = turbo::str_split("aaaa\n", ByAsciiWhitespace());
  EXPECT_THAT(results, ElementsAre("aaaa", ""));

  results = turbo::str_split("aaaa\n", ByAsciiWhitespace(), SkipEmpty());
  EXPECT_THAT(results, ElementsAre("aaaa"));

  results = turbo::str_split(" ", ByAsciiWhitespace());
  EXPECT_THAT(results, ElementsAre("", ""));

  results = turbo::str_split(" ", ByAsciiWhitespace(), SkipEmpty());
  EXPECT_THAT(results, IsEmpty());

  results = turbo::str_split("a", ByAsciiWhitespace());
