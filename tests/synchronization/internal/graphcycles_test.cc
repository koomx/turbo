...
static void PrintGCTransitiveClosure(Nodes *nodes, const IdMap &id,
                                     GraphCycles *gc) {
  KLOG(INFO) << "GC Transitive closure";
  for (int a : *nodes) {
    for (int b : *nodes) {
  ...
  for (const auto &a : *nodes) {
    for (const auto &b : *nodes) {
      ...
      if (gc_reachable != reachable) {
        ...
        KLOG(FATAL) << "gc_reachable " << gc_reachable << " reachable "
                   << reachable << " a " << a << " b " << b;
      }
    }
  }
...
static void CheckEdges(Nodes *nodes, Edges *edges, const IdMap &id,
  ...
  if (count != edges->size()) {
    PrintEdges(edges);
    PrintGCEdges(nodes, id, gc);
    KLOG(FATAL) << "edges->size() " << edges->size() << "  count " << count;
  }
}

static void CheckInvariants(const GraphCycles &gc) {
  KCHECK(gc.CheckInvariants()) << "CheckInvariants";
}
...
class GraphCyclesTest : public ::testing::Test {
 public:
  ...
  std::string Path(int x, int y) {
    GraphId path[5];
    int np = g_.FindPath(Get(id_, x), Get(id_, y), KUMO_ARRAYSIZE(path), path);
    std::string result;
    for (int i = 0; i < np; i++) {
      if (i >= KUMO_ARRAYSIZE(path)) {
        result += " ...";
        break;
      }
