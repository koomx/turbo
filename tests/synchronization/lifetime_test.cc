...
//
// These tests use KCHECK to validate invariants, rather than EXPECT or ASSERT
...
void ThreadOne(turbo::Mutex* mutex, turbo::CondVar* condvar,
  ...
  KCHECK(!notification->HasBeenNotified()) << "invalid Notification";
  KCHECK(!*state) << "*state not initialized";
...
KUMO_CONST_INIT turbo::Mutex const_init_mutex(turbo::kConstInit);
...
KUMO_CONST_INIT turbo::Mutex early_const_init_mutex(turbo::kConstInit);
...
KUMO_CONST_INIT turbo::Mutex const_init_sanity_mutex(turbo::kConstInit);
...
KUMO_CONST_INIT turbo::Mutex late_const_init_mutex(turbo::kConstInit);
