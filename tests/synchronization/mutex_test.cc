...
std::unique_ptr<turbo::synchronization_internal::ThreadPool> CreatePool(
    int threads) {
  return std::make_unique<turbo::synchronization_internal::ThreadPool>(threads);
}
...
static void CheckSumG0G1(void *v) {
  ...
  KCHECK_EQ(cxt->g0, -cxt->g1) << "Error in CheckSumG0G1";
...
static void TestRW(TestContext *cxt, int c) {
  if ((c & 1) == 0) {
    ...
    for (int i = 0; i != cxt->iterations; i++) {
      turbo::ReaderMutexLock l(cxt->mu);
      KCHECK_EQ(cxt->g0, -cxt->g1) << "Error in TestRW";
      cxt->mu.AssertReaderHeld();
    }
  }
}
...
static void TestAwait(TestContext *cxt, int c) {
  ...
  while (cxt->g0 < cxt->iterations) {
    ...
    KCHECK(mc.MyTurn()) << "Error in TestAwait";
    cxt->mu.AssertHeld();
...
static void TestSignal(TestContext *cxt, int c) {
  KCHECK_EQ(cxt->threads, 2) << "TestSignal should use 2 threads";
  int target = c;
  turbo::MutexLock l(cxt->mu);
  cxt->mu.AssertHeld();
  while (cxt->g0 < cxt->iterations) {
...
static void TestTime(TestContext *cxt, int c, bool use_cv) {
  KCHECK_EQ(cxt->iterations, 1) << "TestTime should only use 1 iteration";
  KCHECK_GT(cxt->threads, 2) << "TestTime should use more than 2 threads";
  ...
  if (c == 0) {
    ...
    if (use_cv) {
      ...
      KCHECK(!cxt->mu.AwaitWithTimeout(false_cond, turbo::Seconds(1)))
          << "TestTime failed";
    }
    turbo::Duration elapsed = turbo::Now() - start;
    KCHECK(turbo::Seconds(0.9) <= elapsed && elapsed <= turbo::Seconds(2.0))
        << "TestTime failed";
    KCHECK_EQ(cxt->g0, 1) << "TestTime failed";
    ...
    if (use_cv) {
      ...
      KCHECK(!cxt->mu.AwaitWithTimeout(false_cond, turbo::Seconds(4)))
          << "TestTime failed";
    }
    elapsed = turbo::Now() - start;
    KCHECK(turbo::Seconds(3.9) <= elapsed && elapsed <= turbo::Seconds(6.0))
        << "TestTime failed";
    KCHECK_GE(cxt->g0, 3) << "TestTime failed";
    ...
    KCHECK(turbo::Seconds(0.9) <= elapsed && elapsed <= turbo::Seconds(2.0))
        << "TestTime failed";
    KCHECK_EQ(cxt->g0, cxt->threads) << "TestTime failed";
    ...
    if (use_cv) {
      ...
      KCHECK(!cxt->mu.AwaitWithTimeout(false_cond, turbo::Milliseconds(500)))
          << "TestTime failed";
    }
    const turbo::Duration elapsed = turbo::Now() - start;
    KCHECK(turbo::Seconds(0.4) <= elapsed && elapsed <= turbo::Seconds(0.9))
        << "TestTime failed";
    cxt->g0++;
  } else if (c == 2) {
    turbo::MutexLock l(cxt->mu);
    if (use_cv) {
      ...
      KCHECK(cxt->mu.AwaitWithTimeout(g0ge2, turbo::Seconds(100)))
          << "TestTime failed";
    }
    cxt->g0++;
  } else {
...
static int RunTestWithInvariantDebugging(void (*test)(TestContext *cxt, int),
  ...
  KCHECK(GetInvariantChecked()) << "Invariant not checked";
  return ret;
}
#endif
...
TEST(Mutex, LockedMutexDestructionBug) TURBO_NO_THREAD_SAFETY_ANALYSIS {
  ...
  for (int i = 0; i != 10; i++) {
    ...
    auto mu = std::make_unique<turbo::Mutex[]>(kNumLocks);
...
TEST(Mutex, FunctionPointerCondition) {
  ...
  EXPECT_FALSE((std::is_constructible_v<turbo::Condition, decltype(Equals42),
                                        decltype(&const_x)>));
  // Validate use of is_constructible by contrasting to a well-formed case.
  EXPECT_TRUE((std::is_constructible_v<turbo::Condition, decltype(ConstEquals42),
                                       decltype(&const_x)>));
}
...
TEST(Mutex, FunctionPointerConditionWithDerivedToBaseConversion) {
  ...
  EXPECT_FALSE((
      std::is_constructible_v<turbo::Condition, decltype(derived_pred), Base*>));
  EXPECT_FALSE((std::is_constructible_v<turbo::Condition, decltype(derived_pred),
                                        const Base*>));
  // Validate use of is_constructible by contrasting to well-formed cases.
  EXPECT_TRUE((std::is_constructible_v<turbo::Condition, decltype(derived_pred),
                                       Derived*>));
  EXPECT_TRUE((std::is_constructible_v<turbo::Condition, decltype(derived_pred),
                                       const Derived*>));
}
...
static bool ConditionWithAcquire(AcquireFromConditionStruct *x) {
  ...
  KCHECK_LT(x->value, 4) << "should not be invoked a fourth time";
...
TEST(Mutex, DeadlockDetectorStressTest) TURBO_NO_THREAD_SAFETY_ANALYSIS {
  ...
  auto array_of_locks = std::make_unique<turbo::Mutex[]>(n_locks);
...
static bool DelayIsWithinBounds(turbo::Duration expected_delay,
  ...
  if (actual_delay < expected_delay) {
    KLOG(WARNING) << "Actual delay " << actual_delay
...
TEST(Mutex, LoggingAddressReuse) {
  ...
  ScopedInvariantDebugging scoped_debugging;
  alignas(turbo::Mutex) unsigned char storage[sizeof(turbo::Mutex)];
  auto invariant =
