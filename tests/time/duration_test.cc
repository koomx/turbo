...
TEST(Duration, ConstExpr) {
  static_assert(std::is_trivially_destructible_v<turbo::Duration>,
...
#ifdef TURBO_INTERNAL_TIME_HAS_THREE_WAY_COMPARISON
  EXPECT_EQ(range_future <=> turbo::InfiniteDuration(),
            std::strong_ordering::less);
  EXPECT_EQ(range_past <=> -turbo::InfiniteDuration(),
            std::strong_ordering::greater);
  EXPECT_EQ(full_range <=> turbo::ZeroDuration(),  //
            std::strong_ordering::greater);
  EXPECT_EQ(full_range <=> -turbo::InfiniteDuration(),
            std::strong_ordering::greater);
  EXPECT_EQ(neg_full_range <=> -turbo::InfiniteDuration(),
            std::strong_ordering::greater);
...
TEST(Duration, turbo_stringify) {
  ...
  // verify that str_sprintf("%v", d) works as expected.
  turbo::Duration d = turbo::Seconds(1);
  EXPECT_EQ(turbo::str_sprintf("%v", d), turbo::FormatDuration(d));
}
