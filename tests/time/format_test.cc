...
TEST(format_time, LocaleSpecific) {
  ...
  const std::string s =
      turbo::format_time("%c", turbo::FromTimeT(0), turbo::UTCTimeZone());
...
TEST(format_time, InfiniteTime) {
  ...
  EXPECT_EQ("infinite-future",
            turbo::format_time("%H:%M blah", turbo::InfiniteFuture(), tz));
  EXPECT_EQ("infinite-past",
            turbo::format_time("%H:%M blah", turbo::InfinitePast(), tz));
}
...
TEST(parse_time, ErrorCases) {
  ...
  EXPECT_FALSE(
      turbo::parse_time("%Y", std::string("2026\0payload", 12), &t, &err));
  EXPECT_THAT(err, HasSubstr("Illegal trailing data"));
  ...
  for (int i = 128; i < 256; ++i) {
    ...
    EXPECT_FALSE(turbo::parse_time("%Y-%m-%d", input, &t, &err));
  }
}
...
TEST(FormatParse, RoundTrip) {
  ...
  {
    turbo::Time out;
    const std::string s = turbo::format_time("%c", in, turbo::UTCTimeZone());
