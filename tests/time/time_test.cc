...
#include <tests/time/internal/test_util.h>
...
TEST(Time, ConstExpr) {
  static_assert(std::is_trivially_destructible_v<turbo::Time>,
                "Time is documented as being trivially destructible");
  constexpr turbo::Time t0 = turbo::UnixEpoch();
  static_assert(t0 == turbo::UnixEpoch(), "UnixEpoch");
  constexpr turbo::Time t1 = turbo::InfiniteFuture();
  static_assert(t1 != turbo::UnixEpoch(), "InfiniteFuture");
  constexpr turbo::Time t2 = turbo::InfinitePast();
  static_assert(t2 != turbo::UnixEpoch(), "InfinitePast");
  constexpr turbo::Time t3 = turbo::FromUnixNanos(0);
  static_assert(t3 == turbo::UnixEpoch(), "FromUnixNanos");
  constexpr turbo::Time t4 = turbo::FromUnixMicros(0);
  static_assert(t4 == turbo::UnixEpoch(), "FromUnixMicros");
  constexpr turbo::Time t5 = turbo::FromUnixMillis(0);
  static_assert(t5 == turbo::UnixEpoch(), "FromUnixMillis");
  constexpr turbo::Time t6 = turbo::FromUnixSeconds(0);
  static_assert(t6 == turbo::UnixEpoch(), "FromUnixSeconds");
  constexpr turbo::Time t7 = turbo::FromTimeT(0);
  static_assert(t7 == turbo::UnixEpoch(), "FromTimeT");
}
...
TEST(Time, RelationalOperators) {
  ...
  static_assert(turbo::UnixEpoch() == t1, "");
  static_assert(t1 == t1, "");
  static_assert(t2 == t2, "");
  static_assert(t3 == t3, "");
...
#undef TEST_FLOOR_CONVERSION
  ...
  EXPECT_EQ(0, turbo::ToUnixNanos(turbo::UnixEpoch() + turbo::ZeroDuration()));
  EXPECT_EQ(-1,
  ...
  EXPECT_EQ(0,
            turbo::ToUniversal(turbo::UniversalEpoch() + turbo::ZeroDuration()));
  EXPECT_EQ(-1,
            turbo::ToUniversal(turbo::UniversalEpoch() + turbo::Nanoseconds(-1)));
  EXPECT_EQ(-1,
            turbo::ToUniversal(turbo::UniversalEpoch() + turbo::Nanoseconds(-99)));
  ...
  const struct {
    ...
    timespec ts;
      ...
      {turbo::FromUnixSeconds(1) + turbo::ZeroDuration(), {1, 0}},
      {turbo::FromUnixSeconds(0) + turbo::ZeroDuration(), {0, 0}},
  ...
  const struct {
    ...
    turbo::Time t;
      ...
      {{1, 0}, turbo::FromUnixSeconds(1) + turbo::ZeroDuration()},
      {{0, 0}, turbo::FromUnixSeconds(0) + turbo::ZeroDuration()},
  ...
  // Tests  turbo::ToTimeval()/TimeFromTimeval() (same as timespec above)
  const struct {
    turbo::Time t;
    timeval tv;
  } to_tv[] = {
  ...
  for (const auto& test : to_tv) {
    EXPECT_THAT(turbo::ToTimeval(test.t), TimevalMatcher(test.tv));
  }
  const struct {
    timeval tv;
    turbo::Time t;
...
#define TEST_CONVERSION_ROUND_TRIP(SOURCE, FROM, TO, MATCHER) \
  ...
  // TimeFromTimeval() and  turbo::ToTimeval()
  timeval tv;
  tv.tv_sec = -1;
  tv.tv_usec = 0;
...
TEST(Time, TimeZoneAt) {
  ...
  EXPECT_EQ("Wed, 31 Dec 1969 23:59:59 +0000 (UTC)",
            turbo::format_time(fmt, minus1_cl.pre, turbo::UTCTimeZone()));
}
...
TEST(Time, FromCivilUTC) {
  ...
  EXPECT_EQ("infinite-future", turbo::format_time(fmt, t, utc));  // no overflow
  ...
  EXPECT_EQ("infinite-past", turbo::format_time(fmt, t, utc));  // no underflow
...
TEST(Time, ToTM) {
  ...
  // Checks overflow.
  tm = turbo::ToTM(turbo::InfiniteFuture(), nyc);
  ...
  // Checks underflow.
  tm = turbo::ToTM(turbo::InfinitePast(), nyc);
...
TEST(Time, FromTM) {
  ...
  tm.tm_sec = 3;
  tm.tm_isdst = -1;
  turbo::Time t = turbo::FromTM(tm, nyc);
  EXPECT_EQ("2014-06-28T01:02:03-04:00", turbo::format_time(t, nyc));  // DST
  tm.tm_isdst = 0;
  t = turbo::FromTM(tm, nyc);
  EXPECT_EQ("2014-06-28T01:02:03-04:00", turbo::format_time(t, nyc));  // DST
  tm.tm_isdst = 1;
  t = turbo::FromTM(tm, nyc);
  EXPECT_EQ("2014-06-28T01:02:03-04:00", turbo::format_time(t, nyc));  // DST
  ...
  tm.tm_isdst = -1;
  t = turbo::FromTM(tm, nyc);
  EXPECT_EQ("2014-11-02T01:30:42-04:00", turbo::format_time(t, nyc));  // DST
  tm.tm_isdst = 0;
  t = turbo::FromTM(tm, nyc);
  EXPECT_EQ("2014-11-02T01:30:42-05:00", turbo::format_time(t, nyc));  // STD
  tm.tm_isdst = 1;
  t = turbo::FromTM(tm, nyc);
  EXPECT_EQ("2014-11-02T01:30:42-04:00", turbo::format_time(t, nyc));  // DST
  ...
  tm.tm_isdst = -1;
  t = turbo::FromTM(tm, nyc);
  EXPECT_EQ("2014-03-09T03:30:42-04:00", turbo::format_time(t, nyc));  // DST
  tm.tm_isdst = 0;
  t = turbo::FromTM(tm, nyc);
  EXPECT_EQ("2014-03-09T01:30:42-05:00", turbo::format_time(t, nyc));  // STD
  tm.tm_isdst = 1;
  t = turbo::FromTM(tm, nyc);
  EXPECT_EQ("2014-03-09T03:30:42-04:00", turbo::format_time(t, nyc));  // DST
  ...
  tm.tm_sec = 3;
  tm.tm_isdst = -1;
  t = turbo::FromTM(tm, turbo::UTCTimeZone());
  EXPECT_EQ("2147483648-06-28T01:02:03+00:00",
            turbo::format_time(t, turbo::UTCTimeZone()));
...
TEST(Time, TMRoundTrip) {
  ...
  for (turbo::Time t = start; t < end; t += turbo::Minutes(1)) {
    struct tm tm = turbo::ToTM(t, nyc);
    turbo::Time rt = turbo::FromTM(tm, nyc);
    EXPECT_EQ(rt, t);
  }
...
TEST(Time, ConversionSaturation) {
  ...
  EXPECT_EQ("infinite-future", turbo::format_time(turbo::RFC3339_full, t, utc));
  ...
  EXPECT_EQ("infinite-past", turbo::format_time(turbo::RFC3339_full, t, utc));
}
...
TEST(Time, FromCivilAlignment) {
  ...
  EXPECT_EQ("2015-02-03T04:05:06+00:00", turbo::format_time(t, utc));
  t = turbo::FromCivil(turbo::CivilMinute(cs), utc);
  EXPECT_EQ("2015-02-03T04:05:00+00:00", turbo::format_time(t, utc));
  t = turbo::FromCivil(turbo::CivilHour(cs), utc);
  EXPECT_EQ("2015-02-03T04:00:00+00:00", turbo::format_time(t, utc));
  t = turbo::FromCivil(turbo::CivilDay(cs), utc);
  EXPECT_EQ("2015-02-03T00:00:00+00:00", turbo::format_time(t, utc));
  t = turbo::FromCivil(turbo::CivilMonth(cs), utc);
  EXPECT_EQ("2015-02-01T00:00:00+00:00", turbo::format_time(t, utc));
  t = turbo::FromCivil(turbo::CivilYear(cs), utc);
  EXPECT_EQ("2015-01-01T00:00:00+00:00", turbo::format_time(t, utc));
}
...
TEST(Time, turbo_stringify) {
  // format_time is already well tested, so just use one test case here to
  // verify that str_sprintf("%v", t) works as expected.
  turbo::Time t = turbo::Now();
  EXPECT_EQ(turbo::str_sprintf("%v", t), turbo::format_time(t));
}

TEST(Time, SupportsHash) {
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly({
      turbo::UTCTimeZone(),
      turbo::FixedTimeZone(-8 * 60 * 60),
      turbo::UTCTimeZone(),
  }));
  ...
  EXPECT_TRUE(turbo::VerifyTypeImplementsTurboHashCorrectly({
      turbo::Now(),
      turbo::UnixEpoch(),
      turbo::UnixEpoch() + turbo::Seconds(60),
      turbo::UnixEpoch() + turbo::Minutes(1),
      turbo::InfiniteFuture(),
      turbo::InfinitePast(),
  }));
