...
static_assert(!turbo::type_traits_internal::IsOwner<AnySpan<int>>::value &&
                  turbo::type_traits_internal::IsView<AnySpan<int>>::value,
...
TEST(ConstAnySpanTest, InitializerListString) {
  auto test = [](AnySpan<const std::string> span) {
    ...
    for (const std::string& i : span) {
      EXPECT_EQ(turbo::StrCat(expected++), i);
    }
    EXPECT_EQ(expected, 5);
  };
  test({"1", "2", "3", "4"});
...
bool WantsStringOrAnySpan(std::string_view /* ignored */) { return false; }
