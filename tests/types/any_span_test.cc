...
namespace turbo {
    ...
    namespace {
        ...
        static_assert(!turbo::type_traits_internal::IsOwner<AnySpan<int>>::value && turbo::type_traits_internal::IsView<AnySpan<int>>::value,
        ...
        TYPED_TEST(AnySpanTest, NullAndEmpty) {
            ...
            for (const std::string& a : empty) {
                (void)a;
                TURBO_RAW_LOG(FATAL, "Empty container should not iterate.");
            }
        ...
        TYPED_TEST(AnySpanTest, AtThrows) {
            ...
            std::vector<std::string> v = { "a", "b", "c", "d" };
            AnySpan<T> span(v);
            TURBO_BASE_INTERNAL_EXPECT_FAIL(span.at(4), std::out_of_range,
                "failed bounds check");
        }
        ...
        TYPED_TEST(AnySpanTest, CheapTypes) {
            ...
            EXPECT_TRUE(IsCheap(AnySpan<T>(DefaultInstance<turbo::Span<int>>())));
            EXPECT_TRUE(IsCheap(AnySpan<T>(DefaultInstance<std::vector<int>>())));
            EXPECT_TRUE(IsCheap(AnySpan<T>(DefaultInstance<std::vector<int*>>(),
                any_span_transform::Deref())));
            EXPECT_TRUE(IsCheap(AnySpan<T>(DefaultInstance<std::vector<int*>>(),
            ...
            struct SomeContainer {
                int& operator[](std::size_t) { TURBO_RAW_LOG(FATAL, "Not implemented"); }
                const int& operator[](std::size_t) const {
                    TURBO_RAW_LOG(FATAL, "Not implemented");
                }
        ...
        TEST(ConstAnySpanTest, CheapTypes) {
            ...
            EXPECT_TRUE(IsCheap(AnySpan<const int>(std::vector<const int*>(),
                any_span_transform::Deref())));
            EXPECT_TRUE(
                IsCheap(AnySpan<const int>(DefaultInstance<turbo::Span<const int>>())));
        ...
        TEST(ConstAnySpanTest, InitializerListString) {
            auto test = [](AnySpan<const std::string> span) {
                ...
                for (const std::string& i : span) {
                    EXPECT_EQ(turbo::str_cat(expected++), i);
                }
                EXPECT_EQ(expected, 5);
            };
            test({ "1", "2", "3", "4" });
        ...
        TYPED_TEST(AnySpanTest, SubspanToEnd) {
            ...
            if (IsHardened()) {
                auto hardener = turbo::base_internal::ScopedSetTurboHardeningForTesting(true);
        ...
        TEST(MutableAnySpanTest, ImplicitConversionFromMutableSpan) {
            std::vector<std::string> v = { "a", "b", "c", "d" };
            turbo::Span<std::string> mutable_slice(v);
            auto set_all_to_e = [](AnySpan<std::string> span) {
                for (std::string& s : span) {
                    s = "e";
                }
        ...
        bool WantsStringOrAnySpan(std::string_view /* ignored */) {
            return false;
        }
        bool WantsStringOrAnySpan(AnySpan<std::string> /* ignored */) {
            return true;
        }
        bool WantsStringOrConstAnySpan(std::string_view /* ignored */) {
            return false;
        }
        ...
        TEST(AnySpanTest, SupportsTurboHash) {
        ...
        class MarkIfCopied {
        public:
            explicit MarkIfCopied(bool* copied)
                : copied_(copied) { }
