...
namespace {
    ...
    TEST(FixedArrayTest, MoveCtor) {
        ...
        turbo::FixedArray<std::unique_ptr<int>, 10> alloced_copy = std::move(allocated);
        for (int i = 0; i < 15; ++i)
    ...
    static void TestArrayOfArrays(int n) {
        ...
        ConstructionTester::destructions = 0;
        {
            ...
            auto array_ptr = std::make_unique<turbo::FixedArray<InnerArray, inline_elements>>(n);
    ...
    TEST(IteratorConstructorTest, NonInline) {
        ...
        turbo::FixedArray<int, KUMO_ARRAYSIZE(kInput) - 1> const fixed(
            kInput, kInput + KUMO_ARRAYSIZE(kInput));
        ASSERT_EQ(KUMO_ARRAYSIZE(kInput), fixed.size());
        for (size_t i = 0; i < KUMO_ARRAYSIZE(kInput); ++i) {
    ...
    TEST(IteratorConstructorTest, Inline) {
        int const kInput[] = { 2, 3, 5, 7, 11, 13, 17 };
        turbo::FixedArray<int, KUMO_ARRAYSIZE(kInput)> const fixed(
            kInput, kInput + KUMO_ARRAYSIZE(kInput));
        ASSERT_EQ(KUMO_ARRAYSIZE(kInput), fixed.size());
        for (size_t i = 0; i < KUMO_ARRAYSIZE(kInput); ++i) {
            ASSERT_EQ(kInput[i], fixed[i]);
    ...
    TEST(IteratorConstructorTest, FromNonEmptyVector) {
        int const kInput[] = { 2, 3, 5, 7, 11, 13, 17 };
        std::vector<int> const items(kInput, kInput + KUMO_ARRAYSIZE(kInput));
    ...
    TEST(IteratorConstructorTest, FromBidirectionalIteratorRange) {
        int const kInput[] = { 2, 3, 5, 7, 11, 13, 17 };
        std::list<int> const items(kInput, kInput + KUMO_ARRAYSIZE(kInput));
        turbo::FixedArray<int> const fixed(items.begin(), items.end());
    ...
    TEST(FixedArrayTest, UsesGlobalAlloc) {
        turbo::FixedArray<PickyDelete, 0> a(5);
        EXPECT_EQ(a.size(), 5);
    }
    ...
    TEST(FixedArrayTest, DefaultCtorDoesNotValueInit) {
        ...
        FixedArrType* arr = ::new (static_cast<void*>(std::addressof(buff))) FixedArrType(length);
    ...
    TEST(AllocatorSupportTest, CountOutoflineAllocations) {
...

        {
            ...
            AllocFxdArr arr(ia, ia + KUMO_ARRAYSIZE(ia), alloc);
