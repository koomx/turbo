...
#include <tests/base/internal/exception_safety_testing.h>
...
namespace {
    ...
    using ThrowerVec = turbo::InlinedVector<Thrower, kInlinedCapacity>;
    using MovableThrowerVec = turbo::InlinedVector<MovableThrower, kInlinedCapacity>;

    using ThrowAllocThrowerVec = turbo::InlinedVector<Thrower, kInlinedCapacity, ThrowAlloc>;
    using ThrowAllocMovableThrowerVec = turbo::InlinedVector<MovableThrower, kInlinedCapacity, ThrowAlloc>;
...
#define TURBO_INTERNAL_MAKE_INIT_LIST(T, N)                           \
    ...
    using NoSizeTestParams = ::testing::Types<TestParams<ThrowerVec>, TestParams<MovableThrowerVec>,
        TestParams<ThrowAllocThrowerVec>,
        TestParams<ThrowAllocMovableThrowerVec>>;

    using OneSizeTestParams = ::testing::Types<TestParams<ThrowerVec, kLargeSize>,
    ...
    bool InlinedVectorInvariants(VecT* vec) {
        ...
        if (vec->begin() > vec->end())
            return false;
        if ((vec->end() - vec->begin()) != vec->size())
            return false;
        if (std::distance(vec->begin(), vec->end()) != vec->size())
            return false;
        return true;
    }
    ...
    TYPED_TEST(OneSizeTest, MoveConstructor) {
        ...
        if (!turbo::allocator_is_nothrow<allocator_type>::value) {
