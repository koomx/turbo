...
namespace {
    ...
    using turbo::container_internal::CountingAllocator;
    ...
    using IntVec = turbo::InlinedVector<int, 8>;
    ...
    class RefCounted {
    public:
        RefCounted(int value, int* count)
            : value_(value)
            , count_(count) { Ref(); }
    ...
    using RefCountedVec = turbo::InlinedVector<RefCounted, 8>;
    ...
    using DynamicVec = turbo::InlinedVector<Dynamic, 8>;
    ...
    TEST(IntVec, LengthThrows) {
        IntVec v = { 1, 2, 3 };
        TURBO_BASE_INTERNAL_EXPECT_FAIL(v.insert(v.begin(), v.max_size() + 1, 0),
            std::length_error, "failed length check");
        TURBO_BASE_INTERNAL_EXPECT_FAIL(
            v.insert(v.begin(), static_cast<size_t>(-1), 0), std::length_error,
            "failed length check");
        TURBO_BASE_INTERNAL_EXPECT_FAIL(v.resize(v.max_size() + 1), std::length_error,
            "failed length check");
        TURBO_BASE_INTERNAL_EXPECT_FAIL(v.reserve(v.max_size() + 1), std::length_error,
            "failed length check");
        TURBO_BASE_INTERNAL_EXPECT_FAIL(static_cast<void>(IntVec(v.max_size() + 1, 0)),
            std::length_error, "failed length check");
    }
    ...
    TEST(IntVec, EmplaceLengthThrows) {
        turbo::InlinedVector<int, 1, SmallMaxAllocator<int>> v = { 1, 2 };
        TURBO_BASE_INTERNAL_EXPECT_FAIL(v.push_back(3), std::length_error,
            "failed length check");
        TURBO_BASE_INTERNAL_EXPECT_FAIL(v.emplace_back(3), std::length_error,
            "failed length check");
        TURBO_BASE_INTERNAL_EXPECT_FAIL(v.insert(v.begin(), 3), std::length_error,
            "failed length check");
        TURBO_BASE_INTERNAL_EXPECT_FAIL(v.emplace(v.begin(), 3), std::length_error,
            "failed length check");
    }
    ...
    TEST(UniquePtr, MoveConstruct) {
        for (size_t size = 0; size < 16; ++size) {
            ...
            turbo::InlinedVector<std::unique_ptr<size_t>, 2> a;
            for (size_t i = 0; i < size; ++i) {
                a.push_back(std::make_unique<size_t>(i));
            }

            turbo::InlinedVector<std::unique_ptr<size_t>, 2> b(std::move(a));
    ...
    TEST(UniquePtr, MoveAssign) {
        for (size_t size = 0; size < 16; ++size) {
            ...
            turbo::InlinedVector<std::unique_ptr<size_t>, 2> b;
            b = std::move(a);
    ...
    // leaks, despite the fact that unique pointers are trivially relocatable but
    // not trivially destructible.
    // TODO(turbo-team): Using unique_ptr here is technically correct, but
    ...
    TEST(UniquePtr, EraseSingle) {
        for (size_t size = 4; size < 16; ++size) {
            turbo::InlinedVector<std::unique_ptr<size_t>, 8> a;
            for (size_t i = 0; i < size; ++i) {
                a.push_back(std::make_unique<size_t>(i));
            }
            a.erase(a.begin());
    ...
    TEST(InlinedVectorTest, NoDefaultCtor) {
        turbo::InlinedVector<NoDefaultCtor, 1> v(10, NoDefaultCtor(2));
        (void)v;
    }
    TEST(InlinedVectorTest, NoCopy) {
        turbo::InlinedVector<NoCopy, 1> v(10);
        (void)v;
    }
    TEST(InlinedVectorTest, NoAssign) {
        turbo::InlinedVector<NoAssign, 1> v(10);
        (void)v;
    }
    TEST(InlinedVectorTest, MoveOnly) {
        turbo::InlinedVector<MoveOnly, 2> v;
    ...
    TEST(InlinedVectorTest, Noexcept) {
        ...
        EXPECT_TRUE(
            (std::is_nothrow_move_constructible_v<turbo::InlinedVector<MoveOnly, 2>>));

        struct MoveCanThrow {
            MoveCanThrow(MoveCanThrow&&) { }
        };
        EXPECT_EQ(turbo::default_allocator_is_nothrow::value,
            (std::is_nothrow_move_constructible_v<
                turbo::InlinedVector<MoveCanThrow, 2>>));
    }

    TEST(InlinedVectorTest, EmplaceBack) {
        turbo::InlinedVector<std::pair<std::string, int>, 1> v;
    ...
    TEST(InlinedVectorTest, ShrinkToFitEdgeCases) {
        ...
        {
            turbo::InlinedVector<std::string, 2> v(100);
            v.resize(0);
            v.shrink_to_fit();
        ...
        for (size_t len = 0; len < 20; len++) {
            for (ptrdiff_t pos = 0; pos <= static_cast<ptrdiff_t>(len); pos++) {
                ...
                }
                {
                    ...
                    IntVec::iterator it = v.insert(v.cbegin() + pos, input.cbegin(), input.cend());
                    EXPECT_THAT(v, ElementsAreArray(std_v));
                    EXPECT_EQ(it, v.cbegin() + pos);
                }
                {
                ...
                }
                {
                    ...
                    IntVec::iterator it = v.insert(v.cbegin() + pos, std::istream_iterator<int>(input),
    ...
    TEST(IntPairVec, Insert) {
        for (size_t len = 0; len < 20; len++) {
            for (ptrdiff_t pos = 0; pos <= static_cast<ptrdiff_t>(len); pos++) {
                ...
                const turbo::meta_internal::Cpp20ForwardZipIterator<
                ...
                turbo::InlinedVector<std::pair<int, int>, 8> v;
                for (size_t i = 0; i < len; ++i) {
    ...
    public:
        NotTriviallyDestructible()
            : p_(new int(1)) { }
        explicit NotTriviallyDestructible(int i)
            : p_(new int(i)) { }
    ...
    TEST(AliasingTest, Emplace) {
        for (size_t i = 2; i < 20; ++i) {
            turbo::InlinedVector<NotTriviallyDestructible, 10> vec;
            for (size_t j = 0; j < i; ++j) {
    ...
    TEST(OverheadTest, Storage) {
        ...
        EXPECT_EQ((2 * expected_overhead),
            sizeof(turbo::InlinedVector<T, 1>) - sizeof(T[1]));
        EXPECT_EQ(expected_overhead,
            sizeof(turbo::InlinedVector<T, 2>) - sizeof(T[2]));
        EXPECT_EQ(expected_overhead,
            sizeof(turbo::InlinedVector<T, 3>) - sizeof(T[3]));
        EXPECT_EQ(expected_overhead,
            sizeof(turbo::InlinedVector<T, 4>) - sizeof(T[4]));
        EXPECT_EQ(expected_overhead,
            sizeof(turbo::InlinedVector<T, 5>) - sizeof(T[5]));
        EXPECT_EQ(expected_overhead,
            sizeof(turbo::InlinedVector<T, 6>) - sizeof(T[6]));
        EXPECT_EQ(expected_overhead,
            sizeof(turbo::InlinedVector<T, 7>) - sizeof(T[7]));
        EXPECT_EQ(expected_overhead,
            sizeof(turbo::InlinedVector<T, 8>) - sizeof(T[8]));
    }
    ...
    TEST(StringVec, SelfRefPushBack) {
        ...
        turbo::InlinedVector<std::string, 4> v;
    ...
    TEST(StringVec, SelfMove) {
        ...
        for (int len = 0; len < 20; len++) {
            SCOPED_TRACE(len);
            turbo::InlinedVector<std::string, 8> v;
            for (int i = 0; i < len; ++i) {
                SCOPED_TRACE(i);
                v.push_back(s);
            }
    ...
    TYPED_TEST_P(InstanceTest, Swap) {
        ...
        using InstanceVec = turbo::InlinedVector<Instance, 8>;
        for (size_t l1 = 0; l1 < 20; l1++) {
            SCOPED_TRACE(l1);
            for (size_t l2 = 0; l2 < 20; l2++) {
                SCOPED_TRACE(l2);
    ...
    TEST(CountElemAssign, SimpleTypeWithInlineBacking) {
        const size_t inlined_capacity = turbo::InlinedVector<int, 2>().capacity();
        ...
        for (size_t original_size = 0; original_size <= 5; ++original_size) {
            ...
            turbo::InlinedVector<int, 2> v(original_contents.begin(),
    ...
    TYPED_TEST_P(InstanceTest, CountElemAssignInlineBacking) {
        ...
        for (size_t original_size = 0; original_size <= 5; ++original_size) {
            ...
            turbo::InlinedVector<Instance, 2> v(original_contents.begin(),
                original_contents.end());
    ...
    void InstanceCountElemAssignWithAllocationTest() {
        for (size_t original_size = 0; original_size <= 5; ++original_size) {
            ...
            EXPECT_THAT(v, AllOf(SizeIs(3u), ElementsAre(ValueIs(123), ValueIs(123), ValueIs(123))));
            EXPECT_LE(v.size(), v.capacity());
        }
    }
    ...
    TEST(RangedConstructor, SimpleType) {
        ...
        turbo::InlinedVector<int, 4> v(source_v.begin(), source_v.end());
        EXPECT_EQ(3u, v.size());
        EXPECT_EQ(4u,
        ...
        turbo::InlinedVector<int, 2> realloc_v(source_v.begin(), source_v.end());
    ...
    void InstanceRangedConstructorTestForContainer() {
        ...
        tracker.ResetCopiesMovesSwaps();
        turbo::InlinedVector<Instance, inlined_capacity> v(source_v.begin(),
            source_v.end());
        EXPECT_EQ(2u, v.size());
    ...
    TEST(RangedConstructor, ElementsAreConstructed) {
        ...
        turbo::InlinedVector<std::string, 1> v(source_v.begin(), source_v.end());
        EXPECT_EQ("cat", v[0]);
        EXPECT_EQ("dog", v[1]);
    }

    TEST(RangedAssign, SimpleType) {
        const size_t inlined_capacity = turbo::InlinedVector<int, 3>().capacity();
    ...
    void InstanceRangedAssignTestForContainer() {
        ...
        for (size_t original_size = 0; original_size <= 5; ++original_size) {
            ...
            for (size_t target_size = 0; target_size <= 5; ++target_size) {
                ...
                turbo::InlinedVector<Instance, 3> v(original_contents.begin(),
                    original_contents.end());
                v.assign(new_contents.begin(), new_contents.end());
    ...
    TEST(InitializerListConstructor, SimpleTypeWithInlineBacking) {
        EXPECT_THAT((turbo::InlinedVector<int, 4> { 4, 5, 6 }),
            AllOf(SizeIs(3u), CapacityIs(4u), ElementsAre(4, 5, 6)));
    }

    TEST(InitializerListConstructor, SimpleTypeWithReallocationRequired) {
        EXPECT_THAT((turbo::InlinedVector<int, 2> { 4, 5, 6 }),
            AllOf(SizeIs(3u), CapacityIs(Gt(2u)), ElementsAre(4, 5, 6)));
    }

    TEST(InitializerListConstructor, DisparateTypesInList) {
        EXPECT_THAT((turbo::InlinedVector<int, 2> { -7, 8ULL }), ElementsAre(-7, 8));

        EXPECT_THAT((turbo::InlinedVector<std::string, 2> { "foo", std::string("bar") }),
            ElementsAre("foo", "bar"));
    }

    TEST(InitializerListConstructor, ComplexTypeWithInlineBacking) {
        const size_t inlined_capacity = turbo::InlinedVector<CopyableMovableInstance, 1>().capacity();
        EXPECT_THAT(
            (turbo::InlinedVector<CopyableMovableInstance, 1> {
                CopyableMovableInstance(0) }),
            AllOf(SizeIs(1u), CapacityIs(inlined_capacity), ElementsAre(ValueIs(0))));
    }

    TEST(InitializerListConstructor, ComplexTypeWithReallocationRequired) {
        EXPECT_THAT((turbo::InlinedVector<CopyableMovableInstance, 1> {
    ...
    TEST(InitializerListAssign, SimpleTypeFitsInlineBacking) {
        for (size_t original_size = 0; original_size <= 4; ++original_size) {
            ...
            turbo::InlinedVector<int, 2> v1(original_size, 12345);
            const size_t original_capacity_v1 = v1.capacity();
            v1.assign({ 3 });
            EXPECT_THAT(v1, AllOf(SizeIs(1u), CapacityIs(original_capacity_v1), ElementsAre(3)));

            turbo::InlinedVector<int, 2> v2(original_size, 12345);
            const size_t original_capacity_v2 = v2.capacity();
            v2 = { 3 };
            EXPECT_THAT(v2, AllOf(SizeIs(1u), CapacityIs(original_capacity_v2), ElementsAre(3)));
        }
    }
    ...
    TYPED_TEST_P(InstanceTest, InitializerListAssign) {
        ...
        for (size_t original_size = 0; original_size <= 4; ++original_size) {
            SCOPED_TRACE(original_size);
            turbo::InlinedVector<Instance, 2> v(original_size, Instance(12345));
            const size_t original_capacity = v.capacity();
            v.assign({ Instance(3) });
            EXPECT_THAT(v, AllOf(SizeIs(1u), CapacityIs(original_capacity), ElementsAre(ValueIs(3))));
        }
        for (size_t original_size = 0; original_size <= 4; ++original_size) {
            SCOPED_TRACE(original_size);
            turbo::InlinedVector<Instance, 2> v(original_size, Instance(12345));
    ...
    using InstanceTypes = ::testing::Types<CopyableOnlyInstance, CopyableMovableInstance>;
    ...
    TEST(AllocatorSupportTest, Constructors) {
        ...
        using AllocVec = turbo::InlinedVector<int, 4, MyAlloc>;
        const int ia[] = { 0, 1, 2, 3, 4, 5, 6, 7 };
        int64_t allocated = 0;
        MyAlloc alloc(&allocated);
        { AllocVec KUMO_ATTRIBUTE_UNUSED v; }
        { AllocVec KUMO_ATTRIBUTE_UNUSED v(alloc); }
        { AllocVec KUMO_ATTRIBUTE_UNUSED v(ia, ia + KUMO_ARRAYSIZE(ia), alloc); }
        { AllocVec KUMO_ATTRIBUTE_UNUSED v({ 1, 2, 3 }, alloc); }

        AllocVec v2;
        { AllocVec KUMO_ATTRIBUTE_UNUSED v(v2, alloc); }
        { AllocVec KUMO_ATTRIBUTE_UNUSED v(std::move(v2), alloc); }
    }
    ...
    TEST(AllocatorSupportTest, CountAllocations) {
        ...
        {
            AllocVec KUMO_ATTRIBUTE_UNUSED v(ia, ia + 4, alloc);
        ...
        EXPECT_THAT(instance_count, Eq(0));
        {
            ...
            KUMO_ATTRIBUTE_UNUSED AllocVec v2(v, alloc2);
            EXPECT_THAT(bytes_allocated2, Eq(0));

            int64_t bytes_allocated3 = 0;
            MyAlloc alloc3(&bytes_allocated3);
            KUMO_ATTRIBUTE_UNUSED AllocVec v3(std::move(v), alloc3);
    ...
    TEST(AllocatorSupportTest, SwapBothAllocated) {
        ...
        int64_t allocated2 = 0;
        {
            ...
            AllocVec v1(ia1, ia1 + KUMO_ARRAYSIZE(ia1), a1);
            AllocVec v2(ia2, ia2 + KUMO_ARRAYSIZE(ia2), a2);
    ...
    TEST(AllocatorSupportTest, ScopedAllocatorWorksInlined) {
        ...
        using AllocVec = turbo::InlinedVector<StdVector, 1, ScopedAlloc>;
    ...
    TEST(AllocatorSupportTest, SizeAllocConstructor) {
        ...
        using AllocVec = turbo::InlinedVector<int, inlined_size, Alloc>;
    ...
    TEST(InlinedVectorTest, MinimumAllocatorCompilesUsingTraits) {
        ...
        turbo::InlinedVector<T, 1, MinimumAllocator> vec;
        vec.emplace_back();
        vec.resize(0);
    }
    ...
    class MoveConstructibleOnlyInstance
        : public turbo::test_internal::BaseCountedInstance {
    public:
        explicit MoveConstructibleOnlyInstance(int x)
            : BaseCountedInstance(x) { }
        MoveConstructibleOnlyInstance(MoveConstructibleOnlyInstance&& other) = default;
    ...
    TEST(NonAssignableMoveAssignmentTest, AllocatedToInline) {
        ...
        InstanceTracker tracker;
        turbo::InlinedVector<X, 2> inlined;
        inlined.emplace_back(1);
        turbo::InlinedVector<X, 2> allocated;
    ...
    TEST(NonAssignableMoveAssignmentTest, InlineToInline) {
        using X = MoveConstructibleOnlyInstance;
        InstanceTracker tracker;
        turbo::InlinedVector<X, 2> inlined_a;
        inlined_a.emplace_back(1);
        turbo::InlinedVector<X, 2> inlined_b;
    ...
    class NonSwappableInstance : public turbo::test_internal::BaseCountedInstance {
    public:
        explicit NonSwappableInstance(int x)
            : BaseCountedInstance(x) { }
    ...
    template <size_t N>
    using CharVec = turbo::InlinedVector<char, N>;
    ...
    TEST(StorageTest, InlinedCapacityAutoIncrease) {
        ...
        EXPECT_GT((turbo::InlinedVector<int, 1>().capacity()), 1);
        EXPECT_EQ((turbo::InlinedVector<int, 1>().capacity()),
            sizeof(MySpan<int>) / sizeof(int));
    }

    TEST(IntVec, EraseIf) {
        IntVec v = { 3, 1, 2, 0 };
        EXPECT_EQ(turbo::erase_if(v, [](int i) { return i > 1; }), 2u);
        EXPECT_THAT(v, ElementsAre(1, 0));
    }

    TEST(IntVec, EraseIfMatchesNone) {
        IntVec v = { 1, 2, 3 };
        EXPECT_EQ(turbo::erase_if(v, [](int i) { return i > 10; }), 0u);
        ;
    ...
    TEST(IntVec, EraseIfMatchesAll) {
        IntVec v = { 1, 2, 3 };
        EXPECT_EQ(turbo::erase_if(v, [](int i) { return i > 0; }), 3u);
    ...
    struct ThrowOnMove {
        ...
        explicit ThrowOnMove(int* count)
            : alive_count(count)
            , sentinel(kAlive) {
            if (alive_count)
                ++(*alive_count);
    ...
    TEST(InlinedVectorTest, SwapExceptionSafety) {
        int alive_count = 0;
        try {
            turbo::InlinedVector<ThrowOnMove, 2> a;
            a.emplace_back(&alive_count);
            turbo::InlinedVector<ThrowOnMove, 2> b;
    ...
    TEST(InlinedVectorTest, TruncatesBeforeThrowingMoveConstruction) {
        using Vec = turbo::InlinedVector<ThrowOnMove, 1>;
        int count = 0;
        {
            Vec dest;
