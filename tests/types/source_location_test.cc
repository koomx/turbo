...
namespace {
    ...
    TEST(SourceLocationTest, ConstexprMembers) {
        constexpr SourceLocation loc1 = turbo::SourceLocation::current();
        const SourceLocation loc2 = turbo::SourceLocation::current();
        EXPECT_EQ(loc1.line(), loc2.line() - 1);
        EXPECT_EQ(std::string_view(loc1.file_name()), loc2.file_name());
    }
    ...
    TEST(SourceLocationTest, CopyConstructionWorks) {
        constexpr SourceLocation location = turbo::SourceLocation::current();
    ...
    TEST(SourceLocationTest, CopyAssignmentWorks) {
        ...
        location = turbo::SourceLocation::current();
        constexpr int line = __LINE__ - 1;
    ...
    TEST(SourceLocationTest, ExpectedUsageWorks) {
        SourceLocation location = Echo(turbo::SourceLocation::current());
        constexpr int line = __LINE__ - 1;
...
#endif
        if constexpr (!std::is_same_v<turbo::SourceLocation, StdSourceLocation>) {
            EXPECT_FALSE(TryPassLineAndFile<turbo::SourceLocation>(nullptr));
        }
    }
