...
namespace {
    ...
    static_assert(!turbo::type_traits_internal::IsOwner<turbo::Span<int>>::value && turbo::type_traits_internal::IsView<turbo::Span<int>>::value,
        "Span is a view, not an owner");

    using S = turbo::Span<int>;

    static_assert(
        std::is_trivially_destructible_v<S> && std::is_trivially_copyable_v<S> && std::is_trivially_assignable_v<S, S&> && std::is_trivially_copy_assignable_v<S> && std::is_trivially_move_assignable_v<S> && std::is_trivially_assignable_v<S, const S&&> && std::is_trivially_constructible_v<S, S&> && std::is_trivially_copy_constructible_v<S> && std::is_trivially_move_constructible_v<S> && std::is_trivially_constructible_v<S, const S&&>,
    ...
    TEST(IntSpan, ArrayCtor) {
        ...
        EXPECT_TRUE((std::is_constructible_v<turbo::Span<const int>, int[3]>));
        EXPECT_TRUE((std::is_constructible_v<turbo::Span<const int>, const int[3]>));
        EXPECT_FALSE((std::is_constructible_v<turbo::Span<int>, const int[3]>));
        EXPECT_TRUE((std::is_convertible_v<int[3], turbo::Span<const int>>));
        EXPECT_TRUE((std::is_convertible_v<const int[3], turbo::Span<const int>>));
    }
    ...
    TEST(IntSpan, ContainerCtor) {
        ...
        EXPECT_TRUE(
            (std::is_convertible_v<std::vector<int>&, turbo::Span<const int>>));
        EXPECT_TRUE((std::is_convertible_v<turbo::Span<int>&, turbo::Span<const int>>));
    ...
    TEST(CharSpan, StringCtor) {
        ...
        EXPECT_FALSE((std::is_constructible_v<turbo::Span<int>, std::string>));
        EXPECT_FALSE((std::is_constructible_v<turbo::Span<const int>, std::string>));
        EXPECT_TRUE((std::is_convertible_v<std::string, turbo::Span<const char>>));
    }

    TEST(IntSpan, FromConstPointer) {
        EXPECT_TRUE((std::is_constructible_v<turbo::Span<const int* const>,
            std::vector<int*>>));
        EXPECT_TRUE((std::is_constructible_v<turbo::Span<const int* const>,
            std::vector<const int*>>));
        EXPECT_FALSE(
            (std::is_constructible_v<turbo::Span<const int*>, std::vector<int*>>));
        EXPECT_FALSE(
            (std::is_constructible_v<turbo::Span<int*>, std::vector<const int*>>));
    }
    ...
    TEST(IntSpan, EvilTypes) {
        EXPECT_FALSE(
            (std::is_constructible_v<turbo::Span<int>, TypeWithMisleadingData&>));
        EXPECT_FALSE(
            (std::is_constructible_v<turbo::Span<int>, TypeWithMisleadingSize&>));
    }
    ...
    TEST(IntSpan, SpanOfDerived) {
        EXPECT_TRUE((std::is_constructible_v<turbo::Span<int>, Base&>));
        EXPECT_TRUE((std::is_constructible_v<turbo::Span<int>, Derived&>));
        EXPECT_FALSE(
            (std::is_constructible_v<turbo::Span<Base>, std::vector<Derived>>));
    }
    ...
    TEST(ConstIntSpan, InitializerListConversion) {
        ...
        EXPECT_FALSE(
            (std::is_constructible_v<turbo::Span<int>, std::initializer_list<int>>));
        EXPECT_FALSE(
            (std::is_convertible_v<turbo::Span<int>, std::initializer_list<int>>));
    }
...
#define TURBO_TEST_CONSTEXPR(expr)                                             \
    do {                                                                       \
        KUMO_ATTRIBUTE_UNUSED ConstexprTester<(static_cast<void>(expr), 1)> t; \
    } while (0)
    ...
    TEST(ConstIntSpan, ConstexprRelOpsTest) {
        ...
        TURBO_TEST_CONSTEXPR(lhs == rhs_data);
        TURBO_TEST_CONSTEXPR(lhs != rhs_data);
        TURBO_TEST_CONSTEXPR(lhs < rhs_data);
        TURBO_TEST_CONSTEXPR(lhs <= rhs_data);
        TURBO_TEST_CONSTEXPR(lhs > rhs_data);
    ...
    // std::vector is implicitly convertible to turbo::Span.
    ...
    TEST(Span, HashConsistentWithVectorLike) {
        EXPECT_EQ(turbo::HashOf(turbo::Span<const int>({ 1, 2, 3 })),
            turbo::HashOf(std::vector<int> { 1, 2, 3 }));
        EXPECT_EQ(turbo::HashOf(turbo::Span<const int>({ 1, 2, 3 })),
            turbo::HashOf(turbo::InlinedVector<int, 2> { 1, 2, 3 }));
        EXPECT_EQ(turbo::HashOf(turbo::Span<const int>({ 1, 2, 3 })),
            turbo::HashOf(turbo::FixedArray<int> { 1, 2, 3 }));
    }
