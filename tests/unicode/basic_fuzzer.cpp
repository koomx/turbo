...
public:
    ...
    size_t next(std::vector<char>& output) {
        ...
        case THREE_SURROGATE: {
            turbo::tests::reference::utf8::encode(generate(0xd800, 0xdfff),
                consume);
            count = 3;
            break;
        }
        case FOUR_VALID: {
            turbo::tests::reference::utf8::encode(generate(0x10000, 0x10ffff),
                consume);
            count = 4;
            break;
        }
        ...
        case FOUR_TOO_LARGE: {
            turbo::tests::reference::utf8::encode(generate(0x110000, 0x1fffff),
                consume);
            count = 4;
            break;
        }
    ...
    for (int i = 0; i < 6; i++) {
        ...
        while (counter < trials) {
            for (size_t size : input_size) {
                ...
                is_ok_utf8.second = implementation.validate_utf8(input.data(), input.size());
