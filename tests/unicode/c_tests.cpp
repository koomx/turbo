...
TEST(validate_utf8_c) {
  ...
  ASSERT_EQUAL(r.error, UNICODE_ERROR_SUCCESS);
...
TEST(base64_c) {
  ...
  ASSERT_EQUAL(br.error, UNICODE_ERROR_SUCCESS);
...
TEST(ascii_and_detect_c) {
  ...
  ASSERT_EQUAL(et, UNICODE_ENCODING_UTF8);
...
TEST(lengths_and_conversions_c) {
  ...
  ASSERT_EQUAL(cr.error, UNICODE_ERROR_SUCCESS);
}
...
TEST(counts_and_find_utf16_c) {
  ...
  const char16_t *f16 = unicode_find_utf16(u16, u16 + 5, u'e');
  if (f16 == nullptr) {
...
TEST(validate_utf16_c) {
  char16_t u16[5] = {u'h', u'e', u'l', u'l', u'o'};
  bool ok = unicode_validate_utf16(u16, 5);
  ASSERT_TRUE(ok);
  unicode_result r = unicode_validate_utf16_with_errors(u16, 5);
  ASSERT_EQUAL(r.error, UNICODE_ERROR_SUCCESS);
...
TEST(validate_utf32_c) {
  ...
  bool ok = unicode_validate_utf32(u32, 5);
  ASSERT_TRUE(ok);
  unicode_result r = unicode_validate_utf32_with_errors(u32, 5);
  ASSERT_EQUAL(r.error, UNICODE_ERROR_SUCCESS);
  ASSERT_EQUAL(r.count, 5);
}
...
TEST(convert_utf16_to_utf8_c) {
  char16_t u16[5] = {u'h', u'e', u'l', u'l', u'o'};
  char out[8] = {0};
  size_t n = unicode_convert_utf16_to_utf8(u16, 5, out);
  ASSERT_EQUAL(n, 5);
...
TEST(convert_utf32_to_utf8_c) {
  char32_t u32[5] = {U'h', U'e', U'l', U'l', U'o'};
  char out[8] = {0};
  size_t n = unicode_convert_utf32_to_utf8(u32, 5, out);
  ASSERT_EQUAL(n, 5);
  ASSERT_TRUE(std::memcmp(out, "hello", 5) == 0);
}
...
TEST(convert_utf16_to_utf32_c) {
  char16_t u16[5] = {u'h', u'e', u'l', u'l', u'o'};
  char32_t out[8] = {0};
  size_t n = unicode_convert_utf16_to_utf32(u16, 5, out);
  ASSERT_EQUAL(n, 5);
  for (size_t i = 0; i < n; i++) {
...
TEST(convert_utf8_to_latin1_c) {
  ...
  char out[8] = {0};
  size_t n = unicode_convert_utf8_to_latin1(input, 5, out);
  ASSERT_EQUAL(n, 5);
  ASSERT_TRUE(std::memcmp(out, "hello", 5) == 0);
}
...
TEST(convert_latin1_to_utf16_c) {
  ...
  char16_t out[8] = {0};
  size_t n = unicode_convert_latin1_to_utf16(input, 3, out);
  ASSERT_EQUAL(n, 3);
  for (size_t i = 0; i < n; i++) {
...
TEST(to_well_formed_utf16_c) {
  char16_t u16[5] = {u'h', u'e', u'l', u'l', u'o'};
  char16_t out[8] = {0};
  unicode_to_well_formed_utf16(u16, 5, out);
...
TEST(invalid_utf8_c) {
  ...
  bool ok = unicode_validate_utf8(invalid, 2);
  ASSERT_FALSE(ok);
  unicode_result r = unicode_validate_utf8_with_errors(invalid, 2);
  ASSERT_EQUAL(r.error, UNICODE_ERROR_HEADER_BITS);
}
...
TEST(invalid_utf8_conversion_results_c) {
  ...
  char16_t out16[4] = {0};
  unicode_result r16 =
      unicode_convert_utf8_to_utf16_with_errors(invalid, 1, out16);
  ASSERT_EQUAL(r16.error, UNICODE_ERROR_HEADER_BITS);
  ASSERT_EQUAL(r16.count, size_t(0));

  char32_t out32[4] = {0};
  unicode_result r32 =
      unicode_convert_utf8_to_utf32_with_errors(invalid, 1, out32);
  ASSERT_EQUAL(r32.error, UNICODE_ERROR_HEADER_BITS);
...
TEST(invalid_utf16_conversion_result_c) {
  ...
  char out[16] = {0};
  unicode_result r = unicode_convert_utf16_to_utf8_with_errors(invalid, 3, out);
  ASSERT_EQUAL(r.error, UNICODE_ERROR_SURROGATE);
...
TEST(invalid_utf32_conversion_result_c) {
  ...
  char out[16] = {0};
  unicode_result r = unicode_convert_utf32_to_utf8_with_errors(invalid, 3, out);
  ASSERT_EQUAL(r.error, UNICODE_ERROR_TOO_LARGE);
  ASSERT_EQUAL(r.count, size_t(1));
}
...
TEST(base64_utf16_c) {
  ...
  char binout[16] = {0};
  size_t outlen = sizeof(binout);
  unicode_result br = unicode_base64_to_binary_safe_utf16(
      b64_u16, 8, binout, &outlen, UNICODE_BASE64_DEFAULT,
      UNICODE_LAST_CHUNK_LOOSE, true);
  ASSERT_EQUAL(br.error, UNICODE_ERROR_SUCCESS);
  ASSERT_EQUAL(outlen, hello_len);
  ASSERT_TRUE(std::memcmp(binout, "hello", hello_len) == 0);
...
TEST(length_calculations_c) {
  ...
  char16_t u16[5] = {u'h', u'e', u'l', u'l', u'o'};
  size_t utf8_len = unicode_utf8_length_from_utf16(u16, 5);
  ASSERT_EQUAL(utf8_len, 5);

  // Test utf16_length_from_utf8
  const char *utf8 = "hello";
  size_t utf16_len = unicode_utf16_length_from_utf8(utf8, 5);
  ASSERT_EQUAL(utf16_len, 5);

  // Test utf32_length_from_utf8
  size_t utf32_len = unicode_utf32_length_from_utf8(utf8, 5);
  ASSERT_EQUAL(utf32_len, 5);

  // Test latin1_length_from_utf8
  size_t latin1_len = unicode_latin1_length_from_utf8(utf8, 5);
...
TEST(convert_utf16le_to_utf8_with_replacement_c) {
  ...
  char out[32] = {0};
  size_t n =
      unicode_convert_utf16le_to_utf8_with_replacement(input, length, out);
  ...
  ASSERT_TRUE(unicode_validate_utf8(out, n));
  unicode_result len =
      unicode_utf8_length_from_utf16le_with_replacement(input, length);
...
TEST(convert_utf16be_to_utf8_with_replacement_c) {
  ...
  char out[32] = {0};
  size_t n =
      unicode_convert_utf16be_to_utf8_with_replacement(input, length, out);
  ASSERT_EQUAL(n, size_t(1 + 3 + 1 + 3 + 4));
  ASSERT_EQUAL(out[0], 'A');
  ASSERT_TRUE(std::memcmp(out + 1, fffd, 3) == 0);
  ASSERT_EQUAL(out[4], 'B');
  ...
  ASSERT_TRUE(unicode_validate_utf8(out, n));
  unicode_result len =
      unicode_utf8_length_from_utf16be_with_replacement(input, length);
  ASSERT_EQUAL(len.count, n);
}
...
TEST(convert_utf16_to_utf8_with_replacement_c) {
  ...
  char out2[16] = {0};
  size_t n2 =
      unicode_convert_utf16_to_utf8_with_replacement(truncated, 2, out2);
  ASSERT_EQUAL(n2, size_t(4));
  ASSERT_EQUAL(out2[0], 'X');
  ASSERT_TRUE(std::memcmp(out2 + 1, fffd, 3) == 0);
  ASSERT_TRUE(unicode_validate_utf8(out2, n2));

  unicode_result len =
      unicode_utf8_length_from_utf16_with_replacement(truncated, 2);
  ASSERT_EQUAL(len.count, n2);

  // Empty input writes nothing.
  ASSERT_EQUAL(unicode_convert_utf16_to_utf8_with_replacement(nullptr, 0, out),
               size_t(0));
}
