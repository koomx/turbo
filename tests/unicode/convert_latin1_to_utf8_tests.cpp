...
TEST_LOOP(convert_all_latin1) {
  ...
  turbo::tests::helpers::transcode_latin1_to_utf8_test_base test(generator,
                                                                   256);
...
TEST(convert_all_latin1_safe) {
  ...
  for (size_t output_size = 0; output_size < utf8.size(); output_size++) {
    ...
    size_t used_size = turbo::convert_latin1_to_utf8_safe(
...
TEST(compile_time_utf8_length_from_latin1) {
  ...
  static_assert(turbo::utf8_length_from_latin1("x"_latin1) == 1);
  // swedish character "ö":
  static_assert(turbo::utf8_length_from_latin1("\xF6"_latin1) == 2);
}
...
TEST(compile_time_convert_latin1_to_utf8) {
  ...
  static_assert(turbo::utf8_length_from_latin1(input) == expected.size());
...
template <auto input, std::size_t N> constexpr auto convert_safe() {
  turbo::tests::helpers::CTString<char8_t, N> ret{};
  auto written = turbo::convert_latin1_to_utf8_safe(input, ret);
