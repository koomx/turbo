...
TEST(issue_a73) {
    ...
    const size_t expected_length = implementation.utf8_length_from_utf16le(utf16, len);

    std::vector<char> output(expected_length);
    const std::string expected = "\xeb\xb3\xad\xef\x9e\x99\xe8\x9e\x94\xef\x82\xa6\xe8\xaa\x83\xef\x82\xbb"
                                 ...
                                 "\xad";
    const size_t utf8size = implementation.convert_utf16le_to_utf8(utf16, len, output.data());
