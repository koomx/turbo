...
TEST(issue_213) {
  ...
  turbo::result r =
      turbo::convert_utf8_to_utf16be_with_errors(buf + 2, 1, buffer.get());
...
TEST(compile_time_convert_utf8_to_utf16be_with_errors) {
  ...
  constexpr auto output = utf8_to_utf16<std::endian::big, with_errors, input>();
