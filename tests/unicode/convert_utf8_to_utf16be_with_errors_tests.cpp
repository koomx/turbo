...
TEST(issue_213) {
    ...
    turbo::UnicodeResult r = turbo::convert_utf8_to_utf16be_with_errors(buf + 2, 1, buffer.get());
...
TEST_LOOP(convert_pure_ASCII) {
    ...
    auto procedure = [&implementation](const char* utf8, size_t size,
        ...
        const turbo::UnicodeResult res = implementation.convert_utf8_to_utf16be_with_errors(utf8, size, utf16be);
    ...
    for (unsigned int i = 0; i < fix_size; i++) {
        if ((test.input_utf8[i] & 0b11000000) != 0b10000000) { // Only process leading bytes
            auto procedure = [&implementation, &i](const char* utf8, size_t size,
                                 char16_t* utf16be) -> size_t {
                const turbo::UnicodeResult res = implementation.convert_utf8_to_utf16be_with_errors(utf8, size,
                    utf16be);
