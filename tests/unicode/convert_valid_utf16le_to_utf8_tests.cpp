...
template <auto input> constexpr auto convert() {
  ...
  const auto ret = turbo::convert_valid_utf16_to_utf8(input, tmp);
  if (ret != tmp.size()) {
...
TEST(compile_time_convert_valid_utf16_to_utf8) {
  using namespace turbo::tests::helpers;
...
template <auto input> constexpr auto convert_le() {
  ...
  const auto ret = turbo::convert_valid_utf16le_to_utf8(input, tmp);
  if (ret != tmp.size()) {
    throw "unexpected write size";
  }
  return tmp;
