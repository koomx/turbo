...
TEST(issue_627) {
    ...
    const auto r1 = implementation.detect_encodings((const char*)data.data(), data.size());
    ASSERT_EQUAL(r1, 2);
    ...
    const auto r2 = implementation.detect_encodings((const char*)data2.data(), data2.size());
    ASSERT_EQUAL(r2, 0);
}
#endif
