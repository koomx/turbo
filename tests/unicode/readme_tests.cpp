...
int main_demo() {
    ...
    // convert to UTF-16LE
    size_t utf16words = turbo::convert_utf8_to_utf16le(source, 4, utf16_output.get());
...
TEST(utf8_streaming) {
    ...
    size_t utf16words = turbo::convert_utf8_to_utf16le(unicode, length, utf16.get());
    ...
    size_t next_utf16words = turbo::convert_utf8_to_utf16le(next, next_length, next_utf16.get());
...
TEST(utf16_streaming) {
    ...
    size_t utf8words = turbo::convert_utf16le_to_utf8(unicode, length, utf8.get());
    // We can then transcode the next batch
    const char16_t* next = unicode + length;
    size_t next_length = 6 - length;
    size_t next_budget_utf8 = turbo::utf8_length_from_utf16le(next, next_length);
    std::unique_ptr<char[]> next_utf8 { new char[next_budget_utf8] };
    size_t next_utf8words = turbo::convert_utf16le_to_utf8(next, next_length, next_utf8.get());
