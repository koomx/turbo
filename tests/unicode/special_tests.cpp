...
TEST(special_cases_utf8_utf32_roundtrip) {
  ...
  for (const std::string &source : cases) {
    ...
    size_t expected_utf32words =
        turbo::utf32_length_from_utf8(source.c_str(), source.size());
    ...
    size_t expected_utf8words =
        turbo::utf8_length_from_utf32(utf32_output.get(), utf32words);
...
TEST(special_cases_utf8_utf16le_roundtrip) {
  ...
  for (const std::string &source : cases) {
    ...
    size_t expected_utf16words =
        turbo::utf16_length_from_utf8(source.c_str(), source.size());
...
TEST(special_cases_utf8_utf16be_roundtrip) {
  ...
  for (const std::string &source : cases) {
    ...
    // We need a buffer where to write the UTF-8 code units.
    size_t expected_utf8words =
        turbo::utf8_length_from_utf16be(utf16_output.get(), utf16words);
    ASSERT_EQUAL(expected_utf8words, source.size());
    std::unique_ptr<char[]> utf8_output{new char[expected_utf8words]};
    // convert to UTF-8
...
TEST(special_cases_utf8_utf16_roundtrip) {
  ...
  for (const std::string &source : cases) {
    ...
    // We need a buffer where to write the UTF-8 code units.
    size_t expected_utf8words =
        turbo::utf8_length_from_utf16(utf16_output.get(), utf16words);
    ASSERT_EQUAL(expected_utf8words, source.size());
    std::unique_ptr<char[]> utf8_output{new char[expected_utf8words]};
    // convert to UTF-8
...
TEST(special_cases_utf8_utf16_invalid) {
  ...
  for (const std::string &source : cases) {
    ...
    size_t expected_latin1words =
        turbo::latin1_length_from_utf8(source.c_str(), source.size());
...
TEST(special_cases_utf16_utf8_roundtrip) {
  ...
  for (std::vector<char16_t> &source : cases) {
    ...
    size_t expected_utf8words =
        turbo::utf8_length_from_utf16(utf16_input.get(), source.size());
    std::unique_ptr<char[]> utf8_output{new char[expected_utf8words]};
    size_t utf8words = turbo::convert_utf16_to_utf8(
    ...
    size_t expected_utf16words =
        turbo::utf16_length_from_utf8(utf8_output.get(), utf8words);
    size_t utf16words = turbo::convert_utf8_to_utf16(
