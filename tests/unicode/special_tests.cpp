...
TEST(visualstudio2019icelakeissue) {
    ...
    size_t out_size = implementation.convert_utf16le_to_utf8(source, length, buffer.get());
...
TEST(special_cases_utf8_utf32_roundtrip) {
    ...
    for (const std::string& source : cases) {
        ...
        size_t expected_utf32words = turbo::utf32_length_from_utf8(source.c_str(), source.size());
        std::unique_ptr<char32_t[]> utf32_output { new char32_t[expected_utf32words] };
        // convert to UTF-32
        size_t utf32words = turbo::convert_utf8_to_utf32(
            source.c_str(), source.size(), utf32_output.get());
        // It wrote utf32words * sizeof(char32_t) bytes.
        bool validutf32 = turbo::validate_utf32(utf32_output.get(), utf32words);
        ...
        // convert to UTF-16LE
        size_t utf32words_valid = turbo::convert_valid_utf8_to_utf32(
        ...
        size_t expected_utf8words = turbo::utf8_length_from_utf32(utf32_output.get(), utf32words);
        ASSERT_EQUAL(expected_utf8words, source.size());
        std::unique_ptr<char[]> utf8_output { new char[expected_utf8words] };
        // convert to UTF-8
        size_t utf8words = turbo::convert_utf32_to_utf8(
        ...
        size_t utf8words_valid = turbo::convert_valid_utf32_to_utf8(
            utf32_output.get(), utf32words, utf8_output.get());
...
TEST(special_cases_utf8_utf16le_roundtrip) {
    ...
    for (const std::string& source : cases) {
        ...
        ASSERT_TRUE(validutf8);
        // We need a buffer where to write the UTF-16LE code units.
        size_t expected_utf16words = turbo::utf16_length_from_utf8(source.c_str(), source.size());
        std::unique_ptr<char16_t[]> utf16_output { new char16_t[expected_utf16words] };
        // convert to UTF-16LE
        size_t utf16words = turbo::convert_utf8_to_utf16le(
        ...
        // convert to UTF-16LE
        size_t utf16words_valid = turbo::convert_valid_utf8_to_utf16le(
        ...
        // convert it back:
        // We need a buffer where to write the UTF-8 code units.
        size_t expected_utf8words = turbo::utf8_length_from_utf16le(utf16_output.get(), utf16words);
        ASSERT_EQUAL(expected_utf8words, source.size());
        std::unique_ptr<char[]> utf8_output { new char[expected_utf8words] };
        // convert to UTF-8
        ...
        size_t utf8words_valid = turbo::convert_valid_utf16le_to_utf8(
            utf16_output.get(), utf16words, utf8_output.get());
        ASSERT_EQUAL(expected_utf8words, utf8words_valid);
        const std::string final_string_valid(utf8_output.get(), utf8words_valid);
        ASSERT_EQUAL(final_string_valid, source);
...
TEST(special_cases_utf8_utf16be_roundtrip) {
    ...
    for (const std::string& source : cases) {
        ...
        std::unique_ptr<char16_t[]> utf16_output { new char16_t[expected_utf16words] };
        // convert to UTF-16BE
        size_t utf16words = turbo::convert_utf8_to_utf16be(
            source.c_str(), source.size(), utf16_output.get());
        // It wrote utf16words * sizeof(char16_t) bytes.
        bool validutf16 = turbo::validate_utf16be(utf16_output.get(), utf16words);
        ASSERT_TRUE(validutf16);
        ...
        // convert to UTF-16BE
        size_t utf16words_valid = turbo::convert_valid_utf8_to_utf16be(
            source.c_str(), source.size(), utf16_valid_output.get());
        ASSERT_EQUAL(utf16words_valid, utf16words);
        for (size_t z = 0; z < utf16words_valid; z++) {
        ...
        // convert it back:
        // We need a buffer where to write the UTF-8 code units.
        size_t expected_utf8words = turbo::utf8_length_from_utf16be(utf16_output.get(), utf16words);
        ASSERT_EQUAL(expected_utf8words, source.size());
        std::unique_ptr<char[]> utf8_output { new char[expected_utf8words] };
        // convert to UTF-8
        const size_t utf8words = turbo::convert_utf16be_to_utf8(
            utf16_output.get(), utf16words, utf8_output.get());
        ASSERT_EQUAL(expected_utf8words, utf8words);
        const std::string final_string(utf8_output.get(), utf8words);
        ASSERT_EQUAL(final_string, source);

        const size_t utf8words_valid = turbo::convert_valid_utf16be_to_utf8(
            utf16_output.get(), utf16words, utf8_output.get());
        ASSERT_EQUAL(expected_utf8words, utf8words_valid);
        const std::string final_string_valid(utf8_output.get(), utf8words_valid);
        ASSERT_EQUAL(final_string_valid, source);
...
TEST(special_cases_utf8_utf16_roundtrip) {
    ...
    for (const std::string& source : cases) {
        ...
        std::unique_ptr<char16_t[]> utf16_output { new char16_t[expected_utf16words] };
        // convert to UTF-16
        size_t utf16words = turbo::convert_utf8_to_utf16(
            source.c_str(), source.size(), utf16_output.get());
        // It wrote utf16words * sizeof(char16_t) bytes.
        bool validutf16 = turbo::validate_utf16(utf16_output.get(), utf16words);
        ASSERT_TRUE(validutf16);
        ...
        // convert to UTF-16
        size_t utf16words_valid = turbo::convert_valid_utf8_to_utf16(
            source.c_str(), source.size(), utf16_valid_output.get());
        ASSERT_EQUAL(utf16words_valid, utf16words);
        for (size_t z = 0; z < utf16words_valid; z++) {
            // Note: casting due to Visual Studio's lack of operator<< for char16_t.
        ...
        // convert it back:
        // We need a buffer where to write the UTF-8 code units.
        size_t expected_utf8words = turbo::utf8_length_from_utf16(utf16_output.get(), utf16words);
        ASSERT_EQUAL(expected_utf8words, source.size());
        std::unique_ptr<char[]> utf8_output { new char[expected_utf8words] };
        // convert to UTF-8
        size_t utf8words = turbo::convert_utf16_to_utf8(
            utf16_output.get(), utf16words, utf8_output.get());
        ASSERT_EQUAL(expected_utf8words, utf8words);
        std::string final_string(utf8_output.get(), utf8words);
        ASSERT_EQUAL(final_string, source);
        size_t utf8words_valid = turbo::convert_valid_utf16_to_utf8(
            utf16_output.get(), utf16words, utf8_output.get());
        ASSERT_EQUAL(expected_utf8words, utf8words_valid);
...
TEST(special_cases_utf8_utf16_invalid) {
    ...
    for (const std::string& source : cases) {
        ...
        size_t expected_latin1words = turbo::latin1_length_from_utf8(source.c_str(), source.size());
        std::unique_ptr<char[]> latin1_output { new char[expected_latin1words] };
        size_t latin1words = turbo::convert_utf8_to_latin1(
...
TEST(special_cases_utf16_utf8_roundtrip) {
    ...
    for (std::vector<char16_t>& source : cases) {
        ...
        bool validutf16 = turbo::validate_utf16(utf16_input.get(), source.size());
        ASSERT_TRUE(validutf16);

        size_t expected_utf8words = turbo::utf8_length_from_utf16(utf16_input.get(), source.size());
        std::unique_ptr<char[]> utf8_output { new char[expected_utf8words] };
        size_t utf8words = turbo::convert_utf16_to_utf8(
        ...
        size_t expected_utf16words = turbo::utf16_length_from_utf8(utf8_output.get(), utf8words);
        size_t utf16words = turbo::convert_utf8_to_utf16(
