...
TEST(to_std_encoding_unspecified) {
  constexpr auto enc = turbo::to_std_encoding(turbo::unspecified);
...
TEST(from_std_encoding_unsupported) {
  ...
  constexpr auto result = turbo::from_std_encoding(enc);
  static_assert(result == turbo::unspecified,
                "unsupported encodings map to unspecified");
}

TEST(roundtrip_conversions) {
  static_assert(turbo::from_std_encoding(
                    turbo::to_std_encoding(turbo::UTF8)) == turbo::UTF8,
                "UTF8 roundtrip failed");
  static_assert(turbo::from_std_encoding(turbo::to_std_encoding(
                    turbo::UTF16_LE)) == turbo::UTF16_LE,
                "UTF16_LE roundtrip failed");
  static_assert(turbo::from_std_encoding(turbo::to_std_encoding(
                    turbo::UTF16_BE)) == turbo::UTF16_BE,
                "UTF16_BE roundtrip failed");
  static_assert(turbo::from_std_encoding(turbo::to_std_encoding(
                    turbo::UTF32_LE)) == turbo::UTF32_LE,
                "UTF32_LE roundtrip failed");
  static_assert(turbo::from_std_encoding(turbo::to_std_encoding(
                    turbo::UTF32_BE)) == turbo::UTF32_BE,
                "UTF32_BE roundtrip failed");
  static_assert(turbo::from_std_encoding(turbo::to_std_encoding(
                    turbo::Latin1)) == turbo::Latin1,
                "Latin1 roundtrip failed");
}

TEST(from_std_encoding_native_utf16) {
  constexpr std::text_encoding enc(std::text_encoding::id::UTF16);
  constexpr auto result = turbo::from_std_encoding_native(enc);
  #if SIMDUTF_IS_BIG_ENDIAN
  static_assert(result == turbo::UTF16_BE, "native UTF16 mapping mismatch");
  #else
  static_assert(result == turbo::UTF16_LE, "native UTF16 mapping mismatch");
  #endif
}
...
TEST(from_std_encoding_native_utf32) {
  ...
  constexpr auto result = turbo::from_std_encoding_native(enc);
  #if SIMDUTF_IS_BIG_ENDIAN
  static_assert(result == turbo::UTF32_BE, "native UTF32 mapping mismatch");
  #else
  static_assert(result == turbo::UTF32_LE, "native UTF32 mapping mismatch");
  #endif
}
...
TEST(from_std_encoding_native_explicit_endian) {
  ...
  static_assert(turbo::from_std_encoding_native(enc_le) == turbo::UTF16_LE,
                "UTF16LE mapping mismatch");
  static_assert(turbo::from_std_encoding_native(enc_be) == turbo::UTF16_BE,
