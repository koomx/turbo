...
namespace {
    ...
    using ::turbo::convert_wchar_to_utf8;
    using ::turbo::kMaxEncodedUTF8Size;
    using ::turbo::ShiftState;
...
#endif
    TEST(encode_utf32_to_utf8, BasicFunction) {
        ...
        for (auto& test : tests) {
            ...
            char* buf0_written = &buf0[turbo::encode_utf32_to_utf8(buf0, test.first)];
            char* buf1_written = &buf1[turbo::encode_utf32_to_utf8(buf1, test.first)];
            int apparent_length = 7;
            while (buf0[apparent_length - 1] == '\x00' && buf1[apparent_length - 1] == '\xFF') {
                if (--apparent_length == 0)
                    break;
            }
        ...
        EXPECT_LE(turbo::encode_utf32_to_utf8(buf, 0x00110000),
            turbo::kMaxEncodedUTF8Size);
        char buf2[32] = "Negative is invalid but sane";
        EXPECT_LE(turbo::encode_utf32_to_utf8(buf2, -1),
            turbo::kMaxEncodedUTF8Size);
    }
#if defined(__clang__)
#pragma clang diagnostic pop
    ...
    TEST_P(WideToUtf8ParamTest, SingleCharConversion) {
        ...
        size_t bytes_written = convert_wchar_to_utf8(test_case.input, buffer.data(), state);
...
#define UTF8_STRING_LITERAL u8"Holá €1 你好 שָׁלוֹם 👍🏻🇺🇸👩‍❤️‍💋‍👨 中"
    ...
    std::string_view GetUtf8TestString() {
        ...
        // `char*`
        static std::string_view kUtf8TestString = [] {
