...
concept passable_to_validate = requires(InputPtr p) {
  turbo::scalar::ascii::validate_with_errors(p, 10u);
};
...
TEST(compile_time_valid) {
  ...
  static_assert(turbo::validate_ascii_with_errors(ascii).is_ok());
  static_assert(
      turbo::validate_ascii_with_errors(ascii.as_array<unsigned char>())
          .is_ok());
  static_assert(
      turbo::validate_ascii_with_errors(ascii.as_array<signed char>())
          .is_ok());
  static_assert(
      turbo::validate_ascii_with_errors(ascii.as_array<std::byte>()).is_ok());
...
TEST(compile_time_invalid) {
  ...
  static_assert(turbo::validate_ascii_with_errors(not_ascii).is_err());
}
#endif
