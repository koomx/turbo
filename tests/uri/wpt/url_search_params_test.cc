...
#include <turbo/uri/uri_search_params.h>

TEST(UriSearchParams, append) {
    auto search_params = turbo::UriSearchParams();
...
TEST(UriSearchParams, to_string) {
    auto search_params = turbo::UriSearchParams();
...
TEST(UriSearchParams, with_accents) {
    auto search_params = turbo::UriSearchParams();
 ...
 */
TEST(UriSearchParams, to_string_serialize_space) {
    auto params = turbo::UriSearchParams();
    params.append("a", "b c");
...
TEST(UriSearchParams, to_string_serialize_plus) {
    auto params = turbo::UriSearchParams();
    params.append("a", "b+c");
...
TEST(UriSearchParams, to_string_serialize_ampersand) {
    auto params = turbo::UriSearchParams();
    params.append("&", "a");
...
TEST(UriSearchParams, string_constructor) {
    auto p = turbo::UriSearchParams("?a=b");
    ASSERT_EQ(p.to_string(), "a=b");
    SUCCEED();
}

TEST(UriSearchParams, string_constructor_with_empty_input) {
    auto p = turbo::UriSearchParams("");
    ASSERT_EQ(p.to_string(), "");
    ASSERT_EQ(p.size(), 0);
    SUCCEED();
}

TEST(UriSearchParams, string_constructor_without_value) {
    auto p = turbo::UriSearchParams("a=b&c");
...
TEST(UriSearchParams, string_constructor_with_edge_cases) {
    auto p = turbo::UriSearchParams("&a&&& &&&&&a+b=& c&m%c3%b8%c3%b8");
    p.to_string();
    ASSERT_TRUE(p.has("a"));
...
TEST(UriSearchParams, has) {
    auto search_params = turbo::UriSearchParams("key1=value1&key2=value2");
...
TEST(UriSearchParams, iterators) {
    // JS style iterators
    auto search_params = turbo::UriSearchParams("key1=value1&key1=value2&key2=value3");
...
TEST(UriSearchParams, test_to_string_encoding) {
    auto search_params = turbo::UriSearchParams("q1=foo&q2=foo+bar&q3=foo bar&q4=foo/bar");
...
// https://github.com/cloudflare/workerd/issues/1777
TEST(UriSearchParams, test_character_set) {
    auto search_params = turbo::UriSearchParams("key=value");
