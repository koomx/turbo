...
TEST(wpt_tests, setters_tests_encoding) {
  ...
  try {
    for (auto element : doc.get_array()) {
      if (element.type() == ondemand::json_type::string) {
        ...
        auto current =
            turbo::parse_wpt_uri("https://" + std::string(input) + "/x");
        ...
        auto setter = turbo::parse_wpt_uri("https://x/x");
  ...
  try {
    for (auto element : doc.get_array()) {
      if (element.type() == ondemand::json_type::string) {
        ...
        turbo::WptUri input_url = wpt_parse(input);
