...
// -----------------------------------------------------------------------------
//
// This header file defines the `turbo::partial_ordering`, `turbo::weak_ordering`,
// and `turbo::strong_ordering` types for storing the results of three way
// comparisons.
//
// Example:
//   turbo::weak_ordering compare(const std::string& a, const std::string& b);
//
...
#else  // KUMO_HAVE_ATTRIBUTE(enable_if)
...
// TURBO_INTERNAL_INLINE_CONSTEXPR here because the variables here are of
// incomplete types so they need to be defined after the types are complete.
#ifdef __cpp_inline_variables

// A no-op expansion that can be followed by a semicolon at class level.
#define TURBO_COMPARE_INLINE_BASECLASS_DECL(name) static_assert(true, "")

#define TURBO_COMPARE_INLINE_SUBCLASS_DECL(type, name) static const type name
...
#define TURBO_COMPARE_INLINE_BASECLASS_DECL(name) \
  KUMO_CONST_INIT static const T name
...
class partial_ordering
 ...
 public:
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(partial_ordering, less);
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(partial_ordering, equivalent);
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(partial_ordering, greater);
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(partial_ordering, unordered);
...
TURBO_COMPARE_INLINE_INIT(partial_ordering, less, compare_internal::ord::less);
TURBO_COMPARE_INLINE_INIT(partial_ordering, equivalent,
                         compare_internal::eq::equivalent);
TURBO_COMPARE_INLINE_INIT(partial_ordering, greater,
                         compare_internal::ord::greater);
TURBO_COMPARE_INLINE_INIT(partial_ordering, unordered,
...
class weak_ordering
 ...
 public:
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(weak_ordering, less);
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(weak_ordering, equivalent);
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(weak_ordering, greater);
...
TURBO_COMPARE_INLINE_INIT(weak_ordering, less, compare_internal::ord::less);
TURBO_COMPARE_INLINE_INIT(weak_ordering, equivalent,
                         compare_internal::eq::equivalent);
TURBO_COMPARE_INLINE_INIT(weak_ordering, greater,
                         compare_internal::ord::greater);
...
class strong_ordering
 ...
 public:
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(strong_ordering, less);
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(strong_ordering, equal);
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(strong_ordering, equivalent);
  TURBO_COMPARE_INLINE_SUBCLASS_DECL(strong_ordering, greater);
...
TURBO_COMPARE_INLINE_INIT(strong_ordering, less, compare_internal::ord::less);
TURBO_COMPARE_INLINE_INIT(strong_ordering, equal, compare_internal::eq::equal);
TURBO_COMPARE_INLINE_INIT(strong_ordering, equivalent,
                         compare_internal::eq::equivalent);
TURBO_COMPARE_INLINE_INIT(strong_ordering, greater,
                         compare_internal::ord::greater);

#undef TURBO_COMPARE_INLINE_BASECLASS_DECL
#undef TURBO_COMPARE_INLINE_SUBCLASS_DECL
#undef TURBO_COMPARE_INLINE_INIT

#endif  // KUMO_USES_STD_ORDERING

namespace compare_internal {
// We also provide these comparator adapter functions for internal turbo use.
...
constexpr turbo::weak_ordering compare_result_as_ordering(
    const turbo::weak_ordering c) {
  return c;
}
...
template <
    ...
    std::enable_if_t<
        !std::is_same_v<bool, turbo::result_of_t<Compare(const K&, const LK&)>>,
        int> = 0>
...
constexpr turbo::weak_ordering do_three_way_comparison(const Compare& compare,
                                                      const K& x, const LK& y) {
  return compare(x, y)   ? turbo::weak_ordering::less
         : compare(y, x) ? turbo::weak_ordering::greater
                         : turbo::weak_ordering::equivalent;
}
