...
// `turbo::c_xx(container, ...) are equivalent to std:: functions such as
...
constexpr void AssertCopyNSize(InputSequence& input, Size n,
  ...
  if constexpr (meta_internal::IsAtLeastForwardIterator<InputIter>::value) {
  ...
  if constexpr (meta_internal::IsAtLeastForwardIterator<OutputIter>::value) {
    base_internal::HardeningAssertLE(
...
constexpr void AssertCopySize(InputSequence& input, OutputRange& output) {
  ...
  using OutputIter = ContainerIter<OutputRange>;
  if constexpr (meta_internal::IsAtLeastForwardIterator<InputIter>::value &&
                meta_internal::IsAtLeastForwardIterator<OutputIter>::value) {
    base_internal::HardeningAssertLE(
...
using ResultOfRangeToIteratorTransfer =
    std::enable_if_t<container_algorithm_internal::IsIterator<
                         turbo::remove_cvref_t<OutputIterator>>::value &&
...
template <typename T>
struct IsSpan<turbo::Span<T>> : std::true_type {};
...
// as turbo::c_fill(). Similar idea as std::ranges::borrowed_range.
//
...
// c_linear_search()
//
// Container-based version of turbo::linear_search() for performing a linear
// search within a container.
//
// For a generalization that uses a predicate, see turbo::c_any_of().
template <typename C, typename EqualityComparable>
constexpr bool c_linear_search(const C& c, EqualityComparable&& value) {
  return turbo::linear_search(container_algorithm_internal::c_begin(c),
                             container_algorithm_internal::c_end(c),
...
constexpr bool c_contains(const Sequence& sequence, T&& value) {
  return turbo::c_find(sequence, std::forward<T>(value)) !=
...
constexpr bool c_contains_subrange(Sequence1& sequence,
  ...
  return turbo::c_search(sequence, subsequence) !=
         container_algorithm_internal::c_end(sequence);
}
...
// Copies elements from `input` to `output`. `turbo::c_copy(input, output)` is
...
c_copy(const InputSequence& input, OutputRange&& output) {
  ...
  turbo::c_copy(input, container_algorithm_internal::c_begin(output));
}
...
// `turbo::c_copy_n(input, n, output)` is equivalent to
...
c_copy_n(const C& input, Size n, OutputRange&& output) {
  ...
  turbo::c_copy_n(input, n, container_algorithm_internal::c_begin(output));
}
...
// Moves elements from `src` to `dest`. `turbo::c_move(src, dest)` is
...
c_move(C&& src, OutputRange&& dest) {
  ...
  turbo::c_move(std::forward<C>(src),
...
// `output`. `turbo::c_transform(input, output, unary_op)` is equivalent to
...
c_transform(const InputSequence1& input1, const InputSequence2& input2,
  ...
  if constexpr (meta_internal::IsAtLeastForwardIterator<OutputIter>::value) {
    constexpr bool input1_has_size =
        meta_internal::IsAtLeastForwardIterator<InputIter1>::value;
    constexpr bool input2_has_size =
        meta_internal::IsAtLeastForwardIterator<InputIter2>::value;
    auto output_size =
        std::distance(container_algorithm_internal::c_begin(output),
                      container_algorithm_internal::c_end(output));
...
constexpr OutputIterator c_set_intersection(const C1& c1, const C2& c2,
  ...
  // using operator<.
  KUMO_ASSERT(turbo::c_is_sorted(c1));
  KUMO_ASSERT(turbo::c_is_sorted(c2));
...
constexpr OutputIterator c_set_intersection(const C1& c1, const C2& c2,
  ...
  KUMO_ASSERT(turbo::c_is_sorted(c1, comp));
  KUMO_ASSERT(turbo::c_is_sorted(c2, comp));
  return std::set_intersection(container_algorithm_internal::c_begin(c1),
                               container_algorithm_internal::c_end(c1),
                               container_algorithm_internal::c_begin(c2),
                               container_algorithm_internal::c_end(c2), output,
