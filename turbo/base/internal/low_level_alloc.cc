    ...
    namespace base_internal {
        ...
        static int LLA_SkiplistLevels(size_t size, size_t base, uint32_t* random) {
            ...
            if (level > kMaxLevel - 1)
                level = kMaxLevel - 1;
            TURBO_RAW_CHECK(level >= 1, "block not big enough for even one level");
            return level;
        }
        ...
        static void LLA_SkiplistDelete(AllocList* head, AllocList* e,
            ...
            TURBO_RAW_CHECK(e == found, "element not in freelist");
        ...
        struct LowLevelAlloc::Arena {
            ...
            AllocList freelist TURBO_GUARDED_BY(mu);
            // Count of allocated blocks
            int32_t allocation_count TURBO_GUARDED_BY(mu);
            ...
            // PRNG state
            uint32_t random TURBO_GUARDED_BY(mu);
        };
...
#endif
                base_internal::LowLevelAlloc::Arena* new_arena = base_internal::LowLevelAlloc::NewArena(flags);
        ...
        namespace {
            ...
            turbo::once_flag create_globals_once;
        ...
        namespace {
            class TURBO_SCOPED_LOCKABLE ArenaLock {
            public:
                explicit ArenaLock(LowLevelAlloc::Arena* arena)
                    TURBO_EXCLUSIVE_LOCK_FUNCTION(arena->mu)
                    : arena_(arena) {
#ifndef TURBO_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
                ...
                ~ArenaLock() { TURBO_RAW_CHECK(left_, "haven't left Arena region"); }
                void Leave() TURBO_UNLOCK_FUNCTION() {
                    arena_->mu.unlock();
#ifndef TURBO_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
                    if (mask_valid_) {
                        const int err = pthread_sigmask(SIG_SETMASK, &mask_, nullptr);
                        if (err != 0) {
                            TURBO_RAW_LOG(FATAL, "pthread_sigmask failed: %d", err);
                        }
                    }
#endif
                    left_ = true;
        ...
        bool LowLevelAlloc::DeleteArena(Arena* arena) {
            ...
            while (arena->freelist.next[0] != nullptr) {
                ...
                TURBO_RAW_CHECK(region->header.arena == arena,
                    "bad arena pointer in DeleteArena()");
                TURBO_RAW_CHECK(size % arena->pagesize == 0,
                    "empty arena has non-page-aligned block size");
                TURBO_RAW_CHECK(reinterpret_cast<uintptr_t>(region) % arena->pagesize == 0,
                    "empty arena has non-page-aligned block");
                int munmap_result;
#ifdef _WIN32
                munmap_result = VirtualFree(region, 0, MEM_RELEASE);
                TURBO_RAW_CHECK(munmap_result != 0,
...
#else
                munmap_result = munmap(region, size);
#endif // TURBO_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
                if (munmap_result != 0) {
                    TURBO_RAW_LOG(FATAL, "LowLevelAlloc::DeleteArena: munmap failed: %d",
                        errno);
                }
#endif // _WIN32
            }
        ...
        static inline uintptr_t CheckedAdd(uintptr_t a, uintptr_t b) {
            uintptr_t sum = a + b;
            TURBO_RAW_CHECK(sum >= a, "LowLevelAlloc arithmetic overflow");
            return sum;
        }
        ...
        static AllocList* Next(int i, AllocList* prev, LowLevelAlloc::Arena* arena)
            TURBO_EXCLUSIVE_LOCKS_REQUIRED(arena->mu) {
            TURBO_RAW_CHECK(i < prev->levels, "too few levels in Next()");
            AllocList* next = prev->next[i];
            if (next != nullptr) {
                TURBO_RAW_CHECK(
                    next->header.magic == Magic(kMagicUnallocated, &next->header),
                    "bad magic number in Next()");
                TURBO_RAW_CHECK(next->header.arena == arena, "bad arena pointer in Next()");
                if (prev != &arena->freelist) {
                    TURBO_RAW_CHECK(prev < next, "unordered freelist");
                    TURBO_RAW_CHECK(reinterpret_cast<char*>(prev) + prev->header.size < reinterpret_cast<char*>(next),
                        "malformed freelist");
                }
            }
            return next;
        ...
        static void Coalesce(AllocList* a) {
            ...
            if (n != nullptr && reinterpret_cast<char*>(a) + a->header.size == reinterpret_cast<char*>(n)) {
                ...
                a->levels = LLA_SkiplistLevels(a->header.size, arena->min_size, &arena->random);
        ...
        static void AddToFreelist(void* v, LowLevelAlloc::Arena* arena)
            ...
            TURBO_RAW_CHECK(f->header.magic == Magic(kMagicAllocated, &f->header),
                "bad magic number in AddToFreelist()");
            TURBO_RAW_CHECK(f->header.arena == arena,
        ...
        void LowLevelAlloc::Free(void* v) {
            if (v != nullptr) {
                ...
                TURBO_RAW_CHECK(arena->allocation_count > 0, "nothing in arena to free");
        ...
        static void* DoAllocWithArena(size_t request, LowLevelAlloc::Arena* arena) {
            ...
            if (request != 0) {
                ...
                size_t req_rnd = RoundUp(CheckedAdd(request, sizeof(s->header)), arena->round_up);
                for (;;) { // loop until we find a suitable region
                    // find the minimum levels that a block of this size must have
                    int i = LLA_SkiplistLevels(req_rnd, arena->min_size, nullptr) - 1;
                    if (i < arena->freelist.levels) { // potential blocks exist
                        AllocList* before = &arena->freelist; // predecessor of s
                        while ((s = Next(i, before, arena)) != nullptr && s->header.size < req_rnd) {
                            before = s;
                        }
                    ...
                    TURBO_RAW_CHECK(new_pages != nullptr, "VirtualAlloc failed");
#else
#ifndef TURBO_LOW_LEVEL_ALLOC_ASYNC_SIGNAL_SAFE_MISSING
                    if ((arena->flags & LowLevelAlloc::kAsyncSignalSafe) != 0) {
                    ...
                    if (new_pages == MAP_FAILED) {
                        TURBO_RAW_LOG(FATAL, "mmap error: %d", errno);
                    }
                ...
                s->header.magic = Magic(kMagicAllocated, &s->header);
                TURBO_RAW_CHECK(s->header.arena == arena, "");
                arena->allocation_count++;
                section.Leave();
                result = &s->levels;
            }
            TURBO_ANNOTATE_MEMORY_IS_UNINITIALIZED(result, request);
            return result;
        }
        ...
        void* LowLevelAlloc::AllocWithArena(size_t request, Arena* arena) {
            TURBO_RAW_CHECK(arena != nullptr, "must pass a valid arena");
