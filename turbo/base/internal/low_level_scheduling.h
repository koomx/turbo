    ...
    namespace base_internal {
        ...
        inline bool SchedulingGuard::ReschedulingIsAllowed() {
            ...
            if (identity != nullptr) {
                ...
                return state->bound_schedulable.load(std::memory_order_relaxed) != nullptr && state->scheduling_disabled_depth.load(std::memory_order_relaxed) == 0;
            } else {
        ...
        inline SchedulingGuard::ScopedEnable::ScopedEnable() {
            ...
            identity = CurrentThreadIdentityIfPresent();
            if (identity != nullptr) {
                scheduling_disabled_depth_ = identity->scheduler_state.scheduling_disabled_depth.load(
                    std::memory_order_relaxed);
