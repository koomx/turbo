...
// This is similar to KLOG(severity) << format..., but
// * it is to be used ONLY by low-level modules that can't use normal KLOG()
...
#define TURBO_RAW_LOG(severity, ...)                                                                                                 \
    do {                                                                                                                             \
        constexpr const char* turbo_raw_log_internal_basename = ::turbo::raw_log_internal::Basename(__FILE__, sizeof(__FILE__) - 1); \
...
// Similar to KCHECK(condition) << message, but for low-level modules:
...
#define TURBO_RAW_LOG_INTERNAL_MAYBE_UNREACHABLE_FATAL KUMO_UNREACHABLE()
...
namespace turbo {
    ...
    namespace raw_log_internal {
        ...
        void RawLog(turbo::LogSeverity severity, const char* file, int line,
            const char* format, ...) KUMO_PRINTF_ATTRIBUTE(4, 5);
        ...
        KUMO_INTERNAL_ATOMIC_HOOK_ATTRIBUTES KUMO_DLL extern base_internal::AtomicHook<
            InternalLogFunction>
