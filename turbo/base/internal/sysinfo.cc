...
#pragma comment(lib, "advapi32.lib") // For Reg* functions.
...
#endif
            if (rc != 0) {
                TURBO_RAW_LOG(FATAL, "clock_gettime() failed: (%d)", errno);
            }
        ...
        KUMO_CONST_INIT static once_flag init_num_cpus_once;
        KUMO_CONST_INIT static int num_cpus = 0;
        ...
        KUMO_CONST_INIT static once_flag init_nominal_cpu_frequency_once;
        KUMO_CONST_INIT static double nominal_cpu_frequency = 1.0;
...
#define TURBO_INTERNAL_HAVE_GETTID 1
#endif
#endif

#ifdef TURBO_INTERNAL_HAVE_GETTID
        pid_t GetTID() {
...
#endif
#undef TURBO_INTERNAL_HAVE_GETTID
        ...
        // get_cached_tid() caches the thread ID in thread-local storage (which is a
...
#define TURBO_INTERNAL_USING_EMULATED_TLS 1
#endif
#endif

#if KUMO_HAVE_THREAD_LOCAL && !defined(TURBO_INTERNAL_USING_EMULATED_TLS)
            static thread_local pid_t thread_id = GetTID();
            return thread_id;
#else
            return GetTID();
#endif // KUMO_HAVE_THREAD_LOCAL &&
       // !defined(TURBO_INTERNAL_USING_EMULATED_TLS)
#undef TURBO_INTERNAL_USING_EMULATED_TLS
        }
