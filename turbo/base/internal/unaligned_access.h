...
namespace turbo {
    ...
    namespace base_internal {
        ...
        inline uint16_t UnalignedLoad16(const void* turbo_nonnull p) {
            uint16_t t;
            memcpy(&t, p, sizeof t);
            return t;
        }

        inline uint32_t UnalignedLoad32(const void* turbo_nonnull p) {
            uint32_t t;
            memcpy(&t, p, sizeof t);
            return t;
        }

        inline uint64_t UnalignedLoad64(const void* turbo_nonnull p) {
            uint64_t t;
            memcpy(&t, p, sizeof t);
            return t;
        }

        inline void UnalignedStore16(void* turbo_nonnull p, uint16_t v) {
            memcpy(p, &v, sizeof v);
        }

        inline void UnalignedStore32(void* turbo_nonnull p, uint32_t v) {
            memcpy(p, &v, sizeof v);
        }

        inline void UnalignedStore64(void* turbo_nonnull p, uint64_t v) {
            memcpy(p, &v, sizeof v);
        }
