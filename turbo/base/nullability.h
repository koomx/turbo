...
//
//  * "Non-null" (for pointers annotated `turbo_nonnull`), indicating that it is
//    invalid for the given pointer to ever be null.
//  * "Nullable" (for pointers annotated `turbo_nullable`), indicating that it is
//    valid for the given pointer to be null.
//  * "Unknown" (for pointers annotated `turbo_nullability_unknown`), indicating
...
// `turbo_nullability_unknown`; you should rarely, if ever, see this annotation
...
// Employee* turbo_nonnull const e;
//
// // A non-null pointer to a const `Employee`.
// const Employee* turbo_nonnull e;
//
// // A non-null pointer to a const nullable pointer to an `Employee`.
// Employee* turbo_nullable const* turbo_nonnull e;
//
// // A non-null function pointer.
// void (*turbo_nonnull func)(int, double);
//
// // A non-null array of `Employee`s as a parameter.
// void func(Employee employees[turbo_nonnull]);
//
...
//
// Employee* turbo_nullable find(Map& employees, std::string_view name);
//
...
// annotations by adding the TURBO_NULLABILITY_COMPATIBLE attribute.
//
// // Example:
// struct TURBO_NULLABILITY_COMPATIBLE MyPtr {
//   ...
// };
//
...
// marked with `TURBO_NULLABILITY_COMPATIBLE`.
//
// DISCLAIMER:
// ===========================================================================
...
// `turbo_nonnull` is *not guaranteed* to be non-null, and the compiler won't
// alert or prevent assignment of a `T* turbo_nullable` to a `T* turbo_nonnull`.
// ===========================================================================
// SKIP_TURBO_INLINE_NAMESPACE_CHECK
#ifndef TURBO_BASE_NULLABILITY_H_
#define TURBO_BASE_NULLABILITY_H_

#include <turbo/macros/config.h>

// TURBO_POINTERS_DEFAULT_NONNULL
//
...
//     T* turbo_nullable GetNullablePtr();           // explicitly nullable
//     T* turbo_nullability_unknown GetUnknownPtr();  // explicitly unknown
//
// The macro can be safely used in header files - it will not affect any files
// that include it.
//
// In files with the macro, plain `T*` syntax means `T* turbo_nonnull`, and the
// exceptions (`turbo_nullable` and `turbo_nullability_unknown`) must be marked
...
//     #include <turbo/base/nullability.h>
//
//     void FillMessage(Message* turbo_nonnull m);  // explicitly non-null
//     T* turbo_nullable GetNullablePtr();          // explicitly nullable
...
// void PaySalary(Employee* turbo_nonnull employee) {
//   pay(*employee);  // OK to dereference
// }
#define turbo_nonnull _Nonnull
...
// void PaySalary(Employee* turbo_nullable employee) {
//   if (employee != nullptr) {
//     Pay(*employee);  // OK to dereference
//   }
// }
#define turbo_nullable _Nullable

// turbo_nullability_unknown  (default without `TURBO_POINTERS_DEFAULT_NONNULL`)
//
...
// migrated into one of the above two nullability states: `turbo_nonnull` or
//  `turbo_nullable`.
//
// NOTE: For files that do not specify `TURBO_POINTERS_DEFAULT_NONNULL`,
...
// void PaySalary(Employee* turbo_nullability_unknown employee) {
...
//   Pay(*employee); // Potentially dangerous. API provider should investigate.
// }
#define turbo_nullability_unknown _Null_unspecified
#else
// No-op for non-Clang compilers or Objective-C.
#define turbo_nonnull
// No-op for non-Clang compilers or Objective-C.
#define turbo_nullable
// No-op for non-Clang compilers or Objective-C.
#define turbo_nullability_unknown
#endif

// TURBO_NULLABILITY_COMPATIBLE
//
...
// with `TURBO_NULLABILITY_COMPATIBLE`.
#if KUMO_HAVE_FEATURE(nullability_on_classes)
#define TURBO_NULLABILITY_COMPATIBLE _Nullable
#else
