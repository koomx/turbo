...
namespace turbo {
    ...
    [[nodiscard]] constexpr std::enable_if_t<std::is_unsigned_v<T>, T> rotl(
        T x, int s) noexcept {
        return bits_internal::RotateLeft(x, s);
    }

    template<class T>
    [[nodiscard]] constexpr std::enable_if_t<std::is_unsigned_v<T>, T> rotr(
        T x, int s) noexcept {
        return bits_internal::RotateRight(x, s);
    }
    ...
    // intrinsics.
    template<class T>
    TURBO_INTERNAL_CONSTEXPR_CLZ inline std::enable_if_t<std::is_unsigned_v<T>, int>
    countl_zero(T x) noexcept {
        return bits_internal::CountLeadingZeroes(x);
    }
    ...
    template<class T>
    TURBO_INTERNAL_CONSTEXPR_CTZ inline std::enable_if_t<std::is_unsigned_v<T>, int>
    countr_zero(T x) noexcept {
        return bits_internal::CountTrailingZeroes(x);
    }
    ...
    template<class T>
    TURBO_INTERNAL_CONSTEXPR_POPCOUNT inline std::enable_if_t<std::is_unsigned_v<T>,
        int>
    popcount(T x) noexcept {
        return bits_internal::Popcount(x);
    }
    ...
    template<class T>
    TURBO_INTERNAL_CONSTEXPR_CLZ inline std::enable_if_t<std::is_unsigned_v<T>, T>
    ...
    bit_ceil(T x) {
        ...
        return has_single_bit(x)
                   ? T{1} << (bit_width(x) - 1)
                   : bits_internal::BitCeilNonPowerOf2(x);
    }
...
#error "Endian detection needs to be set up for this platform"
    ...
    //
    // `turbo::byteswap` participates in overload resolution only if `T` satisfies
