...
constexpr std::enable_if_t<
    ...
    To>
implicit_cast(turbo::type_identity_t<To> to) {
  return to;
}
template <typename To>
constexpr std::enable_if_t<
    ...
    To>
implicit_cast(turbo::type_identity_t<To> to KUMO_ATTRIBUTE_LIFETIME_BOUND) {
  return to;
}
template <typename To>
constexpr std::enable_if_t<std::is_reference_v<To>, To> implicit_cast(
    turbo::type_identity_t<To> to KUMO_ATTRIBUTE_LIFETIME_BOUND) {
  return std::forward<turbo::type_identity_t<To>>(to);
}
...
//
// The requirements of `turbo::bit_cast` are more strict than that of
...
// default-constructible. In C++20, `turbo::bit_cast` is replaced by
// `std::bit_cast`.
#if defined(__cpp_lib_bit_cast) && __cpp_lib_bit_cast >= 201806L
...
#if !KUMO_HAVE_BUILTIN(__builtin_bit_cast)
                               && std::is_default_constructible_v<Dest>
#endif  // !KUMO_HAVE_BUILTIN(__builtin_bit_cast)
                           ,
                           int> = 0>
#if KUMO_HAVE_BUILTIN(__builtin_bit_cast)
...
#else  // KUMO_HAVE_BUILTIN(__builtin_bit_cast)
...
[[noreturn]] KUMO_ATTRIBUTE_NOINLINE void BadDownCastCrash(
    const char* source_type, const char* target_type);

template <typename To, typename From>
inline void ValidateDownCast(From* f KUMO_ATTRIBUTE_UNUSED) {
  // Assert only if RTTI is enabled and in debug mode or hardened asserts are
  // enabled.
#if KUMO_HAVE_RTTI
#if !defined(NDEBUG) || (KUMO_OPTION_HARDENED == 1)
  // Suppress erroneous nonnull comparison warning on older GCC.
#if defined(__GNUC__) && !defined(__clang__)
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wnonnull-compare"
#endif
  if (KUMO_UNLIKELY(f != nullptr && dynamic_cast<To>(f) == nullptr)) {
#if defined(__GNUC__) && !defined(__clang__)
#pragma GCC diagnostic pop
#endif
    turbo::base_internal::BadDownCastCrash(
...
inline To down_cast(From* f) {  // so we only accept pointers
  ...
  turbo::base_internal::ValidateDownCast<To>(f);

  return static_cast<To>(f);
}

// Overload of down_cast for references. Use like this:
// turbo::down_cast<T&>(foo). The code is slightly convoluted because we're still
...
inline To down_cast(From& f) {
  ...
  turbo::base_internal::ValidateDownCast<std::remove_reference_t<To>*>(
      std::addressof(f));
