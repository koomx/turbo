...
namespace xcli {
    ...
    App::set_version_flag(std::string flag_name, std::function<std::string()> vfunc, const std::string& version_help) {
        ...
        // Empty name will simply remove the version flag
        if (!flag_name.empty()) {
            version_ptr_ = add_flag_callback(flag_name, [vfunc]() { throw(xcli::CallForVersion(vfunc(), 0)); }, version_help);
            version_ptr_->configurable(false)->callback_priority(CallbackPriority::First);
        }
    ...
    Option* App::_add_flag_internal(std::string flag_name, xcli::callback_t fun, std::string flag_description) {
        Option* opt = nullptr;
    ...
    bool App::remove_option(Option* opt) {
        ...
        auto iterator = std::find_if(std::begin(options_), std::end(options_), [opt](const Option_p& v) { return v.get() == opt; });
        ...
        if (!subcommand_name.empty() && !detail::valid_name_string(subcommand_name)) {
            ...
            for (auto c : subcommand_name) {
                if (!detail::valid_later_char(c)) {
                    throw IncorrectConstruction(std::string("Subcommand name contains invalid character ('") + c + "'), all characters are allowed except"
    ...
    App* App::add_subcommand(xcli::App_p subcom) {
        if (!subcom)
    ...
    xcli::App_p App::get_subcommand_ptr(App* subcom) const {
        if (subcom == nullptr)
            throw OptionNotFound("nullptr passed");
        for (const App_p& subcomptr : subcommands_)
            if (subcomptr.get() == subcom)
    ...
    [[nodiscard]] xcli::App_p App::get_subcommand_ptr(std::string subcom) const {
        for (const App_p& subcomptr : subcommands_)
    ...
    [[nodiscard]] xcli::App_p App::get_subcommand_ptr(int index) const {
        if (index >= 0) {
            auto uindex = static_cast<unsigned>(index);
            if (uindex < subcommands_.size())
    ...
    [[nodiscard]] xcli::App* App::get_option_group(std::string group_name) const {
    ...
    namespace detail {
        ...
        std::string maybe_narrow(const wchar_t* str) {
            return turbo::convert_wchar_to_utf8(std::wstring_view(str));
        }
    ...
    void App::parse(std::wstring commandline, bool program_name_included) {
        parse(turbo::convert_wchar_to_utf8(commandline), program_name_included);
    }

    void App::_parse_setup() {
        // Clear if parsed
        if (parsed_ > 0)
            clear();
    ...
    void App::_validate() const {
        ...
        if (pcount > 1) {
            auto pcount_req = std::count_if(std::begin(options_), std::end(options_), [](const Option_p& opt) {
                return opt->get_items_expected_max() >= detail::expected_max_vector_size && !opt->nonpositional() && opt->get_required();
            });
    ...
    [[nodiscard]] detail::Classifier App::_recognize(const std::string& current,
        ...
        if (detail::split_short(current, dummy1, dummy2)) {
            if ((dummy1[0] >= '0' && dummy1[0] <= '9') || (dummy1[0] == '.' && !dummy2.empty() && (dummy2[0] >= '0' && dummy2[0] <= '9'))) {
    ...
    void App::_process_completion_callbacks(bool with_help_flags) {
        _process_callbacks(CallbackPriority::FirstPreHelp);
        if (with_help_flags) {
            _process_help_flags(CallbackPriority::First);
        }
    ...
    App::_add_flag_like_result(Option* op, const ConfigItem& item, const std::vector<std::string>& inputs) {
        ...
        if (static_cast<int>(inputs.size()) > op->get_items_expected_max() && op->get_multi_option_policy() != MultiOptionPolicy::TakeAll && op->get_multi_option_policy() != MultiOptionPolicy::Join) {
    ...
    bool App::_parse_single_config(const ConfigItem& item, std::size_t level) {
        ...
        if (op == nullptr || !op->get_configurable()) {
            ...
            auto options = get_options([&iname](const xcli::Option* opt) {
                return (opt->get_configurable() && (opt->check_name(iname) || opt->check_lname(iname) || opt->check_sname(iname)));
            });
            if (!options.empty()) {
                op = options[0];
            }
    ...
    bool App::_parse_positional(std::vector<std::string>& args, bool haltOnSubcommand) {
        ...
        if (posOpt == nullptr) {
            for (const Option_p& opt : options_) {
                ...
                if (opt->get_positional() && (static_cast<int>(opt->count()) < opt->get_items_expected_max() || opt->get_allow_extra_args())) {
                    if (validate_positionals_) {
                        std::string pos = positional;
                        pos = opt->_validate(pos, 0);
                        if (!pos.empty()) {
        ...
        if (subcommand_fallthrough_) {
            ...
            if (com != nullptr && (com->parent_->require_subcommand_max_ == 0 || com->parent_->require_subcommand_max_ > com->parent_->parsed_subcommands_.size())) {
                return false;
            }
        }
        if (positionals_at_end_) {
    ...
    App::_parse_arg(std::vector<std::string>& args, detail::Classifier current_type, bool local_processing_only) {
        ...
        while (op_ptr == std::end(options_)) {
            ...
            if (dotloc != std::string::npos && dotloc < arg_name.size() - 1) {
                ...
                if (sub != nullptr) {
                    ...
                    std::string dummy1, dummy2;
                    bool val = false;
                    if ((current_type == detail::Classifier::SHORT && detail::valid_first_char(args.back()[1])) || detail::split_long(args.back(), dummy1, dummy2)) {
        ...
        if (max_num > collected || op->get_allow_extra_args()) { // we allow optional arguments
            ...
            while ((collected < max_num || op->get_allow_extra_args()) && !args.empty() && _recognize(args.back(), false) == detail::Classifier::NONE) {
