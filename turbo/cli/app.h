...
class App {
  ...
  public:
    ...
    XCLI_NODISCARD char **ensure_utf8(char **argv);
    ...
    XCLI_NODISCARD bool parsed() const { return parsed_ > 0; }
    ...
    Option *add_option(std::string option_name,
        ...
        auto fun = [&variable](const xcli::results_t &res) {  // comment for spacing
            return detail::lexical_conversion<AssignTo, ConvertTo>(res, variable);
        };

        Option *opt = add_option(option_name, fun, option_description, false, [&variable]() {
            return xcli::detail::checked_to_string<AssignTo, ConvertTo>(variable);
        });
    ...
    Option *add_option_function(std::string option_name,
        ...
        auto fun = [func](const xcli::results_t &res) {
            ArgType variable;
    ...
    Option *add_option(std::string option_name) {
        return add_option(option_name, xcli::callback_t{}, std::string{}, false);
    }
    ...
    Option *add_option(std::string option_name, T &option_description) {
        return add_option(option_name, xcli::callback_t(), option_description, false);
    }
  ...
  private:
    ...
    Option *_add_flag_internal(std::string flag_name, xcli::callback_t fun, std::string flag_description);

  public:
    /// Add a flag with no description or variable assignment
    Option *add_flag(std::string flag_name) { return _add_flag_internal(flag_name, xcli::callback_t(), std::string{}); }
    ...
    Option *add_flag(std::string flag_name, T &&flag_description) {
        return _add_flag_internal(flag_name, xcli::callback_t(), std::forward<T>(flag_description));
    }
    ...
    Option *add_flag(std::string flag_name,
        ...
        xcli::callback_t fun = [&flag_result](const xcli::results_t &res) {
    ...
    Option *add_flag(std::string flag_name,
                     ...
                     std::string flag_description = "") {
        xcli::callback_t fun = [&flag_results](const xcli::results_t &res) {
            bool retval = true;
    ...
    App *add_subcommand(xcli::App_p subcom);
    ...
    XCLI_NODISCARD App *get_subcommand(std::string subcom) const;

    /// Get a subcommand by name (noexcept non-const version)
    /// returns null if subcommand doesn't exist
    XCLI_NODISCARD App *get_subcommand_no_throw(std::string subcom) const noexcept;

    /// Get a pointer to subcommand by index
    XCLI_NODISCARD App *get_subcommand(int index = 0) const;

    /// Check to see if a subcommand is part of this command and get a shared_ptr to it
    xcli::App_p get_subcommand_ptr(App *subcom) const;

    /// Check to see if a subcommand is part of this command (text version)
    XCLI_NODISCARD xcli::App_p get_subcommand_ptr(std::string subcom) const;

    /// Get an owning pointer to subcommand by index
    XCLI_NODISCARD xcli::App_p get_subcommand_ptr(int index = 0) const;

    /// Check to see if an option group is part of this App
    XCLI_NODISCARD App *get_option_group(std::string group_name) const;
  ...
  public:
    ...
    XCLI_NODISCARD std::size_t count(std::string option_name) const { return get_option(option_name)->count(); }

    /// Get a subcommand pointer list to the currently selected subcommands (after parsing by default, in command
    /// line order; use parsed = false to get the original definition list.)
    XCLI_NODISCARD std::vector<App *> get_subcommands() const { return parsed_subcommands_; }
    ...
    XCLI_NODISCARD bool got_subcommand(std::string subcommand_name) const noexcept {
    ...
    XCLI_NODISCARD std::string config_to_str() const { return config_to_str(ConfigOutputMode::Active, false); }

    /// Produce a string that could be read in as a config of the current values of the App.
    XCLI_NODISCARD std::string config_to_str(ConfigOutputMode mode, bool write_description = false) const {
    ...
    XCLI_NODISCARD std::string config_to_str(bool default_also, bool write_description = false) const {
    ...
    XCLI_NODISCARD std::string help(std::string prev = "", AppFormatMode mode = AppFormatMode::Normal) const;

    /// Displays a version string
    XCLI_NODISCARD std::string version() const;
    ///@}
    /// @name Getters
    ///@{

    /// Access the formatter
    XCLI_NODISCARD std::shared_ptr<FormatterBase> get_formatter() const { return formatter_; }

    /// Access the config formatter
    XCLI_NODISCARD std::shared_ptr<Config> get_config_formatter() const { return config_formatter_; }

    /// Access the config formatter as a configBase pointer
    XCLI_NODISCARD std::shared_ptr<ConfigBase> get_config_formatter_base() const {
        // This is safer as a dynamic_cast if we have RTTI, as Config -> ConfigBase
#if XCLI_USE_STATIC_RTTI == 0
    ...
    XCLI_NODISCARD std::string get_description() const { return description_; }
    ...
    XCLI_NODISCARD Option *get_option_no_throw(std::string option_name) noexcept;

    /// Get an option by name (noexcept const version)
    XCLI_NODISCARD const Option *get_option_no_throw(std::string option_name) const noexcept;

    /// Get an option by name
    XCLI_NODISCARD const Option *get_option(std::string option_name) const;

    /// Get an option by name (non-const version)
    XCLI_NODISCARD Option *get_option(std::string option_name);
    ...
    XCLI_NODISCARD bool get_ignore_case() const { return ignore_case_; }

    /// Check the status of ignore_underscore
    XCLI_NODISCARD bool get_ignore_underscore() const { return ignore_underscore_; }

    /// Check the status of fallthrough
    XCLI_NODISCARD bool get_fallthrough() const { return fallthrough_; }

    /// Check the status of subcommand fallthrough
    XCLI_NODISCARD bool get_subcommand_fallthrough() const { return subcommand_fallthrough_; }

    /// Check the status of the allow windows style options
    XCLI_NODISCARD bool get_allow_windows_style_options() const { return allow_windows_style_options_; }

    /// Check the status of the allow windows style options
    XCLI_NODISCARD bool get_positionals_at_end() const { return positionals_at_end_; }

    /// Check the status of the allow windows style options
    XCLI_NODISCARD bool get_configurable() const { return configurable_; }

    /// Get the group of this subcommand
    XCLI_NODISCARD const std::string &get_group() const { return group_; }

    /// Generate and return the usage.
    XCLI_NODISCARD std::string get_usage() const {
        return (usage_callback_) ? usage_callback_() + '\n' + usage_ : usage_;
    }

    /// Generate and return the footer.
    XCLI_NODISCARD std::string get_footer() const {
        return (footer_callback_) ? footer_callback_() + '\n' + footer_ : footer_;
    }

    /// Get the required min subcommand value
    XCLI_NODISCARD std::size_t get_require_subcommand_min() const { return require_subcommand_min_; }

    /// Get the required max subcommand value
    XCLI_NODISCARD std::size_t get_require_subcommand_max() const { return require_subcommand_max_; }

    /// Get the required min option value
    XCLI_NODISCARD std::size_t get_require_option_min() const { return require_option_min_; }

    /// Get the required max option value
    XCLI_NODISCARD std::size_t get_require_option_max() const { return require_option_max_; }

    /// Get the prefix command status
    XCLI_NODISCARD bool get_prefix_command() const { return static_cast<bool>(prefix_command_); }

    /// Get the prefix command status
    XCLI_NODISCARD PrefixCommandMode get_prefix_command_mode() const { return prefix_command_; }

    /// Get the status of allow extras
    XCLI_NODISCARD bool get_allow_extras() const { return allow_extras_ > ExtrasMode::Ignore; }

    /// Get the mode of allow_extras
    XCLI_NODISCARD ExtrasMode get_allow_extras_mode() const { return allow_extras_; }

    /// Get the status of required
    XCLI_NODISCARD bool get_required() const { return required_; }

    /// Get the status of disabled
    XCLI_NODISCARD bool get_disabled() const { return disabled_; }

    /// Get the status of silence
    XCLI_NODISCARD bool get_silent() const { return silent_; }

    /// Get the status of allowing non standard option names
    XCLI_NODISCARD bool get_allow_non_standard_option_names() const { return allow_non_standard_options_; }

    /// Get the status of allowing prefix matching for subcommands
    XCLI_NODISCARD bool get_allow_subcommand_prefix_matching() const { return allow_prefix_matching_; }

    /// Get the status of disabled
    XCLI_NODISCARD bool get_immediate_callback() const { return immediate_callback_; }

    /// Get the status of disabled by default
    XCLI_NODISCARD bool get_disabled_by_default() const { return (default_startup == startup_mode::disabled); }

    /// Get the status of disabled by default
    XCLI_NODISCARD bool get_enabled_by_default() const { return (default_startup == startup_mode::enabled); }
    /// Get the status of validating positionals
    XCLI_NODISCARD bool get_validate_positionals() const { return validate_positionals_; }
    /// Get the status of validating optional vector arguments
    XCLI_NODISCARD bool get_validate_optional_arguments() const { return validate_optional_arguments_; }

    /// Get the status of allow extras
    XCLI_NODISCARD config_extras_mode get_allow_config_extras() const {
    ...
    XCLI_NODISCARD const Option *get_help_ptr() const { return help_ptr_; }

    /// Get a pointer to the help all flag. (const)
    XCLI_NODISCARD const Option *get_help_all_ptr() const { return help_all_ptr_; }

    /// Get a pointer to the config option.
    Option *get_config_ptr() { return config_ptr_; }

    /// Get a pointer to the config option. (const)
    XCLI_NODISCARD const Option *get_config_ptr() const { return config_ptr_; }

    /// Get a pointer to the version option.
    Option *get_version_ptr() { return version_ptr_; }

    /// Get a pointer to the version option. (const)
    XCLI_NODISCARD const Option *get_version_ptr() const { return version_ptr_; }

    /// Get the parent of this subcommand (or nullptr if main app)
    App *get_parent() { return parent_; }

    /// Get the parent of this subcommand (or nullptr if main app) (const version)
    XCLI_NODISCARD const App *get_parent() const { return parent_; }

    /// Get the name of the current app
    XCLI_NODISCARD const std::string &get_name() const { return name_; }

    /// Get the aliases of the current app
    XCLI_NODISCARD const std::vector<std::string> &get_aliases() const { return aliases_; }
    ...
    XCLI_NODISCARD std::string get_display_name(bool with_aliases = false) const;

    /// Check the name, case-insensitive and underscore insensitive, and prefix matching if set
    /// @return true if matched
    XCLI_NODISCARD bool check_name(std::string name_to_check) const;
    ...
    XCLI_NODISCARD NameMatch check_name_detail(std::string name_to_check) const;

    /// Get the groups available directly from this option (in order)
    XCLI_NODISCARD std::vector<std::string> get_groups() const;

    /// This gets a vector of pointers with the original parse order
    XCLI_NODISCARD const std::vector<Option *> &parse_order() const { return parse_order_; }

    /// This returns the missing options from the current subcommand
    XCLI_NODISCARD std::vector<std::string> remaining(bool recurse = false) const;

    /// This returns the missing options in a form ready for processing by another command line program
    XCLI_NODISCARD std::vector<std::string> remaining_for_passthrough(bool recurse = false) const;

    /// This returns the number of remaining options, minus the -- separator
    XCLI_NODISCARD std::size_t remaining_size(bool recurse = false) const;
  ...
  protected:
    ...
    XCLI_NODISCARD bool _valid_subcommand(const std::string &current, bool ignore_used = true) const;

    /// Selects a Classifier enum based on the type of the current argument
    XCLI_NODISCARD detail::Classifier _recognize(const std::string &current,
    ...
    XCLI_NODISCARD std::size_t _count_remaining_positionals(bool required_only = false) const;

    /// Count the required remaining positional arguments
    XCLI_NODISCARD bool _has_remaining_positionals() const;
    ...
    XCLI_NODISCARD App *_get_fallthrough_parent() noexcept;

    /// Get the appropriate parent to fallthrough to which is the first one that has a name or the main app
    XCLI_NODISCARD const App *_get_fallthrough_parent() const noexcept;

    /// Helper function to run through all possible comparisons of subcommand names to check there is no overlap
    XCLI_NODISCARD const std::string &_compare_subcommand_names(const App &subcom, const App &base) const;
...
#ifndef XCLI_COMPILE
#include <turbo/cli/impl/app_inl.h>  // IWYU pragma: export
#endif
