...
#define XCLI_PARSE(app, ...)              \
    ...
    enum class ConfigExtrasMode : std::uint8_t { Error = 0,
        Ignore,
        IgnoreAll,
        Capture };

    /// @brief  enumeration of modes of how to deal with extras in config files
    enum class config_extras_mode : std::uint8_t { error = 0,
        ignore,
        ignore_all,
        capture };
    ...
    /// recognized options
    enum class PrefixCommandMode : std::uint8_t { Off = 0,
        SeparatorOnly = 1,
        On = 2 };
    ...
    protected:
        ...
        enum class startup_mode : std::uint8_t { stable,
            enabled,
            disabled };
        ...
        Option* add_option(std::string option_name,
            ...
            Option* opt = add_option(
                ...
                [&flag](const xcli::results_t& res) {
                    ...
                    turbo::CommandLineFlag* cl = turbo::FindCommandLineFlag(turbo::GetFlagReflectionHandle(flag).Name());
                    if (cl == nullptr) {
                        return false;
                    }
    ...
    public:
        ...
        template <typename T,
            enable_if_t<(std::is_const<typename std::remove_reference<T>::type>::value || std::is_rvalue_reference<T&&>::value) && std::is_constructible<std::string, typename std::remove_reference<T>::type>::value,
                detail::enabler> = detail::dummy>
        ...
        template <typename T,
            enable_if_t<!detail::is_mutable_container<T>::value && !std::is_const<T>::value && !detail::is_turbo_flag<T>::value && !std::is_constructible<std::function<void(int)>, T>::value,
                detail::enabler> = detail::dummy>
        ...
        /// Check to see if a subcommand is part of this command (text version)
        [[nodiscard]] xcli::App_p get_subcommand_ptr(std::string subcom) const;

        /// Get an owning pointer to subcommand by index
        [[nodiscard]] xcli::App_p get_subcommand_ptr(int index = 0) const;
    ...
    public:
        ...
        [[nodiscard]] std::string config_to_str() const { return config_to_str(ConfigOutputMode::Active, false); }
        ...
        [[nodiscard]] bool get_subcommand_fallthrough() const { return subcommand_fallthrough_; }
        ...
        [[nodiscard]] bool get_prefix_command() const { return static_cast<bool>(prefix_command_); }

        /// Get the prefix command status
        [[nodiscard]] PrefixCommandMode get_prefix_command_mode() const { return prefix_command_; }

        /// Get the status of allow extras
        [[nodiscard]] bool get_allow_extras() const { return allow_extras_ > ExtrasMode::Ignore; }

        /// Get the mode of allow_extras
        [[nodiscard]] ExtrasMode get_allow_extras_mode() const { return allow_extras_; }
        ...
        [[nodiscard]] bool get_allow_non_standard_option_names() const { return allow_non_standard_options_; }

        /// Get the status of allowing prefix matching for subcommands
        [[nodiscard]] bool get_allow_subcommand_prefix_matching() const { return allow_prefix_matching_; }
        ...
        enum class NameMatch : std::uint8_t { none = 0,
            exact = 1,
            prefix = 2 };
