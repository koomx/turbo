...
namespace xcli {
    ...
    namespace detail {
        ...
        bool is_printable(const std::string& test_string) {
        ...
        generate_parents(const std::string& section, std::string& name, char parentSeparator) {
            ...
            try {
                ...
                throw xcli::ParseError(iarg.what(), xcli::ExitCodes::InvalidError);
            }
            return parents;
        }
        ...
        bool hasMLString(std::string const& fullString, char check) {
        ...
        void clean_name_string(std::string& name, const std::string& keyChars) {
            if (name.find_first_of(keyChars) != std::string::npos || (name.front() == '[' && name.back() == ']') || (name.find_first_of("'`\"\\") != std::string::npos)) {
    ...
    std::vector<ConfigItem> ConfigBase::from_config(std::istream& input) const {
        std::string line;
        std::string buffer;
        ...
        while (getline(input, buffer)) {
            ...
            std::string name;
            line = turbo::trim_all_copy(buffer);
            ...
            if (line.front() == '[' && line.back() != ']' && line.find_first_of(commentChar) != std::string::npos) {
                ...
                if (comment_search < line.size() && line[comment_search] == commentChar) {
                    line = turbo::trim_all_copy(line.substr(0, comment_search));
                    len = line.length();
                    if (len < 3) {
                        continue;
                    }
            ...
            if (delimiter_pos != std::string::npos) {
                ...
                name = turbo::trim_all_copy(line.substr(0, delimiter_pos));
                std::string item = turbo::trim_all_copy(line.substr(delimiter_pos + 1));
                ...
                if (!mlquote && comment_pos != std::string::npos) {
                    ...
                    item = turbo::trim_all_copy(citems.front());
                }
                if (mlquote) {
                    // multiline string
                    ...
                    while (inMLineValue) {
                        ...
                        line = l2;
                        turbo::trim_right(&line);
                        if (detail::hasMLString(line, keyChar)) {
                            line.pop_back();
                            line.pop_back();
                            line.pop_back();
                            if (lineExtension) {
                                turbo::trim_left(&line);
                    ...
                    for (std::string multiline; item.back() != aEnd && std::getline(input, multiline);) {
                       multiline = turbo::trim_all_copy(multiline);
                        item += multiline;
                    }
                    if (item.back() == aEnd) {
                ...
                name = turbo::trim_all_copy(line.substr(0, comment_pos));
                items_buffer = { "true" };
            }
            std::vector<std::string> parents;
            try {
                ...
                throw xcli::ParseError(ia.what(), xcli::ExitCodes::InvalidError);
            }
            ...
            if (match != output.end()) {
                if ((match->inputs.size() > 1 && items_buffer.size() > 1) || allowMultipleDuplicateFields) {
                    ...
                    if (!(match->inputs.back().empty() || items_buffer.front().empty() || match->inputs.back() == "%%" || items_buffer.front() == "%%")) {
    ...
    ConfigBase::to_config(const App* app, ConfigOutputMode mode, bool write_description, std::string prefix) const {
        ...
        for (auto& group : groups) {
            ...
            for (const Option* opt : app->get_options({ })) {
                ...
                if (opt->get_configurable()) {
                    ...
                    if (results.size() > 1 && opt->get_multi_option_policy() == xcli::MultiOptionPolicy::Reverse) {
                        std::reverse(results.begin(), results.end());
                    }
                    if (opt->get_multi_option_policy() == xcli::MultiOptionPolicy::Sum && opt->count() >= 1 && results.size() == 1) {
                    ...
                    if (opt->get_multi_option_policy() == xcli::MultiOptionPolicy::Join && opt->count() > 1) {
                        ...
                        if (delim == '\0') {
                            ...
                            if (results[0].back() == delim || static_cast<decltype(delim_count)>(opt->count()) <= delim_count || results[0].find(std::string(2, delim)) != std::string::npos) {
                                results = opt->results();
                            }
                        }
                    }
                    ...
                    if (!value.empty()) {
                        if (!opt->get_fnames().empty()) {
                            ...
                            } catch (const xcli::ArgumentMismatch&) {
                                bool valid { false };
        ...
        for (const App* subcom : subcommands) {
            if (!subcom->get_name().empty()) {
                if ((!include_default_values && (subcom->count_all() == 0)) || (mode == ConfigOutputMode::ActiveSubcommandDefaults && !app->got_subcommand(subcom))) {
                    continue;
                }
        ...
        if (write_description && !out.str().empty()) {
            std::string outString = commentChar + commentLead + detail::fix_newlines(commentChar + commentLead, app->get_description()) + '\n';
