...
namespace xcli {
    ...
    inline const detail::IPV4Validator ValidIPV4;
    ...
    public:
        ...
        explicit AsNumberWithUnit(std::map<std::string, Number> mapping,
            ...
            func_ = [mapping, opts](std::string& input) -> std::string {
                ...
                if (it == mapping.end()) {
                    throw ValidationError(unit + " unit not recognized. "
                                                 "Allowed values: "
                ...
                if (!input.empty()) {
                    ...
                    if (!ok) {
                        throw ValidationError(detail::to_string(num) + " multiplied by " + unit + " factor would cause number overflow. Use smaller value.");
                    }
                } else {
            ...
            if (opts & CASE_INSENSITIVE) {
                ...
                for (auto& kv : mapping) {
                    ...
                    if (lower_mapping.count(s)) {
                        throw ValidationError(std::string("Several matching lowercase unit representations are found: ") + s);
                    }
    ...
    namespace detail {
        enum class Permission : std::uint8_t { none = 0,
            read = 1,
            write = 2,
            exec = 4 };
