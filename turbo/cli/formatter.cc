...
namespace xcli {
    namespace detail {
         std::string indent_block(const std::string& input, const std::string& indent) {
    ...
    Formatter::make_group(std::string group, bool is_positional, std::vector<const Option*> opts) const {
     ...
     std::string Formatter::make_positionals(const App* app) const {
        std::vector<const Option*> opts = app->get_options([](const Option* opt) { return !opt->get_group().empty() && opt->get_positional(); });
     ...
     std::string Formatter::make_description(const App* app) const {
        ...
        if (min_options > 0) {
            if (max_options == min_options) {
                ...
                desc += " \n[Between " + std::to_string(min_options) + " and " + std::to_string(max_options) + " of the following options are required]";
            } else {
     ...
     std::string Formatter::make_usage(const App* app, std::string name) const {
        ...
        std::vector<const Option*> non_pos_options = app->get_options([](const Option* opt) { return opt->nonpositional(); });
        if (!non_pos_options.empty())
            out << " [" << get_label("OPTIONS") << "]";

        // Positionals need to be listed here
        std::vector<const Option*> positionals = app->get_options([](const Option* opt) { return !opt->get_group().empty() && opt->get_positional(); });
     ...
     std::string Formatter::make_help(const App* app, std::string name, AppFormatMode mode) const {
        ...
        if (is_footer_paragraph_formatting_enabled()) {
            detail::stream_out_as_paragraph(out, footer_string, footer_paragraph_width_); // Format footer as paragraph
        } else {
            out << footer_string;
        }
     ...
     std::string Formatter::make_subcommands(const App* app, AppFormatMode mode) const {
        ...
        for (const App* com : subcommands) {
            ...
            if (!group_key.empty() && std::find_if(subcmd_groups_seen.begin(), subcmd_groups_seen.end(), [&group_key_lower](const std::string& a) {
     ...
     std::string Formatter::make_expanded(const App* sub, AppFormatMode mode) const {
        std::stringstream out;
     ...
     std::string Formatter::make_option(const Option* opt, bool is_positional) const {
        std::stringstream out;
        if (is_positional) {
            ...
            shortNamesOverSize = (std::min)(shortNamesOverSize, longNamesColumnWidth); // Prevent negative result with unsigned integers
