...
XCLI_INLINE App::App(std::string app_description, std::string app_name, App *parent)
...
XCLI_NODISCARD XCLI_INLINE char **App::ensure_utf8(char **argv) {
#ifdef _WIN32
    (void)argv;
...
XCLI_INLINE App *App::name(std::string app_name) {
...
XCLI_INLINE App *App::alias(std::string app_name) {
...
XCLI_INLINE App *App::immediate_callback(bool immediate) {
...
XCLI_INLINE App *App::ignore_case(bool value) {
...
XCLI_INLINE App *App::ignore_underscore(bool value) {
...
XCLI_INLINE Option *App::add_option(std::string option_name,
...
XCLI_INLINE Option *App::set_help_flag(std::string flag_name, const std::string &help_description) {
...
XCLI_INLINE Option *App::set_help_all_flag(std::string help_name, const std::string &help_description) {
...
App::set_version_flag(std::string flag_name, const std::string &versionString, const std::string &version_help) {
    ...
    if(!flag_name.empty()) {
        version_ptr_ = add_flag_callback(
            flag_name, [versionString]() { throw(xcli::CallForVersion(versionString, 0)); }, version_help);
...
App::set_version_flag(std::string flag_name, std::function<std::string()> vfunc, const std::string &version_help) {
    ...
    if(!flag_name.empty()) {
        version_ptr_ =
            add_flag_callback(flag_name, [vfunc]() { throw(xcli::CallForVersion(vfunc(), 0)); }, version_help);
        version_ptr_->configurable(false)->callback_priority(CallbackPriority::First);
    }
...
XCLI_INLINE Option *App::_add_flag_internal(std::string flag_name, xcli::callback_t fun, std::string flag_description) {
    Option *opt = nullptr;
...
XCLI_INLINE Option *App::add_flag_callback(std::string flag_name,
                                            std::function<void(void)> function,  ///< A function to call, void(void)
                                            std::string flag_description) {

    xcli::callback_t fun = [function](const xcli::results_t &res) {
...
XCLI_INLINE Option *App::set_config(std::string option_name,
...
XCLI_INLINE bool App::remove_option(Option *opt) {
...
XCLI_INLINE App *App::add_subcommand(std::string subcommand_name, std::string subcommand_description) {
    ...
    xcli::App_p subcom = std::shared_ptr<App>(new App(std::move(subcommand_description), subcommand_name, this));
    return add_subcommand(std::move(subcom));
}

XCLI_INLINE App *App::add_subcommand(xcli::App_p subcom) {
    if(!subcom)
...
XCLI_INLINE bool App::remove_subcommand(App *subcom) {
    // Make sure no links exist
...
XCLI_INLINE App *App::get_subcommand(const App *subcom) const {
    if(subcom == nullptr)
...
XCLI_NODISCARD XCLI_INLINE App *App::get_subcommand(std::string subcom) const {
...
XCLI_NODISCARD XCLI_INLINE App *App::get_subcommand_no_throw(std::string subcom) const noexcept {
    return _find_subcommand(subcom, false, false);
}

XCLI_NODISCARD XCLI_INLINE App *App::get_subcommand(int index) const {
    if(index >= 0) {
...
XCLI_INLINE xcli::App_p App::get_subcommand_ptr(App *subcom) const {
    if(subcom == nullptr)
        throw OptionNotFound("nullptr passed");
    for(const App_p &subcomptr : subcommands_)
        if(subcomptr.get() == subcom)
...
XCLI_NODISCARD XCLI_INLINE xcli::App_p App::get_subcommand_ptr(std::string subcom) const {
    for(const App_p &subcomptr : subcommands_)
...
XCLI_NODISCARD XCLI_INLINE xcli::App_p App::get_subcommand_ptr(int index) const {
    if(index >= 0) {
        auto uindex = static_cast<unsigned>(index);
        if(uindex < subcommands_.size())
...
XCLI_NODISCARD XCLI_INLINE xcli::App *App::get_option_group(std::string group_name) const {
...
XCLI_NODISCARD XCLI_INLINE std::size_t App::count_all() const {
    std::size_t cnt{0};
...
XCLI_INLINE const char *maybe_narrow(const char *str) { return str; }
XCLI_INLINE std::string maybe_narrow(const wchar_t *str) { return narrow(str); }
...
XCLI_INLINE void App::parse(std::string commandline, bool program_name_included) {
    ...
    try {
        ...
        throw xcli::ParseError(arg.what(), xcli::ExitCodes::InvalidError);
    }
    std::reverse(args.begin(), args.end());
    parse(std::move(args));
}

XCLI_INLINE void App::parse(std::wstring commandline, bool program_name_included) {
    parse(narrow(commandline), program_name_included);
}

XCLI_INLINE void App::_parse_setup() {
    // Clear if parsed
    if(parsed_ > 0)
        clear();
...
XCLI_INLINE void App::parse(std::vector<std::string> &args) {
    _parse_setup();
    _parse(args);
    run_callback();
}
...
XCLI_INLINE void App::parse_from_stream(std::istream &input) {
    if(parsed_ == 0) {
        _validate();
        _configure();
        // set the parent as nullptr as this object should be the top now
...
XCLI_INLINE int App::exit(const Error &e, std::ostream &out, std::ostream &err) const {

    /// Avoid printing anything if this is a xcli::RuntimeError
...
XCLI_INLINE std::vector<const App *> App::get_subcommands(const std::function<bool(const App *)> &filter) const {
...
XCLI_INLINE std::vector<App *> App::get_subcommands(const std::function<bool(App *)> &filter) {
...
XCLI_INLINE bool App::remove_excludes(Option *opt) {
...
XCLI_INLINE bool App::remove_excludes(App *app) {
...
XCLI_INLINE bool App::remove_needs(Option *opt) {
...
XCLI_INLINE bool App::remove_needs(App *app) {
...
XCLI_NODISCARD XCLI_INLINE std::string App::help(std::string prev, AppFormatMode mode) const {
    if(prev.empty())
        prev = get_name();
    else
        prev += " " + get_name();
...
XCLI_NODISCARD XCLI_INLINE std::string App::version() const {
    std::string val;
    if(version_ptr_ != nullptr) {
        ...
        } catch(const xcli::CallForVersion &cfv) {
            val = cfv.what();
        }
        version_ptr_->clear();
...
XCLI_INLINE std::vector<const Option *> App::get_options(const std::function<bool(const Option *)> filter) const {
...
XCLI_INLINE std::vector<Option *> App::get_options(const std::function<bool(Option *)> filter) {
...
XCLI_NODISCARD XCLI_INLINE const Option *App::get_option(std::string option_name) const {
...
XCLI_NODISCARD XCLI_INLINE Option *App::get_option(std::string option_name) {
...
XCLI_NODISCARD XCLI_INLINE Option *App::get_option_no_throw(std::string option_name) noexcept {
...
XCLI_NODISCARD XCLI_INLINE const Option *App::get_option_no_throw(std::string option_name) const noexcept {
    for(const Option_p &opt : options_) {
        if(opt->check_name(option_name)) {
            return opt.get();
        }
...
XCLI_NODISCARD XCLI_INLINE std::string App::get_display_name(bool with_aliases) const {
    if(name_.empty()) {
...
XCLI_NODISCARD XCLI_INLINE bool App::check_name(std::string name_to_check) const {
    auto result = check_name_detail(std::move(name_to_check));
    return (result != NameMatch::none);
}

XCLI_NODISCARD XCLI_INLINE App::NameMatch App::check_name_detail(std::string name_to_check) const {
...
XCLI_NODISCARD XCLI_INLINE std::vector<std::string> App::get_groups() const {
...
XCLI_NODISCARD XCLI_INLINE std::vector<std::string> App::remaining(bool recurse) const {
...
XCLI_NODISCARD XCLI_INLINE std::vector<std::string> App::remaining_for_passthrough(bool recurse) const {
    std::vector<std::string> miss_list = remaining(recurse);
    std::reverse(std::begin(miss_list), std::end(miss_list));
    return miss_list;
}

XCLI_NODISCARD XCLI_INLINE std::size_t App::remaining_size(bool recurse) const {
...
XCLI_INLINE void App::_validate() const {
...
XCLI_INLINE void App::_configure() {
...
XCLI_INLINE void App::run_callback(bool final_mode, bool suppress_final_callback) {
    pre_callback();
...
XCLI_NODISCARD XCLI_INLINE bool App::_valid_subcommand(const std::string &current, bool ignore_used) const {
...
XCLI_NODISCARD XCLI_INLINE detail::Classifier App::_recognize(const std::string &current,
...
XCLI_INLINE bool App::_process_config_file(const std::string &config_file, bool throw_error) {
...
XCLI_INLINE void App::_process_config_file() {
    if(config_ptr_ != nullptr) {
...
XCLI_INLINE void App::_process_env() {
    for(const Option_p &opt : options_) {
...
XCLI_INLINE void App::_process_callbacks(CallbackPriority priority) {
...
XCLI_INLINE void App::_process_help_flags(CallbackPriority priority, bool trigger_help, bool trigger_all_help) const {
...
XCLI_INLINE void App::_process_requirements() {
    // check excludes
    bool excluded{false};
    std::string excluder;
    ...
    for(App_p &sub : subcommands_) {
        ...
        if(sub->required_ && sub->count_all() == 0) {
            throw(xcli::RequiredError(sub->get_display_name()));
        }
    }
}

XCLI_INLINE void App::_process() {
...
XCLI_INLINE void App::_process_extras() {
...
XCLI_INLINE void App::increment_parsed() {
    ++parsed_;
    for(App_p &sub : subcommands_) {
...
XCLI_INLINE void App::_process_completion_callbacks(bool with_help_flags) {
    _process_callbacks(CallbackPriority::FirstPreHelp);
    if(with_help_flags) {
        _process_help_flags(CallbackPriority::First);
    }
...
XCLI_INLINE void App::_parse(std::vector<std::string> &args) {
    increment_parsed();
...
XCLI_INLINE void App::_parse_stream(std::istream &input) {
...
XCLI_INLINE void App::_parse_config(const std::vector<ConfigItem> &args) {
...
XCLI_INLINE bool App::_parse_single_config(const ConfigItem &item, std::size_t level) {
    ...
    if(op == nullptr || !op->get_configurable()) {
        ...
        auto options = get_options([&iname](const xcli::Option *opt) {
...
XCLI_INLINE bool App::_parse_single(std::vector<std::string> &args, bool &positional_only) {
    bool retval = true;
...
XCLI_NODISCARD XCLI_INLINE std::size_t App::_count_remaining_positionals(bool required_only) const {
    std::size_t retval = 0;
    for(const Option_p &opt : options_) {
...
XCLI_NODISCARD XCLI_INLINE bool App::_has_remaining_positionals() const {
    for(const Option_p &opt : options_) {
...
XCLI_INLINE bool App::_parse_positional(std::vector<std::string> &args, bool haltOnSubcommand) {
    ...
    if(positionals_at_end_) {
        ...
        throw xcli::ExtrasError(name_, rargs);
    }
...
XCLI_NODISCARD XCLI_INLINE App *
...
XCLI_INLINE bool App::_parse_subcommand(std::vector<std::string> &args) {
...
XCLI_INLINE void App::_trigger_pre_parse(std::size_t remaining_args) {
...
XCLI_INLINE App *App::_get_fallthrough_parent() noexcept {
    if(parent_ == nullptr) {
        return nullptr;
    }
...
XCLI_INLINE const App *App::_get_fallthrough_parent() const noexcept {
    if(parent_ == nullptr) {
        return nullptr;
    }
...
XCLI_NODISCARD XCLI_INLINE const std::string &App::_compare_subcommand_names(const App &subcom,
...
XCLI_INLINE void App::_move_to_missing(detail::Classifier val_type, const std::string &val) {
...
XCLI_INLINE void App::_move_option(Option *opt, App *app) {
    if(opt == nullptr) {
...
XCLI_INLINE void TriggerOn(App *trigger_app, App *app_to_enable) {
    app_to_enable->enabled_by_default(false);
    app_to_enable->disabled_by_default();
    trigger_app->preparse_callback([app_to_enable](std::size_t) { app_to_enable->disabled(false); });
}

XCLI_INLINE void TriggerOn(App *trigger_app, std::vector<App *> apps_to_enable) {
...
XCLI_INLINE void TriggerOff(App *trigger_app, App *app_to_enable) {
...
XCLI_INLINE void TriggerOff(App *trigger_app, std::vector<App *> apps_to_enable) {
    for(auto &app : apps_to_enable) {
...
XCLI_INLINE void deprecate_option(Option *opt, const std::string &replacement) {
...
XCLI_INLINE void retire_option(App *app, Option *opt) {
    App temp;
...
XCLI_INLINE void retire_option(App &app, Option *opt) { retire_option(&app, opt); }

XCLI_INLINE void retire_option(App *app, const std::string &option_name) {
...
XCLI_INLINE void retire_option(App &app, const std::string &option_name) { retire_option(&app, option_name); }

namespace FailureMessage {

XCLI_INLINE std::string simple(const App *app, const Error &e) {
...
XCLI_INLINE std::string help(const App *app, const Error &e) {
