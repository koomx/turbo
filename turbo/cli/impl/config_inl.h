...
XCLI_INLINE bool is_printable(const std::string &test_string) {
...
XCLI_INLINE std::string ini_join(const std::vector<std::string> &args,
                                  char sepChar,
                                  char arrayStart,
                                  char arrayEnd,
                                  char stringQuote,
...
XCLI_INLINE std::vector<std::string>
...
XCLI_INLINE bool hasMLString(std::string const &fullString, char check) {
...
XCLI_INLINE void clean_name_string(std::string &name, const std::string &keyChars) {
    ...
    for(auto &group : groups) {
        ...
        for(const Option *opt : app->get_options({})) {
            ...
            if(opt->get_configurable()) {
                ...
                if(opt->get_multi_option_policy() == xcli::MultiOptionPolicy::Sum && opt->count() >= 1 &&
                   results.size() == 1) {
