...
XCLI_INLINE IPV4Validator::IPV4Validator() : Validator("IPV4") {
...
XCLI_INLINE AsSizeValue::AsSizeValue(bool kb_is_1000) : AsNumberWithUnit(get_mapping(kb_is_1000)) {
    if(kb_is_1000) {
...
XCLI_INLINE std::map<std::string, AsSizeValue::result_t> AsSizeValue::init_mapping(bool kb_is_1000) {
...
XCLI_INLINE const std::map<std::string, AsSizeValue::result_t> &AsSizeValue::get_mapping(bool kb_is_1000) {
    if(kb_is_1000) {
...
namespace detail {
XCLI_INLINE PermissionValidator::PermissionValidator(Permission permission) {
...
XCLI_INLINE FileSizeValidator::FileSizeValidator(std::uint64_t min_size, std::uint64_t max_size) {
    std::string desc;
    if(max_size == 0) {
