...
XCLI_INLINE Option *Option::expected(int value) {
    if(value < 0) {
        expected_min_ = -value;
...
XCLI_INLINE Option *Option::expected(int value_min, int value_max) {
    if(value_min < 0) {
        value_min = -value_min;
    }
...
XCLI_INLINE Option *Option::check(Validator_p validator) {
...
XCLI_INLINE Option *Option::check(Validator validator, const std::string &validator_name) {
    validator.non_modifying();
...
XCLI_INLINE Option *Option::check(std::function<std::string(const std::string &)> validator_func,
...
XCLI_INLINE Option *Option::transform(Validator_p validator) {
    validators_.insert(validators_.begin(), std::move(validator));

    return this;
}

XCLI_INLINE Option *Option::transform(Validator validator, const std::string &transform_name) {
    auto vp = std::make_shared<Validator>(std::move(validator));
...
XCLI_INLINE Option *Option::transform(const std::function<std::string(std::string)> &transform_func,
...
XCLI_INLINE Option *Option::each(const std::function<void(std::string)> &func) {
    auto vp = std::make_shared<Validator>(
...
XCLI_INLINE Validator *Option::get_validator(const std::string &validator_name) {
...
XCLI_INLINE Validator *Option::get_validator(int index) {
...
XCLI_INLINE bool Option::remove_needs(Option *opt) {
...
XCLI_INLINE Option *Option::excludes(Option *opt) {
    if(opt == this) {
...
XCLI_INLINE bool Option::remove_excludes(Option *opt) {
...
XCLI_INLINE Option *Option::multi_option_policy(MultiOptionPolicy value) {
...
XCLI_NODISCARD XCLI_INLINE std::string
...
XCLI_INLINE void Option::run_callback() {
...
XCLI_NODISCARD XCLI_INLINE const std::string &Option::matching_name(const Option &other) const {
...
XCLI_NODISCARD XCLI_INLINE bool Option::check_name(const std::string &name) const {
...
XCLI_NODISCARD XCLI_INLINE std::string Option::get_flag_value(const std::string &name,
...
XCLI_INLINE Option *Option::add_result(std::string s) {
    _add_result(std::move(s), results_);
    current_option_state_ = option_state::parsing;
    return this;
}

XCLI_INLINE Option *Option::add_result(std::string s, int &results_added) {
    results_added = _add_result(std::move(s), results_);
    current_option_state_ = option_state::parsing;
    return this;
}

XCLI_INLINE Option *Option::add_result(std::vector<std::string> s) {
    current_option_state_ = option_state::parsing;
    for(auto &str : s) {
...
XCLI_NODISCARD XCLI_INLINE results_t Option::reduced_results() const {
...
XCLI_INLINE Option *Option::type_size(int option_type_size) {
...
XCLI_INLINE Option *Option::type_size(int option_type_size_min, int option_type_size_max) {
...
XCLI_NODISCARD XCLI_INLINE std::string Option::get_type_name() const {
...
XCLI_INLINE void Option::_validate_results(results_t &res) const {
    // Run the Validators (can change the string)
    if(!validators_.empty()) {
        if(type_size_max_ > 1) {  // in this context index refers to the index in the type
            int index = 0;
            if(get_items_expected_max() < static_cast<int>(res.size()) &&
               (multi_option_policy_ == xcli::MultiOptionPolicy::TakeLast ||
                multi_option_policy_ == xcli::MultiOptionPolicy::Reverse)) {
...
XCLI_INLINE void Option::_reduce_results(results_t &out, const results_t &original) const {
...
XCLI_INLINE std::string Option::_validate(std::string &result, int index) const {
    std::string err_msg;
...
XCLI_INLINE int Option::_add_result(std::string &&result, std::vector<std::string> &res) const {
    int result_count = 0;
    ...
    if((allow_extra_args_ || get_expected_max() > 1 || get_type_size() > 1) && !result.empty() &&
       ...
       result.back() == ']') {  // this is now a vector string likely from the default or user entry
        ...
        for(auto &var : xcli::detail::split_up(result, ',')) {
            if(!var.empty()) {
    ...
    if(delimiter_ == '\0') {
        ...
        if((result.find_first_of(delimiter_) != std::string::npos)) {
            for(const auto &var : xcli::detail::split(result, delimiter_)) {
                if(!var.empty()) {
                    res.push_back(var);
                    ++result_count;
                }
