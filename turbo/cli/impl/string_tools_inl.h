...
namespace detail {
XCLI_INLINE std::vector<std::string> split(const std::string &s, char delim) {
...
XCLI_INLINE std::string &ltrim(std::string &str) {
    auto it = std::find_if(str.begin(), str.end(), [](char ch) { return !std::isspace<char>(ch, std::locale()); });
    str.erase(str.begin(), it);
    return str;
}

XCLI_INLINE std::string &ltrim(std::string &str, const std::string &filter) {
    auto it = std::find_if(str.begin(), str.end(), [&filter](char ch) { return filter.find(ch) == std::string::npos; });
    str.erase(str.begin(), it);
    return str;
}

XCLI_INLINE std::string &rtrim(std::string &str) {
    auto it = std::find_if(str.rbegin(), str.rend(), [](char ch) { return !std::isspace<char>(ch, std::locale()); });
    str.erase(it.base(), str.end());
    return str;
}

XCLI_INLINE std::string &rtrim(std::string &str, const std::string &filter) {
    auto it =
...
XCLI_INLINE std::string &remove_quotes(std::string &str) {
...
XCLI_INLINE std::string &remove_outer(std::string &str, char key) {
...
XCLI_INLINE std::string fix_newlines(const std::string &leader, std::string input) {
...
XCLI_INLINE std::ostream &format_aliases(std::ostream &out, const std::vector<std::string> &aliases, std::size_t wid) {
    if(!aliases.empty()) {
...
XCLI_INLINE bool valid_name_string(const std::string &str) {
...
XCLI_INLINE std::string get_group_separators() {
...
XCLI_INLINE std::string find_and_replace(std::string str, std::string from, std::string to) {
...
XCLI_INLINE void remove_default_flag_values(std::string &flags) {
...
XCLI_MODULE_INLINE const std::string &escapedChars() {
    static const std::string s{"\b\t\n\f\r\"\\"};
    return s;
}
XCLI_MODULE_INLINE const std::string &escapedCharsCode() {
    static const std::string s{"btnfr\"\\"};
    return s;
}
XCLI_MODULE_INLINE const std::string &bracketChars() {
    static const std::string s{"\"'`[(<{"};
    return s;
}
XCLI_MODULE_INLINE const std::string &matchBracketChars() {
    static const std::string s{"\"'`])>}"};
    return s;
}

XCLI_INLINE bool has_escapable_character(const std::string &str) {
    return (str.find_first_of(escapedChars()) != std::string::npos);
}

XCLI_INLINE std::string add_escaped_characters(const std::string &str) {
    std::string out;
...
XCLI_INLINE std::uint32_t hexConvert(char hc) {
    int hcode{0};
    if(hc >= '0' && hc <= '9') {
        hcode = (hc - '0');
    } else if(hc >= 'A' && hc <= 'F') {
...
XCLI_INLINE char make_char(std::uint32_t code) { return static_cast<char>(static_cast<unsigned char>(code)); }

XCLI_INLINE void append_codepoint(std::string &str, std::uint32_t code) {
...
XCLI_INLINE std::string remove_escaped_characters(const std::string &str) {
...
XCLI_INLINE std::size_t close_string_quote(const std::string &str, std::size_t start, char closure_char) {
    std::size_t loc{0};
...
XCLI_INLINE std::size_t close_literal_quote(const std::string &str, std::size_t start, char closure_char) {
    auto loc = str.find_first_of(closure_char, start + 1);
    return (loc != std::string::npos ? loc : str.size());
}

XCLI_INLINE std::size_t close_sequence(const std::string &str, std::size_t start, char closure_char) {
...
XCLI_INLINE std::vector<std::string> split_up(std::string str, char delimiter) {
...
XCLI_INLINE std::size_t escape_detect(std::string &str, std::size_t offset) {
...
XCLI_INLINE std::string binary_escape_string(const std::string &string_to_escape, bool force) {
...
XCLI_INLINE bool is_binary_escaped_string(const std::string &escaped_string) {
...
XCLI_INLINE std::string extract_binary_string(const std::string &escaped_string) {
    std::size_t start{0};
    std::size_t tail{0};
    size_t ssize = escaped_string.size();
    if(escaped_string.compare(0, 3, "B\"(") == 0 && escaped_string.compare(ssize - 2, 2, ")\"") == 0) {
...
XCLI_INLINE void remove_quotes(std::vector<std::string> &args) {
    for(auto &arg : args) {
        if(arg.empty()) {
            continue;
        }
...
XCLI_INLINE void handle_secondary_array(std::string &str) {
...
XCLI_INLINE std::ostream &streamOutAsParagraph(std::ostream &out,
