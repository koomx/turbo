...
XCLI_INLINE std::string Validator::operator()(std::string &str) const {
...
XCLI_NODISCARD XCLI_INLINE Validator Validator::description(std::string validator_desc) const {
    Validator newval(*this);
    newval.desc_function_ = [validator_desc]() { return validator_desc; };
    return newval;
}

XCLI_INLINE Validator Validator::operator&(const Validator &other) const {
    Validator newval;
...
XCLI_INLINE Validator Validator::operator!() const {
    Validator newval;
...
XCLI_INLINE ExistingFileValidator::ExistingFileValidator() : Validator("FILE") {
...
XCLI_INLINE ExistingDirectoryValidator::ExistingDirectoryValidator() : Validator("DIR") {
...
XCLI_INLINE ExistingPathValidator::ExistingPathValidator() : Validator("PATH(existing)") {
    func_ = [](std::string &filename) {
        auto path_result = check_path(filename.c_str());
        if(path_result == path_type::nonexistent) {
...
XCLI_INLINE NonexistentPathValidator::NonexistentPathValidator() : Validator("PATH(non-existing)") {
    func_ = [](std::string &filename) {
        auto path_result = check_path(filename.c_str());
        if(path_result != path_type::nonexistent) {
...
XCLI_INLINE EscapedStringTransformer::EscapedStringTransformer() {
...
XCLI_INLINE FileOnDefaultPath::FileOnDefaultPath(std::string default_path, bool enableErrorReturn)
    : Validator("FILE") {
