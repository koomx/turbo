...
namespace xcli {
    ...
    Option* Option::check(Validator_p validator) {
    ...
    Option* Option::check(std::function<std::string(const std::string&)> validator_func,
    ...
    Option* Option::transform(Validator_p validator) {
        validators_.insert(validators_.begin(), std::move(validator));

        return this;
    }

    Option* Option::transform(Validator validator, const std::string& transform_name) {
        auto vp = std::make_shared<Validator>(std::move(validator));
    ...
    Option* Option::transform(const std::function<std::string(std::string)>& transform_func,
    ...
    Option* Option::multi_option_policy(MultiOptionPolicy value) {
        if (value != multi_option_policy_) {
            if (multi_option_policy_ == MultiOptionPolicy::Throw && expected_max_ == detail::expected_max_vector_size && expected_min_ > 1) { // this bizarre condition is to maintain backwards compatibility
    ...
    void Option::_validate_results(results_t& res) const {
        ...
        if (!validators_.empty()) {
            if (type_size_max_ > 1) { // in this context index refers to the index in the type
                int index = 0;
                if (get_items_expected_max() < static_cast<int>(res.size()) && (multi_option_policy_ == xcli::MultiOptionPolicy::TakeLast || multi_option_policy_ == xcli::MultiOptionPolicy::Reverse)) {
                ...
                int index = 0;
                if (expected_max_ < static_cast<int>(res.size()) && (multi_option_policy_ == xcli::MultiOptionPolicy::TakeLast || multi_option_policy_ == xcli::MultiOptionPolicy::Reverse)) {
                    // create a negative index for the earliest ones
    ...
    int Option::_add_result(std::string&& result, std::vector<std::string>& res) const {
        ...
        if ((allow_extra_args_ || get_expected_max() > 1 || get_type_size() > 1) && !result.empty() && result.front() == '[' && result.back() == ']') { // this is now a vector string likely from the default or user entry
