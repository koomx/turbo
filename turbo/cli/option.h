...
template <typename CRTP> class OptionBase {
  ...
  public:
    ...
    /// The status of configurable
    XCLI_NODISCARD bool get_disable_flag_override() const { return disable_flag_override_; }

    /// Get the current delimiter char
    XCLI_NODISCARD char get_delimiter() const { return delimiter_; }

    /// Return true if this will automatically capture the default value for help printing
    XCLI_NODISCARD bool get_always_capture_default() const { return always_capture_default_; }

    /// The status of the multi option policy
    XCLI_NODISCARD MultiOptionPolicy get_multi_option_policy() const { return multi_option_policy_; }

    /// The priority of callback
    XCLI_NODISCARD CallbackPriority get_callback_priority() const { return callback_priority_; }
...
class Option : public OptionBase<Option> {
  ...
  public:
    ...
    XCLI_NODISCARD std::size_t count() const { return results_.size(); }

    /// True if the option was not passed
    XCLI_NODISCARD bool empty() const { return results_.empty(); }
    ...
    XCLI_NODISCARD bool get_allow_extra_args() const { return allow_extra_args_; }
    ...
    XCLI_NODISCARD bool get_trigger_on_parse() const { return trigger_on_result_; }
    ...
    XCLI_NODISCARD bool get_force_callback() const { return force_callback_; }
    ...
    XCLI_NODISCARD bool get_run_callback_for_default() const { return run_callback_for_default_; }
    ...
    XCLI_NODISCARD int get_type_size() const { return type_size_min_; }

    /// The minimum number of arguments the option expects
    XCLI_NODISCARD int get_type_size_min() const { return type_size_min_; }
    /// The maximum number of arguments the option expects
    XCLI_NODISCARD int get_type_size_max() const { return type_size_max_; }

    /// Return the inject_separator flag
    XCLI_NODISCARD bool get_inject_separator() const { return inject_separator_; }

    /// The environment variable associated to this value
    XCLI_NODISCARD std::string get_envname() const { return envname_; }

    /// The set of options needed
    XCLI_NODISCARD std::set<Option *> get_needs() const { return needs_; }

    /// The set of options excluded
    XCLI_NODISCARD std::set<Option *> get_excludes() const { return excludes_; }

    /// The default value (for help printing)
    XCLI_NODISCARD std::string get_default_str() const { return default_str_; }

    /// Get the callback function
    XCLI_NODISCARD callback_t get_callback() const { return callback_; }

    /// Get the long names
    XCLI_NODISCARD const std::vector<std::string> &get_lnames() const { return lnames_; }

    /// Get the short names
    XCLI_NODISCARD const std::vector<std::string> &get_snames() const { return snames_; }

    /// Get the flag names with specified default values
    XCLI_NODISCARD const std::vector<std::string> &get_fnames() const { return fnames_; }
    /// Get a single name for the option, first of lname, sname, pname, envname
    XCLI_NODISCARD const std::string &get_single_name() const {
        if(!lnames_.empty()) {
            return lnames_[0];
        }
        if(!snames_.empty()) {
    ...
    XCLI_NODISCARD int get_expected() const { return expected_min_; }

    /// The number of times the option expects to be included
    XCLI_NODISCARD int get_expected_min() const { return expected_min_; }
    /// The max number of times the option expects to be included
    XCLI_NODISCARD int get_expected_max() const { return expected_max_; }

    /// The total min number of expected  string values to be used
    XCLI_NODISCARD int get_items_expected_min() const { return type_size_min_ * expected_min_; }

    /// Get the maximum number of items expected to be returned and used for the callback
    XCLI_NODISCARD int get_items_expected_max() const {
        int t = type_size_max_;
        return detail::checked_multiply(t, expected_max_) ? t : detail::expected_max_vector_size;
    }
    /// The total min number of expected  string values to be used
    XCLI_NODISCARD int get_items_expected() const { return get_items_expected_min(); }

    /// True if the argument can be given directly
    XCLI_NODISCARD bool get_positional() const { return !pname_.empty(); }

    /// True if option has at least one non-positional name
    XCLI_NODISCARD bool nonpositional() const { return (!lnames_.empty() || !snames_.empty()); }

    /// True if option has description
    XCLI_NODISCARD bool has_description() const { return !description_.empty(); }

    /// Get the description
    XCLI_NODISCARD const std::string &get_description() const { return description_; }
    ...
    XCLI_NODISCARD const std::string &get_option_text() const { return option_text_; }
    ...
    XCLI_NODISCARD std::string get_name(bool positional = false,                  ///< Show the positional name
    ...
    XCLI_NODISCARD const std::string &matching_name(const Option &other) const;

    /// If options share any of the same names, they are equal (not counting positional)
    bool operator==(const Option &other) const { return !matching_name(other).empty(); }

    /// Check a name. Requires "-" or "--" for short / long, supports positional name
    XCLI_NODISCARD bool check_name(const std::string &name) const;

    /// Requires "-" to be removed from string
    XCLI_NODISCARD bool check_sname(std::string name) const {
        return (detail::find_member(std::move(name), snames_, ignore_case_) >= 0);
    }

    /// Requires "--" to be removed from string
    XCLI_NODISCARD bool check_lname(std::string name) const {
        return (detail::find_member(std::move(name), lnames_, ignore_case_, ignore_underscore_) >= 0);
    }

    /// Requires "--" to be removed from string
    XCLI_NODISCARD bool check_fname(std::string name) const {
        if(fnames_.empty()) {
            return false;
        }
    ...
    /// disabled
    XCLI_NODISCARD std::string get_flag_value(const std::string &name, std::string input_value) const;
    ...
    XCLI_NODISCARD const results_t &results() const { return results_; }

    /// Get a copy of the results
    XCLI_NODISCARD results_t reduced_results() const;
    ...
    template <typename T> XCLI_NODISCARD T as() const {
        T output;
        results(output);
        return output;
    }
    ...
    XCLI_NODISCARD bool get_callback_run() const { return (current_option_state_ == option_state::callback_run); }
    ...
    XCLI_NODISCARD std::string get_type_name() const;
...
#ifndef XCLI_COMPILE
#include <turbo/cli/impl/option_inl.h>  // IWYU pragma: export
#endif
