    ...
    namespace detail {
        ...
        std::vector<std::string> split_names(std::string current) {
            ...
            while ((val = current.find(',')) != std::string::npos) {
                output.push_back(std::string(turbo::trim_all(current.substr(0, val))));
                current = current.substr(val + 1);
            }
            output.push_back(std::string(turbo::trim_all((current))));
            return output;
        }
        ...
        std::vector<std::pair<std::string, std::string>> get_default_flag_values(const std::string& str) {
            ...
            flags.erase(std::remove_if(flags.begin(),
                            ...
                            [](const std::string& name) {
                                return ((name.empty()) || (!(((name.find_first_of('{') != std::string::npos) && (name.back() == '}')) || (name[0] == '!'))));
                            }),
                flags.end());
