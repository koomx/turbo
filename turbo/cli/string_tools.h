...
XCLI_MODULE_INLINE constexpr int expected_max_vector_size{1 << 29};
...
XCLI_INLINE std::ptrdiff_t find_member(std::string name,
...
XCLI_INLINE std::vector<std::string> split_up(std::string str, char delimiter = '\0');

/// get the value of an environmental variable or empty string if empty
XCLI_INLINE std::string get_environment_value(const std::string &env_name);
...
XCLI_INLINE std::string binary_escape_string(const std::string &string_to_escape, bool force = false);
...
XCLI_INLINE bool process_quoted_string(std::string &str,
                                        char string_char = '\"',
                                        char literal_char = '\'',
...
#ifndef XCLI_COMPILE
#include <turbo/cli/impl/string_tools_inl.h>  // IWYU pragma: export
#endif
