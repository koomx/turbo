...
namespace xcli {
    ...
    inline const detail::ExistingFileValidator ExistingFile;

    /// Check for an existing directory (returns error message if check fails)
    inline const detail::ExistingDirectoryValidator ExistingDirectory;

    /// Check for an existing path
    inline const detail::ExistingPathValidator ExistingPath;

    /// Check for an non-existing path
    inline const detail::NonexistentPathValidator NonexistentPath;

    /// convert escaped characters into their associated values
    inline const detail::EscapedStringTransformer EscapedString;
