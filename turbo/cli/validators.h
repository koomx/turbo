...
class Validator {
  ...
  public:
    ...
    /// Specify the type string
    XCLI_NODISCARD Validator description(std::string validator_desc) const;

    /// Generate type description information for the Validator
    XCLI_NODISCARD std::string get_description() const {
        if(active_) {
    ...
    /// Specify the type string
    XCLI_NODISCARD Validator name(std::string validator_name) const {
    ...
    /// Specify whether the Validator is active or not
    XCLI_NODISCARD Validator active(bool active_val = true) const {
        Validator newval(*this);
    ...
    /// Specify the application index of a validator
    XCLI_NODISCARD Validator application_index(int app_index) const {
        Validator newval(*this);
        newval.application_index_ = app_index;
        return newval;
    }
    /// Get the current value of the application index
    XCLI_NODISCARD int get_application_index() const { return application_index_; }
    /// Get a boolean if the validator is active
    XCLI_NODISCARD bool get_active() const { return active_; }

    /// Get a boolean if the validator is allowed to modify the input returns true if it can modify the input
    XCLI_NODISCARD bool get_modifying() const { return !non_modifying_; }
...
XCLI_INLINE path_type check_path(const char *file) noexcept;
...
XCLI_MODULE_INLINE const detail::ExistingFileValidator ExistingFile;

/// Check for an existing directory (returns error message if check fails)
XCLI_MODULE_INLINE const detail::ExistingDirectoryValidator ExistingDirectory;

/// Check for an existing path
XCLI_MODULE_INLINE const detail::ExistingPathValidator ExistingPath;

/// Check for an non-existing path
XCLI_MODULE_INLINE const detail::NonexistentPathValidator NonexistentPath;

/// convert escaped characters into their associated values
XCLI_MODULE_INLINE const detail::EscapedStringTransformer EscapedString;
...
XCLI_MODULE_INLINE const Range NonNegativeNumber((std::numeric_limits<double>::max)(), "NONNEGATIVE");

/// Check for a positive valued number (val>0.0), <double>::denorm_min is the smallest positive number
XCLI_MODULE_INLINE const
...
/// everything else.
XCLI_INLINE std::pair<std::string, std::string> split_program_name(std::string commandline);
...
#ifndef XCLI_COMPILE
#include <turbo/cli/impl/validators_inl.h>  // IWYU pragma: export
#endif
