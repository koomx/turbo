...
//
// `turbo::chunked_queue` provides the same minus random access. It is
// implemented as a double-linked list of fixed or variable sized blocks.
//
// `turbo::chunked_queue` is useful when memory usage is paramount as it provides
...
// one.
//
//   turbo::chunked_queue<int64> q = {1, 2, 3};
//   q.clear();
//   q.shrink_to_fit();
//
...
#ifndef TURBO_CONTAINER_CHUNKED_QUEUE_H_
#define TURBO_CONTAINER_CHUNKED_QUEUE_H_
...
#include <turbo/container/internal/chunked_queue.h>
 ...
 public:
  ...
  template <typename Iter,
            typename = std::enable_if_t<
                meta_internal::IsAtLeastInputIterator<Iter>::value>>
  ...
  void swap(chunked_queue& other) noexcept {
    ...
    if (AllocatorTraits::propagate_on_container_swap::value) {
      ...
      turbo::base_internal::HardeningAssert(get_allocator() ==
                                           other.get_allocator());
...
#endif  // TURBO_CONTAINER_CHUNKED_QUEUE_H_
