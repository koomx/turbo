...
// -----------------------------------------------------------------------------
//
// An `turbo::flat_hash_map<K, V>` is an unordered associative container of
...
// -----------------------------------------------------------------------------
//
// An `turbo::flat_hash_map<K, V>` is an unordered associative container which
...
// consider using `turbo::flat_hash_map<Key, std::unique_ptr<Value>>` instead.
// If your types are not moveable or you require pointer stability for keys,
// consider `turbo::node_hash_map`.
//
...
//   turbo::flat_hash_map<std::string, std::string> ducks =
...
class KUMO_ATTRIBUTE_OWNER flat_hash_map
    : public turbo::container_internal::InstantiateRawHashMap<
          turbo::container_internal::FlatHashMapPolicy<K, V>, Hash, Eq,
          Allocator>::type {
 ...
 public:
  ...
  //
  //   std::vector<std::pair<int, std::string>> v = {{1, "a"}, {2, "b"}};
  //   turbo::flat_hash_map<int, std::string> map8(std::from_range, v);
  flat_hash_map() {}
  using Base::Base;
  ...
  //
  // NOTE: this member function is particular to `turbo::flat_hash_map` and is
...
struct IsUnorderedContainer<
    turbo::flat_hash_map<Key, T, Hash, KeyEqual, Allocator>> : std::true_type {};
