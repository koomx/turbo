...
// -----------------------------------------------------------------------------
//
// An `turbo::flat_hash_set<T>` is an unordered associative container designed to
...
#ifndef TURBO_CONTAINER_FLAT_HASH_SET_H_
#define TURBO_CONTAINER_FLAT_HASH_SET_H_
...
// -----------------------------------------------------------------------------
//
// An `turbo::flat_hash_set<T>` is an unordered associative container which has
...
//
// By default, `flat_hash_set` uses the `turbo::Hash` hashing framework. All
// fundamental and Abseil types that support the `turbo::Hash` framework have a
...
// types.
//
// Using `turbo::flat_hash_set` at interface boundaries in dynamically loaded
// libraries (e.g. .dll, .so) is unsupported due to way `turbo::Hash` values may
...
// `turbo_container_hash` and (optionally) `turbo_container_eq`. In either case,
...
//
// In most cases `T` needs only to provide the `turbo_container_hash`. In this
...
// `turbo::flat_hash_set<std::unique_ptr<T>>`. If your type is not moveable and
// you require pointer stability, consider `turbo::node_hash_set` instead.
//
...
//   turbo::flat_hash_set<std::string> ducks =
//     {"huey", "dewey", "louie"};
//
...
class KUMO_ATTRIBUTE_OWNER flat_hash_set
    : public turbo::container_internal::InstantiateRawHashSet<
          turbo::container_internal::FlatHashSetPolicy<T>, Hash, Eq,
          Allocator>::type {
 ...
 public:
  ...
  //
  //   turbo::flat_hash_set<std::string> set2 =
  //       {{"huey"}, {"dewey"}, {"louie"},};
  //
  // * Copy constructor
  //
  ...
  //
  //   std::vector<std::string> v = {"a", "b"};
  //   turbo::flat_hash_set<std::string> set8(std::from_range, v);
  flat_hash_set() {}
  using Base::Base;
  ...
  //
  // NOTE: this member function is particular to `turbo::flat_hash_set` and is
...
// Specialization of trait in turbo/algorithm/container.h
template <class Key, class Hash, class KeyEqual, class Allocator>
struct IsUnorderedContainer<turbo::flat_hash_set<Key, Hash, KeyEqual, Allocator>>
    : std::true_type {};
...
#endif  // TURBO_CONTAINER_FLAT_HASH_SET_H_
