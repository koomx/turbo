...
using btree_is_key_compare_to =
    std::is_convertible<compare_result_t<Compare, T, T>, turbo::weak_ordering>;
...
// greater<string_view>, less<turbo::Cord>, and greater<turbo::Cord>.
  ...
  struct checked_compare : checked_compare_base<Compare> {
   ...
   public:
    ...
    turbo::weak_ordering operator()(const T& lhs, const U& rhs) const {
      ...
      assert(is_self_equivalent(lhs));
      assert(is_self_equivalent(rhs));
      const turbo::weak_ordering lhs_comp_rhs = comp()(lhs, rhs);
#ifndef NDEBUG
      const turbo::weak_ordering rhs_comp_lhs = comp()(rhs, lhs);
      if (lhs_comp_rhs > 0) {
...
template <>
struct key_compare_adapter<std::less<turbo::Cord>, turbo::Cord> {
  using type = StringBtreeDefaultLess;
};

template <>
struct key_compare_adapter<std::greater<turbo::Cord>, turbo::Cord> {
  using type = StringBtreeDefaultGreater;
};
...
constexpr bool compare_has_valid_result_type() {
  ...
  return std::is_same_v<compare_result_type, bool> ||
         std::is_convertible_v<compare_result_type, turbo::weak_ordering>;
}
...
class btree_node {
 ...
 private:
  using layout_type =
      turbo::container_internal::Layout<btree_node *, uint32_t, field_type,
 ...
 public:
  ...
  SearchResult<size_type, true> linear_search_impl(
    ...
    while (s < e) {
      const turbo::weak_ordering c = comp(key(s), k);
      if (c == 0) {
  ...
  SearchResult<size_type, true> binary_search_impl(
    ...
    if (params_type::template can_have_multiple_equivalent_keys<K>()) {
      ...
      while (s != e) {
        const size_type mid = (s + e) >> 1;
        const turbo::weak_ordering c = comp(key(mid), k);
        if (c < 0) {
          s = mid + 1;
        } else {
          e = mid;
...
class btree_iterator : private btree_iterator_generation_info {
 ...
 public:
  ...
  reference operator*() const {
    turbo::base_internal::HardeningAssertNonNull(node_);
    assert_valid_generation(node_);
    turbo::base_internal::HardeningAssertGE(position_,
                                           static_cast<int>(node_->start()));
    if (position_ >= node_->finish()) {
      // If this assertion fails, we have tried to dereference an end()
      // iterator.
      turbo::base_internal::HardeningAssert(!IsEndIterator());
      turbo::base_internal::HardeningAssertLT(position_,
...
class btree {
 ...
 private:
  ...
  turbo::CompressedTuple<key_compare, allocator_type,
                                            node_type *>
      rightmost_;
  ...
  while (n > 0) {
    if (node->is_leaf()) {
      if (position + n < node->finish()) {
        ...
        if (position == node->finish()) {
          turbo::base_internal::HardeningAssert(n == 0);
          return *this = save;
        }
      }
    } else {
  ...
  while (n > 0) {
    if (node->is_leaf()) {
      if (position - n >= node->start()) {
        ...
        turbo::base_internal::HardeningAssertGE(position,
...
constexpr bool btree<P>::static_assert_validation() {
  ...
  // Verify that key_compare returns an turbo::{weak,strong}_ordering or bool.
  static_assert(
      compare_has_valid_result_type<key_compare, key_type>(),
      "key comparison function must return turbo::{weak,strong}_ordering or "
      "bool.");
