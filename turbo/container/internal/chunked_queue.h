...
#ifndef TURBO_CONTAINER_INTERNAL_CHUNKED_QUEUE_H_
#define TURBO_CONTAINER_INTERNAL_CHUNKED_QUEUE_H_
...
class ChunkedQueueBlock {
 ...
 private:
  ...
  static constexpr size_t AllocSize(size_t min_element_count) {
    return turbo::container_internal::Layout<ChunkedQueueBlock, T>(
               1, min_element_count)
        .AllocSize();
  }

  static constexpr ptrdiff_t start_offset() {
    return turbo::container_internal::Layout<ChunkedQueueBlock, T>(1, 1)
        .template Offset<1>();
  }
...
#endif  // TURBO_CONTAINER_INTERNAL_CHUNKED_QUEUE_H_
