...
struct StringHash {
 ...
 private:
  friend struct turbo::hash_internal::HashWithSeed;

  size_t hash_with_seed(std::string_view v, size_t seed) const {
    return turbo::hash_internal::HashWithSeed().hash(
        turbo::Hash<std::string_view>{}, v, seed);
  }
  size_t hash_with_seed(const turbo::Cord& v, size_t seed) const {
    return turbo::hash_internal::HashWithSeed().hash(turbo::Hash<turbo::Cord>{}, v,
                                                    seed);
  }
};
...
// Supports heterogeneous lookup for w/u8/u16/u32 string + std::string_view + char*.
template <typename TChar>
...
template <typename T>
struct HasTurboContainerHash<T, std::void_t<typename T::turbo_container_hash>>
    : std::true_type {};
...
template <typename T>
struct HasTurboContainerEq<T, std::void_t<typename T::turbo_container_eq>>
    : std::true_type {};
