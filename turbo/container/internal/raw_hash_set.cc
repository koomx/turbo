...
#include <turbo/container/internal/raw_hash_set_resize_impl.h>
...
void ValidateMaxSize([[maybe_unused]] size_t size,
  ...
  TURBO_SWISSTABLE_ASSERT(size <= MaxValidSize(key_size, slot_size));
}
...
void IterateOverFullSlotsImpl(const CommonFields& c, size_t slot_size, Fn cb) {
  ...
  if (is_half_group(cap)) {
    ...
    TURBO_SWISSTABLE_ASSERT(cap <= GroupPortableImpl::kWidth &&
  ...
  KUMO_ATTRIBUTE_UNUSED const size_t original_size_for_assert = remaining;
  while (remaining != 0) {
    for (uint32_t i : GroupFullEmptyOrDeleted(ctrl).MaskFull()) {
      TURBO_SWISSTABLE_ASSERT(IsFull(ctrl[i]) &&
  ...
  // turbo::erase_if specialization. So we use `>=`.
  TURBO_SWISSTABLE_ASSERT(original_size_for_assert >= c.size() &&
                         "hash table was modified unexpectedly");
}
...
void CommonFields::AssertNotDebugCapacityImpl() const {
  ...
  assert(!cap.IsReentrance() &&
         ...
         "is not allowed.");
  if (cap.IsDestroyed()) {
    TURBO_RAW_LOG(FATAL, "Use of destroyed hash table.");
  }
  if (SwisstableGenerationsEnabled() && KUMO_UNLIKELY(cap.IsMovedFrom())) {
    if (cap.IsSelfMovedFrom()) {
      ...
      TURBO_RAW_LOG(FATAL, "Use of self-move-assigned hash table.");
    }
    TURBO_RAW_LOG(FATAL, "Use of moved-from hash table.");
  }
}
...
KUMO_ATTRIBUTE_NOINLINE GrowthInfoLowerBound
GrowthInfoAccessor::RebalanceGrowthLeftLowerBoundLargeCapacity() {
  ...
  TURBO_SWISSTABLE_ASSERT(result.HasNoDeleted() ==
...
void ConvertDeletedToEmptyAndFullToDeleted(ctrl_t* ctrl, size_t capacity) {
  TURBO_SWISSTABLE_ASSERT(ctrl[capacity] == ctrl_t::kSentinel);
...
inline void SetCtrlNoSanitizeImpl(const CommonFields& c, size_t i, ctrl_t h) {
  TURBO_SWISSTABLE_ASSERT(i < c.capacity());
...
inline void SetCtrl(const CommonFields& c, size_t i, ctrl_t h,
                    size_t slot_size) {
  TURBO_SWISSTABLE_ASSERT(!c.is_small());
...
inline void SetCtrlInSingleGroupTable(const CommonFields& c, size_t i, ctrl_t h,
  ...
  const size_t cap = c.capacity();
  TURBO_SWISSTABLE_ASSERT(!c.is_small());
  TURBO_SWISSTABLE_ASSERT(is_single_group(cap));
  DoSanitizeOnSetCtrl(c, i, h, slot_size);
  ctrl_t* ctrl = c.control();
  ctrl[i] = h;
  ctrl[i + cap + 1] = h;
...
inline void SetCtrlInLargeTable(const CommonFields& c, size_t i, ctrl_t h,
                                size_t slot_size) {
  TURBO_SWISSTABLE_ASSERT(c.capacity() >= Group::kWidth - 1);
  DoSanitizeOnSetCtrl(c, i, h, slot_size);
  ctrl_t* ctrl = c.control();
  ctrl[i] = h;
...
void* DropDeletesWithoutResizeAndPrepareInsert(
  ...
  const size_t capacity = common.capacity();
  TURBO_SWISSTABLE_ASSERT(IsValidCapacity(capacity));
  TURBO_SWISSTABLE_ASSERT(!is_single_group(capacity));
  KUMO_ASSUME(capacity > kMaxSmallCapacity);
  ...
  for (size_t i = 0; i != capacity;
    ...
    TURBO_SWISSTABLE_ASSERT(slot_ptr == SlotAddress(slot_array, i, slot_size));
    if (IsEmpty(ctrl[i])) {
      tmp_space_id = i;
      continue;
    }
    ...
    if (KUMO_LIKELY(probe_index(new_i) == probe_index(i))) {
    ...
    if (IsEmpty(ctrl[new_i])) {
      ...
      TURBO_SWISSTABLE_ASSERT(IsDeleted(ctrl[new_i]));
      SetCtrlInLargeTable(common, new_i, h2, slot_size);
...
void ResetCtrl(CommonFields& common, size_t slot_size,
  ...
  TURBO_SWISSTABLE_ASSERT(IsCapacityValidForBlockedElements(common.capacity()) ||
                         blocked_element_count == 0);
  const size_t capacity = common.capacity();
  ctrl_t* ctrl = common.control();
  static constexpr size_t kTwoGroupCapacity = 2 * Group::kWidth - 1;
  if (KUMO_LIKELY(capacity <= kTwoGroupCapacity)) {
...
KUMO_ATTRIBUTE_ALWAYS_INLINE inline void InitializeThreeElementsControlBytes(
  ...
  static_assert(SooSlotIndex() == 1);
  TURBO_SWISSTABLE_ASSERT(new_offset == 0 || new_offset == 2);
  ...
  turbo::little_endian::Store64(new_ctrl, first_ctrl_bytes);
...
void ClearBackingArrayNoReuse(CommonFields& c,
                              ...
                              void* alloc) {
  TURBO_SWISSTABLE_ASSERT(c.capacity() > policy.soo_capacity());
  (*policy.dealloc)(alloc, c.capacity(), c.control(), policy.slot_size,
                    policy.slot_align, c.blocked_element_count());
...
void EraseMetaOnlyLarge(CommonFields& c, size_t index, size_t slot_size) {
  TURBO_SWISSTABLE_ASSERT(!c.is_small());
  TURBO_SWISSTABLE_ASSERT(IsFull(c.control()[index]) &&
...
void ClearBackingArray(CommonFields& c,
                       ...
                       bool reuse) {
  TURBO_SWISSTABLE_ASSERT(c.capacity() > kMaxSmallCapacity);
  if (reuse) {
    ...
    TURBO_SWISSTABLE_ASSERT(c.blocked_element_count() == blocked_element_count);
  } else {
...
void DestroySlots(CommonFields& c, size_t slot_size,
  ...
  TURBO_SWISSTABLE_ASSERT(!c.is_small());
  TURBO_SWISSTABLE_ASSERT(destroy_slot != nullptr);
...
void DeallocBackingArray(CommonFields& c, size_t slot_size, size_t slot_align,
  ...
  const size_t cap = c.capacity();
  dealloc(alloc, cap, c.control(), slot_size, slot_align,
...
void DestructSoo(CommonFields& c, size_t slot_size, size_t slot_align,
                 ...
                 void* alloc) {
  TURBO_SWISSTABLE_ASSERT(!c.is_small() || !c.empty());
  if (c.is_small()) {
    TURBO_SWISSTABLE_ASSERT(destroy_slot != nullptr);
...
void DestructNonSoo(CommonFields& c, size_t slot_size, size_t slot_align,
                    DestroySlotFn destroy_slot, DeallocBackingArrayFn dealloc,
                    void* alloc) {
  TURBO_SWISSTABLE_ASSERT(c.capacity() > 0);
  if (destroy_slot != nullptr) {
    if (c.is_small()) {
      if (!c.empty()) {
...
void ResizeEmptyNonAllocatedTableImpl(CommonFields& common,
                                      ...
                                      size_t new_capacity,
                                      size_t blocked_element_count) {
  TURBO_SWISSTABLE_ASSERT(IsValidCapacity(new_capacity));
  TURBO_SWISSTABLE_ASSERT(new_capacity > policy.soo_capacity());
  TURBO_SWISSTABLE_ASSERT(common.capacity() == policy.soo_capacity());
  TURBO_SWISSTABLE_ASSERT(common.empty());
  const size_t slot_size = policy.slot_size;
  ...
  const auto [new_ctrl, new_slots] = AllocBackingArray(
      common, policy, new_capacity, alloc, blocked_element_count);
...
void InsertOldSooSlotAndInitializeControlBytes(
    ...
    void* new_slots) {
  TURBO_SWISSTABLE_ASSERT(c.size() == policy.soo_capacity());
  TURBO_SWISSTABLE_ASSERT(policy.soo_enabled);
...
void AssertFullSoo([[maybe_unused]] CommonFields& common,
  ...
  TURBO_SWISSTABLE_ASSERT(common.size() == policy.soo_capacity());
}
...
void ResizeFullSooTable(CommonFields& common,
  ...
  const auto [new_ctrl, new_slots] =
      AllocBackingArray(common, policy, new_capacity, alloc,
                        /*blocked_element_count=*/0);
...
void GrowIntoSingleGroupShuffleControlBytes(ctrl_t* __restrict old_ctrl,
                                            ...
                                            size_t new_capacity) {
  TURBO_SWISSTABLE_ASSERT(is_single_group(new_capacity));
  constexpr size_t kHalfWidth = Group::kWidth / 2;
  KUMO_ASSUME(old_capacity < kHalfWidth);
  KUMO_ASSUME(old_capacity > 0);
  ...
  uint64_t copied_bytes = turbo::little_endian::Load64(old_ctrl + old_capacity);
  ...
  if (KUMO_UNLIKELY(old_blocked_element_count > 0)) {
  ...
  if (Group::kWidth == 8) {
    ...
    TURBO_SWISSTABLE_ASSERT(old_capacity < 8 &&
                           "old_capacity is too large for group size 8");
    turbo::little_endian::Store64(new_ctrl, copied_bytes);
    ...
    turbo::little_endian::Store64(new_ctrl + new_capacity, copied_bytes);
  ...
  TURBO_SWISSTABLE_ASSERT(Group::kWidth == 16);  // NOLINT(misc-static-assert)
  ...
  turbo::little_endian::Store64(new_ctrl + new_capacity + 1, copied_bytes);
...
KUMO_ATTRIBUTE_NOINLINE size_t DecodeAndInsertImpl(
  ...
  for (; start < end; ++start) {
    ...
    const size_t new_i = target.offset;
    TURBO_SWISSTABLE_ASSERT(old_index < new_capacity.capacity() / 2);
    TURBO_SWISSTABLE_ASSERT(new_i < new_capacity.capacity());
    TURBO_SWISSTABLE_ASSERT(IsEmpty(new_ctrl[new_i]));
...
KUMO_ATTRIBUTE_NOINLINE size_t ProcessProbedMarkedElements(
...
class ProbedItemEncoder {
 public:
  ...
  void EncodeItem(ProbedItem item) {
    if (KUMO_UNLIKELY(!kGuaranteedFitToBuffer && pos_ >= end_)) {
      return ProcessEncodeWithOverflow(item);
    }
    TURBO_SWISSTABLE_ASSERT(pos_ < end_);
    *pos_ = item;
    ++pos_;
  }
 ...
 private:
  ...
  ProbedItem* OverflowBufferStart() const {
    TURBO_SWISSTABLE_ASSERT(!kGuaranteedFitToBuffer &&
    ...
    return AlignToNextItem(
        control_ - MetadataBeforeControlSize(
                                 NextCapacity(kMaxLocalBufferOldCapacity)));
  }
  ...
  // element.
  KUMO_ATTRIBUTE_NOINLINE void ProcessEncodeWithOverflow(ProbedItem item) {
    if (!local_buffer_full_) {
    ...
    // buffer.
    if (KUMO_UNLIKELY(marked_elements_starting_position_ !=
  ...
  // Finds new position for each element and transfers it to the new slots.
  // Returns the total probe length.
  KUMO_ATTRIBUTE_NOINLINE size_t DecodeAndInsertToTableOverflow(
      CommonFields& common, const PolicyFunctions& __restrict policy,
      void* old_slots) const {
    TURBO_SWISSTABLE_ASSERT(local_buffer_full_ &&
                           "must not be called when local buffer is not full");
    size_t total_probe_length = DecodeAndInsertImpl(
        common, policy, OverflowBufferStart(), pos_, old_slots);
    if (KUMO_LIKELY(marked_elements_starting_position_ ==
                          kNoMarkedElementsSentinel)) {
      return total_probe_length;
    }
    total_probe_length +=
...
size_t GrowToNextCapacity(CommonFields& common,
  ...
  TURBO_SWISSTABLE_ASSERT(common.capacity() <= ProbedItem::kMaxNewCapacity);
...
size_t GrowToNextCapacityThatFitsInLocalBuffer(
    CommonFields& common, const PolicyFunctions& __restrict policy,
    ctrl_t* old_ctrl, void* old_slots) {
  TURBO_SWISSTABLE_ASSERT(common.capacity() <= kMaxLocalBufferNewCapacity);
...
size_t GrowToNextCapacityOverflowLocalBuffer(
  ...
  if (KUMO_LIKELY(new_capacity <= ProbedItem4Bytes::kMaxNewCapacity)) {
    return GrowToNextCapacity4BytesEncoder(common, policy, old_ctrl, old_slots);
  }
  if (KUMO_LIKELY(new_capacity <= ProbedItem8Bytes::kMaxNewCapacity)) {
...
size_t GrowToNextCapacityDispatch(CommonFields& common,
                                  ...
                                  ctrl_t* old_ctrl, void* old_slots) {
  const size_t new_capacity = common.capacity();
  if (KUMO_LIKELY(new_capacity <= kMaxLocalBufferNewCapacity)) {
...
void IncrementSmallSizeNonSoo(CommonFields& common,
                              const PolicyFunctions& __restrict policy) {
  TURBO_SWISSTABLE_ASSERT(common.is_small());
  common.increment_size();
...
void* Grow1To3AndPrepareInsert(CommonFields& common,
  ...
  TURBO_SWISSTABLE_ASSERT(common.capacity() == 1);
  TURBO_SWISSTABLE_ASSERT(!common.empty());
  TURBO_SWISSTABLE_ASSERT(!policy.soo_enabled);
  // 1-element tables can't have any blocked elements.
  TURBO_SWISSTABLE_ASSERT(common.blocked_element_count() == 0);
  ...
  const auto [new_ctrl, new_slots] =
      AllocBackingArray(common, policy, kNewCapacity, alloc,
                        /*blocked_element_count=*/0);
  common.set_control(new_ctrl);
  ...
  policy.dealloc(alloc, kOldCapacity,
                 ...
                 /*blocked_element_count=*/0);
  PrepareInsertCommon(common);
  TURBO_SWISSTABLE_ASSERT(common.size() == 2);
...
void* GrowToNextCapacityAndPrepareInsert(
  ...
  TURBO_SWISSTABLE_ASSERT(old_capacity > policy.soo_capacity());
  TURBO_SWISSTABLE_ASSERT(!IsSmallCapacity(old_capacity));
  KUMO_ASSUME(old_capacity > kMaxSmallCapacity);
  ...
  h2_t new_h2 = H2(new_hash);
  FindInfo find_info;
  if (KUMO_LIKELY(is_single_group(new_capacity))) {
    size_t offset;
    ...
    TURBO_SWISSTABLE_ASSERT(IsEmpty(new_ctrl[offset]));
    ...
    TURBO_SWISSTABLE_ASSERT(common.size() + old_blocked_element_count ==
                           old_capacity);
...
void* PrepareInsertSmallNonSoo(CommonFields& common,
  ...
  TURBO_SWISSTABLE_ASSERT(common.capacity() == 0);
...
KUMO_ATTRIBUTE_NOINLINE void ReserveAllocatedTable(
    CommonFields& common, const PolicyFunctions& __restrict policy,
    size_t new_size) {
  const size_t cap = common.capacity();
  ValidateMaxSize(new_size, policy.key_size, policy.slot_size);
  ...
  if (cap == policy.soo_capacity()) {
    ...
    TURBO_SWISSTABLE_ASSERT(cap > policy.soo_capacity());
...
void ResizeAllocatedTableWithSeedChange(
  ...
  TURBO_SWISSTABLE_ASSERT(old_capacity > 0);
  FindNewPositionsAndTransferSlots(common, policy, old_ctrl, old_slots,
                                   old_capacity);
  (*policy.dealloc)(alloc, old_capacity, old_ctrl, slot_size, slot_align,
                    old_blocked_element_count);
  if (GrowthInfoSizeForCapacity(new_capacity) > 0) {
...
void* GrowSooTableToNextCapacityAndPrepareInsert(
  ...
  // SOO data.
  const auto [new_ctrl, new_slots] = AllocBackingArray(
      common, policy, kNewCapacity, alloc,
      /*blocked_element_count=*/0);
  ...
  if constexpr (TransferUsesMemcpy) {
    ...
    TURBO_SWISSTABLE_ASSERT(SooSlotMemcpySize <= 2 * slot_size);
    TURBO_SWISSTABLE_ASSERT(SooSlotMemcpySize >= slot_size);
...
void Rehash(CommonFields& common, const PolicyFunctions& __restrict policy,
  ...
  if (n == 0) {
    ...
    if (common.size() <= policy.soo_capacity()) {
      TURBO_SWISSTABLE_ASSERT(slot_size <= sizeof(HeapOrSoo));
      TURBO_SWISSTABLE_ASSERT(policy.slot_align <= alignof(HeapOrSoo));
      HeapOrSoo tmp_slot;
...
void Copy(CommonFields& common, const PolicyFunctions& __restrict policy,
  ...
  TURBO_SWISSTABLE_ASSERT(size > 0);
  ...
  const size_t blocked_element_count = common.blocked_element_count();
  TURBO_SWISSTABLE_ASSERT(other.capacity() > soo_capacity);
  const size_t cap = common.capacity();
  TURBO_SWISSTABLE_ASSERT(cap > soo_capacity);
  KUMO_ASSUME(cap > kMaxSmallCapacity);
  size_t offset = cap;
  const void* hash_fn = policy.hash_fn(common);
  auto hasher = policy.hash_slot;
...
void ReserveTableToFitNewSize(CommonFields& common,
  ...
  TURBO_SWISSTABLE_ASSERT(new_size > policy.soo_capacity());
  const size_t cap = common.capacity();
  if (KUMO_LIKELY(common.empty() && cap <= policy.soo_capacity())) {
    return ReserveEmptyNonAllocatedTableToFitNewSize(common, policy, new_size);
  }

  TURBO_SWISSTABLE_ASSERT(!common.empty() || cap > policy.soo_capacity());
  TURBO_SWISSTABLE_ASSERT(cap > 0);
...
void* PrepareInsertLarge(CommonFields& common,
  ...
  TURBO_SWISSTABLE_ASSERT(!SwisstableGenerationsEnabled());
