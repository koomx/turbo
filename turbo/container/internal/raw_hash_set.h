...
#include <turbo/container/internal/common.h>  // IWYU pragma: export // for node_handle
...
#include <turbo/container/internal/hashtable_control_bytes.h>
...
#elif (KUMO_HAVE_ADDRESS_SANITIZER ||   \
...
#ifdef TURBO_SWISSTABLE_ASSERT
#error TURBO_SWISSTABLE_ASSERT cannot be directly set
#else
// We use this macro for assertions that users may see when the table is in an
// invalid state that sanitizers may help diagnose.
#define TURBO_SWISSTABLE_ASSERT(CONDITION) \
...
KUMO_DLL extern char kDefaultIterSlot;
...
KUMO_DLL extern const ctrl_t kSooControl[2];
...
KUMO_DLL extern const ctrl_t kInsertIteratorControl[2];
...
constexpr size_t NextCapacity(size_t n) {
  TURBO_SWISSTABLE_ASSERT(IsValidCapacity(n) || n == 0);
  return n * 2 + 1;
}

// Returns the previous valid capacity before `n`.
constexpr size_t PreviousCapacity(size_t n) {
  TURBO_SWISSTABLE_ASSERT(IsValidCapacity(n));
  return n / 2;
}
...
constexpr size_t CapacityToGrowth(size_t capacity) {
  TURBO_SWISSTABLE_ASSERT(IsValidCapacity(capacity));
  // `capacity*7/8`
...
constexpr size_t SizeToCapacity(size_t size) {
  ...
  int leading_zeros = turbo::countl_zero(size);
...
class HashtableCapacityImpl {
 ...
 public:
  ...
  explicit constexpr HashtableCapacityImpl(size_t capacity)
    ...
    TURBO_SWISSTABLE_ASSERT(capacity == 0 || IsValidCapacity(capacity));
  }
  ...
  constexpr size_t capacity() const {
    TURBO_SWISSTABLE_ASSERT(IsValid());
 ...
 private:
  ...
  explicit constexpr HashtableCapacityImpl(InvalidCapacity capacity)
    ...
    TURBO_SWISSTABLE_ASSERT(capacity_data_ > kAboveMaxValidCapacity);
  }
...
class PerTableSeedImpl {
 public:
  ...
  // zero-extended move instead of mov+and). When turbo::Hash is inlined, it can
...
// 1) kBlockedElementsBitCount bits that stores number of blocked elements in
//    the table.
...
class HashtableInlineDataImpl {
 public:
  ...
  static constexpr size_t kSizeBitCount =
      64 -
      (kBlockedElementBitCount + PerTableSeed::kBitCount +
  ...
  void init_blocked_element_count(uint64_t count) {
    TURBO_SWISSTABLE_ASSERT(blocked_element_count() == 0);
    TURBO_SWISSTABLE_ASSERT(count <= kMaxBlockedElementCount);
 ...
 private:
  ...
  static constexpr size_t kDataBitCount =
      PerTableSeed::kBitCount + kSizeBitCount + kBlockedElementBitCount;
  ...
  static constexpr uint64_t kBlockedElementsShift = PerTableSeed::kBitCount;
...
#ifndef TURBO_SWISSTABLE_INTERNAL_ENABLE_CAPACITY_BY_VALUE
...
#endif  // TURBO_SWISSTABLE_INTERNAL_ENABLE_CAPACITY_BY_VALUE
...
class GrowthInfoAccessor {
 public:
  ...
  void OverwriteEmptyAsFull() {
    TURBO_SWISSTABLE_ASSERT(GetGrowthLeftLowerBound() > 0);
  ...
  void OverwriteControlAsFull(ctrl_t ctrl) {
    TURBO_SWISSTABLE_ASSERT(GetGrowthLeftLowerBound() >=
...
// Computes the size of the metadata before the control bytes. growth_info and
// generation are stored at the beginning of the backing array.
constexpr size_t MetadataBeforeControlSize(size_t capacity) {
...
class RawHashSetLayout {
 public:
  explicit RawHashSetLayout(size_t capacity, size_t slot_size,
      ...
      : control_offset_(MetadataBeforeControlSize(capacity)),
    ...
    TURBO_SWISSTABLE_ASSERT(!IsSmallCapacity(capacity) ||
...
class CommonFields : public CommonFieldsGenerationInfo {
 public:
  ...
  ctrl_t* control() const {
    TURBO_SWISSTABLE_ASSERT(capacity() > 0);
    ...
    KUMO_ASSUME(reinterpret_cast<uintptr_t>(ctrl + num_control_bytes) <=
  ...
  // KUMO_ASSUME conditions. We require passing it explicitly to eliminate
  // branches inside of NumControlBytes in majority of cases.
  void* slot_array(size_t capacity) const {
    TURBO_SWISSTABLE_ASSERT(capacity == this->capacity());
    ctrl_t* ctrl = control();
  ...
  // Sets the size to zero, but keeps seed/metadata bits.
  ...
  void increment_size() {
    TURBO_SWISSTABLE_ASSERT(size() < capacity());
    inline_data_.increment_size();
  }
  void increment_size(size_t n) {
    TURBO_SWISSTABLE_ASSERT(size() + n <= capacity());
    inline_data_.increment_size(n);
  }
  void decrement_size() {
    TURBO_SWISSTABLE_ASSERT(!empty());
  ...
  void generate_new_seed() { inline_data_.generate_new_seed(); }
  ...
  HashtableCapacity capacity_impl() const {
    ...
    TURBO_SWISSTABLE_ASSERT(cap.IsValid());
    return cap;
  }
  ...
  GrowthInfoAccessor growth_info() const {
    TURBO_SWISSTABLE_ASSERT(GrowthInfoSizeForCapacity(capacity()) > 0);
 ...
 private:
  ...
  void AssertInSooMode() const {
    TURBO_SWISSTABLE_ASSERT(capacity() == SooCapacity());
  }
...
size_t SelectReservationSizeForIterRange(InputIter first, InputIter last,
  ...
  if (meta_internal::IsAtLeastIterator<std::random_access_iterator_tag,
                                       InputIter>()) {
...
#if defined(TURBO_SWISSTABLE_ENABLE_GENERATIONS) || \
    KUMO_OPTION_HARDENED == 1 || !defined(NDEBUG)
  return true;
#else
  return false;
#endif
...
inline void AssertIsFull(const ctrl_t* const& ctrl, const void* slot,
  ...
  if (SwisstableGenerationsEnabled()) {
    ...
    if (KUMO_UNLIKELY(!IsFull(CrashIfIteratorIsInvalid(ctrl)))) {
      TURBO_RAW_LOG(
          FATAL,
...
inline void AssertIsValidForComparison(const ctrl_t* const& ctrl,
  ...
  if (SwisstableGenerationsEnabled()) {
    ...
    if (KUMO_UNLIKELY(!ctrl_is_valid_for_comparison)) {
      TURBO_RAW_LOG(
...
inline void AssertSameContainer(const ctrl_t* const& ctrl_a,
  ...
  if (SwisstableGenerationsEnabled()) {
    if (KUMO_LIKELY(generation_ptr_a == generation_ptr_b)) return;
...
inline size_t TryFindNewIndexWithoutProbing(size_t h1, size_t old_index,
  ...
  if (KUMO_LIKELY((index_diff & old_capacity) == 0)) {
    size_t new_index = (h1 + in_floating_group_index) & new_capacity;
    KUMO_ASSUME(new_index != kProbedElementIndexSentinel);
    return new_index;
  }
  TURBO_SWISSTABLE_ASSERT(((old_index - h1) & old_capacity) >= Group::kWidth);
  ...
  if (KUMO_UNLIKELY((h1 & old_capacity) >= old_index)) {
    return kProbedElementIndexSentinel;
  }
  size_t offset = h1 & new_capacity;
  Group new_g(new_ctrl + offset);
  if (auto mask = new_g.MaskNonFull(); KUMO_LIKELY(mask)) {
    size_t result = offset + mask.LowestBitSet();
    KUMO_ASSUME(result != kProbedElementIndexSentinel);
    return result;
  }
  return kProbedElementIndexSentinel;
}
...
void IterateOverFullSlots(const CommonFields& c, size_t slot_size,
                          turbo::FunctionRef<void(const ctrl_t*, void*)> cb);
...
void DeallocateBackingArray(void* alloc, size_t capacity, ctrl_t* ctrl,
                            ...
                            size_t blocked_element_count) {
  RawHashSetLayout layout(capacity, slot_size, slot_align,
                          blocked_element_count);
...
struct PolicyFunctions {
  ...
  void (*transfer_unprobed_elements_to_next_capacity)(
      ...
      // TODO(b/382423690): Try to use turbo::FunctionRef here.
      void* probed_storage,
...
// we KUMO_ASSUME that the size is less than MaxValidSize, which can enable
...
void Copy(CommonFields& common, const PolicyFunctions& policy,
          ...
          turbo::FunctionRef<void(void*, const void*)> copy_fn);
...
void* GrowSooTableToNextCapacityAndPrepareInsert(
    ...
    turbo::FunctionRef<size_t(size_t)> get_hash);
...
KUMO_ATTRIBUTE_NOINLINE void TransferNRelocatable(void*, void* dst, void* src,
                                                  size_t count) {
...
void* PrepareInsertLargeGenerationsEnabled(
    ...
    turbo::FunctionRef<size_t(size_t)> recompute_hash);
...
class raw_hash_set {
 ...
 private:
  ...
  constexpr static bool kIsDefaultHash =
      std::is_same_v<hasher, turbo::Hash<key_type>> ||
      std::is_same_v<hasher, turbo::container_internal::StringHash>;
  ...
  using key_hash_result =
      turbo::remove_cvref_t<decltype(std::declval<const Hash&>()(
  ...
  using Insertable = std::disjunction<
      std::is_same<turbo::remove_cvref_t<reference>, turbo::remove_cvref_t<T>>,
 ...
 public:
  ...
  class iterator : private HashSetIteratorGenerationInfo {
   ...
   public:
    ...
    iterator& operator++() {
      ...
      if (KUMO_UNLIKELY(*ctrl_ == ctrl_t::kSentinel)) slot_ = nullptr;
      return *this;
    }
    // PRECONDITION: not an end() iterator.
   ...
   private:
    ...
    // An equality check which skips TURBO Hardening iterator invalidation
    // checks.
    ...
    // Dereferences the iterator without TURBO Hardening iterator invalidation
    // checks.
  ...
  class const_iterator {
    ...
    friend struct turbo::container_internal::hashtable_debug_internal::
  ...
  //   // copies the strings into the set.
  //   turbo::flat_hash_set<std::string> s = {"abc", "def"};
  //
  ...
  KUMO_ATTRIBUTE_NOINLINE raw_hash_set(raw_hash_set&& that) noexcept(
  ...
  raw_hash_set& operator=(const raw_hash_set& that) {
    that.AssertNotDebugCapacity();
    if (KUMO_UNLIKELY(this == &that)) return *this;
  ...
  iterator begin() KUMO_ATTRIBUTE_LIFETIME_BOUND {
    if (KUMO_UNLIKELY(empty())) return end();
    if (is_small()) return single_iterator();
    iterator it = {control(), slot_array(capacity()),
                   common().generation_ptr()};
    it.skip_empty_or_deleted();
    TURBO_SWISSTABLE_ASSERT(IsFull(*it.control()));
    return it;
  }
  iterator end() KUMO_ATTRIBUTE_LIFETIME_BOUND {
    AssertNotDebugCapacity();
    return iterator(common().generation_ptr());
  }

  const_iterator begin() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
    return const_cast<raw_hash_set*>(this)->begin();
  }
  const_iterator end() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
    return const_cast<raw_hash_set*>(this)->end();
  }
  const_iterator cbegin() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
    return begin();
  }
  const_iterator cend() const KUMO_ATTRIBUTE_LIFETIME_BOUND { return end(); }
  ...
  size_t size() const {
    ...
    KUMO_ASSUME(size <= kMaxValidSize);
    return size;
  }
  size_t capacity() const {
    ...
    KUMO_ASSUME(kIsValid || cap == 0);
    KUMO_ASSUME(cap >= kDefaultCapacity);
    KUMO_ASSUME(cap <= kMaxValidCapacity);
    return cap;
  }
  ...
  template <class T,
            int = std::enable_if_t<IsDecomposableAndInsertable<T>::value &&
                                   ...
                                   int>()>
  std::pair<iterator, bool> insert(T&& value) KUMO_ATTRIBUTE_LIFETIME_BOUND {
  ...
  std::pair<iterator, bool> insert(
      T&& value KUMO_INTERNAL_ATTRIBUTE_CAPTURED_BY_THIS)
  ...
  template <class T, int&...,
            std::enable_if_t<IsDecomposableAndInsertable<const T&>::value &&
                             ...
                             int> = 0>
  std::pair<iterator, bool> insert(
      const T& value KUMO_INTERNAL_ATTRIBUTE_CAPTURED_BY_THIS)
      KUMO_ATTRIBUTE_LIFETIME_BOUND {
...
#if KUMO_CPLUSPLUS_LANG >= 202002L
  std::pair<iterator, bool> insert(
      init_type&& value KUMO_INTERNAL_ATTRIBUTE_CAPTURED_BY_THIS)
      KUMO_ATTRIBUTE_LIFETIME_BOUND
    requires(IsLifetimeBoundAssignmentFrom<init_type>::value)
  {
    return emplace(std::move(value));
  ...
  template <class T,
            int = std::enable_if_t<IsDecomposableAndInsertable<T>::value &&
                                       ...
                                       !IsLifetimeBoundAssignmentFrom<T>::value,
                                   int>()>
  iterator insert(const_iterator, T&& value) KUMO_ATTRIBUTE_LIFETIME_BOUND {
  ...
  iterator insert(const_iterator,
                  const T& value) KUMO_ATTRIBUTE_LIFETIME_BOUND {
    return insert(value).first;
  }

  iterator insert(const_iterator,
                  init_type&& value) KUMO_ATTRIBUTE_LIFETIME_BOUND {
  ...
  insert_return_type insert(node_type&& node) KUMO_ATTRIBUTE_LIFETIME_BOUND {
  ...
  iterator insert(const_iterator,
                  node_type&& node) KUMO_ATTRIBUTE_LIFETIME_BOUND {
  ...
  iterator erase(const_iterator first,
                 const_iterator last) KUMO_ATTRIBUTE_LIFETIME_BOUND {
    AssertNotDebugCapacity();
  ...
  void reserve(size_t n) {
    if (KUMO_LIKELY(n > DefaultCapacity())) {
  ...
  iterator find(const key_arg<K>& key,
                size_t) KUMO_ATTRIBUTE_LIFETIME_BOUND {
    return find(key);
  }
  ...
  template <class K = key_type>
  iterator find(const key_arg<K>& key) KUMO_ATTRIBUTE_LIFETIME_BOUND {
    AssertOnFind(key);
  ...
  const_iterator find(const key_arg<K>& key,
                      size_t) const KUMO_ATTRIBUTE_LIFETIME_BOUND {
    return find(key);
  }
  template <class K = key_type>
  ...
  std::pair<const_iterator, const_iterator> equal_range(
      const key_arg<K>& key) const KUMO_ATTRIBUTE_LIFETIME_BOUND {
    auto it = find(key);
    if (it != end()) return {it, std::next(it)};
    return {it, it};
  }
 ...
 private:
  ...
  friend struct turbo::container_internal::HashtableFreeFunctionsAccess;
  ...
  // SOO functionality.
  template <class K = key_type>
  KUMO_ATTRIBUTE_ALWAYS_INLINE iterator find_small(const key_arg<K>& key) {
    TURBO_SWISSTABLE_ASSERT(is_small());
  ...
  iterator find_large(const key_arg<K>& key) {
    TURBO_SWISSTABLE_ASSERT(!is_small());
    const size_t cap = common().capacity();
    KUMO_ASSUME(cap > kMaxSmallCapacity);
    ...
    while (true) {
      ...
      for (uint32_t i : g.Match(h2)) {
        ...
        if (KUMO_LIKELY(equal_to(key, slot_array + offset)))
          return iterator_at_ptr(ctrl + offset, slot_array + offset);
      }
      if (KUMO_LIKELY(g.MaskEmpty())) return end();
      seq.next();
      TURBO_SWISSTABLE_ASSERT(seq.index() <= cap && "full table!");
    }
  }

  void clear_backing_array(bool reuse) {
    TURBO_SWISSTABLE_ASSERT(capacity() > kMaxSmallCapacity);
  ...
  void dealloc() {
    TURBO_SWISSTABLE_ASSERT(capacity() > DefaultCapacity());
  ...
  static KUMO_ATTRIBUTE_ALWAYS_INLINE slot_type* to_slot(void* buf) {
  ...
  template <class K>
  KUMO_ATTRIBUTE_ALWAYS_INLINE std::pair<slot_type*, bool>
  find_or_prepare_insert_soo(const K& key) {
    TURBO_SWISSTABLE_ASSERT(is_soo());
    slot_type* slot = single_slot();
    if (empty()) {
      common().set_full_soo();
      return {slot, true};
    }
    if (equal_to(key, slot)) {
      return {slot, false};
    }
    TURBO_SWISSTABLE_ASSERT(capacity() == 1);
  ...
  std::pair<slot_type*, bool> find_or_prepare_insert_large(const K& key) {
    ...
    while (true) {
      ...
      for (uint32_t i : g.Match(h2)) {
        ...
        if (KUMO_LIKELY(equal_to(key, slot))) {
          return {slot, false};
        }
      }
      auto mask_empty = g.MaskEmpty();
      if (KUMO_LIKELY(mask_empty)) {
      ...
      seq.next();
      TURBO_SWISSTABLE_ASSERT(seq.index() <= capacity() && "full table!");
    }
  }
 ...
 protected:
  ...
  void AssertHashEqConsistent(const K& key) {
    ...
    if (std::is_same_v<hasher, turbo::container_internal::StringHash> &&
        std::is_same_v<key_equal, turbo::container_internal::StringEq>) {
      return;
    }
  ...
  iterator iterator_at(size_t i) KUMO_ATTRIBUTE_LIFETIME_BOUND {
    return {control() + i, slot_array() + i, common().generation_ptr()};
  }
  const_iterator iterator_at(size_t i) const KUMO_ATTRIBUTE_LIFETIME_BOUND {
 ...
 private:
  ...
  void prefetch_heap_block() const {
    TURBO_SWISSTABLE_ASSERT(!is_soo());
#if KUMO_HAVE_BUILTIN(__builtin_prefetch) || defined(__GNUC__)
  ...
  static void transfer_unprobed_elements_to_next_capacity_fn(
    ...
    KUMO_ASSUME(new_capacity > kMaxSmallCapacity);
    const size_t old_capacity = PreviousCapacity(new_capacity);
    KUMO_ASSUME(old_capacity + 1 >= Group::kWidth);
    KUMO_ASSUME((old_capacity + 1) % Group::kWidth == 0);
    ...
    for (size_t group_index = 0; group_index < old_capacity;
      ...
      for (auto in_fixed_group_index : old_g.MaskFull()) {
        ...
        if (KUMO_UNLIKELY(new_index == kProbedElementIndexSentinel)) {
          encode_probed_element(probed_storage, h2, old_index, h1);
          continue;
        }
        TURBO_SWISSTABLE_ASSERT((new_index & old_capacity) <= old_index);
        TURBO_SWISSTABLE_ASSERT(IsEmpty(new_ctrl[new_index]));
  ...
  static const PolicyFunctions& GetPolicyFunctions() {
    static_assert(sizeof(slot_type) <= (std::numeric_limits<uint32_t>::max)(),
                  ...
                  "or use turbo::node_hash_{map,set}.");
  ...
  turbo::CompressedTuple<CommonFields, hasher, key_equal,
                                            CharAlloc>
...
struct HashtableFreeFunctionsAccess {
  ...
  static typename Set::size_type EraseIf(Predicate& pred, Set* c) {
    ...
    if (c->is_small()) {
      ...
      if (!pred(*it)) {
        TURBO_SWISSTABLE_ASSERT(c->size() == 1 &&
    ...
    TURBO_SWISSTABLE_ASSERT(original_size_for_assert - num_deleted ==
                               c->size() &&
                           "hash table was modified unexpectedly");
    return num_deleted;
  }
...
extern template void* GrowSooTableToNextCapacityAndPrepareInsert<0, false>(
    CommonFields&, const PolicyFunctions&, turbo::FunctionRef<size_t(size_t)>);
...
extern template void DeallocateBackingArray<
    ...
    size_t slot_align, size_t blocked_element_count);
