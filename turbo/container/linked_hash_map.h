...
#ifndef TURBO_CONTAINER_LINKED_HASH_MAP_H_
#define TURBO_CONTAINER_LINKED_HASH_MAP_H_
...
template <typename Key, typename Value,
          typename KeyHash = typename turbo::flat_hash_set<Key>::hasher,
          typename KeyEq =
              typename turbo::flat_hash_set<Key, KeyHash>::key_equal,
          typename Alloc = std::allocator<std::pair<const Key, Value>>>
class linked_hash_map {
  using KeyArgImpl = turbo::container_internal::KeyArg<
      turbo::container_internal::IsTransparent<KeyEq>::value &&
      turbo::container_internal::IsTransparent<KeyHash>::value>;
 ...
 private:
  ...
  using SetType =
      turbo::flat_hash_set<typename ListType::iterator, Wrapped<hasher>,
 ...
 public:
  ...
  mapped_type& at(const key_arg<K>& key) {
    auto it = find(key);
    if (KUMO_UNLIKELY(it == end())) {
      ThrowStdOutOfRange("turbo::linked_hash_map::at");
    }
    return it->second;
  }
...
#endif  // TURBO_CONTAINER_LINKED_HASH_MAP_H_
