...
// -----------------------------------------------------------------------------
//
// An `turbo::node_hash_set<T>` is an unordered associative container designed to
...
// -----------------------------------------------------------------------------
//
// An `turbo::node_hash_set<T>` is an unordered associative container which
...
//   turbo::node_hash_set<std::string> ducks =
//     {"huey", "dewey", "louie"};
//
...
class KUMO_ATTRIBUTE_OWNER node_hash_set
    : public turbo::container_internal::InstantiateRawHashSet<
          turbo::container_internal::NodeHashSetPolicy<T>, Hash, Eq,
          Alloc>::type {
 ...
 public:
  ...
  //    turbo::node_hash_set<std::string> set1;
  //
  // * Initializer List constructor
  //
  //   turbo::node_hash_set<std::string> set2 =
  //       {{"huey"}, {"dewey"}, {"louie"}};
  //
  // * Copy constructor
  //
  //   turbo::node_hash_set<std::string> set3(set2);
  //
  // * Copy assignment operator
  //
  //   // Hash functor and Comparator are copied as well
  //   turbo::node_hash_set<std::string> set4;
  //   set4 = set3;
  //
  // * Move constructor
  //
  //   // Move is guaranteed efficient
  //   turbo::node_hash_set<std::string> set5(std::move(set4));
  //
  ...
  //   turbo::node_hash_set<std::string> set6;
  //   set6 = std::move(set5);
  //
  ...
  //   turbo::node_hash_set<std::string> set7(v.begin(), v.end());
  //
  // * from_range constructor (C++23)
  //
  //   std::vector<std::string> v = {"a", "b"};
  //   turbo::node_hash_set<std::string> set8(std::from_range, v);
  node_hash_set() {}
  using Base::Base;
  ...
  //
  // NOTE: this member function is particular to `turbo::node_hash_set` and is
...
template <class T>
struct NodeHashSetPolicy
    : turbo::container_internal::node_slot_policy<T&, NodeHashSetPolicy<T>> {
  using key_type = T;
  using init_type = T;
...
struct IsUnorderedContainer<turbo::node_hash_set<Key, Hash, KeyEqual, Allocator>>
    : std::true_type {};
