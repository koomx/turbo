...
namespace turbo {
    ...
    static void dump_node(CordRep* turbo_nonnull nonnull_rep, bool include_data,
        std::ostream* turbo_nonnull os, int indent = 0);
    static bool verify_node(CordRep* turbo_nonnull root,
        CordRep* turbo_nonnull start_node);

    static inline CordRep* turbo_nullable VerifyTree(CordRep* turbo_nullable node) {
        assert(node == nullptr || verify_node(node, node));
        static_cast<void>(&verify_node);
        return node;
    }

    static CordRepFlat* turbo_nonnull CreateFlat(const char* turbo_nonnull data,
    ...
    static CordRep* turbo_nonnull NewBtree(const char* turbo_nonnull data,
        size_t length, size_t alloc_hint) {
    ...
    // The returned node has a refcount of 1.
    static CordRep* turbo_nullable NewTree(const char* turbo_nullable data,
        size_t length, size_t alloc_hint) {
        if (length == 0)
            return nullptr;
    ...
    static CordRep* turbo_nonnull CordRepFromString(std::string&& src) {
        ...
        auto* rep = static_cast<::turbo::cord_internal::CordRepExternalImpl<StringReleaser>*>(
    ...
    static CordRepBtree* turbo_nonnull ForceBtree(CordRep* rep) {
        return rep->IsBtree()
            ? rep->btree()
            : CordRepBtree::Create(cord_internal::RemoveCrcNode(rep));
    }

    void Cord::InlineRep::AppendTreeToInlined(CordRep* turbo_nonnull tree) {
        assert(!is_tree());
        if (!data_.is_empty()) {
    ...
    void Cord::InlineRep::AppendTreeToTree(CordRep* turbo_nonnull tree) {
        assert(is_tree());
        tree = CordRepBtree::Append(ForceBtree(data_.as_tree()), tree);
        SetTree(tree);
    }

    void Cord::InlineRep::AppendTree(CordRep* turbo_nonnull tree) {
        assert(tree != nullptr);
        assert(tree->length != 0);
        assert(!tree->IsCrc());
        if (data_.is_tree()) {
            AppendTreeToTree(tree);
        } else {
            AppendTreeToInlined(tree);
        }
    }

    void Cord::InlineRep::PrependTreeToInlined(CordRep* turbo_nonnull tree) {
        assert(!is_tree());
        if (!data_.is_empty()) {
            CordRepFlat* flat = MakeFlatWithExtraCapacity(0);
    ...
    void Cord::InlineRep::PrependTreeToTree(CordRep* turbo_nonnull tree) {
        assert(is_tree());
        tree = CordRepBtree::Prepend(ForceBtree(data_.as_tree()), tree);
        SetTree(tree);
    }

    void Cord::InlineRep::PrependTree(CordRep* turbo_nonnull tree) {
        assert(tree != nullptr);
        assert(tree->length != 0);
        assert(!tree->IsCrc());
        if (data_.is_tree()) {
            PrependTreeToTree(tree);
        } else {
            PrependTreeToInlined(tree);
        }
    }
    ...
    static inline bool PrepareAppendRegion(CordRep* turbo_nonnull root,
        char* turbo_nullable* turbo_nonnull region,
        size_t* turbo_nonnull size,
        size_t max_length) {
    ...
    void Cord::InlineRep::AssignSlow(const Cord::InlineRep& src) {
        ...
        if (KUMO_LIKELY(!is_tree())) {
            EmplaceTree(CordRep::Ref(src.as_tree()));
            return;
        }
    ...
    Cord& Cord::operator=(std::string_view src) {
        ...
        if (tree != nullptr) {
            if (tree->IsFlat() && tree->flat()->Capacity() >= length && tree->refcount.IsOne()) {
            ...
            contents_.SetTree(NewTree(data, length, 0));
            CordRep::Unref(tree);
        } else {
            contents_.EmplaceTree(NewTree(data, length, 0));
        }
        return *this;
    }
    ...
    void Cord::InlineRep::append_array(std::string_view src) {
        if (src.empty())
            return; // memcpy(_, nullptr, 0) is undefined.
    ...
    inline CordRep* turbo_nonnull Cord::TakeRep() const& {
        return CordRep::Ref(contents_.tree());
    }

    inline CordRep* turbo_nonnull Cord::TakeRep() && {
    ...
    inline void Cord::AppendImpl(C&& src) {
        ...
        if (src.empty())
            return;
        TURBO_RAW_CHECK(src.contents_.size() <= std::numeric_limits<size_t>::max() - contents_.size(),
        ...
        if (src_size <= kMaxBytesToCopy) {
            ...
            if (src_tree == nullptr) {
                ...
                contents_.append_array({ src.contents_.data(), src_size });
                return;
            }
            if (src_tree->IsFlat()) {
                // src tree just has one flat node.
                contents_.append_array({ src_tree->flat()->Data(), src_size });
                return;
            }
            if (&src == this) {
    ...
    static CordRep::ExtractResult ExtractAppendBuffer(CordRep* turbo_nonnull rep,
        size_t min_capacity) {
        switch (rep->tag) {
        case cord_internal::BTREE:
            return CordRepBtree::ExtractAppendBuffer(rep->btree(), min_capacity);
        default:
            if (rep->IsFlat() && rep->refcount.IsOne() && rep->flat()->Capacity() - rep->length >= min_capacity) {
                return { nullptr, rep };
            }
            return { rep, nullptr };
        }
    ...
    static CordBuffer CreateAppendBuffer(InlineData& data, size_t block_size,
        ...
        CordBuffer buffer = block_size ? CordBuffer::create_with_custom_limit(block_size, capacity)
    ...
    CordBuffer Cord::GetAppendBufferSlowPath(size_t block_size, size_t capacity,
        ...
        if (tree != nullptr) {
            ...
            if (result.extracted != nullptr) {
                contents_.SetTreeOrEmpty(result.tree);
    ...
    void Cord::PrependArray(std::string_view src) {
        contents_.MaybeRemoveEmptyCrcNode();
        if (src.empty())
            return; // memcpy(_, nullptr, 0) is undefined.
    ...
    void Cord::AppendPrecise(std::string_view src) {
        assert(!src.empty());
        ...
        if (contents_.remaining_inline_capacity() >= src.size()) {
            ...
            contents_.AppendTree(CordRepFlat::Create(src));
        }
    }

    void Cord::PrependPrecise(std::string_view src) {
        assert(!src.empty());
        assert(src.size() <= cord_internal::kMaxFlatLength);
        if (contents_.remaining_inline_capacity() >= src.size()) {
            ...
            contents_.PrependTree(CordRepFlat::Create(src));
        }
    }
    ...
    namespace {
        ...
        int CompareChunks(std::string_view* turbo_nonnull lhs,
            std::string_view* turbo_nonnull rhs,
            size_t* turbo_nonnull size_to_compare) {
    ...
    // initializing the iterator, and returns a std::string_view referencing the data.
    ...
    const crc_internal::CrcCordState* turbo_nullable Cord::MaybeGetCrcCordState()
        const {
    ...
    inline int Cord::CompareSlowPath(std::string_view rhs, size_t compared_size,
        ...
        auto advance = [](Cord::ChunkIterator* turbo_nonnull it,
                           std::string_view* turbo_nonnull chunk) {
            if (!chunk->empty())
                return true;
            ++*it;
            if (it->bytes_remaining_ == 0)
                return false;
            *chunk = **it;
            return true;
        };
        ...
        std::string_view lhs_chunk = (lhs_it.bytes_remaining_ != 0) ? *lhs_it : std::string_view();
    ...
    inline int Cord::CompareSlowPath(const Cord& rhs, size_t compared_size,
        ...
        std::string_view lhs_chunk = (lhs_it.bytes_remaining_ != 0) ? *lhs_it : std::string_view();
        std::string_view rhs_chunk = (rhs_it.bytes_remaining_ != 0) ? *rhs_it : std::string_view();
        assert(compared_size <= lhs_chunk.size());
        ...
        return static_cast<int>(rhs_chunk.empty()) - static_cast<int>(lhs_chunk.empty());
    }
    ...
    inline int shared_compare_impl(const Cord& lhs, const RHS& rhs) {
    ...
    int Cord::compare(std::string_view rhs) const {
        return shared_compare_impl(*this, rhs);
    }

    int Cord::compare_impl(const Cord& rhs) const {
        return shared_compare_impl(*this, rhs);
    }
    ...
    size_t copy_cord_to_span(const Cord& src, turbo::Span<char> dst) {
        if (src.size() <= dst.size()) {
            src.copy_to_array_impl(dst.data());
            return src.size();
        }
    ...
    Cord Cord::ChunkIterator::advance_and_read_bytes(size_t n) {
        ...
        if (btree_reader_) {
            ...
            if (n <= chunk_size && n <= kMaxBytesToCopy) {
                ...
                subcord.contents_.EmplaceTree(rep);
            }
            bytes_remaining_ -= n;
            return subcord;
        }
        ...
        if (n == current_leaf_->length) {
            ...
            subcord.contents_.EmplaceTree(VerifyTree(tree));
            return subcord;
        }
    ...
    namespace {
        ...
        bool IsSubstringInCordAt(turbo::Cord::CharIterator position,
            ...
            auto haystack_chunk = turbo::Cord::chunk_remaining(position);
            while (true) {
                // Precondition is that `turbo::Cord::chunk_remaining(position)` is not
                ...
                haystack_chunk = turbo::Cord::chunk_remaining(position);
            }
        }
    ...
    //     forward iterator.
    // (c) Use std::string_view::find in each fragment, and specifically handle fragment
    //     boundaries.
    //
    ...
    turbo::Cord::CharIterator turbo::Cord::FindImpl(CharIterator it,
        ...
        while (it.chunk_iterator_.bytes_remaining_ >= needle.size()) {
            auto haystack_chunk = Cord::chunk_remaining(it);
            assert(!haystack_chunk.empty());
    ...
    namespace {
        ...
        bool IsSubcordInCordAt(turbo::Cord::CharIterator haystack,
            ...
            while (needle_begin != needle_end) {
                auto haystack_chunk = turbo::Cord::chunk_remaining(haystack);
                assert(!haystack_chunk.empty());
                auto needle_chunk = turbo::Cord::chunk_remaining(needle_begin);
                auto min_length = std::min(haystack_chunk.size(), needle_chunk.size());
                if (haystack_chunk.substr(0, min_length) != needle_chunk.substr(0, min_length)) {
                    return false;
                }
    ...
    turbo::Cord::CharIterator turbo::Cord::find(const turbo::Cord& needle) const {
        ...
        const auto needle_chunk = Cord::chunk_remaining(needle.char_begin());
    ...
    std::string_view Cord::flatten_slow_path() {
        ...
        contents_.SetTree(new_rep);
        return std::string_view(new_buffer, total_size);
    }

    /* static */ bool Cord::get_flat_aux(CordRep* turbo_nonnull rep,
    ...
    /* static */ void Cord::for_each_chunk_aux(
        ...
        bool success = get_flat_aux(current_node, &chunk);
        assert(success);
        if (success) {
            callback(chunk);
        }
    ...
    static void dump_node(CordRep* turbo_nonnull nonnull_rep, bool include_data,
        std::ostream* turbo_nonnull os, int indent) {
    ...
    static std::string report_error(CordRep* turbo_nonnull root,
        CordRep* turbo_nonnull node) {
        std::ostringstream buf;
        buf << "Error at node " << node << " in:";
        dump_node(root, true, &buf);
        return buf.str();
    }

    static bool verify_node(CordRep* turbo_nonnull root,
        CordRep* turbo_nonnull start_node) {
        turbo::InlinedVector<CordRep * turbo_nonnull, 2> worklist;
        worklist.push_back(start_node);
        do {
            CordRep* node = worklist.back();
            worklist.pop_back();

            TURBO_INTERNAL_CHECK(node != nullptr, report_error(root, node));
            if (node != root) {
                TURBO_INTERNAL_CHECK(node->length != 0, report_error(root, node));
                TURBO_INTERNAL_CHECK(!node->IsCrc(), report_error(root, node));
            }

            if (node->IsFlat()) {
                TURBO_INTERNAL_CHECK(node->length <= node->flat()->Capacity(),
                    report_error(root, node));
                ...
                TURBO_INTERNAL_CHECK(
                    ...
                    report_error(root, node));
                TURBO_INTERNAL_CHECK(node->substring()->start + node->length <= node->substring()->child->length,
                    report_error(root, node));
            } else if (node->IsCrc()) {
                TURBO_INTERNAL_CHECK(
    ...
    namespace strings_internal {
        size_t CordTestAccess::FlatOverhead() {
            return cord_internal::kFlatOverhead;
        }
