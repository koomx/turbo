...
static void DumpNode(CordRep* turbo_nonnull nonnull_rep, bool include_data,
                     std::ostream* turbo_nonnull os, int indent = 0);
static bool VerifyNode(CordRep* turbo_nonnull root,
...
inline void Cord::AppendImpl(C&& src) {
  ...
  if (src.empty()) return;
  TURBO_RAW_CHECK(src.contents_.size() <=
...
void CopyCordToString(const Cord& src, std::string* turbo_nonnull dst) {
  if (!src.contents_.is_tree()) {
...
void AppendCordToString(const Cord& src, std::string* turbo_nonnull dst) {
...
size_t CopyCordToSpan(const Cord& src, turbo::Span<char> dst) {
...
std::string_view Cord::FlattenSlowPath() {
  assert(contents_.is_tree());
...
/* static */ bool Cord::GetFlatAux(CordRep* turbo_nonnull rep,
...
static std::string ReportError(CordRep* turbo_nonnull root,
