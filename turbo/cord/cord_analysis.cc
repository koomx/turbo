    ...
    namespace cord_internal {
        namespace {
            ...
            struct CordRepRef {
                ...
                explicit CordRepRef(const CordRep* turbo_nonnull r)
                    : rep(r) { }

                // Creates a child reference holding the provided child.
                // Overloaded to add cumulative reference count for kFairShare.
                CordRepRef Child(const CordRep* turbo_nonnull child) const {
                    return CordRepRef(child);
                }

                const CordRep* turbo_nonnull rep;
            };
            ...
            struct RawUsage<Mode::kTotalMorePrecise> {
                ...
                std::unordered_set<const CordRep * turbo_nonnull> counted;
            ...
            struct CordRepRef<Mode::kFairShare> {
                ...
                explicit CordRepRef(const CordRep* turbo_nonnull r, double frac = 1.0)
                    : rep(r)
                    , fraction(MaybeDiv(frac, r->refcount.Get())) { }
            ...
            void AnalyzeDataEdge(CordRepRef<mode> rep, RawUsage<mode>& raw_usage) {
                ...
                const size_t size = rep.rep->tag >= FLAT
            ...
            template <Mode mode>
            size_t GetEstimatedUsage(const CordRep* turbo_nonnull rep) {
        ...
        size_t get_estimated_memory_usage(const CordRep* turbo_nonnull rep) {
            return GetEstimatedUsage<Mode::kTotal>(rep);
        }

        size_t get_estimated_fair_share_memory_usage(const CordRep* turbo_nonnull rep) {
            return GetEstimatedUsage<Mode::kFairShare>(rep);
        }

        size_t get_more_precise_memory_usage(const CordRep* turbo_nonnull rep) {
