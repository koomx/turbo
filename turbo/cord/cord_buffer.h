...
// -----------------------------------------------------------------------------
//
// This file defines an `turbo::CordBuffer` data structure to hold data for
...
namespace turbo {
    ...
    private:
        ...
        // Power2 functions
        static bool IsPow2(size_t size) { return turbo::has_single_bit(size); }
        ...
        static CordBuffer create_with_custom_limit_impl(size_t block_size,
            size_t capacity,
        ...
        cord_internal::CordRep* consume_value(std::string_view& short_value) {
    ...
    template <typename... AllocationHints>
    inline CordBuffer CordBuffer::create_with_custom_limit_impl(
    ...
    inline CordBuffer CordBuffer::create_with_custom_limit(size_t block_size,
        size_t capacity) {
        return create_with_custom_limit_impl(block_size, capacity);
    }
    ...
    inline void CordBuffer::set_length(size_t length) {
        turbo::base_internal::HardeningAssertLE(length, capacity());
        if (rep_.is_short()) {
    ...
    inline void CordBuffer::increase_length_by(size_t n) {
        turbo::base_internal::HardeningAssertLE(n, capacity());
        turbo::base_internal::HardeningAssertLE(length() + n, capacity());
        if (rep_.is_short()) {
