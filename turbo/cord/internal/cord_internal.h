...
#if KUMO_HAVE_CONSTANT_EVALUATED && (KUMO_HAVE_ADDRESS_SANITIZER || KUMO_HAVE_MEMORY_SANITIZER)
#define TURBO_INTERNAL_CORD_HAVE_SANITIZER 1
#endif

#define KUMO_CORD_INTERNAL_NO_SANITIZE \
    KUMO_ATTRIBUTE_NO_SANITIZE_ADDRESS KUMO_ATTRIBUTE_NO_SANITIZE_MEMORY
...
#pragma GCC diagnostic pop
        ...
        public:
            ...
            inline void Increment() {
                const int32_t prev_count = count_.fetch_add(kRefIncrement, std::memory_order_relaxed);
                if (KUMO_UNLIKELY(
                        prev_count >= ((std::numeric_limits<decltype(count_)::value_type>::max)() / 3) * 2)) {
                    IncrementOverflow();
                }
            }
            ...
            inline bool Decrement() {
                ...
                return refcount != kRefIncrement && count_.fetch_sub(kRefIncrement, std::memory_order_acq_rel) != kRefIncrement;
            }

            // Same as Decrement but expect that refcount is greater than 1.
            inline bool DecrementExpectHighRefcount() {
                int32_t refcount = count_.fetch_sub(kRefIncrement, std::memory_order_acq_rel);
                assert(refcount > 0 || refcount & kImmortalFlag);
            ...
            inline size_t Get() const {
                return static_cast<size_t>(count_.load(std::memory_order_acquire) >> kNumFlags);
            }
        ...
        template <typename Releaser,
            typename = ::std::invoke_result_t<Releaser, std::string_view>>
        ...
        struct CordRepExternalImpl
            : public CordRepExternal,
              public ::turbo::CompressedTuple<Releaser> {
        ...
        inline CordRepSubstring* CordRepSubstring::Create(CordRep* child, size_t pos,
            ...
            if (KUMO_UNLIKELY(!(child->IsExternal() || child->IsFlat()))) {
        ...
        struct ConstInitExternalStorage {
            KUMO_CONST_INIT static CordRepExternal value;
        };

        template <typename Str>
        KUMO_CONST_INIT CordRepExternal
        ...
        // Tree tag occupies the first 8 bytes of the InlineData union so that
        // `tag() & 1` (the is_tree bit in byte 0) stays set for tree values.
        using tree_tag_t = int64_t;

        static_assert(sizeof(tree_tag_t) * 2 == kMaxInline + 1, "");
        ...
        static constexpr tree_tag_t LittleEndianByte(unsigned char value) {
#if KUMO_ENDIAN_BIG
            return static_cast<tree_tag_t>(value) << ((sizeof(tree_tag_t) - 1) * 8);
#else
            return value;
#endif
        }
        ...
        public:
            ...
            // Odd tag stored in the first byte so is_tree() is true for tree values.
            static constexpr tree_tag_t kTreeTag = LittleEndianByte(1);
            ...
            // if the current compiler supports `turbo::is_constant_evaluated()`.
#ifdef TURBO_INTERNAL_CORD_HAVE_SANITIZER
            ~InlineData() noexcept { unpoison(); }
#endif

            constexpr InlineData() noexcept { poison_this(); }

            explicit InlineData(DefaultInitType) noexcept
                : rep_(kDefaultInit) {
                poison_this();
            }

            explicit InlineData(CordRep* rep) noexcept
                : rep_(rep) {
                KUMO_ASSERT(rep != nullptr);
            }
        ...
        private:
            struct Rep {
                struct AsTree {
                    explicit constexpr AsTree(turbo::cord_internal::CordRep* tree)
                        : rep(tree) { }
                    tree_tag_t tree_tag = kTreeTag;
                ...
                explicit constexpr Rep(CordRep* rep)
                    : as_tree(rep) { }
                ...
                KUMO_CORD_INTERNAL_NO_SANITIZE
                ...
                size_t inline_size() const {
                    KUMO_ASSERT(!self()->is_tree());
                    return static_cast<size_t>(self()->tag()) >> 1;
                }

                void set_inline_size(size_t size) {
                    KUMO_ASSERT(size <= kMaxInline);
                ...
                void make_tree(CordRep* tree) {
                    ...
                    self()->as_tree.tree_tag = kTreeTag;
                }
                ...
                // `as_tree.rep` with `as_tree.tree_tag` odd so `tag() & 1` is non zero.
                union {
...
#else // TURBO_INTERNAL_CORD_HAVE_SANITIZER
...
#endif // TURBO_INTERNAL_CORD_HAVE_SANITIZER
        ...
        inline CordRep* CordRep::Ref(CordRep* rep) {
            // KUMO_ASSUME is a workaround for
        ...
        inline void CordRep::Unref(CordRep* rep) {
            assert(rep != nullptr);
            if (KUMO_UNLIKELY(!rep->refcount.DecrementExpectHighRefcount())) {
                Destroy(rep);
            }
        }
