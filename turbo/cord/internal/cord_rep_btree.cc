...
#include <turbo/cord/internal/cord_rep_consume.h>
...
namespace turbo {
    ...
    namespace cord_internal {
        ...
        namespace {
            ...
            KUMO_CONST_INIT std::atomic<bool> cord_btree_exhaustive_validation(false);
            ...
            void DumpAll(const CordRep* rep,
                ...
                if (rep->IsBtree()) {
                    ...
                    std::string label = node->height() ? turbo::str_cat("Node(", node->height(), ")") : "Leaf";
            ...
            inline CordRep* MakeSubstring(CordRep* rep, size_t offset, size_t n) {
                if (n == rep->length)
                    return rep;
                if (n == 0)
                    return CordRep::Unref(rep), nullptr;
            ...
            inline std::string_view Consume(std::string_view s, size_t n) {
                return edge_type == kBack ? s.substr(n) : s.substr(0, s.size() - n);
            }

            template <EdgeType edge_type>
            inline std::string_view Consume(char* dst, std::string_view s, size_t n) {
                if (edge_type == kBack) {
                    memcpy(dst, s.data(), n);
                    return s.substr(n);
                } else {
            ...
            struct StackOperations {
                ...
                static inline CordRepBtree* Finalize(CordRepBtree* tree, OpResult result) {
                    ...
                    case CordRepBtree::kPopped:
                        ...
                        if (KUMO_UNLIKELY(tree->height() > CordRepBtree::kMaxHeight)) {
        ...
        void CordRepBtree::Dump(const CordRep* rep, std::string_view label,
        ...
        void CordRepBtree::Dump(const CordRep* rep, std::ostream& stream) {
            Dump(rep, std::string_view(), false, stream);
        }
...
#define NODE_CHECK_EQ(x, y)                                               \
    if ((x) != (y)) {                                                     \
        ...
        return false;                                                     \
            ...
            if ((!shallow || IsCordBtreeExhaustiveValidationEnabled()) && tree->height() > 0) {
                for (CordRep* edge : tree->Edges()) {
                    if (!IsValid(edge->btree(), shallow))
                        return false;
                }
            }
            return true;
...
#undef NODE_CHECK_EQ
        ...
        CordRepBtree* CordRepBtree::AddCordRep(CordRepBtree* tree, CordRep* rep) {
            ...
            const OpResult result = leaf->AddEdge<edge_type>(ops.owned(depth), rep, length);
            return ops.Unwind(tree, depth, length, result);
        }

        template <>
        CordRepBtree* CordRepBtree::NewLeaf<kBack>(std::string_view data,
            size_t extra) {
        ...
        template <>
        CordRepBtree* CordRepBtree::NewLeaf<kFront>(std::string_view data,
            size_t extra) {
            CordRepBtree* leaf = CordRepBtree::New(0);
            size_t length = 0;
        ...
        template <>
        std::string_view CordRepBtree::AddData<kBack>(std::string_view data,
            size_t extra) {
            assert(!data.empty());
        ...
        template <>
        std::string_view CordRepBtree::AddData<kFront>(std::string_view data,
            size_t extra) {
            assert(!data.empty());
            assert(size() < capacity());
            AlignEnd();
        ...
        template <EdgeType edge_type>
        CordRepBtree* CordRepBtree::AddData(CordRepBtree* tree, std::string_view data,
            size_t extra) {
            if (KUMO_UNLIKELY(data.empty()))
                return tree;
        ...
        CopyResult CordRepBtree::CopySuffix(size_t offset) {
            ...
            if (offset == 0)
                return { CordRep::Ref(node), height };
        ...
        CopyResult CordRepBtree::CopyPrefix(size_t n, bool allow_folding) {
            ...
            if (allow_folding) {
                while (front->length >= n) {
                    if (--height < 0)
                        return { MakeSubstring(CordRep::Ref(front), 0, n), -1 };
                    node = front->btree();
        ...
        CordRep* CordRepBtree::remove_suffix(CordRepBtree* tree, size_t n) {
            ...
            if (KUMO_UNLIKELY(n == 0)) {
                return tree;
            }
            if (KUMO_UNLIKELY(n >= len)) {
            ...
            while (pos.index == tree->begin()) {
                ...
                if (height-- == 0)
                    return ResizeEdge(edge, length, is_mutable);
                tree = edge->btree();
        ...
        CordRep* CordRepBtree::SubTree(size_t offset, size_t n) {
            ...
            while (front.n + n <= left->length) {
                if (--height < 0)
                    return MakeSubstring(CordRep::Ref(left), front.n, n);
                node = left->btree();
        ...
        bool CordRepBtree::IsFlat(std::string_view* fragment) const {
            if (height() == 0 && size() == 1) {
                if (fragment)
                    *fragment = Data(begin());
                return true;
        ...
        bool CordRepBtree::IsFlat(size_t offset, const size_t n,
            ...
            for (;;) {
                ...
                if (--height < 0) {
                    if (fragment)
                        *fragment = EdgeData(edge).substr(front.n, n);
                    return true;
                }
                offset = front.n;
        ...
        Span<char> CordRepBtree::GetAppendBufferSlow(size_t size) {
            ...
            if (!edge->refcount.IsOne() || edge->tag < FLAT)
                return { };
            ...
            if (tree->height() == 0) {
                for (CordRep* edge : tree->Edges()) {
                    ...
                    while (result.action == CordRepBtree::kPopped) {
                        ...
                        if (KUMO_UNLIKELY(++height >= kMaxDepth)) {
                            TURBO_RAW_LOG(FATAL, "CordRepBtree::Rebuild() exceeded max depth");
                        }
