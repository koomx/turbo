...
namespace turbo {
    ...
    namespace cord_internal {
        ...
        public:
            ...
            // Use CordRep::Unref() as we overload for turbo::Span<CordRep* const>.
            using CordRep::Unref;

            // Unrefs all edges in `edges` which are assumed to be 'likely one'.
            static void Unref(turbo::Span<CordRep* const> edges);
            ...
            // the caller. For example, a caller may need to add 2 std::string_views of data
            ...
            // to be null.
            bool IsFlat(size_t offset, size_t n, std::string_view* fragment) const;
            ...
            inline turbo::Span<CordRep* const> Edges() const;
            inline turbo::Span<CordRep* const> Edges(size_t begin, size_t end) const;

            // Returns reference to the data edge at `index`.
            // Requires this instance to be a leaf node, and `index` to be valid index.
            inline std::string_view Data(size_t index) const;
            ...
            static void Dump(const CordRep* rep, std::string_view label,
                std::ostream& stream);
            static void Dump(const CordRep* rep, std::string_view label,
        ...
        private:
            ...
            template <EdgeType edge_type>
            inline void Add(turbo::Span<CordRep* const>);
        ...
        inline turbo::Span<CordRep* const> CordRepBtree::Edges() const {
            return { edges_ + begin(), size() };
        }

        inline turbo::Span<CordRep* const> CordRepBtree::Edges(size_t begin,
            size_t end) const {
            assert(begin <= end);
        ...
        inline std::string_view CordRepBtree::Data(size_t index) const {
            assert(height() == 0);
        ...
        inline void CordRepBtree::Unref(turbo::Span<CordRep* const> edges) {
            for (CordRep* edge : edges) {
                if (KUMO_UNLIKELY(!edge->refcount.Decrement())) {
        ...
        inline void CordRepBtree::AlignBegin() {
            ...
            if (KUMO_UNLIKELY(delta != 0)) {
                ...
                KUMO_ASSUME(new_end <= kMaxCapacity);
#ifdef __clang__
#pragma unroll 1
#endif
...
#pragma unroll 1
        ...
        inline void CordRepBtree::Add<CordRepBtree::kBack>(
            turbo::Span<CordRep* const> edges) {
            AlignBegin();
            size_t new_end = end();
            for (CordRep* edge : edges)
                edges_[new_end++] = edge;
