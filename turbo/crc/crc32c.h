...
class crc32c_t final {
 public:
  ...
  friend void turbo_stringify(Sink& sink, crc32c_t crc) {
    turbo::str_printf_to(&sink, "%08x", static_cast<uint32_t>(crc));
  }
...
inline crc32c_t ComputeCrc32c(std::string_view buf) {
...
inline std::ostream& operator<<(std::ostream& os, crc32c_t crc) {
  return os << turbo::stream_printf("%08x", static_cast<uint32_t>(crc));
}
