...
void CRC32::InitTables() {
  ...
  TURBO_RAW_CHECK(j <= static_cast<int>(KUMO_ARRAYSIZE(this->zeroes_)), "");
  for (int i = 0; i < j; i++) {
  ...
  TURBO_RAW_CHECK(j <= static_cast<int>(KUMO_ARRAYSIZE(this->reverse_zeroes_)),
                 "");
}
...
void CRC32::Scramble(uint32_t* crc) const {
  ...
  *crc = turbo::rotr(static_cast<uint32_t>(*crc + kScrambleLo), scramble_rotate);
}

void CRC32::Unscramble(uint32_t* crc) const {
  constexpr int scramble_rotate = (32 / 2) + 1;
  uint64_t rotated = turbo::rotl(*crc, scramble_rotate);
