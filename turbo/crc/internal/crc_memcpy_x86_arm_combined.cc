...
#if defined(TURBO_INTERNAL_HAVE_X86_64_ACCELERATED_CRC_MEMCPY_ENGINE) || defined(TURBO_INTERNAL_HAVE_ARM_ACCELERATED_CRC_MEMCPY_ENGINE)
...
namespace turbo {
    ...
    namespace crc_internal {
            ...
            KUMO_ATTRIBUTE_NO_SANITIZE_UNDEFINED inline void LargeTailCopy(
                ...
                while (copy_rounds > 0) {
                    ...
                    for (size_t i = 0; i < int_regions; i++) {
                        ...
                        auto* usrc = reinterpret_cast<const uint64_t*>(*src + region_size * region);
        ...
        public:
            ...
            AcceleratedCrcMemcpyEngine operator=(const AcceleratedCrcMemcpyEngine&) = delete;
        ...
        AcceleratedCrcMemcpyEngine<vec_regions, int_regions>::Compute(
            ...
            constexpr std::size_t kBlocksPerCacheLine = KUMO_CACHELINE_SIZE / kBlockSize;
            ...
            static_assert(KUMO_CACHELINE_SIZE % kBlockSize == 0,
            ...
            constexpr std::size_t kPrefetchAhead = 2 * KUMO_CACHELINE_SIZE;

            // Small-size CRC-memcpy : just do CRC + memcpy
            if (length < kCrcSmallSize) {
                crc32c_t crc = ExtendCrc32c(initial_crc, std::string_view(src_bytes, length));
            ...
            std::size_t bytes_from_last_aligned = reinterpret_cast<uintptr_t>(dst) & (kBlockSize - 1);
            if (bytes_from_last_aligned != 0) {
                ...
                initial_crc = ShortCrcCopy(dst_bytes, src_bytes, bytes_for_alignment, initial_crc);
            ...
            while (copy_rounds > kBlocksPerCacheLine) {
                ...
                for (size_t i = 0; i < kBlocksPerCacheLine; i++) {
                    ...
                    for (size_t j = 0; j < vec_regions; j++) {
                        ...
                        auto* vsrc = reinterpret_cast<const V128*>(src_bytes + region_size * region);
                    ...
                    for (size_t j = 0; j < int_regions; j++) {
                        ...
                        auto* usrc = reinterpret_cast<const uint64_t*>(src_bytes + region_size * region);
                        auto* udst = reinterpret_cast<uint64_t*>(dst_bytes + region_size * region);
            ...
            crcs[kRegions - 1] = ShortCrcCopy(dst_bytes, src_bytes, tail_size - tail_blocks * kBlockSize,
                crcs[kRegions - 1]);
            ...
            crcs[kRegions - 1] = crc32c_t { static_cast<uint32_t>(crcs[kRegions - 1]) ^ kCrcDataXor };
