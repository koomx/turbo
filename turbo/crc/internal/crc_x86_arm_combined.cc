...
#if defined(TURBO_CRC_INTERNAL_HAVE_ARM_SIMD) || defined(TURBO_CRC_INTERNAL_HAVE_X86_SIMD)
...
namespace turbo {
    ...
    namespace crc_internal {
        ...
        using ::turbo::CpuType;
        using ::turbo::get_cpu_type;
        using ::turbo::supports_arm_crc32_pmull;
...
#define TURBO_INTERNAL_STEP1(crc, p)                      \
    do {                                                  \
        crc = CRC32_u8(static_cast<uint32_t>(crc), *p++); \
    } while (0)
#define TURBO_INTERNAL_STEP2(crc, p)                                                  \
    do {                                                                              \
        crc = CRC32_u16(static_cast<uint32_t>(crc), turbo::little_endian::Load16(p)); \
        p += 2;                                                                       \
    } while (0)
#define TURBO_INTERNAL_STEP4(crc, p)                                                  \
    do {                                                                              \
        crc = CRC32_u32(static_cast<uint32_t>(crc), turbo::little_endian::Load32(p)); \
        p += 4;                                                                       \
    } while (0)
#define TURBO_INTERNAL_STEP8(crc, p)                                                  \
    do {                                                                              \
        crc = CRC32_u64(static_cast<uint32_t>(crc), turbo::little_endian::Load64(p)); \
        p += 8;                                                                       \
    } while (0)
...
// treated as zeroes; the caller is assumed to handle any nonzero bits there.
#if defined(NDEBUG) && KUMO_HAVE_CPP_ATTRIBUTE(clang::no_sanitize)
            ...
            protected:
                ...
                KUMO_FORCE_INLINE void Process64BytesPclmul(
                    ...
                    V128 loopMultiplicands = V128_Load(reinterpret_cast<const V128*>(kFoldAcross512Bits));
                ...
                FinalizePclmulStream(V128* partialCRC) const {
                    ...
                    V128 reductionMultiplicands = V128_Load(reinterpret_cast<const V128*>(kFoldAcross256Bits));
                    ...
                    reductionMultiplicands = V128_Load(reinterpret_cast<const V128*>(kFoldAcross128Bits));
                ...
                KUMO_FORCE_INLINE void Process64BytesCRC2Streams(
                    ...
                    for (int i = 0; i < 8; i++) {
                        crc0 = CRC32_u64(static_cast<uint32_t>(crc0),
                            turbo::little_endian::Load64(p0));
                        crc1 = CRC32_u64(static_cast<uint32_t>(crc1),
                            turbo::little_endian::Load64(p1));
                        p0 += 8;
                        p1 += 8;
                    }
                    crc[0] = crc0;
...
#if defined(TURBO_CRC_INTERNAL_HAVE_X86_SIMD) && defined(__AVX__) && (!defined(_MSC_VER) || defined(__clang__))
                ...
                void Extend(uint32_t* crc, const void* bytes,
                    ...
                    if (strategy != CutoffStrategy::Unroll64CRC && (length < kSmallCutoff)) {
                        ...
                        if (strategy == CutoffStrategy::Fold3) {
                            ...
                            l64 = static_cast<uint64_t>(V128_Low64(x)) ^ turbo::little_endian::Load64(p2);
                        ...
                        while (p != x) {
                            TURBO_INTERNAL_STEP1(l, p);
                        }

                        size_t bs = static_cast<size_t>(e - p) / (num_crc_streams + num_pclmul_streams + num_vpclmul_streams) / 64;
                        ...
                        loopMultiplicands = V256_Broadcast128(reinterpret_cast<const V128*>(kFoldAcross512Bits));
                        for (size_t i = 0; i < num_vpclmul_streams; i++) {
                    ...
                    if (remaining_bytes & 4) {
                        TURBO_INTERNAL_STEP4(l, p);
                    }
                    if (remaining_bytes & 2) {
                        TURBO_INTERNAL_STEP2(l, p);
                    }
