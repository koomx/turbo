...
namespace turbo {
    ...
    namespace crc_internal {
        ...
        constexpr size_t kCacheLineSize = KUMO_CACHELINE_SIZE;
        ...
        inline void* non_temporal_store_memcpy(void* __restrict dst,
...
#if defined(__SSE3__) || defined(__aarch64__) || (defined(_MSC_VER) && defined(__AVX__))
            ...
            if (reinterpret_cast<uintptr_t>(d) & (kCacheLineSize - 1)) {
                uintptr_t bytes_before_alignment_boundary = kCacheLineSize - (reinterpret_cast<uintptr_t>(d) & (kCacheLineSize - 1));
...
#if KUMO_HAVE_CPP_ATTRIBUTE(gnu::target) && !defined(_MSC_VER) && (defined(__x86_64__) || defined(__i386__))
#define TURBO_INTERNAL_CAN_FORCE_AVX 1
#endif

// If the objects overlap, the behavior is undefined. Uses regular memcpy
// instead of non-temporal memcpy if the required CPU intrinsics are unavailable
// at compile time.
#ifdef TURBO_INTERNAL_CAN_FORCE_AVX
        [[gnu::target("avx")]]
#endif
...
#if ((defined(__AVX__) || defined(TURBO_INTERNAL_CAN_FORCE_AVX)) && defined(__SSE3__)) || (defined(_MSC_VER) && defined(__AVX__))
            uint8_t* d = reinterpret_cast<uint8_t*>(dst);
            const uint8_t* s = reinterpret_cast<const uint8_t*>(src);
...
#undef TURBO_INTERNAL_CAN_FORCE_AVX
