...
// TURBO_HAVE_PTHREAD_CPU_NUMBER_NP
//
// Checks whether pthread_cpu_number_np is available.
#ifdef TURBO_HAVE_PTHREAD_CPU_NUMBER_NP
#error TURBO_HAVE_PTHREAD_CPU_NUMBER_NP cannot be directly set
...
#define TURBO_HAVE_PTHREAD_CPU_NUMBER_NP 1
#endif

namespace turbo {


KUMO_CONST_INIT static FailureSignalHandlerOptions fsh_options;
...
KUMO_CONST_INIT static FailureSignalData failure_signal_data[] = {
...
#ifdef TURBO_HAVE_SIGALTSTACK
  KUMO_ATTRIBUTE_UNUSED static const bool kOnce = SetupAlternateStackOnce();
  return SA_ONSTACK;
#else
  return 0;
#endif
...
// Many of the turbo::debugging_internal::Dump* functions in
...
KUMO_ATTRIBUTE_NOINLINE static void WriteStackTrace(
  ...
  int depth = turbo::GetStackFramesWithContext(
      stack, frame_sizes, kNumStackFrames,
      1,  // Do not include this function in stack trace.
      ucontext, &min_dropped_frames);
  turbo::debugging_internal::DumpPCAndFrameSizesAndStackTrace(
      turbo::debugging_internal::GetProgramCounter(ucontext), stack, frame_sizes,
...
KUMO_CONST_INIT static std::atomic<GetTidType> failed_tid(0);

static int GetCpuNumber() {
#if KUMO_HAVE_SCHED_GETCPU
  return sched_getcpu();
#elif defined(TURBO_HAVE_PTHREAD_CPU_NUMBER_NP)
  size_t cpu_num;
...
static void TurboFailureSignalHandler(int signo, siginfo_t*, void* ucontext) {
  ...
  WriteFailureInfo(
      signo, ucontext, my_cpu, +[](const char* data) {
        turbo::raw_log_internal::AsyncSignalSafeWriteError(data, strlen(data));
      });
