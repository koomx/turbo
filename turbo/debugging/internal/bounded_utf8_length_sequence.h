...
#ifndef TURBO_DEBUGGING_INTERNAL_BOUNDED_UTF8_LENGTH_SEQUENCE_H_
#define TURBO_DEBUGGING_INTERNAL_BOUNDED_UTF8_LENGTH_SEQUENCE_H_
...
class BoundedUtf8LengthSequence {
 public:
  ...
  uint32_t InsertAndReturnSumOfPredecessors(
    ...
    int full_popcount = turbo::popcount(same_word_bits_below_insertion);
    int odd_popcount =
        turbo::popcount(same_word_bits_below_insertion & odd_bits_mask);
    for (uint32_t j = word_index; j > 0; --j) {
      const uint64_t word_below_insertion = rep_[j - 1];
      full_popcount += turbo::popcount(word_below_insertion);
      odd_popcount += turbo::popcount(word_below_insertion & odd_bits_mask);
    }
...
#endif  // TURBO_DEBUGGING_INTERNAL_BOUNDED_UTF8_LENGTH_SEQUENCE_H_
