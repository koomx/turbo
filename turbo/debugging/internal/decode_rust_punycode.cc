...
#include <turbo/debugging/internal/bounded_utf8_length_sequence.h>
#include <turbo/macros/config.h>
#include <turbo/unicode/api/utf8_for_code_point.h>
...
namespace turbo {
    ...
    namespace debugging_internal {
        ...
        namespace {
            ...
            bool ConsumeOptionalAsciiPrefix(const char*& punycode_begin,
                ...
                for (int i = 0; i < punycode_end - punycode_begin; ++i) {
                    ...
                    // We write out the meaning of turbo::ascii_isalnum rather than call that
                ...
                if (last_underscore + 1 > out_end - out_begin)
                    return false;
            ...
            bool ScanNextDelta(const char*& punycode_begin, const char* const punycode_end,
                ...
                for (uint32_t k = kBase; punycode_begin != punycode_end; k += kBase) {
                    ...
                    if (static_cast<uint32_t>(digit_value) < t)
                        return true;
        ...
        char* turbo_nullable DecodeRustPunycode(DecodeRustPunycodeOptions options) {
            ...
            *out_begin = '\0';
            ...
            while (punycode_begin != punycode_end) {
                if (num_chars >= kMaxChars)
                    return nullptr;

                const uint32_t old_i = i;

                if (!ScanNextDelta(punycode_begin, punycode_end, bias, i))
                    return nullptr;
