...
#include <turbo/debugging/internal/demangle_rust.h>
...
#include <turbo/debugging/internal/decode_rust_punycode.h>
...
class RustSymbolParser {
 public:
  ...
  [[nodiscard]] bool Parse() && {
    ...
    // appear in only one TURBO_DEMANGLER_RECURSE call and be listed in the
    ...
    // ParseIdentifier.
#define TURBO_DEMANGLER_RECURSE(callee, caller) \
    do { \
    ...
    for (; iter < kMaxReturns && recursion_depth_ > 0; ++iter) {
      ...
      switch (recursion_stack_[--recursion_depth_]) {
        ...
        whole_encoding:
          ...
          TURBO_DEMANGLER_RECURSE(path, kInstantiatingCrate);
          if (IsAlpha(Peek())) {
            ++silence_depth_;  // Print nothing more from here on.
            TURBO_DEMANGLER_RECURSE(path, kVendorSpecificSuffix);
          }
          switch (Take()) {
        ...
        //
        // Note that TURBO_DEMANGLER_RECURSE does not work inside a nested switch
        ...
        inherent_impl:
          ...
          TURBO_DEMANGLER_RECURSE(impl_path, kInherentImplType);
          TURBO_DEMANGLER_RECURSE(type, kInherentImplEnding);
          if (!Emit(">")) return false;
          continue;
        ...
        trait_impl:
          if (!Emit("<")) return false;
          TURBO_DEMANGLER_RECURSE(impl_path, kTraitImplType);
          TURBO_DEMANGLER_RECURSE(type, kTraitImplInfix);
          if (!Emit(" as ")) return false;
          TURBO_DEMANGLER_RECURSE(path, kTraitImplEnding);
          if (!Emit(">")) return false;
          continue;
        ...
        impl_path:
          ...
          TURBO_DEMANGLER_RECURSE(path, kImplPathEnding);
          --silence_depth_;
          continue;
        ...
        trait_definition:
          if (!Emit("<")) return false;
          TURBO_DEMANGLER_RECURSE(type, kTraitDefinitionInfix);
          if (!Emit(" as ")) return false;
          TURBO_DEMANGLER_RECURSE(path, kTraitDefinitionEnding);
          if (!Emit(">")) return false;
          continue;
        ...
        nested_path:
          ...
          if (IsUpper(Peek())) {
            ...
            TURBO_DEMANGLER_RECURSE(path, kIdentifierInUppercaseNamespace);
            if (!Emit("::")) return false;
          ...
          if (IsLower(Take())) {
            TURBO_DEMANGLER_RECURSE(path, kIdentifierInLowercaseNamespace);
            if (!Emit("::")) return false;
            if (!ParseIdentifier()) return false;
            continue;
          }
        ...
        // TURBO_DEMANGLER_RECURSE works fine in the 'S' case.
        type:
          if (IsLower(Peek())) {
          ...
          if (Eat('A')) {
            ...
            if (!Emit("[")) return false;
            TURBO_DEMANGLER_RECURSE(type, kArraySize);
            if (!Emit("; ")) return false;
            TURBO_DEMANGLER_RECURSE(constant, kFinishArray);
            if (!Emit("]")) return false;
            continue;
          }
          if (Eat('S')) {
            if (!Emit("[")) return false;
            TURBO_DEMANGLER_RECURSE(type, kSliceEnding);
            if (!Emit("]")) return false;
            continue;
          }
        ...
        tuple_type:
          ...
          TURBO_DEMANGLER_RECURSE(type, kAfterFirstTupleElement);
          if (Eat('E')) {
            if (!Emit(",)")) return false;
            continue;
          }
          ...
          if (!Emit(", ")) return false;
          TURBO_DEMANGLER_RECURSE(type, kAfterSecondTupleElement);
          if (Eat('E')) {
            if (!Emit(")")) return false;
            continue;
          }
          ...
          if (!Emit(", ")) return false;
          TURBO_DEMANGLER_RECURSE(type, kAfterThirdTupleElement);
          if (Eat('E')) {
            if (!Emit(")")) return false;
            continue;
          }
          ...
          ++silence_depth_;
          while (!Eat('E')) {
            TURBO_DEMANGLER_RECURSE(type, kAfterSubsequentTupleElement);
          }
          --silence_depth_;
          continue;
        ...
        fn_type:
          ...
          while (!Eat('E')) {
            TURBO_DEMANGLER_RECURSE(type, kContinueParameterList);
          }
          TURBO_DEMANGLER_RECURSE(type, kFinishFn);
          --silence_depth_;
          continue;
        ...
        dyn_trait_type:
          ...
          if (!ParseOptionalBinder()) return false;
          if (!Eat('E')) {
            TURBO_DEMANGLER_RECURSE(dyn_trait, kBeginAutoTraits);
            while (!Eat('E')) {
              if (!Emit(" + ")) return false;
              TURBO_DEMANGLER_RECURSE(dyn_trait, kContinueAutoTraits);
            }
          }
        ...
        // for generic-args.
        dyn_trait:
          TURBO_DEMANGLER_RECURSE(path, kContinueDynTrait);
          if (Peek() == 'p') {
            if (!Emit("<>")) return false;
            ++silence_depth_;
            while (Eat('p')) {
              if (!ParseUndisambiguatedIdentifier()) return false;
              TURBO_DEMANGLER_RECURSE(type, kContinueAssocBinding);
            }
            --silence_depth_;
          }
          continue;
        ...
        constant:
          ...
          ++silence_depth_;
          TURBO_DEMANGLER_RECURSE(type, kConstData);
          --silence_depth_;
        ...
        generic_args:
          TURBO_DEMANGLER_RECURSE(path, kBeginGenericArgList);
          if (!Emit("::<>")) return false;
          ++silence_depth_;
          while (!Eat('E')) {
            TURBO_DEMANGLER_RECURSE(generic_arg, kContinueGenericArgList);
          }
          --silence_depth_;
          continue;
        ...
        path_backref:
          ...
          if (silence_depth_ == 0) {
            TURBO_DEMANGLER_RECURSE(path, kPathBackrefEnding);
          }
          EndBackref();
          continue;
        ...
        type_backref:
          if (!BeginBackref()) return false;
          if (silence_depth_ == 0) {
            TURBO_DEMANGLER_RECURSE(type, kTypeBackrefEnding);
          }
          EndBackref();
          continue;
        ...
        const_backref:
          if (!BeginBackref()) return false;
          if (silence_depth_ == 0) {
            TURBO_DEMANGLER_RECURSE(constant, kConstantBackrefEnding);
          }
          EndBackref();
          continue;
      }
 ...
 private:
  // Enumerates resumption points for TURBO_DEMANGLER_RECURSE calls.
  ...
  // ReturnAddresses pushed by TURBO_DEMANGLER_RECURSE calls not yet completed.
