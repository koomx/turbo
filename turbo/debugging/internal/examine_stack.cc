...
KUMO_CONST_INIT SymbolizeUrlEmitter debug_stack_trace_hook = nullptr;
...
void DumpPCAndSymbol(OutputWriter* writer, void* writer_arg, void* const pc,
  ...
  // a call to a function annotated noreturn (e.g. KCHECK).
  ...
  if (turbo::Symbolize(reinterpret_cast<const char*>(prev_pc), tmp,
                      sizeof(tmp)) ||
      turbo::Symbolize(pc, tmp, sizeof(tmp))) {
    symbol = tmp;
  }
  char buf[1024];
...
void DumpPCAndFrameSizeAndSymbol(OutputWriter* writer, void* writer_arg,
  ...
  char tmp[1024];
  const char* symbol = "(unknown)";
  if (turbo::Symbolize(symbolize_pc, tmp, sizeof(tmp))) {
    symbol = tmp;
  }
  char buf[1024];
  if (framesize <= 0) {
...
void* GetProgramCounter(void* const vuc) {
  ...
  if (vuc != nullptr) {
    ...
    if (14 < KUMO_ARRAYSIZE(context->uc_mcontext.gregs))
    ...
    if (16 < KUMO_ARRAYSIZE(context->uc_mcontext.gregs))
      return reinterpret_cast<void*>(context->uc_mcontext.gregs[16]);
...
void DumpPCAndFrameSizesAndStackTrace(void* const pc, void* const stack[],
  ...
  for (int i = 0; i < depth; i++) {
    if (symbolize_stacktrace) {
      ...
      // call to a function annotated noreturn (e.g. KCHECK). Note that we don't
...
void DumpStackTrace(int min_dropped_frames, int max_num_frames,
  ...
  int depth = turbo::GetStackTrace(stack, num_stack, min_dropped_frames + 1);
  for (int i = 0; i < depth; i++) {
    if (symbolize_stacktrace) {
