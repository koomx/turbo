...
// stored. (Note that the frame generated for the `turbo::GetStackFrames()`
...
//        void* result[10];
//        int sizes[10];
//        int depth = turbo::GetStackFrames(result, sizes, 10, 1);
//      }
//
...
// that the frame generated for the `turbo::GetStackFramesWithContext()` routine
...
// stored. Note that this function is similar to `turbo::GetStackFrames()`
...
// the `turbo::GetStackFramesWithContext()` routine itself is also skipped.)
//
// The `uc` parameter, if non-null, should be a pointer to a `ucontext_t` value
// passed to a signal handler registered via the `sa_sigaction` field of a
// `sigaction` struct. (See
...
// arguments passed to `turbo::GetStackFramesWithContext()` except that sizes
...
// *****************************************************************************
//
// turbo::SetStackUnwinder is not suitable for general purpose use.  It is
// provided for custom runtimes.
// Some things to watch out for when calling `turbo::SetStackUnwinder()`:
//
...
// case where `turbo::SetStackUnwinder()` was never called. Equivalents are:
//
