...
namespace turbo {
    ...
    struct Base32Rfc4648 {
        static constexpr char encodeChar(uint8_t c) {
            KUMO_ASSERT(c < 32);
            if (c < 26)
                return 'A' + c;
            return '2' + (c - 26);
        }
        static constexpr uint8_t decodeChar(uint8_t c) {
            if (c >= 'A' && c <= 'Z')
                return c - 'A';
        ...
        static constexpr int8_t padding_char = '=';
    };
    ...
    struct Base32<Traits, Base32NaiveTag> {
        static size_t base32_encode(const uint8_t* src, size_t src_length, uint8_t* dst) {
        ...
        static std::optional<size_t> base32_decode(const uint8_t* src, size_t src_length, uint8_t* dst) {
            if (src_length % 8 != 0) {
                return std::nullopt;
            }
            ...
            for (size_t i = 0; i < src_length; ++i) {
                ...
                uint8_t value = Traits::decodeChar(c);
                if (value == 0xFF) {
    ...
    inline size_t base32_encode(const uint8_t* src, size_t src_length, uint8_t* dst) {
        return Base32<Base32Rfc4648, Base32NaiveTag>::base32_encode(src, src_length, dst);
    }

    inline std::optional<size_t> base32_decode(const uint8_t* src, size_t src_length, uint8_t* dst) {
        return Base32<Base32Rfc4648, Base32NaiveTag>::base32_decode(src, src_length, dst);
    }
