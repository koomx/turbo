...
namespace turbo {
    ...
    namespace flags_internal {
        ...
        // if TURBO_FLAGS_STRIP_HELP is set to 1 before including turbo/flags/flag.h
...
#define DONT_VALIDATE(T, _)                     \
    if (flag_type_id == turbo::FastTypeId<T>()) \
        return false;
...
#undef DONT_VALIDATE
            ...
            class MutexRelock {
            public:
                explicit MutexRelock(std::mutex& mu)
                    : mu_(mu) { mu_.unlock(); }
            ...
            KUMO_CONST_INIT std::vector<void*>* s_freelist TURBO_GUARDED_BY(FreelistMutex())
                TURBO_PT_GUARDED_BY(FreelistMutex()) = nullptr;

            void AddToFreelist(void* p) {
                std::lock_guard<std::mutex> l(FreelistMutex());
                if (!s_freelist) {
        ...
        public:
            ...
            ~FlagState() override {
                if (flag_impl_.ValueStorageKind() != FlagValueStorageKind::kHeapAllocated && flag_impl_.ValueStorageKind() != FlagValueStorageKind::kSequenceLocked)
                    return;
        ...
        private:
            ...
            void Restore() && override {
                if (!std::move(flag_impl_).RestoreState(*this))
                    return;
            ...
            FlagImpl& flag_impl_;
            union SavedValue {
                explicit SavedValue(void* v)
                    : heap_allocated(v) { }
                explicit SavedValue(int64_t v)
                    : one_word(v) { }
        ...
        void FlagImpl::AssertValidType(FlagFastTypeId rhs_type_id,
            ...
            if (KUMO_LIKELY(lhs_type_id == rhs_type_id))
                return;

            const std::type_info* lhs_runtime_type_id = flags_internal::RuntimeTypeId(op_);
            const std::type_info* rhs_runtime_type_id = (*gen_rtti)();

            if (lhs_runtime_type_id == rhs_runtime_type_id)
                return;
            ...
            TURBO_INTERNAL_LOG(
                FATAL, turbo::str_cat("Flag '", Name(), "' is defined as one type and declared as another"));
        }
        ...
        std::string_view FlagImpl::Name() const {
            return name_;
        }

        std::string_view FlagImpl::TypeName() const {
            return type_name_;
        }
        ...
        bool FlagImpl::IsSpecifiedOnCommandLine() const {
            std::lock_guard<std::mutex> l(DataGuard());
        ...
        std::string FlagImpl::CurrentValue() const {
            ...
            case FlagValueStorageKind::kValueAndInitBit:
            case FlagValueStorageKind::kOneWordAtomic: {
                const auto one_word_val = turbo::bit_cast<std::array<char, sizeof(int64_t)>>(
        ...
        void FlagImpl::set_validator(FlagValidatorBridge bridge, void (*typed_fn)()) {
            std::lock_guard<std::mutex> l(DataGuard());

            if (validator_ == nullptr) {
                validator_ = new FlagValidator;
            }
            validator_->bridge = bridge;
            validator_->typed_fn = typed_fn;
        }

        bool FlagImpl::InvokeUserValidate(const void* value, std::string* err) const {
            if (validator_ == nullptr)
                return true;

            FlagValidatorBridge bridge = validator_->bridge;
            void (*typed_fn)() = validator_->typed_fn;
            MutexRelock relock(DataGuard());
            std::lock_guard<std::mutex> lock(validator_->guard);
            return bridge(value, err, typed_fn);
        }
        ...
        void FlagImpl::InvokeCallback() const {
            ...
            MutexRelock relock(DataGuard());
            std::lock_guard<std::mutex> lock(callback_->guard);
            cb();
        }
        ...
        std::unique_ptr<FlagStateInterface> FlagImpl::SaveState() {
            ...
            case FlagValueStorageKind::kSequenceLocked: {
                ...
                bool success = seq_lock_.TryRead(cloned, AtomicBufferValue(), Sizeof(op_));
                assert(success);
        ...
        std::atomic<int64_t>& FlagImpl::OneWordValue() const {
            assert(ValueStorageKind() == FlagValueStorageKind::kOneWordAtomic || ValueStorageKind() == FlagValueStorageKind::kValueAndInitBit);
        ...
        std::unique_ptr<void, DynValueDeleter> FlagImpl::TryParse(
            ...
            if (!InvokeUserValidate(tentative_value.get(), &validate_err)) {
                std::string_view err_sep = validate_err.empty() ? "" : "; ";
                err = turbo::str_cat("Illegal value '", value, "' specified for flag '",
                    Name(), "'", err_sep, validate_err);
                return nullptr;
            }
        ...
        void FlagImpl::Read(void* dst) const {
            ...
            case FlagValueStorageKind::kValueAndInitBit:
            case FlagValueStorageKind::kOneWordAtomic: {
                const int64_t one_word_val = OneWordValue().load(std::memory_order_acquire);
        ...
        void FlagImpl::ReadSequenceLockedData(void* dst) const {
            ...
            if (KUMO_LIKELY(seq_lock_.TryRead(dst, AtomicBufferValue(), size))) {
                return;
            }
        ...
        bool FlagImpl::Write(const void* src, std::string* error) {
            std::lock_guard<std::mutex> l(DataGuard());
            ...
            if (ShouldValidateFlagValue(flags_internal::FastTypeId(op_))) {
                ...
                if (!flags_internal::Parse(op_, src_as_str, obj.get(), &ignored_error)) {
                    TURBO_INTERNAL_LOG(ERROR, turbo::str_cat("Attempt to set flag '", Name(), "' to invalid value ", src_as_str));
                }
            }

            std::string validate_err;
            std::string* err_ptr = error != nullptr ? error : &validate_err;
            if (!InvokeUserValidate(src, err_ptr)) {
                return false;
            }
        ...
        void FlagImpl::CheckDefaultValueParsingRoundtrip() const {
            ...
            std::lock_guard<std::mutex> lock(DataGuard());
        ...
        bool FlagImpl::ValidateInputValue(std::string_view value) const {
            ...
            auto obj = MakeInitValue();
            std::string ignored_error;
            if (!flags_internal::Parse(op_, value, obj.get(), &ignored_error)) {
                return false;
            }
            return InvokeUserValidate(obj.get(), &ignored_error);
        }
