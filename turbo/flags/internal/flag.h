...
#ifndef TURBO_FLAGS_INTERNAL_FLAG_H_
#define TURBO_FLAGS_INTERNAL_FLAG_H_
...
namespace turbo {
    ...
    ///////////////////////////////////////////////////////////////////////////////
    // Forward declaration of turbo::Flag<T> public API.
    ...
    template <typename T>
    [[nodiscard]] T GetFlag(const turbo::Flag<T>& flag);

    template <typename T>
    bool SetFlag(turbo::Flag<T>* flag, const T& v, std::string* error = nullptr);

    template <typename T, typename V>
    bool SetFlag(turbo::Flag<T>* flag, const V& v, std::string* error = nullptr);
    ...
    namespace flags_internal {
        ...
        inline FlagFastTypeId FastTypeId(FlagOpFn op) {
            return turbo::bit_cast<FlagFastTypeId>(
        ...
        // turbo::Flag<T> data. Given FlagImpl pointer p you can get the
        ...
        // This is help argument for turbo::Flag encapsulating the string literal pointer
        ...
        struct FixedCharArray {
            ...
            static constexpr FixedCharArray<N> FromLiteralString(
                std::string_view str, std::index_sequence<I...>) {
        ...
        enum class FlagHelpKind : uint8_t { kLiteral = 0,
            kGenFunc = 1 };
        ...
        // way to pass Help argument to turbo::Flag. We'll be passing
        // TurboFlagHelpGenFor##name as Gen and integer 0 as a single argument to prefer
        // first overload if possible. If help message is evaluatable on constexpr
        // context We'll be able to make FixedCharArray out of it and we'll choose first
        // overload. In this case the help message expression is immediately evaluated
        // and is used to construct the turbo::Flag. No additional code is generated by
        // TURBO_FLAG Otherwise SFINAE kicks in and first overload is dropped from the
        ...
        // generated in the TURBO_FLAG code.
...
#define TURBO_FLAGS_INTERNAL_DFLT_FOR_TYPE(T, name) \
    T name##_value;                                 \
    constexpr explicit FlagDefaultSrc(T value)      \
        : name##_value(value) { } // NOLINT
            TURBO_FLAGS_INTERNAL_BUILTIN_TYPES(TURBO_FLAGS_INTERNAL_DFLT_FOR_TYPE)
#undef TURBO_FLAGS_INTERNAL_DFLT_FOR_TYPE
        ...
        // facilitate usage of {} as default value in TURBO_FLAG macro.
        ...
        template <typename T>
        using FlagUseValueAndInitBitStorage = std::bool_constant<std::is_trivially_copyable_v<T> && std::is_default_constructible_v<T> && (sizeof(T) < 8)>;

        template <typename T>
        using FlagUseOneWordStorage = std::integral_constant<bool,
        ...
        public:
            ...
            constexpr MaskedPointer()
                : ptr_(nullptr) { }
            constexpr explicit MaskedPointer(ptr_t rhs)
                : ptr_(rhs) { }
            ...
            void* Ptr() const {
                return reinterpret_cast<void*>(reinterpret_cast<mask_t>(ptr_) & kPtrValueMask);
            }
            bool AllowsUnprotectedRead() const {
                return (reinterpret_cast<mask_t>(ptr_) & kAllowsUnprotectedRead) == kAllowsUnprotectedRead;
            }
        ...
        private:
            ...
            static constexpr mask_t kAllowsUnprotectedRead = kUnprotectedReadCandidate | kHasBeenRead;
        ...
        struct FlagValue<T, FlagValueStorageKind::kOneWordAtomic> : FlagOneWordValue {
            ...
            bool Get(const SequenceLock&, T& dst) const {
                ...
                if (KUMO_UNLIKELY(one_word_val == FlagOneWordValue::Uninitialized())) {
                    return false;
                }
        ...
        struct FlagValue<T, FlagValueStorageKind::kHeapAllocated>
            ...
            constexpr FlagValue()
                : FlagMaskedPointerValue(&buffer[0]) { }

            bool Get(const SequenceLock&, T& dst) const {
                MaskedPointer ptr_value = value.load(std::memory_order_acquire);

                if (KUMO_LIKELY(ptr_value.AllowsUnprotectedRead())) {
        ...
        // Typed validator: receives pointer to value of type T (type-erased as void*).
        // `typed_fn` is the user callback bitcast to void (*)().
        using FlagValidatorBridge =
            bool (*)(const void* value, std::string* err, void (*typed_fn)()) noexcept;

        struct FlagValidator {
            FlagValidatorBridge bridge = nullptr;
            void (*typed_fn)() = nullptr;
            std::mutex guard; // Guard for concurrent validator invocations.
        };

        template <typename T>
        bool ValidatorBridge(const void* value, std::string* err,
                             void (*typed_fn)()) noexcept {
            using TypedFn = bool (*)(const T&, std::string*) noexcept;
            auto* cb = reinterpret_cast<TypedFn>(typed_fn);
            return cb(*static_cast<const T*>(value), err);
        }
...
#pragma GCC diagnostic ignored "-Wnon-virtual-dtor"
        ...
        public:
            ...
            int64_t ReadOneWord() const TURBO_LOCKS_EXCLUDED(DataGuard());
            bool ReadOneBool() const TURBO_LOCKS_EXCLUDED(DataGuard());
            void Read(void* dst) const override TURBO_LOCKS_EXCLUDED(DataGuard());
            void Read(bool* value) const TURBO_LOCKS_EXCLUDED(DataGuard()) {
                *value = ReadOneBool();
            }
            template <typename T,
                std::enable_if_t<flags_internal::StorageKind<T>() == FlagValueStorageKind::kOneWordAtomic,
                    int> = 0>
            void Read(T* value) const TURBO_LOCKS_EXCLUDED(DataGuard()) {
                int64_t v = ReadOneWord();
                std::memcpy(value, static_cast<const void*>(&v), sizeof(T));
            }
            template <typename T,
                std::enable_if_t<flags_internal::StorageKind<T>() == FlagValueStorageKind::kValueAndInitBit,
                    int> = 0>
            void Read(T* value) const TURBO_LOCKS_EXCLUDED(DataGuard()) {
                *value = turbo::bit_cast<FlagValueAndInitBit<T>>(ReadOneWord()).value;
            }

            // Mutating access methods
            // Writes `src` if validation succeeds. Returns false and leaves the flag
            // unchanged when a user validator rejects the value.
            bool Write(const void* src, std::string* error = nullptr)
            ...
            // Interfaces to operate on user validators (typed value pointed by void*).
            void set_validator(FlagValidatorBridge bridge, void (*typed_fn)())
                TURBO_LOCKS_EXCLUDED(DataGuard());
        ...
        private:
            ...
            void ReadSequenceLockedData(void* dst) const TURBO_LOCKS_EXCLUDED(DataGuard());

            // Invokes user validator while DataGuard is held (releases it during call).
            // `value` points to a value of the flag's type T.
            bool InvokeUserValidate(const void* value, std::string* err) const
                TURBO_EXCLUSIVE_LOCKS_REQUIRED(DataGuard());
            ...
            bool ValidateInputValue(std::string_view value) const override
                TURBO_LOCKS_EXCLUDED(DataGuard());
            ...
            bool ParseFrom(std::string_view value, FlagSettingMode set_mode,
                ValueSource source, std::string& error) override
                TURBO_LOCKS_EXCLUDED(DataGuard());

            // Immutable flag's state.

            // Flags name passed to TURBO_FLAG as second arg.
            const char* const name_;

            // Flags type passed to TURBO_FLAG as first arg.
            const char* const type_name_;

            // The file name where TURBO_FLAG resides.
            ...
            bool modified_ : 1 TURBO_GUARDED_BY(DataGuard());
            // Has this flag been specified on command line.
            bool on_command_line_ : 1 TURBO_GUARDED_BY(DataGuard());

            // Unique tag for turbo::call_once call to initialize this flag.
            turbo::once_flag init_control_;
            ...
            // Optional typed user validator (ParseFrom / SetFlag / Write).
            FlagValidator* validator_ TURBO_GUARDED_BY(DataGuard());
            ...
            // problems.
            alignas(std::mutex) mutable unsigned char data_guard_[sizeof(std::mutex)];
        };
#if defined(__GNUC__) && !defined(__clang__)
#pragma GCC diagnostic pop
...
#if !defined(NDEBUG)
                impl_.AssertValidType(turbo::FastTypeId<T>(), &GenRuntimeTypeId<T>);
#endif

                if (KUMO_UNLIKELY(!value_.Get(impl_.seq_lock_, u.value))) {
                    impl_.Read(&u.value);
                }
                return std::move(u.value);
            }
            bool Set(const T& v, std::string* error = nullptr) {
                impl_.AssertValidType(turbo::FastTypeId<T>(), &GenRuntimeTypeId<T>);
                return impl_.Write(&v, error);
            }
        ...
        public:
            ...
            static bool InvokeSet(FlagType& flag, const T& v, std::string* error) {
                return flag.Set(v, error);
            }
        ...
        void* FlagOps(FlagOp op, const void* v1, void* v2, void* v3) {
            ...
            case FlagOp::kFastTypeId:
                return turbo::bit_cast<void*>(turbo::FastTypeId<T>());
            ...
            case FlagOp::kParse: {
                ...
                if (!turbo::ParseFlag<T>(*static_cast<const std::string_view*>(v1), &temp,
            ...
            case FlagOp::kUnparse:
                *static_cast<std::string*>(v2) = turbo::UnparseFlag<T>(*static_cast<const T*>(v1));
                return nullptr;
        ...
        public:
            ...
            FlagRegistrar on_validate(
                bool (*cb)(const T&, std::string*) noexcept) && {
                flag_.impl_.set_validator(&ValidatorBridge<T>,
...
#endif // TURBO_FLAGS_INTERNAL_FLAG_H_
