...
std::mutex& HelpAttributesMutex() {
  static std::mutex mutex;
  return mutex;
}
KUMO_CONST_INIT std::string* match_substr TURBO_GUARDED_BY(HelpAttributesMutex())
    TURBO_PT_GUARDED_BY(HelpAttributesMutex()) = nullptr;
KUMO_CONST_INIT HelpMode help_mode TURBO_GUARDED_BY(HelpAttributesMutex()) =
    HelpMode::kNone;
KUMO_CONST_INIT HelpFormat help_format TURBO_GUARDED_BY(HelpAttributesMutex()) =
    HelpFormat::kHumanReadable;

}  // namespace

std::string GetFlagsHelpMatchSubstr() {
  std::lock_guard<std::mutex> l(HelpAttributesMutex());
