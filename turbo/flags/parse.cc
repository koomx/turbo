...
std::mutex& ProcessingChecksMutex() {
  static std::mutex mutex;
  return mutex;
}

KUMO_CONST_INIT bool flagfile_needs_processing
    TURBO_GUARDED_BY(ProcessingChecksMutex()) = false;
KUMO_CONST_INIT bool fromenv_needs_processing
    TURBO_GUARDED_BY(ProcessingChecksMutex()) = false;
KUMO_CONST_INIT bool tryfromenv_needs_processing
    TURBO_GUARDED_BY(ProcessingChecksMutex()) = false;

std::mutex& SpecifiedFlagsMutex() {
  static std::mutex mutex;
  return mutex;
}

KUMO_CONST_INIT std::vector<const CommandLineFlag*>* specified_flags
    TURBO_GUARDED_BY(SpecifiedFlagsMutex()) = nullptr;

// Suggesting at most kMaxHints flags in case of misspellings.
KUMO_CONST_INIT const size_t kMaxHints = 100;
// Suggesting only flags which have a smaller distance than kMaxDistance.
KUMO_CONST_INIT const size_t kMaxDistance = 3;
...
TURBO_FLAG(std::vector<std::string>, flagfile, {},
    ...
    .OnUpdate([]() {
      ...
      std::lock_guard<std::mutex> l(turbo::flags_internal::ProcessingChecksMutex());
...
bool ArgsList::ReadFromFlagfile(const std::string& flag_file_name) {
  ...
  while (std::getline(flag_file, line)) {
    ...
    flags_internal::ReportUsageError(
        turbo::StrCat("Unexpected line ", line_number, " in the flagfile ",
                     flag_file_name),
        true);
...
bool HandleGeneratorFlags(std::vector<ArgsList>& input_args,
  ...
  std::lock_guard<std::mutex> l(flags_internal::ProcessingChecksMutex());
...
bool WasPresentOnCommandLine(std::string_view flag_name) {
  std::lock_guard<std::mutex> l(SpecifiedFlagsMutex());
...
HelpMode ParseAbseilFlagsOnlyImpl(
  ...
  std::lock_guard<std::mutex> l(flags_internal::SpecifiedFlagsMutex());
