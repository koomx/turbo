...
class FlagRegistry {
 public:
  ...
  inline void Lock() TURBO_EXCLUSIVE_LOCK_FUNCTION(lock_) { lock(); }

  void unlock() TURBO_UNLOCK_FUNCTION(lock_) { lock_.unlock(); }
  inline void Unlock() TURBO_UNLOCK_FUNCTION(lock_) { unlock(); }
...
class RetiredFlagObj final : public CommandLineFlag {
 ...
 private:
  ...
  // Any input is valid
  bool ValidateInputValue(std::string_view) const override {
    OnAccess();
    return true;
  }
  ...
  bool ParseFrom(std::string_view, flags_internal::FlagSettingMode,
...
CommandLineFlag* FindCommandLineFlag(std::string_view name) {
