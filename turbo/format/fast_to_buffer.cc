...
namespace turbo::format_internal {
    // Digit conversion.
    KUMO_CONST_INIT KUMO_DLL const char kHexChar[] =
            "0123456789abcdef";

    KUMO_CONST_INIT KUMO_DLL const char kHexTable[513] =
    ...
    namespace {
        ...
        inline KUMO_ATTRIBUTE_ALWAYS_INLINE char * turbo_nonnull EncodeFullU32(
        ...
        inline KUMO_ATTRIBUTE_ALWAYS_INLINE char * turbo_nonnull EncodeFullU64(
        ...
        inline KUMO_ATTRIBUTE_ALWAYS_INLINE char * turbo_nonnull EncodeFullU128(
    ...
    char * turbo_nonnull round_trip_double_to_buffer(
        ...
        if (std::abs(d) <= kDoublePrecisionCheckMax) {
            int snprintf_result =
                    snprintf(buffer, kFastToBufferSize, "%.*g",
                             std::numeric_limits<double>::digits10, d);

            // The snprintf should never overflow because the buffer is significantly
            // larger than the precision we asked for.
            KUMO_ASSERT(snprintf_result > 0 &&
                        snprintf_result < kFastToBufferSize);
