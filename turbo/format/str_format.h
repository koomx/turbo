...
// Example:
//
//   std::string s = turbo::str_sprintf(
//                      "%s %s You have $%d!", "Hello", name, dollars);
//
// The library consists of the following basic utilities:
//
//   * `turbo::str_sprintf()`, a type-safe replacement for `std::sprintf()`, to
//     write a format string to a `string` value.
//   * `turbo::str_append_sprintf()` to append a format string to a `string`
//   * `turbo::stream_printf()` to more efficiently write a format string to a
//     stream, such as`std::cout`.
//   * `turbo::str_printf()`, `turbo::str_fprintf()` and `turbo::str_snprintf()` as
//     drop-in replacements for `std::printf()`, `std::fprintf()` and
//     `std::snprintf()`.
//
//     Note: An `turbo::SPrintF()` drop-in replacement is not supported as it
//     is generally unsafe due to buffer overflows. Use `turbo::str_sprintf` which
...
//
//   * A generic `str_printf_to()` function to write outputs to arbitrary sink types,
//     which must implement a `FormatRawSink` interface.
//
//   * A `str_printf_untyped_to()` function that is similar to `str_printf_to()` except it is
//     loosely typed. `str_printf_untyped_to()` is not a template and does not perform
//     any compile-time checking of the format string; instead, it returns a
//     boolean from a runtime check.
//
// In addition, the `str_format` library provides extension points for
// augmenting formatting to new types.  See "str_sprintf Extensions" below.
...
#include <turbo/format/str_format/arg.h>  // IWYU pragma: export
#include <turbo/format/str_format/bind.h>  // IWYU pragma: export
#include <turbo/format/str_format/checker.h>  // IWYU pragma: export
#include <turbo/format/str_format/extension.h>  // IWYU pragma: export
#include <turbo/format/str_format/parser.h>  // IWYU pragma: export
#include <string_view>
...
namespace turbo {
    ...
    // `str_printf_untyped_to()`.
    //
    // Example:
    //
    //   turbo::UntypedFormatSpec format("%d");
    //   std::string out;
    //   KCHECK(turbo::str_printf_untyped_to(&out, format, {turbo::FormatArg(1)}));
    ...
    protected:
        explicit UntypedFormatSpec(
            const str_format_internal::ParsedFormatBase * turbo_nonnull pc)
            : spec_(pc) {
        }
    ...
    // intrinsic type support within `str_sprintf()` itself.
    //
    // Example:
    //
    //   turbo::str_sprintf("%s", turbo::FormatStreamed(obj));
    template<typename T>
    ...
    // FormatCountCapture
    //
    // This class provides a way to safely wrap `str_sprintf()` captures of `%n`
    // conversions, which denote the number of characters written by a formatting
    // operation to this point, into an integer value.
    //
    // This wrapper is designed to allow safe usage of `%n` within `str_sprintf(); in
    ...
    //
    //   int n = 0;
    //   std::string s = turbo::str_sprintf("%s%d%n", "hello", 123,
    //                       turbo::FormatCountCapture(&n));
    //   EXPECT_EQ(8, n);
    class FormatCountCapture {
    public:
        explicit FormatCountCapture(int * turbo_nonnull p) : p_(p) {
        }
    ...
    private:
        ...
        // class.
        int * turbo_nonnull Unused() { return p_; }
        int * turbo_nonnull p_;
    };
    ...
    //   turbo::str_sprintf(format_string, "TheVillage", 6);
    //
    //   // Verified at runtime.
    //   auto format_runtime = turbo::ParsedFormat<'d'>::New(format_string);
    //   if (format_runtime) {
    //     value = turbo::str_sprintf(*format_runtime, i);
    //   } else {
    //     ... error case ...
    //   }
    ...
    // additional arguments. Use it as you would `sprintf()`. `str_sprintf()` is the
    ...
    // `str_sprintf()`'s other arguments. See the comments for `FormatSpec` for full
    ...
    template<typename... Args>
    [[nodiscard]] std::string str_sprintf(const FormatSpec<Args...> &format,
                                        const Args &... args) {
    ...
    //   std::cout << str_append_sprintf(&orig, "%12.6f", 3.14);
    template<typename... Args>
    std::string &str_append_sprintf(std::string * turbo_nonnull dst,
    ...
    // `turbo::str_sprintf()`. The returned object must be streamed before the full
    // expression ends.
    //
    // Example:
    //
    //   std::cout << stream_printf("%12.6f", 3.14);
    template<typename... Args>
    [[nodiscard]] str_format_internal::Streamable stream_printf(
    ...
    // prefer `turbo::str_printf()` over `std::printf()`.
    //
    // Example:
    //
    //   std::string_view s = "Ulaanbaatar";
    //   turbo::str_printf("The capital of Mongolia is %s", s);
    //
    //   Outputs: "The capital of Mongolia is Ulaanbaatar"
    //
    template<typename... Args>
    int str_printf(const FormatSpec<Args...> &format, const Args &... args) {
    ...
    // prefer `turbo::str_fprintf()` over `std::fprintf()`.
    //
    // Example:
    //
    //   std::string_view s = "Ulaanbaatar";
    //   turbo::str_fprintf(stdout, "The capital of Mongolia is %s", s);
    //
    //   Outputs: "The capital of Mongolia is Ulaanbaatar"
    //
    template<typename... Args>
    int str_fprintf(std::FILE * turbo_nonnull output, const FormatSpec<Args...> &format,
                const Args &... args) {
        return str_format_internal::FprintF(
    ...
    // type-safe); prefer `turbo::str_snprintf()` over `std::snprintf()`.
    //
    // In particular, a successful call to `turbo::str_snprintf()` writes at most `size`
    ...
    //   std::string_view s = "Ulaanbaatar";
    //   char output[128];
    //   turbo::str_snprintf(output, sizeof(output),
    //                  "The capital of Mongolia is %s", s);
    //
    //   Post-condition: output == "The capital of Mongolia is Ulaanbaatar"
    //
    template<typename... Args>
    int str_snprintf(char * turbo_nonnull output, std::size_t size,
                 const FormatSpec<Args...> &format, const Args &... args) {
    ...
    public:
        ...
        FormatRawSink(T * turbo_nonnull raw) // NOLINT
            : sink_(raw) {
        }
    ...
    // appended to it.
    //
    // `turbo::str_printf_to()` is a generic version of `turbo::str_append_sprintf()`, for
    // custom sinks. The format string, like format strings for `str_sprintf()`, is
    // checked at compile-time.
    //
    // On failure, this function returns `false` and the state of the sink is
    // unspecified.
    template<typename... Args>
    bool str_printf_to(FormatRawSink raw_sink, const FormatSpec<Args...> &format,
                const Args &... args) {
        return str_format_internal::str_printf_untyped_to(
    ...
    // `str_printf_untyped_to()`. You may construct `FormatArg` by passing
    ...
    //     if (!turbo::str_printf_untyped_to(&out, format, args)) {
    //       return std::nullopt;
    //     }
    //     return std::move(out);
    //   }
    //
    [[nodiscard]] inline bool str_printf_untyped_to(FormatRawSink raw_sink,
    ...
    //------------------------------------------------------------------------------
    // str_sprintf Extensions
    //------------------------------------------------------------------------------
    //
    // turbo_stringify()
    //
    // A simpler customization API for formatting user-defined types using
    // turbo::str_sprintf(). The API relies on detecting an overload in the
    ...
    //
    // The str_sprintf library provides a customization API for formatting
    // user-defined types using turbo::str_sprintf(). The API relies on detecting an
    // overload in the user-defined type's namespace of a free (non-member)
    // `turbo_format_convert()` function, usually as a friend definition with the
    // following signature:
    //
    ...
    //
    // An `turbo_format_convert()` overload for a type should only be declared in the
    ...
    // turbo_format_convert() routine accepts.  The return value should be {true}.
    // A return value of {false} will result in `str_sprintf()` returning
    ...
    // struct Point {
    //   // To add formatting support to `Point`, we simply need to add a free
    //   // (non-member) function `turbo_format_convert()`.  This method interprets
    ...
    //       turbo::str_printf_to(s, "x=%vy=%v", p.x, p.y);
    //     } else {
    //       turbo::str_printf_to(s, "%v,%v", p.x, p.y);
    //     }
    //     return {true};
    //   }
    //
...
enum class FormatConversionChar : uint8_t {
  ...
  n, p, v                  // misc
    ...
    // FormatConvertResult for custom implementations of `turbo_format_convert`.
    ...
    private:
        ...
        explicit FormatSink(str_format_internal::FormatSinkImpl * turbo_nonnull s)
            : sink_(s) {
        }

        str_format_internal::FormatSinkImpl * turbo_nonnull sink_;
    };

    // FormatConvertResult
    //
    // Indicates whether a call to turbo_format_convert() was successful.
    // This return type informs the str_sprintf extension framework (through
    // ADL but using the return type) of what conversion characters are supported.
    // It is strongly discouraged to return {false}, as this will result in an
    // empty string in str_sprintf.
