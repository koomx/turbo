...
namespace turbo {
    namespace str_format_internal {
        namespace {
            ...
            template<>
            struct MakeUnsigned<turbo::int128> {
                using type = turbo::uint128;
            };

            template<>
            struct MakeUnsigned<turbo::uint128> {
                using type = turbo::uint128;
            };
            ...
            template<>
            struct IsSigned<turbo::int128> : std::true_type {
            };

            template<>
            struct IsSigned<turbo::uint128> : std::false_type {
            };
            ...
            public:
                ...
                void PrintAsDec(T v) {
                    ...
                    start_ = storage_;
                    size_ = static_cast<size_t>(format_internal::fast_int_to_buffer(v, storage_) -
                                                storage_);
                }
                ...
                void PrintAsDec(uint128 v, bool add_neg = false) {
                    // This function can be sped up if needed. We can call fast_int_to_buffer
                    // twice, or fix fast_int_to_buffer to support uint128.
                    char *p = storage_ + sizeof(storage_);
                    do {
                        p -= 2;
                        format_internal::put_two_digits(static_cast<uint32_t>(v % 100), p);
                        v /= 100;
                    } while (v);
                    if (p[0] == '0') {
                ...
                void PrintAsHexLower(T v) {
                    ...
                    do {
                        p -= 2;
                        constexpr const char *table = format_internal::kHexTable;
            ...
            std::string_view BaseIndicator(const IntDigits &as_digits,
                ...
                if (alt && hex && !as_digits.without_neg_or_zero().empty()) {
                    return conv.conversion_char() == FormatConversionCharInternal::X
                               ? "0X"
                               : "0x";
                }
                return {};
            ...
            inline bool ConvertStringArg(const wchar_t *v,
                ...
                return ConvertStringArg(std::string_view(mb.data(), chars_written), conv, sink);
            }
        ...
        IntegralConvertResult FormatConvertImpl(turbo::int128 v,
                                                const FormatConversionSpecImpl conv,
                                                FormatSinkImpl *sink) {
            return {ConvertIntArg(v, conv, sink)};
        }

        IntegralConvertResult FormatConvertImpl(turbo::uint128 v,
                                                const FormatConversionSpecImpl conv,
                                                FormatSinkImpl *sink) {
            return {ConvertIntArg(v, conv, sink)};
        }
