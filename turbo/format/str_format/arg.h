...
namespace turbo {
    ...
    namespace str_format_internal {
        ...
        template<typename T>
        struct HasUserDefinedConvert<T, std::void_t<decltype(turbo_format_convert(
                std::declval<const T &>(),
        ...
        auto FormatConvertImpl(const T &v, FormatConversionSpecImpl conv,
                               FormatSinkImpl *sink)
        -> decltype(turbo_format_convert(v,
                                         std::declval<const FormatConversionSpec &>(),
                                         std::declval<FormatSink *>())) {
            ...
            return turbo_format_convert(v, fcs, &fs);
        }
        ...
        -> std::enable_if_t<std::is_enum_v<T> &&
                            std::is_void_v<decltype(turbo_stringify(
        ...
        auto FormatConvertImpl(const T &v, FormatConversionSpecImpl,
                               FormatSinkImpl *sink)
        -> std::enable_if_t<!std::is_enum_v<T> && !std::is_same_v<T, turbo::Cord> &&
                            std::is_void_v<decltype(turbo_stringify(
                                    std::declval<FormatSink &>(), v))>,
        ...
        // turbo_format_convert(v, conv, sink) is intended to be found by ADL on 'v'
        // as an extension mechanism. These FormatConvertImpl functions are the default
        // implementations.
        // The ADL search is augmented via the 'Sink*' parameter, which also
        // serves as a disambiguator to reject possible unintended 'turbo_format_convert'
        ...
        template<class TurboCord,
                std::enable_if_t<std::is_same_v<TurboCord, turbo::Cord> > * = nullptr>
        ...
        private:
            ...
            struct DecayType<T,
                    std::enable_if_t<!str_format_internal::HasUserDefinedConvert<T>::value &&
                                     !HasTurboStringify<T>::value && std::is_enum_v<T>>
            > {
        ...
        public:
            ...
            template<typename T>
            static bool to_int(Data arg, int *out, std::true_type /* is_integral */,
                              std::false_type) {
            ...
            template<typename T>
            static bool to_int(Data arg, int *out, std::false_type,
            ...
            template<typename T>
            static bool to_int(Data, int *, std::false_type, std::false_type) {
                return false;
            }
            ...
            static bool Dispatch(Data arg, FormatConversionSpecImpl spec, void *out) {
                ...
                if (KUMO_UNLIKELY(spec.conversion_char() ==
                                  FormatConversionCharInternal::kNone)) {
                    return to_int<T>(arg, static_cast<int *>(out), std::is_integral<T>(),
                                    std::is_enum<T>());
                }
                if (KUMO_UNLIKELY(
