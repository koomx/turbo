...
namespace turbo {
    namespace str_format_internal {
        namespace {
            using ::turbo::numeric_internal::IsDoubleDouble;
  ...
  KUMO_ATTRIBUTE_NOINLINE static void RunWithCapacityImpl(Func f) {
            ...
            public:
                ...
                static void RunConversion(uint128 v, int exp,
                                          turbo::FunctionRef<void(BinaryToDecimal)> f) {
                    assert(exp > 0);
                ...
                std::string_view CurrentDigits() const {
                    return std::string_view(&digits_[kDigitsPerChunk - size_], size_);
                }
            ...
            public:
                ...
                static void RunConversion(
                    uint128 v, int exp, turbo::FunctionRef<void(FractionalDigitGenerator)> f) {
                    ...
                    static_assert(StackArray::kMaxCapacity >=
                                  ...
                                  "");
                    StackArray::RunWithCapacity(
                        ChunksNeeded(exp), [=](turbo::Span<uint32_t> input) {
            ...
            template<typename Int>
            void FormatFFast(Int v, int iexp, const FormatState &state) {
                ...
                if (iexp >= 0) {
                    const int total_bits = input_bits - LeadingZeros(v) + iexp;
                    integral_digits_start =
                            total_bits <= 64
                                ? PrintIntegralDigitsFromRightFast(static_cast<uint64_t>(v) << iexp,
                                                                   integral_digits_end)
                                : PrintIntegralDigitsFromRightFast(static_cast<uint128>(v) << iexp,
                                                                   integral_digits_end);
                } else {
                    iexp = -iexp;

                    integral_digits_start = PrintIntegralDigitsFromRightFast(
                        iexp<input_bits ? v >> iexp : 0, integral_digits_end);
                    ...
                    fractional_digits_end =
                            iexp <= 64
                                ? PrintFractionalDigitsFast(v, fractional_digits_start, iexp,
                                                            state.precision)
                                : PrintFractionalDigitsFast(static_cast<uint128>(v),
                                                            fractional_digits_start, iexp,
                                                            state.precision);
                ...
                FinalPrint(state, std::string_view(integral_digits_start, size),
                           /*padding_offset=*/0,
            ...
            void FormatF(Int mantissa, int exp, const FormatState &state) {
                if (exp >= 0) {
                    ...
                    // 128 bit state.
                    if (KUMO_UNLIKELY(total_bits > 128)) {
                        return FormatFPositiveExpSlow(mantissa, exp, state);
                    }
                } else {
                    // Fallback to the slow stack-based approach if we can't do it in a 64 or
                    // 128 bit state.
                    if (KUMO_UNLIKELY(exp < -128)) {
            ...
            void FormatANormalize(const HexFloatTypeParams float_traits, uint8_t *leading,
                ...
                while (*mantissa && !(*mantissa & kHighIntBit)) {
                    if (KUMO_UNLIKELY(*exp - 1 < float_traits.min_exponent)) {
            ...
            void FormatA(const HexFloatTypeParams float_traits, Int mantissa, int exp,
                ...
                *digits_iter++ = digits[leading];
                ...
                auto digits_result = std::string_view(
                    digits_buffer, static_cast<size_t>(digits_iter - digits_buffer));

                // =============== Exponent ==================
                constexpr size_t kBufSizeForExpDecRepr =
                        format_internal::kFastToBufferSize // required for fast_int_to_buffer
                        + 1 // 'p' or 'P'
                        + 1; // '+' or '-'
                char exp_buffer[kBufSizeForExpDecRepr];
                exp_buffer[0] = uppercase ? 'P' : 'p';
                exp_buffer[1] = exp >= 0 ? '+' : '-';
                format_internal::fast_int_to_buffer(exp < 0 ? -exp : exp, exp_buffer + 2);
            ...
            char *CopyStringTo(std::string_view v, char *out) {
            ...
            bool FallbackToSnprintf(const Float v, const FormatConversionSpecImpl &conv,
                ...
                while (true) {
                    ...
                    if (static_cast<size_t>(n) < space.size()) {
                        result = std::string_view(space.data(), static_cast<size_t>(n));
                        break;
                    }
            ...
            DigitRun GetDigits(BinaryToDecimal &btd, std::string_view &digits_view) {
            ...
            void FormatEFast(Int v, int exp, bool uppercase, const FormatState &state) {
                if (!v) {
                    std::string_view mantissa_str = state.ShouldPrintDot() ? "0." : "0";
                ...
                if (exp >= 0) {
                    integral_end = total_bits <= 64
                                       ? format_internal::fast_int_to_buffer(
                    ...
                    *integral_end = '0';
                    ...
                    if (exp < kInputBits) {
                        integral_end =
                                format_internal::fast_int_to_buffer(v >> exp, integral_start);
                    }
                    *integral_end = '0';
                ...
                constexpr size_t kExpBufferSize = format_internal::kFastToBufferSize + 2;
                ...
                *exp_ptr++ = uppercase ? 'E' : 'e';
                ...
                exp_ptr = format_internal::fast_int_to_buffer(scientific_exp, exp_ptr);
                FinalPrint(state,
                           std::string_view(final_start,
                                            static_cast<size_t>(final_end - final_start)),
                           0, trailing_zeros,
                           std::string_view(exp_ptr_start,
            ...
            void FormatENegativeExpSlow(uint128 mantissa, int exp, bool uppercase,
                ...
                FractionalDigitGenerator::RunConversion(
                    ...
                    [&](FractionalDigitGenerator digit_gen) {
                        ...
                        char exp_buffer[format_internal::kFastToBufferSize];
                        ...
                        *exp_start++ = '-';
                        ...
                        scientific_exp *= -1;
                        char *exp_end =
                                format_internal::fast_int_to_buffer(scientific_exp, exp_start);
                        const size_t total_digits =
                                1 // First digit
                                +
                                ((precision > 0 || state.conv.has_alt_flag())
                                     ? 1
                                     : 0) // Decimal point
            ...
            void FormatEPositiveExpSlow(uint128 mantissa, int exp, bool uppercase,
                ...
                BinaryToDecimal::RunConversion(
                    mantissa, exp, [&](BinaryToDecimal btd) {
                        ...
                        std::string_view digits_view = btd.CurrentDigits();
                        ...
                        char exp_buffer[format_internal::kFastToBufferSize];
                        char *exp_buffer_end =
                                format_internal::fast_int_to_buffer(scientific_exp, exp_buffer);
                        const bool print_dot =
                                (state.precision > digits_to_trim) || state.conv.has_alt_flag();
            ...
            void FormatGPositiveExpSlow(Int mantissa, int exp, bool uppercase,
                ...
                BinaryToDecimal::RunConversion(mantissa, exp, [&](BinaryToDecimal btd) {
                    int scientific_exp = static_cast<int>(btd.TotalDigits()) - 1;
                    std::string_view digits = btd.CurrentDigits();
                    size_t digits_to_go = state.precision + 1;
