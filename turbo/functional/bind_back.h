...
// -----------------------------------------------------------------------------
//
// `turbo::bind_back()` returns a functor by binding a number of arguments to
...
// `turbo::bind_back()` allows you to avoid known misuses with `std::bind()`.
//
// `turbo::bind_back()` is meant as a drop-in replacement for C++23's
...
#ifndef TURBO_FUNCTIONAL_BIND_BACK_H_
#define TURBO_FUNCTIONAL_BIND_BACK_H_
...
#include <turbo/functional/internal/back_binder.h>
...
//
// Like `std::bind()`, `turbo::bind_back()` is implicitly convertible to
// `std::function`.  In particular, it may be used as a simpler replacement for
// `std::bind()` in most cases, as it does not require placeholders to be
// specified. More importantly, it provides more reliable correctness guarantees
// than `std::bind()`; while `std::bind()` will silently ignore passing more
// parameters than expected, for example, `turbo::bind_back()` will report such
// mis-uses as errors. In C++23, `turbo::bind_back` is replaced by
// `std::bind_back`.
//
#if defined(__cpp_lib_bind_back) && __cpp_lib_bind_back >= 202202L
using std::bind_back;
...
#endif  // TURBO_FUNCTIONAL_BIND_BACK_H_
