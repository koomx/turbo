...
// FunctionRef
//
// An `turbo::FunctionRef` is a lightweight wrapper to any invocable object with
...
class KUMO_ATTRIBUTE_VIEW FunctionRef<R(Args...)> {
 protected:
  ...
  FunctionRef(std::in_place_t, F&& f KUMO_ATTRIBUTE_LIFETIME_BOUND) noexcept
 ...
 public:
  ...
  template <typename F,
            typename = EnableIfCompatible<std::enable_if_t<
                !std::is_same_v<FunctionRef, turbo::remove_cvref_t<F>>, F&>>>
  // NOLINTNEXTLINE(google-explicit-constructor)
  FunctionRef(F&& f KUMO_ATTRIBUTE_LIFETIME_BOUND) noexcept
  ...
  template <typename F, typename = EnableIfCompatible<F*>,
            turbo::functional_internal::EnableIf<std::is_function_v<F>> = 0>
  // NOLINTNEXTLINE(google-explicit-constructor)
  FunctionRef(F* f KUMO_ATTRIBUTE_LIFETIME_BOUND) noexcept
  ...
  FunctionRef(nontype_t<F>) noexcept  // NOLINT(google-explicit-constructor)
      : invoker_(&turbo::functional_internal::InvokeFunction<decltype(F), F, R,
                                                            Args...>) {}

  template <
      auto F, typename Obj,
      typename = EnableIfCompatible<decltype(F), std::remove_reference_t<Obj>&>,
      turbo::functional_internal::EnableIf<!std::is_rvalue_reference_v<Obj&&>> =
          0>
  // NOLINTNEXTLINE(google-explicit-constructor)
  FunctionRef(nontype_t<F>, Obj&& obj KUMO_ATTRIBUTE_LIFETIME_BOUND) noexcept
      : invoker_(&turbo::functional_internal::InvokeObject<Obj&, decltype(F), F,
                                                          R, Args...>) {
  ...
  // NOLINTNEXTLINE(google-explicit-constructor)
  FunctionRef(nontype_t<F>, Obj* obj KUMO_ATTRIBUTE_LIFETIME_BOUND) noexcept
      : invoker_(&turbo::functional_internal::InvokePtr<Obj, decltype(F), F, R,
                                                       Args...>) {
    ptr_.obj = obj;
  }
#endif
...
class KUMO_ATTRIBUTE_VIEW
 ...
 public:
  template <
      typename F,
      typename = EnableIfCompatible<std::enable_if_t<
          !std::is_same_v<FunctionRef, turbo::remove_cvref_t<F>>, const F&>>>
  // NOLINTNEXTLINE(google-explicit-constructor)
  FunctionRef(const F& f KUMO_ATTRIBUTE_LIFETIME_BOUND) noexcept
      : Base(std::in_place_t(), f) {}

  template <typename F, typename = EnableIfCompatible<F*>,
            turbo::functional_internal::EnableIf<std::is_function_v<F>> = 0>
  // NOLINTNEXTLINE(google-explicit-constructor)
  FunctionRef(F* f KUMO_ATTRIBUTE_LIFETIME_BOUND) noexcept : Base(f) {}
  ...
  template <auto F, typename Obj,
            ...
            turbo::functional_internal::EnableIf<
                !std::is_rvalue_reference_v<Obj&&>> = 0>
  // NOLINTNEXTLINE(google-explicit-constructor)
  FunctionRef(nontype_t<F> arg,
              Obj&& obj KUMO_ATTRIBUTE_LIFETIME_BOUND) noexcept
  ...
  // NOLINTNEXTLINE(google-explicit-constructor)
  FunctionRef(nontype_t<F> arg,
              const Obj* obj KUMO_ATTRIBUTE_LIFETIME_BOUND) noexcept
      : Base(arg, obj) {}
#endif
