...
// IWYU pragma: private, include "turbo/functional/any_invocable.h"
...
template <class Sig>
class TURBO_NULLABILITY_COMPATIBLE AnyInvocable;
...
#pragma GCC diagnostic pop
  ...
  void InitializeRemoteManager(Args&&... args) {
    if constexpr (std::is_trivially_destructible_v<T> &&
                  alignof(T) <= KUMO_DEFAULT_NEW_ALIGNMENT) {
  ...
  class Impl<ReturnType(P...) cv ref noexcept(noex)>                           \
   ...
   public:                                                                     \
    ...
    using CallIsNoexceptIfSigIsNoexcept =                                      \
        TrueAlias<TURBO_INTERNAL_ANY_INVOCABLE_NOEXCEPT_CONSTRAINT_##noex(      \
            inv_quals)>;                                                       \
                                                                               \
    ...
    static ReturnType InvokedAfterMove(                                        \
      ...
      KUMO_HARDENING_ASSERT(false && "AnyInvocable use-after-move");           \
      std::terminate();                                                        \
    }                                                                          \
                                                                               \
