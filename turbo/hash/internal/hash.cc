...
#error TURBO_AES_INTERNAL_HAVE_X86_SIMD cannot be directly set
#elif defined(__SSE4_2__) && defined(__AES__)
#define TURBO_AES_INTERNAL_HAVE_X86_SIMD
#endif
...
#include <wmmintrin.h>
#include <xmmintrin.h>
#endif  // TURBO_AES_INTERNAL_HAVE_X86_SIMD
...
#if defined(TURBO_AES_INTERNAL_HAVE_X86_SIMD) || \
    defined(TURBO_AES_INTERNAL_HAVE_ARM_SIMD)
...
#else  // TURBO_AES_INTERNAL_HAVE_ARM_SIMD
...
[[maybe_unused]] KUMO_ATTRIBUTE_NOINLINE uint64_t
LowLevelHashLenGt64(uint64_t seed, const void* data, size_t len) {
  ...
  auto mix_ab = [&state0,
                 &state1](const uint8_t* p) KUMO_ATTRIBUTE_ALWAYS_INLINE {
  ...
  auto mix_cd = [&state2,
                 &state3](const uint8_t* p) KUMO_ATTRIBUTE_ALWAYS_INLINE {
...
KUMO_ATTRIBUTE_ALWAYS_INLINE inline uint64_t HashBlockOn64Bit(
    uint64_t state, const unsigned char* data, size_t len) {
...
uint64_t CombineLargeContiguousImplOn32BitLengthGt8(uint64_t state,
  ...
  if (KUMO_LIKELY(len <= PiecewiseChunkSize())) {
...
KUMO_CONST_INIT const void* const MixingHashState::kSeed = &kSeed;
