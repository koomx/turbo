...
// For feature testing and determining which headers can be included.
    ...
    defined(TURBO_INTERNAL_VERSION_HEADER_AVAILABLE)
#include <version>
#else
#include <ciso646>
#endif
...
inline uint64_t Read4To8(const unsigned char* p, size_t len) {
  ...
  uint64_t most_significant =
      static_cast<uint64_t>(turbo::base_internal::UnalignedLoad32(p)) << 32;
  uint64_t least_significant =
      turbo::base_internal::UnalignedLoad32(p + len - 4);
...
KUMO_ATTRIBUTE_ALWAYS_INLINE inline uint64_t CombineRawImpl(uint64_t state,
  ...
  s.u32s.high =
      static_cast<uint32_t>(TURBO_HASH_INTERNAL_CRC32_U64(s.u32s.high, value));
