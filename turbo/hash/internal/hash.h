...
// We include config.h here to make sure that KUMO_CPLUSPLUS_LANG is
// defined.
...
#define TURBO_INTERNAL_VERSION_HEADER_AVAILABLE 1
#endif
#endif

// For feature testing and determining which headers can be included.
#if KUMO_CPLUSPLUS_LANG >= 202002L || defined(TURBO_INTERNAL_VERSION_HEADER_AVAILABLE)
#include <version>
#else
#include <ciso646>
#endif

#undef TURBO_INTERNAL_VERSION_HEADER_AVAILABLE
...
#if defined(__cpp_lib_filesystem) && __cpp_lib_filesystem >= 201703L && !defined(__XTENSA__)
#include <filesystem> // NOLINT
#endif

// We are allowed to use a non-portable hardware-accelerated implementation in
// headers if KUMO_OPTION_INLINE_HW_ACCEL_STRATEGY != 0
#if KUMO_OPTION_INLINE_HW_ACCEL_STRATEGY != 0
...
#include <x86intrin.h>
#define TURBO_HASH_INTERNAL_HAS_CRC32
#define TURBO_HASH_INTERNAL_CRC32_U64 _mm_crc32_u64
#define TURBO_HASH_INTERNAL_CRC32_U32 _mm_crc32_u32
#define TURBO_HASH_INTERNAL_CRC32_U8 _mm_crc32_u8

// 32-bit builds with AVX do not have _mm_crc32_u64, so the _M_X64 condition is
// necessary.
#elif defined(_MSC_VER) && !defined(__clang__) && defined(__AVX__) && defined(_M_X64)
...
#define TURBO_HASH_INTERNAL_CRC32_U64(crc, data) \
    __crc32cd(static_cast<uint32_t>(crc), data)
#define TURBO_HASH_INTERNAL_CRC32_U32 __crc32cw
#define TURBO_HASH_INTERNAL_CRC32_U8 __crc32cb

#endif // Platform tests

#endif // KUMO_OPTION_INLINE_HW_ACCEL_STRATEGY != 0

#if KUMO_OPTION_INLINE_HW_ACCEL_STRATEGY == 1
#ifndef TURBO_HASH_INTERNAL_HAS_CRC32
#error "Hardware acceleration is required by KUMO_OPTION_INLINE_HW_ACCEL_STRATEGY but not supported on this platform; see turbo/base/options.h"
#endif
#endif
...
namespace turbo {
    ...
    namespace hash_internal {
        ...
        // Used for the TurboHashValue implementations for composite types below.
        template <typename T>
        struct is_hashable;
        ...
        // inherit from this.
        //
        // A hash state object is the template argument `H` passed to `TurboHashValue`.
        ...
        H hash_bytes(H hash_state, const T& value) {
            ...
            if constexpr (sizeof(T) == 1) {
                ...
                v = turbo::base_internal::UnalignedLoad16(start);
            } else if constexpr (sizeof(T) == 4) {
                v = turbo::base_internal::UnalignedLoad32(start);
            } else {
                static_assert(sizeof(T) == 8);
                v = turbo::base_internal::UnalignedLoad64(start);
            }
        ...
        std::enable_if_t<std::is_same_v<B, bool>, H> TurboHashValue(H hash_state,
            B value) {
        ...
        std::enable_if_t<std::is_enum_v<Enum>, H> TurboHashValue(H hash_state, Enum e) {
        ...
        TurboHashValue(H hash_state, Float value) {
            ...
            using Bits = std::conditional_t<sizeof(Float) == sizeof(uint32_t), uint32_t, uint64_t>;
        ...
        std::enable_if_t<std::is_same_v<LongDouble, long double>, H> TurboHashValue(
        ...
        H TurboHashValue(H hash_state, T (&)[N]) {
            static_assert(
                sizeof(T) == -1,
        ...
        template <typename H, typename T>
        std::enable_if_t<std::is_pointer_v<T>, H> TurboHashValue(H hash_state, T ptr) {
...
#endif
                std::is_same_v<Char, char16_t> || std::is_same_v<Char, char32_t>>>
        H TurboHashValue(H hash_state, std::basic_string_view<Char> str) {
            return H::combine_contiguous(std::move(hash_state), str.data(), str.size());
        }

#if defined(__cpp_lib_filesystem) && __cpp_lib_filesystem >= 201703L && (!defined(__ENVIRONMENT_IPHONE_OS_VERSION_MIN_REQUIRED__) || __ENVIRONMENT_IPHONE_OS_VERSION_MIN_REQUIRED__ >= 130000) && (!defined(__ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__) || __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ >= 101500) && (!defined(__XTENSA__))
...
#define TURBO_INTERNAL_STD_FILESYSTEM_PATH_HASH_AVAILABLE 1
        ...
        H TurboHashValue(H hash_state, const Path& path) {
            ...
            for (const Path& component : path) {
                ...
                if (!part.empty() && (*part.begin() == '/' || *part.begin() == Path::preferred_separator)) {
        ...
        template <typename H, typename T, size_t N>
        std::enable_if_t<is_hashable<T>::value, H> TurboHashValue(
        ...
        std::enable_if_t<is_hashable<Key>::value, H> TurboHashValue(
        ...
        template <typename H, typename... T>
        std::enable_if_t<std::conjunction_v<is_hashable<T>...>, H> TurboHashValue(
        ...
        KUMO_CACHELINE_ALIGNED inline constexpr uint64_t kStaticRandomData[] = {
            0x243f'6a88'85a3'08d3,
            0x1319'8a2e'0370'7344,
            0xa409'3822'299f'31d0,
            0x082e'fa98'ec4e'6c89,
            0x4528'21e6'38d0'1377,
        };
        ...
        inline uint64_t PrecombineLengthMix(uint64_t state, size_t len) {
            KUMO_ASSUME(len + sizeof(uint64_t) <= sizeof(kStaticRandomData));
            uint64_t data = turbo::base_internal::UnalignedLoad64(
...
#pragma GCC diagnostic ignored "-Warray-bounds"
        ...
        inline uint32_t Read4(const unsigned char* p) {
            return turbo::base_internal::UnalignedLoad32(p);
        }
        inline uint64_t Read8(const unsigned char* p) {
            return turbo::base_internal::UnalignedLoad64(p);
        }
#if defined(__GNUC__) && !defined(__clang__)
#pragma GCC diagnostic pop
        ...
        inline uint64_t Read4To8(const unsigned char* p, size_t len) {
            ...
            // `p + len - 4`.
            uint64_t most_significant = static_cast<uint64_t>(turbo::base_internal::UnalignedLoad32(p)) << 32;
            uint64_t least_significant = turbo::base_internal::UnalignedLoad32(p + len - 4);
        ...
        KUMO_FORCE_INLINE uint64_t CombineRawImpl(uint64_t state,
            ...
            // low and high 32 bits of state as CRC states. Note that: (1) when turbo::Hash
            ...
            s.u32s.high = static_cast<uint32_t>(TURBO_HASH_INTERNAL_CRC32_U64(s.u32s.high, value));
            s.u32s.low = static_cast<uint32_t>(
                TURBO_HASH_INTERNAL_CRC32_U64(s.u32s.low, 3 * value));
            return s.u64;
        }
...
#endif // TURBO_HASH_INTERNAL_HAS_CRC32
        ...
        KUMO_FORCE_INLINE uint64_t CombineContiguousImpl17to32(
            ...
            // latency.
            const uint64_t m0 = Mix(Read8(first) ^ kStaticRandomData[1], Read8(first + 8) ^ state);
        ...
        inline uint64_t CombineContiguousImpl(
            ...
            uint64_t mul = turbo::rotr(kMul, static_cast<int>(len));
            ...
            if (len > 8) {
                crcs = { TURBO_HASH_INTERNAL_CRC32_U64(crcs.first, Read8(first)),
                    TURBO_HASH_INTERNAL_CRC32_U64(crcs.second, Read8(first + len - 8)) };
                if (len > 16) {
                    // We compute the second round of dependent CRC32 operations.
                    crcs = { TURBO_HASH_INTERNAL_CRC32_U64(crcs.first, Read8(first + len - 16)),
                        TURBO_HASH_INTERNAL_CRC32_U64(crcs.second, Read8(first + 8)) };
                }
            } else {
                if (len >= 4) {
                    ...
                    // quality.
                    crcs = { TURBO_HASH_INTERNAL_CRC32_U32(static_cast<uint32_t>(crcs.first),
                                 Read4(first)),
                        TURBO_HASH_INTERNAL_CRC32_U32(static_cast<uint32_t>(crcs.second),
                            Read4(first + len - 4)) };
                } else if (len >= 1) {
                    ...
                    // with each other).
                    crcs = { TURBO_HASH_INTERNAL_CRC32_U8(static_cast<uint32_t>(crcs.first),
                                 first[0]),
                        TURBO_HASH_INTERNAL_CRC32_U8(static_cast<uint32_t>(crcs.second),
                            first[len - 1]) };
...
#define TURBO_HASH_INTERNAL_SUPPORT_LEGACY_HASH_ 0
        ...
        private:
            ...
            struct HashValueProbe {
                ...
                static auto Invoke(H state, const T& value) -> std::enable_if_t<
                    std::is_same_v<H, decltype(TurboHashValue(std::move(state), value))>,
                    H> {
        ...
        class KUMO_DLL MixingHashState : public HashStateBase<MixingHashState> {
            template <typename T>
            using IntegralFastPath = std::conjunction<std::is_integral<T>, is_uniquely_represented<T>,
                FitsIn64Bits<T>>;
        ...
        private:
            ...
            template <typename H>
            friend H turbo::hash_internal::hash_weakly_mixed_integer(H,
                WeaklyMixedInteger);
...
#if (!defined(__clang__) || __clang_major__ > 11) && (!defined(__apple_build_version__) || __apple_build_version__ >= 19558921) // Xcode 12
...
#undef TURBO_HASH_INTERNAL_HAS_CRC32
#undef TURBO_HASH_INTERNAL_CRC32_U64
#undef TURBO_HASH_INTERNAL_CRC32_U32
#undef TURBO_HASH_INTERNAL_CRC32_U8
