...
namespace turbo::xxhash {
    ...
    KUMO_FORCE_INLINE XxHash128 xxhash_mult64to128(uint64_t lhs, uint64_t rhs) {
...
#pragma intrinsic(__umulh)
    ...
    KUMO_FORCE_INLINE uint64_t xxhash_mul128_fold64(uint64_t lhs, uint64_t rhs) {
        XxHash128 product = xxhash_mult64to128(lhs, rhs);
    ...
    && defined(__i386__) && defined(__SSE2__) /* x86 + SSE2 */
...
#endif
        {
            ...
            return xxhash_mul128_fold64(
                input_lo ^ (turbo::little_endian::Load64(secret) + seed64),
                input_hi ^ (turbo::little_endian::Load64(secret + 8) - seed64));
        }
    }

    KUMO_FORCE_INLINE uint64_t xxhash_len_9to16_64b(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed) {
        ...
        KUMO_DASSERT(9 <= len && len <= 16);
        {
            ...
            uint64_t const acc = len
                ...
                + xxhash_mul128_fold64(input_lo, input_hi);
            return xxhash_avalanche(acc);
        }
    }

    KUMO_FORCE_INLINE uint64_t xxhash_len_4to8_64b(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed) {
        KUMO_DASSERT(input != NULL);
        KUMO_DASSERT(secret != NULL);
    ...
    xxhash_len_1to3_64b(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed) {
        KUMO_DASSERT(input != NULL);
    ...
    KUMO_FORCE_INLINE uint64_t xxhash_len_0to16_64b(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed) {
        KUMO_DASSERT(len <= 16);
        {
            if (KUMO_LIKELY(len > 8))
                return xxhash_len_9to16_64b(input, len, secret, seed);
            if (KUMO_LIKELY(len >= 4))
                return xxhash_len_4to8_64b(input, len, secret, seed);
            if (len)
                return xxhash_len_1to3_64b(input, len, secret, seed);
            return xxhash_scalar_avalanche64(seed ^ (turbo::little_endian::Load64(secret + 56) ^ turbo::little_endian::Load64(secret + 64)));
        }
    }

    KUMO_FORCE_INLINE uint64_t
    xxhash_len_17to128_64b(const uint8_t* KUMO_RESTRICT input, size_t len,
        ...
        {
            uint64_t acc = len * kXxhPrime64_1;
            if (len > 32) {
                if (len > 64) {
                    if (len > 96) {
                        acc += xxhash_mix16_b(input + 48, secret + 96, seed);
                        acc += xxhash_mix16_b(input + len - 64, secret + 112, seed);
                    }
                    acc += xxhash_mix16_b(input + 32, secret + 64, seed);
                    acc += xxhash_mix16_b(input + len - 48, secret + 80, seed);
                }
                acc += xxhash_mix16_b(input + 16, secret + 32, seed);
                acc += xxhash_mix16_b(input + len - 32, secret + 48, seed);
            }
            acc += xxhash_mix16_b(input + 0, secret + 0, seed);
            acc += xxhash_mix16_b(input + len - 16, secret + 16, seed);
            return xxhash_avalanche(acc);
        }
    }

    KUMO_FORCE_INLINE uint64_t
    xxhash_len_129to240_64b(const uint8_t* KUMO_RESTRICT input, size_t len,
        const uint8_t* KUMO_RESTRICT secret, size_t secretSize,
        uint64_t seed) {
        KUMO_DASSERT(secretSize >= kXxh3SecretSizeMin);
        (void)secretSize;
        KUMO_DASSERT(128 < len && len <= kXxh3MidsizeMax);
...
#pragma clang loop vectorize(disable)
            ...
            for (i = 8; i < nbRounds; i++) {
                ...
                acc_end += xxhash_mix16_b(input + (16 * i), secret + (16 * (i - 8)) + kXxhashMidSizeStartOffset, seed);
            }
            return xxhash_avalanche(acc + acc_end);
        }
    }

    KUMO_FORCE_INLINE XxHash128 xxhash_len_9to16_128b(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed) {
        KUMO_DASSERT(input != NULL);
        KUMO_DASSERT(secret != NULL);
        KUMO_DASSERT(9 <= len && len <= 16);
        {
            ...
            XxHash128 m128 = turbo::xxhash::xxhash_mult64to128(input_lo ^ input_hi ^ bitflipl, kXxhPrime64_1);
            ...
            if (sizeof(void*) < sizeof(uint64_t)) { /* 32-bit */
                ...
                m128.high64 += input_hi + xxhash_mult32to64((uint32_t)input_hi, kXxhPrime32_2 - 1);
            }
            ...
            { /* 128x64 multiply: h128 = m128 * kXxhPrime64_2; */
                ...
                h128.low64 = xxhash_avalanche(h128.low64);
                h128.high64 = xxhash_avalanche(h128.high64);
                return h128;
            }
        }
    }
    ...
    KUMO_FORCE_INLINE XxHash128 xxhash_len_4to8_128b(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed) {
        KUMO_DASSERT(input != NULL);
        KUMO_DASSERT(secret != NULL);
        KUMO_DASSERT(4 <= len && len <= 8);
        seed ^= (uint64_t)turbo::byteswap((uint32_t)seed) << 32;
        {
            ...
            m128.low64 *= turbo::xxhash::kXxhPrimeMX2;
            m128.low64 = xxhash_scalar_xorshift64(m128.low64, 28);
            m128.high64 = xxhash_avalanche(m128.high64);
            return m128;
        }
    }

    KUMO_FORCE_INLINE XxHash128 xxhash_len_0to16_128b(const uint8_t* input, size_t len, const uint8_t* secret, uint64_t seed) {
        KUMO_DASSERT(len <= 16);
        {
            if (len > 8)
                return xxhash_len_9to16_128b(input, len, secret, seed);
            if (len >= 4)
                return xxhash_len_4to8_128b(input, len, secret, seed);
            if (len)
                return xxhash_len_1to3_128b(input, len, secret, seed);
            {
                XxHash128 h128;
    ...
    KUMO_FORCE_INLINE XxHash128 xxhash_128_mix32_b(XxHash128 acc, const uint8_t* input_1, const uint8_t* input_2,
        const uint8_t* secret, uint64_t seed) {
        acc.low64 += turbo::xxhash::xxhash_mix16_b(input_1, secret + 0, seed);
        acc.low64 ^= turbo::little_endian::Load64(input_2) + turbo::little_endian::Load64(input_2 + 8);
        acc.high64 += turbo::xxhash::xxhash_mix16_b(input_2, secret + 16, seed);
        acc.high64 ^= turbo::little_endian::Load64(input_1) + turbo::little_endian::Load64(input_1 + 8);
        return acc;
    }

    KUMO_FORCE_INLINE XxHash128 xxhash_len_17to128_128b(const uint8_t* KUMO_RESTRICT input, size_t len,
        const uint8_t* KUMO_RESTRICT secret, size_t secretSize,
        uint64_t seed) {
        KUMO_DASSERT(secretSize >= kXxh3SecretSizeMin);
        (void)secretSize;
        ...
        {
            XxHash128 acc;
            acc.low64 = len * kXxhPrime64_1;
            acc.high64 = 0;

            if (len > 32) {
                if (len > 64) {
                    if (len > 96) {
                        acc = xxhash_128_mix32_b(acc, input + 48, input + len - 64, secret + 96, seed);
                    }
                    acc = xxhash_128_mix32_b(acc, input + 32, input + len - 48, secret + 64, seed);
                }
                acc = xxhash_128_mix32_b(acc, input + 16, input + len - 32, secret + 32, seed);
            }
            acc = xxhash_128_mix32_b(acc, input, input + len - 16, secret, seed);
            {
                XxHash128 h128;
                h128.low64 = acc.low64 + acc.high64;
                h128.high64 = (acc.low64 * kXxhPrime64_1)
                    + (acc.high64 * kXxhPrime64_4)
                    + ((len - seed) * kXxhPrime64_2);
                h128.low64 = xxhash_avalanche(h128.low64);
                h128.high64 = (uint64_t)0 - xxhash_avalanche(h128.high64);
                return h128;
            }
        }
    }

    KUMO_FORCE_INLINE XxHash128 xxhash_len_129to240_128b(const uint8_t* KUMO_RESTRICT input, size_t len,
        const uint8_t* KUMO_RESTRICT secret, size_t secretSize,
        uint64_t seed) {
        KUMO_DASSERT(secretSize >= kXxh3SecretSizeMin);
        (void)secretSize;
...

        {
            ...
            acc.low64 = xxhash_avalanche(acc.low64);
            acc.high64 = xxhash_avalanche(acc.high64);
            ...
            acc = xxhash_128_mix32_b(acc,
                input + len - 16,
                input + len - 32,
                secret + kXxh3SecretSizeMin - kXxhashMidSizeLastOffset - 16,
                (uint64_t)0 - seed);
    ...
    KUMO_FORCE_INLINE uint64_t xxhash_mix2_accs(const uint64_t* KUMO_RESTRICT acc, const uint8_t* KUMO_RESTRICT secret) {
        return turbo::xxhash::xxhash_mul128_fold64(
            acc[0] ^ turbo::little_endian::Load64(secret),
            acc[1] ^ turbo::little_endian::Load64(secret + 8));
    }

    KUMO_FORCE_INLINE uint64_t xxhash_merge_accs(const uint64_t* KUMO_RESTRICT acc, const uint8_t* KUMO_RESTRICT secret, uint64_t start) {
    ...
    && (defined(__ARM_NEON) || defined(__ARM_NEON__))
        ...
        return xxhash_avalanche(result64);
    }

    KUMO_FORCE_INLINE uint64_t xxhash_finalize_long_64b(const uint64_t* KUMO_RESTRICT acc, const uint8_t* KUMO_RESTRICT secret, uint64_t len) {
        return turbo::xxhash::xxhash_merge_accs(acc, secret + kXxhSecretMergeAccsStart, len * kXxhPrime64_1);
    }
    KUMO_FORCE_INLINE XxHash128 xxhash_finalize_long_128b(const uint64_t* KUMO_RESTRICT acc, const uint8_t* KUMO_RESTRICT secret, size_t secretSize, uint64_t len) {
        XxHash128 h128;
        h128.low64 = xxhash_finalize_long_64b(acc, secret, len);
        h128.high64 = turbo::xxhash::xxhash_merge_accs(acc, secret + secretSize - kXxhStripeLen - kXxhSecretMergeAccsStart,
            ~(len * kXxhPrime64_2));
        return h128;
    }
