...
namespace turbo {
    namespace log_internal {
        ...
        bool LogEveryNState::should_log(int n) {
            return n > 0 && (LossyIncrement(&counter_) % static_cast<uint32_t>(n)) == 0;
        }

        bool LogFirstNState::should_log(int n) {
        ...
        bool LogEveryPow2State::should_log() {
            const uint32_t new_value = LossyIncrement(&counter_) + 1;
            return (new_value & (new_value - 1)) == 0;
        }

        bool LogEveryNSecState::should_log(double seconds) {
