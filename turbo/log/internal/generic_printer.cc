...
namespace turbo {
    namespace internal_generic_printer {
        ...
        std::string TryShorten(T v, F strtox) {
            std::string printed =
                    turbo::str_sprintf("%.*g", std::numeric_limits<T>::max_digits10 / 2, v);
            T parsed = strtox(printed.data());
            if (parsed != v) {
                printed =
                        turbo::str_sprintf("%.*g", std::numeric_limits<T>::max_digits10 + 1, v);
            }
            return printed;
        }
        ...
        std::ostream &PrintPreciseFP(std::ostream &os, float v) {
            // TryShorten formats with turbo::str_sprintf(), which is locale-independent and
            // always emits a '.' radix. Use turbo::SimpleAtof() for locale-independent
            // parsing.
            return os << TryShorten(v, [](const char *buf) {
                float out = 0;
                static_cast<void>(turbo::SimpleAtof(buf, &out));
                return out;
            }) << "f";
        }
        ...
        std::ostream &PrintPreciseFP(std::ostream &os, double v) {
            // TryShorten formats with turbo::str_sprintf(), which is locale-independent and
            // always emits a '.' radix. Use turbo::SimpleAtod() for locale-independent
            // parsing.
            return os << TryShorten(v, [](const char *buf) {
                double out = 0;
                static_cast<void>(turbo::SimpleAtod(buf, &out));
                return out;
            });
        }
        ...
        static std::ostream &PrintCharImpl(std::ostream &os, T v) {
            ...
            // hex-escaped.
            return (turbo::ascii_isprint(static_cast<unsigned char>(v))
                        ? (os << (v == '\'' ? "'\\" : "'") << v)
