...
#ifndef TURBO_LOG_INTERNAL_GENERIC_PRINTER_INTERNAL_H_
#define TURBO_LOG_INTERNAL_GENERIC_PRINTER_INTERNAL_H_
...
#include <turbo/log/internal/container.h>
    ...
    namespace internal_generic_printer {
        ...
        std::ostream &PrintSmartPointerContents(std::ostream &os,
            ...
            if (v == nullptr) {
                ...
                os << turbo::implicit_cast<const void *>(v.get()) << " pointing to ";
        ...
        template<typename T>
        std::ostream &GenericPrintImpl(std::ostream &os, const T &v) {
            if constexpr (is_any_string<T> || std::is_same_v<T, std::string_view>) {
        ...
        struct GenericPrintStreamAdapter {
            ...
            // effects. (See turbo::log_internal::Voidify for an example of why this might
            // be useful.)
...
#endif  // TURBO_LOG_INTERNAL_GENERIC_PRINTER_INTERNAL_H_
