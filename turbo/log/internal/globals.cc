...
#include <string_view>
#include <turbo/strings/strip.h>  // IWYU pragma: keep
...
namespace turbo {
    namespace log_internal {
        namespace {
            ...
            KUMO_CONST_INIT std::atomic<bool> logging_initialized(false);

            // The TimeZone used for logging. This may only be set once.
            KUMO_CONST_INIT std::atomic<turbo::TimeZone *> timezone_ptr{nullptr};

            // If true, the logging library will symbolize stack in fatal messages
            KUMO_CONST_INIT std::atomic<bool> symbolize_stack_trace(true);

            // Specifies maximum number of stack frames to report in fatal messages.
            KUMO_CONST_INIT std::atomic<int> max_frames_in_stack_trace(64);

            KUMO_CONST_INIT std::atomic<bool> exit_on_dfatal_(true);
            KUMO_CONST_INIT std::atomic<bool> suppress_sigabort_trace_(false);
        } // namespace
        ...
        bool exit_on_dfatal() {
            return exit_on_dfatal_.load(std::memory_order_acquire);
        }

        void set_exit_on_dfatal(bool on_off) {
            exit_on_dfatal_.store(on_off, std::memory_order_release);
        }

        bool suppress_sigabort_trace() {
            return suppress_sigabort_trace_.load(std::memory_order_acquire);
        }

        bool set_suppress_sigabort_trace(bool on_off) {
            return suppress_sigabort_trace_.exchange(on_off);
        }
    } // namespace log_internal
} // namespace turbo
