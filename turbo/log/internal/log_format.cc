...
namespace turbo {
    namespace log_internal {
        namespace {
            ...
            size_t format_bounded_fields(turbo::LogSeverity severity, turbo::Time timestamp,
                ...
                if (KUMO_UNLIKELY(buf.size() < kBoundedFieldsMaxLen)) {
                ...
                if (KUMO_UNLIKELY(tz == nullptr)) {
                    ...
                    int snprintf_result = turbo::str_snprintf(
                ...
                *p++ = turbo::LogSeverityName(severity)[0];
                ...
                turbo::format_internal::put_two_digits(static_cast<uint32_t>(ci.cs.month()), p);
                p += 2;
                turbo::format_internal::put_two_digits(static_cast<uint32_t>(ci.cs.day()), p);
                p += 2;
                *p++ = ' ';
                turbo::format_internal::put_two_digits(static_cast<uint32_t>(ci.cs.hour()), p);
                p += 2;
                *p++ = ':';
                turbo::format_internal::put_two_digits(static_cast<uint32_t>(ci.cs.minute()),
                                                       p);
                p += 2;
                *p++ = ':';
                turbo::format_internal::put_two_digits(static_cast<uint32_t>(ci.cs.second()),
                                                       p);
                p += 2;
                *p++ = '.';
                const int64_t usecs = turbo::ToInt64Microseconds(ci.subsecond);
                turbo::format_internal::put_two_digits(static_cast<uint32_t>(usecs / 10000), p);
                p += 2;
                turbo::format_internal::put_two_digits(static_cast<uint32_t>(usecs / 100 % 100),
                                                       p);
                p += 2;
                turbo::format_internal::put_two_digits(static_cast<uint32_t>(usecs % 100), p);
                p += 2;
                *p++ = ' ';
                PutLeadingWhitespace(tid, p);
                p = turbo::format_internal::fast_int_to_buffer(tid, p);
                *p++ = ' ';
            ...
            size_t format_line_number(int line, turbo::Span<char> &buf) {
                constexpr size_t kLineFieldMaxLen =
                        sizeof(":] ") + (1 + std::numeric_limits<int>::digits10 + 1) - sizeof("");
                if (KUMO_UNLIKELY(buf.size() < kLineFieldMaxLen)) {
                ...
                char *p = buf.data();
                *p++ = ':';
                p = format_internal::fast_int_to_buffer(line, p);
                *p++ = ']';
                *p++ = ' ';
                const size_t bytes_formatted = static_cast<size_t>(p - buf.data());
                buf.remove_prefix(bytes_formatted);
        ...
        std::string format_log_message(turbo::LogSeverity severity,
        ...
        size_t format_log_prefix(turbo::LogSeverity severity, turbo::Time timestamp,
                               log_internal::Tid tid, std::string_view basename,
                               int line, PrefixFormat format, turbo::Span<char> &buf) {
            auto prefix_size = format_bounded_fields(severity, timestamp, tid, buf);
            prefix_size += log_internal::append_truncated(basename, buf);
            prefix_size += format_line_number(line, buf);
            if (format == PrefixFormat::kRaw)
                prefix_size += log_internal::append_truncated("RAW: ", buf);
            return prefix_size;
        }
    } // namespace log_internal
} // namespace turbo
