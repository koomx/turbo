...
// through a closed loop within the turbo::LogMessage object. It is not a robust
// protocol buffer encoder or decoder.
//
// Encoding: When `KLOG(INFO) << "foo"` is called, the library uses the Encode*
...
// `finalize_encoding_and_format()`, which uses `ProtoField::DecodeFrom` to parse
...
namespace turbo {
    namespace log_internal {
        ...
        [[nodiscard]] turbo::Span<char> EncodeMessageStart(uint64_t tag,
                                                           uint64_t max_size,
                                                           turbo::Span<char> *buf) {
            const uint64_t tag_type = MakeTagType(tag, WireType::kLengthDelimited);
            const size_t tag_type_size = VarintSize(tag_type);
