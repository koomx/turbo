...
namespace turbo {
    ...
    public:
        ...
        // and `__LINE__` unless overridden by `KLOG(...).at_location(...)`.
        //
        ...
        // Statements like `KLOG(INFO).at_location(std::string(...), ...)` will expose
        // the bug.  If you need the data later, you must copy them.
        std::string_view source_filename() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
            return full_filename_;
        }

        std::string_view source_basename() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        // `KLOG(...).no_prefix()` or globally by `turbo::enable_log_prefix(false)`.
        ...
        // for `KCHECK`, always `turbo::LogSeverity::kFatal`.
        ...
        // entry. Taken from the argument to `VKLOG` or from
        // `KLOG(...).with_verbosity(...)`.
        ...
        // evaluation of `LOG`, but can be overridden by `KLOG(...).with_thread_id(...)`.
        //
        ...
        std::string_view text_message() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
            return std::string_view(
                text_message_with_prefix_and_newline_and_nul_.data() + prefix_len_,
        ...
        // The buffer does not outlive the entry; if you need the data later, you must
        // copy them.
        std::string_view encoded_message() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
            return encoding_;
        }

        // LogEntry::stacktrace()
        //
        // Optional stacktrace, e.g. for `FATAL` logs and failed `KCHECK`s.
        //
        ...
        // The buffer does not outlive the entry; if you need the data later, you must
        // copy them.
        std::string_view stacktrace() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
            return stacktrace_;
        }
    ...
    private:
        ...
        int verbose_level_; // >=0 for `VKLOG`, etc.; otherwise `kNoVerbosityLevel`.
        ...
        friend void PrintTo(const turbo::LogEntry &entry, std::ostream *os);
    };
} // namespace turbo
