...
namespace turbo {
    // LogStreamer
    //
    // Although you can stream into `KLOG(INFO)`, you can't pass it into a function
    ...
    //       yak, turbo::log_info_streamer(__FILE__, __LINE__).stream());
    //
    //   {
    ...
    public:
        ...
        LogStreamer &operator=(LogStreamer &&that) {
            TURBO_LOG_IF(LEVEL(severity_), stream_).at_location(file_, line_) << buf_;
            severity_ = that.severity_;
            file_ = std::move(that.file_);
            line_ = that.line_;
            buf_ = std::move(that.buf_);
        ...
        ~LogStreamer() {
            TURBO_LOG_IF(LEVEL(severity_), stream_.has_value()).at_location(file_, line_)
                    << buf_;
        }
    ...
    inline LogStreamer log_info_streamer(std::string_view file, int line) {
        return turbo::LogStreamer(turbo::LogSeverity::kInfo, file, line);
    }

    // log_warning_streamer()
    //
    // Returns a LogStreamer that writes at level LogSeverity::kWarning.
    inline LogStreamer log_warning_streamer(std::string_view file, int line) {
    ...
    inline LogStreamer log_error_streamer(std::string_view file, int line) {
    ...
    inline LogStreamer log_fatal_streamer(std::string_view file, int line) {
        return turbo::LogStreamer(turbo::LogSeverity::kFatal, file, line);
    }

    // log_debug_fatal_streamer()
    //
    ...
    inline LogStreamer log_debug_fatal_streamer(std::string_view file, int line) {
