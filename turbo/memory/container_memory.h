...
#define TURBO_SWISSTABLE_IGNORE_UNINITIALIZED(x)                    \
  _Pragma("GCC diagnostic push")                                   \
      _Pragma("GCC diagnostic ignored \"-Wmaybe-uninitialized\"")  \
          _Pragma("GCC diagnostic ignored \"-Wuninitialized\"") x; \
  _Pragma("GCC diagnostic pop")
#define TURBO_SWISSTABLE_IGNORE_UNINITIALIZED_RETURN(x) \
  TURBO_SWISSTABLE_IGNORE_UNINITIALIZED(return x)
#else
#define TURBO_SWISSTABLE_IGNORE_UNINITIALIZED(x) x
#define TURBO_SWISSTABLE_IGNORE_UNINITIALIZED_RETURN(x) return x
#endif
        ...
        struct EqualElement {
            ...
            bool operator()(const K2 &lhs, Args &&...) const {
                TURBO_SWISSTABLE_IGNORE_UNINITIALIZED_RETURN(eq(lhs, rhs));
            }
