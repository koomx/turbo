...
//
// Note that `turbo::WrapUnique(p)` is valid only if `delete p` is a valid
// expression. In particular, `turbo::WrapUnique()` cannot wrap pointers to
...
using std::make_unique KUMO_REFACTOR_INLINE;
...
// `turbo::make_unique_for_overwrite<>`.
template <typename T>
...
// initialized.
//
// `turbo::make_unique_for_overwrite` overload for non-array types.
template <typename T>
...
// `turbo::make_unique_for_overwrite` overload for an array T[] of unknown
...
// `turbo::make_unique_for_overwrite` overload for an array T[N] of known bounds.
...
// -----------------------------------------------------------------------------
//
// Extracts the raw pointer from a pointer-like value `ptr`. `turbo::RawPtr` is
...
//    KCHECK_EQ(sp.get(), wp.lock().get());
//    sp.reset();
//    KCHECK(wp.lock() == nullptr);
//
template <typename T>
...
using pointer_traits KUMO_DEPRECATE_AND_INLINE() = std::pointer_traits<Ptr>;
...
using allocator_traits KUMO_DEPRECATE_AND_INLINE() =
