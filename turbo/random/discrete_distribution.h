...
#ifndef TURBO_RANDOM_DISCRETE_DISTRIBUTION_H_
#define TURBO_RANDOM_DISCRETE_DISTRIBUTION_H_
...
// turbo::discrete_distribution takes O(N) time to precompute the probabilities
  ...
  class param_type {
   ...
   private:
    ...
    static_assert(std::is_integral_v<result_type>,
                  "Class-template turbo::discrete_distribution<> must be "
...
discrete_distribution<IntType>::operator()(
    ...
    const param_type& p) {
  const auto idx = turbo::uniform_int_distribution<result_type>(0, p.n())(g);
  const auto& q = p.q_[idx];
  const bool selected = turbo::bernoulli_distribution(q.first)(g);
