...
// the turbo random distributions.
...
// stream_u128_helper allows us to write out either turbo::uint128 or
...
template <>
struct stream_u128_helper<turbo::uint128> {
  template <typename IStream>
  turbo::uint128 read(IStream& in) {
    uint64_t h = 0;
    uint64_t l = 0;
    in >> h >> l;
  ...
  template <typename OStream>
  void write(turbo::uint128 val, OStream& out) {
