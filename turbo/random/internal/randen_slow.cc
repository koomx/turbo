...
#include <turbo/bits/endian.h>  // IWYU pragma: keep
...
#if 0
  turbo::uint128 source[RandenTraits::kFeistelBlocks];
  std::memcpy(source, state, sizeof(source));
  for (size_t i = 0; i < RandenTraits::kFeistelBlocks; i++) {
    const turbo::uint128 v0 = source[shuffle[i]];
    state[i] = v0;
  }
  return;
#endif

  const turbo::uint128 v0 = state[shuffle[0]];
  const turbo::uint128 v1 = state[shuffle[1]];
  const turbo::uint128 v2 = state[shuffle[2]];
  const turbo::uint128 v3 = state[shuffle[3]];
  const turbo::uint128 v4 = state[shuffle[4]];
  const turbo::uint128 v5 = state[shuffle[5]];
  const turbo::uint128 v6 = state[shuffle[6]];
  const turbo::uint128 v7 = state[shuffle[7]];
  const turbo::uint128 w0 = state[shuffle[8]];
  const turbo::uint128 w1 = state[shuffle[9]];
  const turbo::uint128 w2 = state[shuffle[10]];
  const turbo::uint128 w3 = state[shuffle[11]];
  const turbo::uint128 w4 = state[shuffle[12]];
  const turbo::uint128 w5 = state[shuffle[13]];
  const turbo::uint128 w6 = state[shuffle[14]];
  const turbo::uint128 w7 = state[shuffle[15]];
  state[0] = v0;
  state[1] = v1;
  state[2] = v2;
  state[3] = v3;
...
void RandenSlow::Generate(const void* keys_void, void* state_void) {
  static_assert(RandenTraits::kCapacityBytes == sizeof(turbo::uint128),
                "Capacity mismatch");

  auto* state = reinterpret_cast<turbo::uint128*>(state_void);
  const auto* keys = reinterpret_cast<const turbo::uint128*>(keys_void);

  const turbo::uint128 prev_inner = state[0];
