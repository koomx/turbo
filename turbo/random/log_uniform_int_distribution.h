...
#ifndef TURBO_RANDOM_LOG_UNIFORM_INT_DISTRIBUTION_H_
#define TURBO_RANDOM_LOG_UNIFORM_INT_DISTRIBUTION_H_
  ...
  class param_type {
   ...
   private:
    ...
    static_assert(random_internal::IsIntegral<IntType>::value,
                  "Class-template turbo::log_uniform_int_distribution<> must be "
...
log_uniform_int_distribution<IntType>::Generate(
  ...
  // 2 => [b, (b^2)-1]
  // n => [b^(n-1)..(b^n)-1]
  const int e = turbo::uniform_int_distribution<int>(0, p.log_range())(g);
  if (e == 0) {
    return 0;
  }
  const int d = e - 1;
  ...
  return turbo::uniform_int_distribution<result_type>(
