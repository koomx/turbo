...
#ifndef TURBO_RANDOM_POISSON_DISTRIBUTION_H_
#define TURBO_RANDOM_POISSON_DISTRIBUTION_H_
  ...
  class param_type {
   ...
   private:
    ...
    static_assert(random_internal::IsIntegral<IntType>::value,
                  "Class-template turbo::poisson_distribution<> must be "
...
poisson_distribution<IntType>::param_type::param_type(double mean)
  ...
  if (mean_ < 10) {
    ...
    log_k_ = lmu_ * mode - turbo::random_internal::StirlingLogFactorial(mode);
  }
}
...
poisson_distribution<IntType>::operator()(
  ...
  for (;;) {
    ...
    double s = (x <= 1.0) ? 0.0
             ...
             : turbo::random_internal::StirlingLogFactorial(x);
    // clang-format on
