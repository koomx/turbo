...
#ifndef TURBO_STATUS_STATUS_BUILDER_H_
#define TURBO_STATUS_STATUS_BUILDER_H_
...
namespace turbo {
    ...
    namespace status_internal {
        ...
        // StatusBuilder's operator<< overloads use an TurboStringifyStream to allow us
        // to use turbo_stringify. This wraps (but does not own) an OStringStream, which
        ...
        public:
            explicit Stream(std::string& message)
                : ostringstream_(&message)
                , turbo_stringify_stream_(ostringstream_) { }

            template <typename T>
            friend Stream& operator<<(Stream& stream, const T& t) {
                stream.turbo_stringify_stream_ << t;
                return stream;
            }

        private:
            turbo::strings_internal::OStringStream ostringstream_;
            turbo::strings_internal::StringifyStream turbo_stringify_stream_;
        };
        ...
        inline constexpr bool kResultMatches = std::is_same_v<std::decay_t<std::invoke_result_t<Fn, Arg>>, Expected>;
        ...
        template <typename Adaptor, typename Builder>
        using SideEffect = std::enable_if_t<kResultMatches<Adaptor, Builder, turbo::Status>,
            std::invoke_result_t<Adaptor, Builder>>;

        template <typename Adaptor, typename Builder>
        using Conversion = std::enable_if_t<!kResultMatches<Adaptor, Builder, StatusBuilder> && !kResultMatches<Adaptor, Builder, turbo::Status>,
            std::invoke_result_t<Adaptor, Builder>>;
    ...
    void TurboInternalSetErrorCode(StatusBuilder&, turbo::StatusCode);
    ...
    //       .Log(turbo::LogSeverity::kWarning) << "oh no!";
    //
    // In more detail:
    ...
    class KUMO_MUST_USE_RESULT StatusBuilder {
    public:
        ...
        KUMO_MUST_USE_RESULT StatusBuilder&& SetPrepend() &&;
        ...
        KUMO_MUST_USE_RESULT StatusBuilder&& SetAppend() &&;
        ...
        KUMO_MUST_USE_RESULT StatusBuilder&& SetNoLogging() &&;
        ...
        StatusBuilder& Log(turbo::LogSeverity level) &;
        KUMO_MUST_USE_RESULT StatusBuilder&& Log(turbo::LogSeverity level) &&;

        StatusBuilder& LogError() & { return Log(turbo::LogSeverity::kError); }
        KUMO_MUST_USE_RESULT StatusBuilder&& LogError() && {
            return std::move(LogError());
        }
        StatusBuilder& LogWarning() & { return Log(turbo::LogSeverity::kWarning); }
        KUMO_MUST_USE_RESULT StatusBuilder&& LogWarning() && {
            return std::move(LogWarning());
        }
        StatusBuilder& LogInfo() & { return Log(turbo::LogSeverity::kInfo); }
        KUMO_MUST_USE_RESULT StatusBuilder&& LogInfo() && {
        ...
        StatusBuilder& LogEveryN(turbo::LogSeverity level, int n) &;
        KUMO_MUST_USE_RESULT StatusBuilder&& LogEveryN(turbo::LogSeverity level,
            int n) &&;
        ...
        // If period is turbo::ZeroDuration() or less, then this is equivalent to
        // calling the Log() method.
        StatusBuilder& LogEvery(turbo::LogSeverity level, turbo::Duration period) &;
        KUMO_MUST_USE_RESULT StatusBuilder&& LogEvery(turbo::LogSeverity level,
        ...
        // indicates the verbosity level that would be passed to VKLOG().  This
        ...
        KUMO_MUST_USE_RESULT StatusBuilder&& VLog(int verbose_level) &&;
        ...
        KUMO_MUST_USE_RESULT StatusBuilder&& EmitStackTrace() &&;
        ...
        StatusBuilder& AlsoOutputToSink(turbo::LogSink* sink) &;
        KUMO_MUST_USE_RESULT StatusBuilder&& AlsoOutputToSink(turbo::LogSink* sink) &&;
        ...
        // is not configured to log by calling any of the LogXXX methods. Returns
        // `*this` to allow method chaining.
        StatusBuilder& OnlyOutputToSink(turbo::LogSink* sink) &;
        KUMO_MUST_USE_RESULT StatusBuilder&& OnlyOutputToSink(turbo::LogSink* sink) &&;
        ...
        auto AttachPayload(const MessageSetExtension& obj,
            ...
            -> std::enable_if_t<
                std::is_void_v<decltype(TurboInternalAttachPayload(*this, obj, id))>,
                StatusBuilder&> {
            TurboInternalAttachPayload(*this, obj, id);
            return *this;
        }
        ...
        template <typename MessageSetExtension, typename ExtensionIdentifier>
        KUMO_MUST_USE_RESULT auto AttachPayload(const MessageSetExtension& obj,
            const ExtensionIdentifier& id) && //
        ...
        auto AttachPayload(const MessageSetExtension& obj) & //
            -> std::enable_if_t<
                std::is_void_v<decltype(TurboInternalAttachPayload(*this, obj))>,
                StatusBuilder&> {
            TurboInternalAttachPayload(*this, obj);
            return *this;
        }
        ...
        auto SetErrorCode(Enum code) & //
            -> std::enable_if_t<
                std::is_void_v<decltype(TurboInternalSetErrorCode(*this, code))>,
                StatusBuilder&> {
            TurboInternalSetErrorCode(*this, code);
            return *this;
        }
        ...
        // Uses decltype() to propagate template constraints (SFINAE).
        template <typename Enum>
        KUMO_MUST_USE_RESULT auto SetErrorCode(Enum code) && //
        ...
        StatusBuilder& SetCode(turbo::StatusCode code) &;
        KUMO_MUST_USE_RESULT StatusBuilder&& SetCode(turbo::StatusCode code) && {
        ...
        //
        //   TURBO_RETURN_IF_ERROR(foo()).With(TeamPolicy).With(OtherTeamPolicy);
        //
        // Terminal adaptors are often used for type conversion. This allows
        // TURBO_RETURN_IF_ERROR to be used in functions which do not return Status.
        // For example, a function might want to return some default value on error:
        //
        //   int GetSysCounter() {
        //     int value;
        //     TURBO_RETURN_IF_ERROR(ReadCounterFile(filename, &value))
        //         .LogInfo()
        //         .With([](const turbo::Status& unused) { return 0; });
        //     return value;
        //   }
        //
        ...
        //
        //   bool DoMyThing() {
        //     TURBO_RETURN_IF_ERROR(foo()).LogWarning().With(status_macros::Return(false));
        //     ...
        //   }
        //
        ...
        template <typename Adaptor>
        KUMO_MUST_USE_RESULT auto With(
            Adaptor&&
        ...
        // TURBO_RETURN_IF_ERROR with 2 move operations in the common case.
        operator turbo::Status() const&; // NOLINT: Builder converts implicitly.
        operator turbo::Status() &&; // NOLINT: Builder converts implicitly.
    ...
    private:
        ...
        // status macros.
        struct Rep {
            explicit Rep(const turbo::Status& s);
            explicit Rep(turbo::Status&& s);
            Rep(const Rep& r);
            ~Rep();
            void InitStream();
            ...
            turbo::LogSeverity log_severity;
        ...
        static Rep* InitRep(const turbo::Status& s) {
            if (s.ok()) {
                return nullptr;
            } else {
                return new Rep(s);
        ...
        static Rep* InitRep(turbo::Status&& s) {
            if (s.ok()) {
                return nullptr;
            } else {
                Rep* rep = InitRepImpl(std::move(s));
                KUMO_ASSUME(rep != nullptr);
                return rep;
            }
        }
    ...
    //       .With(util::TaskReturn(task));
    //
    // Note in the above example, the TURBO_RETURN_IF_ERROR macro ensures the
    ...
    inline StatusBuilder::StatusBuilder(turbo::StatusCode code,
        ...
        : loc_(location)
        , rep_(InitRep(turbo::Status(code, ""))) { }
    ...
    inline StatusBuilder& StatusBuilder::Log(turbo::LogSeverity level) & {
        if (rep_ == nullptr)
            return *this;
    ...
    inline StatusBuilder& StatusBuilder::LogEveryN(turbo::LogSeverity level,
        int n) & {
        if (rep_ == nullptr)
            return *this;
        if (n < 1)
    ...
    inline StatusBuilder& StatusBuilder::LogEvery(turbo::LogSeverity level,
        turbo::Duration period) & {
        if (rep_ == nullptr)
            return *this;
        if (period <= turbo::ZeroDuration())
            return Log(level);
    ...
    inline StatusBuilder& StatusBuilder::EmitStackTrace() & {
        ...
        if (rep_->logging_mode == Rep::LoggingMode::kDisabled) {
            ...
            rep_->logging_mode = Rep::LoggingMode::kLog;
            rep_->log_severity = turbo::LogSeverity::kInfo;
        }
    ...
    inline StatusBuilder& StatusBuilder::AlsoOutputToSink(turbo::LogSink* sink) & {
        if (rep_ == nullptr)
            return *this;
        rep_->sink = sink;
    ...
    inline StatusBuilder& StatusBuilder::OnlyOutputToSink(turbo::LogSink* sink) & {
        if (rep_ == nullptr)
            return *this;
        rep_->sink = sink;
    ...
    inline turbo::StatusCode StatusBuilder::code() const {
        return rep_ == nullptr ? turbo::StatusCode::kOk : rep_->status.code();
    }

    inline StatusBuilder::operator turbo::Status() && {
...
#endif // TURBO_STATUS_STATUS_BUILDER_H_
