...
namespace turbo {
    ...
    namespace {
        ...
        struct FloatTraits<double> {
            ...
            static double MakeNan(const char* turbo_nonnull tagp) {
#if KUMO_HAVE_BUILTIN(__builtin_nan)
        ...
        struct FloatTraits<float> {
            ...
            static float MakeNan(const char* turbo_nonnull tagp) {
#if KUMO_HAVE_BUILTIN(__builtin_nanf)
        ...
        bool Power10Overflow(int n) {
            return n >= kPower10TableMaxExclusive;
        }
        ...
        bool Power10Underflow(int n) {
            return n < kPower10TableMinInclusive;
        }
        ...
        int TruncateToBitWidth(int bit_width, uint128* turbo_nonnull value) {
        ...
        void EncodeResult(const CalculatedFloat& calculated, bool negative,
            turbo::from_chars_result* turbo_nonnull result,
            FloatType* turbo_nonnull value) {
        ...
        uint64_t ShiftRightAndRound(uint128 value, int shift, bool input_exact,
            bool* turbo_nonnull output_exact) {
            if (shift <= 0) {
        ...
        bool MustRoundUp(uint64_t guess_mantissa, int guess_exponent,
            ...
            turbo::strings_internal::BigUnsigned<84> exact_mantissa;
            ...
            turbo::strings_internal::BigUnsigned<84>& lhs = exact_mantissa;
            int comparison;
            if (exact_exponent >= 0) {
                lhs.MultiplyByFiveToTheNth(exact_exponent);
                turbo::strings_internal::BigUnsigned<84> rhs(guess_mantissa);
                ...
                turbo::strings_internal::BigUnsigned<84> rhs = turbo::strings_internal::BigUnsigned<84>::FiveToTheNth(-exact_exponent);
        ...
        CalculatedFloat CalculateFromParsedDecimal(
            ...
            const int shift = NormalizedShiftSize<FloatType>(mantissa_width, binary_exponent);
            bool result_exact;
        ...
        bool EiselLemire(const strings_internal::ParsedFloat& input, bool negative,
            FloatType* turbo_nonnull value, std::errc* turbo_nonnull ec) {
            ...
            static_assert(
                FloatTraits<FloatType>::kEiselLemireMinInclusiveExp10 >= kPower10TableMinInclusive,
                "(exp10-kPower10TableMinInclusive) in kPower10MantissaHighTable bounds");
            static_assert(
                FloatTraits<FloatType>::kEiselLemireMaxExclusiveExp10 <= kPower10TableMaxExclusive,
                "(exp10-kPower10TableMinInclusive) in kPower10MantissaHighTable bounds");
            ...
            uint64_t ret_exp2 = static_cast<uint64_t>((217706 * exp10 >> 16) + 64 + FloatTraits<FloatType>::kExponentBias - clz);

            // (+) Multiplication.
            uint128 x = static_cast<uint128>(man) * static_cast<uint128>(kPower10MantissaHighTable[exp10 - kPower10TableMinInclusive]);

            // (+) Wider Approximation.
            static constexpr uint64_t high64_mask = FloatTraits<FloatType>::kEiselLemireMask;
            if (((Uint128High64(x) & high64_mask) == high64_mask) && (man > (std::numeric_limits<uint64_t>::max() - Uint128Low64(x)))) {
                uint128 y = static_cast<uint128>(man) * static_cast<uint128>(kPower10MantissaLowTable[exp10 - kPower10TableMinInclusive]);
                x += Uint128High64(y);
                ...
                if (((Uint128High64(x) & high64_mask) == high64_mask) && ((Uint128Low64(x) + 1) == 0) && (man > (std::numeric_limits<uint64_t>::max() - Uint128Low64(y)))) {
                    return false;
                }
            }
            ...
            uint64_t ret_man = Uint128High64(x) >> (msb + FloatTraits<FloatType>::kEiselLemireShift);
            ret_exp2 -= 1 ^ msb;
            ...
            if ((Uint128Low64(x) == 0) && ((Uint128High64(x) & high64_mask) == 0) && ((ret_man & 3) == 1)) {
                return false;
            }
            ...
            static constexpr uint64_t max_exp2 = (1 << FloatTraits<FloatType>::kTargetExponentBits) - 1;
            ...
            if (FloatTraits<FloatType>::kTargetBits == 64) {
                ...
                *value = static_cast<FloatType>(turbo::bit_cast<double>(ret_bits));
                return true;
            } else if (FloatTraits<FloatType>::kTargetBits == 32) {
                uint32_t ret_bits = (static_cast<uint32_t>(ret_exp2) << 23) | (static_cast<uint32_t>(ret_man) & 0x007FFFFFu);
                if (negative) {
                    ret_bits |= 0x80000000u;
                }
                *value = static_cast<FloatType>(turbo::bit_cast<float>(ret_bits));
                return true;
            }
#endif // TURBO_BIT_PACK_FLOATS
            return false;
        ...
        template <typename FloatType>
        from_chars_result FromCharsImpl(const char* turbo_nonnull first,
            const char* turbo_nonnull last, FloatType& value,
            ...
            if ((fmt_flags & chars_format::hex) == chars_format::hex) {
                ...
                if ((decimal_parse.subrange_begin == nullptr) && EiselLemire<FloatType>(decimal_parse, negative, &value, &result.ec)) {
                    return result;
                }
    ...
    from_chars_result from_chars(const char* turbo_nonnull first,
        const char* turbo_nonnull last, double& value,
        chars_format fmt) {
        return FromCharsImpl(first, last, value, fmt);
    }

    from_chars_result from_chars(const char* turbo_nonnull first,
        const char* turbo_nonnull last, float& value,
        chars_format fmt) {
        return FromCharsImpl(first, last, value, fmt);
    }
