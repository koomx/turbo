...
namespace turbo {
    ...
    namespace strings_internal {
        ...
        KUMO_DLL extern const uint32_t
            kFiveToNth[kMaxSmallPowerOfFive + 1];
        KUMO_DLL extern const uint32_t kTenToNth[kMaxSmallPowerOfTen + 1];

        // Large, fixed-width unsigned integer.
        //
        // Exact rounding for decimal-to-binary floating point conversion requires very
        // large integer math, but a design goal of turbo::from_chars is to avoid
        ...
        public:
            ...
            // Constructs a BigUnsigned from the given std::string_view containing a decimal
            // value.  If the input string is not a decimal integer, constructs a 0
            // instead.
            explicit BigUnsigned(std::string_view sv)
                : size_(0)
                , words_ { } {
                // Check for valid input, returning a 0 otherwise.  This is reasonable
                // behavior only because this constructor is for unit tests.
                if (std::find_if_not(sv.begin(), sv.end(), ascii_isdigit) != sv.end() || sv.empty()) {
                    return;
                }
                int exponent_adjust = ReadDigits(sv.data(), sv.data() + sv.size(), Digits10() + 1);
...
#if KUMO_INTERNAL_HAVE_MIN_GNUC_VERSION(14, 0)
...
#pragma GCC diagnostic pop
                        ...
                        for (int i = (std::min)(size_, max_words - 1); i > word_shift; --i) {
                            words_[i] = (words_[i - word_shift] << count) | (words_[i - word_shift - 1] >> (32 - count));
                        }
        ...
        private:
            ...
            void MultiplyBy(int other_size, const uint32_t* other_words) {
                ...
                const int first_step = (std::min)(original_size + other_size - 2, max_words - 1);
