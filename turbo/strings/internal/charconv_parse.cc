...
namespace turbo {
    ...
    namespace {
        ...
        static_assert(std::numeric_limits<uint64_t>::digits10 == kDecimalMantissaDigitsMax,
            "(a) above");
        ...
        // significant bit.
        constexpr int kGuaranteedHexadecimalMantissaBitPrecision = 4 * kHexadecimalMantissaDigitsMax - 3;

        static_assert(kGuaranteedHexadecimalMantissaBitPrecision > std::numeric_limits<double>::digits + 2,
        ...
        static_assert(999999999 + 2 * kDecimalDigitLimit < std::numeric_limits<int>::max(),
            "int type too small");
        static_assert(999999999 + 2 * (4 * kHexadecimalDigitLimit) < std::numeric_limits<int>::max(),
            "int type too small");
        ...
        bool AllowExponent(chars_format flags) {
            ...
            bool scientific = (flags & chars_format::scientific) == chars_format::scientific;
        ...
        bool ParseInfinityOrNan(const char* begin, const char* end,
            ...
            case 'I': {
                ...
                if (end - begin >= 8 && strings_internal::memcasecmp(begin + 3, "inity", 5) == 0) {
                    out->end = begin + 8;
                } else {
                    out->end = begin + 3;
                }
    ...
    namespace strings_internal {
        ...
        strings_internal::ParsedFloat ParseFloat(const char* begin, const char* end,
            ...
            if (AllowExponent(format_flags) && begin < end && IsExponentCharacter<base>(*begin)) {
            ...
            if (result.mantissa > 0) {
                const ptrdiff_t exponent = result.literal_exponent + (DigitMagnitude<base>() * exponent_adjustment);
