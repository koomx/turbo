...
#include <string_view>
#include <turbo/format/str_format/checker.h>
    ...
    namespace str_format_internal {
        ...
        bool parse_format_string(std::string_view src, Consumer consumer) {
            ...
            while (p != end) {
                ...
                if (KUMO_UNLIKELY(percent + 1 >= end))
                    return false;
                ...
                if (tag.is_conv()) {
                    ...
                    if (KUMO_UNLIKELY(
                            !consumer.ConvertOne(conv, std::string_view(percent + 1, 1)))) {
                        return false;
                    }
                } else if (percent[1] != '%') {
                    UnboundConversion conv;
                    p = ConsumeUnboundConversionNoInline(percent + 1, end, &conv, &next_arg);
                    if (KUMO_UNLIKELY(p == nullptr))
                        return false;
                    if (KUMO_UNLIKELY(!consumer.ConvertOne(
                            conv, std::string_view(percent + 1, static_cast<size_t>(p - (percent + 1)))))) {
                        return false;
                    }
                } else {
                    if (KUMO_UNLIKELY(!consumer.Append("%")))
        ...
        constexpr bool EnsureConstexpr(std::string_view s) {
        ...
        public:
            ...
            bool ProcessFormat(Consumer consumer) const {
                ...
                std::string_view text(base, 0);
                for (const auto& item : items_) {
                    const char* const end = text.data() + text.size();
                    text = std::string_view(end, static_cast<size_t>((base + item.text_end) - end));
        ...
        public:
            explicit ExtendedParsedFormat(std::string_view format)
#ifdef TURBO_INTERNAL_ENABLE_FORMAT_CHECKER
                __attribute__((
                    enable_if(str_format_internal::EnsureConstexpr(format),
                        "str_printf_to string is not constexpr."),
                    enable_if(str_format_internal::ValidFormatImpl<C...>(format),
                        "str_printf_to specified does not match the template arguments.")))
            ...
            static std::unique_ptr<ExtendedParsedFormat> New(std::string_view format) {
        ...
        private:
            ...
            ExtendedParsedFormat(std::string_view s, bool allow_ignored)
