    ...
    namespace strings_internal {
        ...
        public:
            ...
            ConvertibleToStringView(char* s)
                : value_(s) { // NOLINT(runtime/explicit)
                assert(s != nullptr);
            }
        ...
        struct SplitterIsConvertibleTo
            : SplitterIsConvertibleToImpl<
                  C,
                  !IsInitializerList<std::remove_reference_t<C>>::value && HasValueType<C>::value && HasConstIterator<C>::value,
        ...
        struct ShouldUseLifetimeBound<
            ...
            std::enable_if_t<
                std::is_same_v<StringType, std::string> && std::is_same_v<typename Container::value_type, std::string_view>>>
            : std::true_type { };

        template <typename StringType, typename First, typename Second>
        using ShouldUseLifetimeBoundForPair = std::integral_constant<bool,
            std::is_same_v<StringType, std::string> && (std::is_same_v<First, std::string_view> || std::is_same_v<Second, std::string_view>)>;

        template <typename StringType, typename ElementType, std::size_t Size>
        using ShouldUseLifetimeBoundForArray = std::integral_constant<bool,
            std::is_same_v<StringType, std::string> && std::is_same_v<ElementType, std::string_view>>;
        ...
        // and returns bool.
        //
        // The StringType parameter can be either std::string_view or string, depending on
        ...
        public:
            ...
            template <
                typename Container,
                std::enable_if_t<ShouldUseLifetimeBound<StringType, Container>::value && SplitterIsConvertibleTo<Container>::value,
                    std::nullptr_t> = nullptr>
            // NOLINTNEXTLINE(google-explicit-constructor)
            operator Container() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
            ...
            template <typename First, typename Second,
                std::enable_if_t<
                    ...
                    std::nullptr_t> = nullptr>
            // NOLINTNEXTLINE(google-explicit-constructor)
            operator std::pair<First, Second>() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
            ...
            template <typename ElementType, std::size_t Size,
                std::enable_if_t<ShouldUseLifetimeBoundForArray<
                    ...
                    std::nullptr_t> = nullptr>
            // NOLINTNEXTLINE(google-explicit-constructor)
            operator std::array<ElementType, Size>() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
