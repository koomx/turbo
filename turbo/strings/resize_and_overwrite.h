...
#define TURBO_INTERNAL_HAS_RESIZE_AND_OVERWRITE 1
#endif
...
namespace turbo {
    ...
    namespace strings_internal {
...
#ifndef TURBO_INTERNAL_HAS_RESIZE_AND_OVERWRITE

        inline size_t ProbeResizeAndOverwriteOp(char*, size_t) {
            return 0;
        }
        ...
        struct has__resize_and_overwrite<
            T, std::void_t<decltype(std::declval<T&>().__resize_and_overwrite(std::declval<size_t>(), ProbeResizeAndOverwriteOp))>>
            : std::true_type { };
        ...
        struct has_Resize_and_overwrite<
            T, std::void_t<decltype(std::declval<T&>()._Resize_and_overwrite(std::declval<size_t>(), ProbeResizeAndOverwriteOp))>>
            : std::true_type { };

#endif // ifndef TURBO_INTERNAL_HAS_RESIZE_AND_OVERWRITE
...
#if KUMO_HAVE_MEMORY_SANITIZER
            if (old_size < n) {
                TURBO_ANNOTATE_MEMORY_IS_UNINITIALIZED(str.data() + old_size, n - old_size);
            }
#endif
            typename T::size_type new_size = static_cast<typename T::size_type>(std::move(op)(str.data(), n));
            turbo::base_internal::HardeningAssertGE(new_size, typename T::size_type { 0 });
            turbo::base_internal::HardeningAssertLE(new_size, n);
            turbo::base_internal::HardeningAssert(str.data()[n] ==
...
#ifdef TURBO_INTERNAL_HAS_RESIZE_AND_OVERWRITE
