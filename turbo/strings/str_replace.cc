...
namespace turbo {
    ...
    namespace strings_internal {
        ...
        using FixedMapping = std::initializer_list<std::pair<std::string_view, std::string_view>>;
        ...
        int ApplySubstitutions(
            std::string_view s,
            std::vector<strings_internal::ViableSubstitution>* turbo_nonnull subs_ptr,
            std::string* turbo_nonnull result_ptr) {
            auto& subs = *subs_ptr;
            int substitutions = 0;
            size_t pos = 0;
            while (!subs.empty()) {
    ...
    int StrReplaceAll(strings_internal::FixedMapping replacements,
        std::string* turbo_nonnull target) {
