...
namespace turbo {
    ...
    namespace substitute_internal {
        ...
        [[nodiscard]] std::enable_if_t<std::is_same_v<T, size_t>, T> CheckedAdd(T a,
            T b) {
            TURBO_INTERNAL_CHECK(b <= (std::numeric_limits<T>::max)() - a,
        ...
        void SubstituteAndAppendArray(std::string* turbo_nonnull output,
            ...
            // Build the string.
            TURBO_INTERNAL_CHECK(size <= output->max_size() - output->size(),
        ...
        Arg::Arg(const void* turbo_nullable value) {
            ...
            if (value == nullptr) {
                ...
                do {
                    *--ptr = turbo::format_internal::kHexChar[num & 0xf];
                    num >>= 4;
                } while (num != 0);
                *--ptr = 'x';
                *--ptr = '0';
        ...
        Arg::Arg(Hex hex) {
            char* const end = &scratch_[format_internal::kFastToBufferSize];
            char* writer = end;
            uint64_t value = hex.value;
            do {
                *--writer = turbo::format_internal::kHexChar[value & 0xF];
                value >>= 4;
            } while (value != 0);
