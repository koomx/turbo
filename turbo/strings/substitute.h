...
namespace turbo {
    ...
    namespace substitute_internal {
        ...
        public:
            ...
            Arg(const char* turbo_nullable value) // NOLINT(google-explicit-constructor)
            ...
            Arg(char value) { // NOLINT(google-explicit-constructor)
                scratch_[0] = value;
                piece_ = std::string_view(scratch_, 1);
            }
            Arg(short value) // NOLINT(*)
                : piece_(scratch_,
                      static_cast<size_t>(
                          format_internal::fast_int_to_buffer(value, scratch_) - scratch_)) { }
            ...
            Arg(float value) // NOLINT(google-explicit-constructor)
                : piece_(scratch_, format_internal::six_digits_to_buffer(value, scratch_)) {
            }
            ...
            template <typename T,
                std::enable_if_t<
                    std::is_class_v<T> && (std::is_same_v<T, std::vector<bool>::reference> || std::is_same_v<T, std::vector<bool>::const_reference>),
                    bool> = true>
        ...
        private:
            ...
            char scratch_[format_internal::kFastToBufferSize];
        };

        // Internal helper function. Don't call this from outside this implementation.
        // This interface may change without notice.
        void SubstituteAndAppendArray(std::string* turbo_nonnull output,
            std::string_view format,
            const std::string_view* turbo_nullable args_array,
            size_t num_args);

#if defined(KUMO_BAD_CALL_IF)
        constexpr int CalculateOneBit(const char* turbo_nonnull format) {
            // Returns:
        ...
        constexpr const char* turbo_nonnull SkipNumber(const char* turbo_nonnull format) {
            return !*format ? format : (format + 1);
        }

        constexpr int PlaceholderBitmask(const char* turbo_nonnull format) {
            return !*format
                ? 0
    ...
    //  }
    //
    inline void substitute_and_append(std::string* turbo_nonnull output,
        std::string_view format) {
    ...
    inline void substitute_and_append(
        std::string* turbo_nonnull output, std::string_view format,
    ...
    void substitute_and_append(std::string* turbo_nonnull output,
        const char* turbo_nonnull format)
        KUMO_BAD_CALL_IF(
    ...
    void substitute_and_append(std::string* turbo_nonnull output,
        const char* turbo_nonnull format,
        const substitute_internal::Arg& a0)
        KUMO_BAD_CALL_IF(substitute_internal::PlaceholderBitmask(format) != 1,
    ...
    void substitute_and_append(
        std::string* turbo_nonnull output, const char* turbo_nonnull format,
        const substitute_internal::Arg& a0, const substitute_internal::Arg& a1,
        const substitute_internal::Arg& a2, const substitute_internal::Arg& a3,
        const substitute_internal::Arg& a4, const substitute_internal::Arg& a5)
        KUMO_BAD_CALL_IF(
    ...
    // This body of functions catches cases where the number of placeholders
    // doesn't match the number of data arguments.
    std::string substitute(const char* turbo_nonnull format)
        KUMO_BAD_CALL_IF(substitute_internal::PlaceholderBitmask(format) != 0,
            "There were no substitution arguments "
    ...
    std::string substitute(
        const char* turbo_nonnull format, const substitute_internal::Arg& a0,
        const substitute_internal::Arg& a1, const substitute_internal::Arg& a2,
        const substitute_internal::Arg& a3, const substitute_internal::Arg& a4,
        const substitute_internal::Arg& a5, const substitute_internal::Arg& a6)
        KUMO_BAD_CALL_IF(
