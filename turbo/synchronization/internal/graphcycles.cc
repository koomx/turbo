...
KUMO_CONST_INIT static turbo::base_internal::SpinLock arena_mu(
    base_internal::SCHEDULE_KERNEL_ONLY);
KUMO_CONST_INIT static base_internal::LowLevelAlloc::Arena* arena;
...
bool GraphCycles::CheckInvariants() const {
  ...
  for (uint32_t x = 0; x < r->nodes_.size(); x++) {
    ...
    if (ptr != nullptr && static_cast<uint32_t>(r->ptrmap_.Find(ptr)) != x) {
      TURBO_RAW_LOG(FATAL, "Did not find live node in hash table %" PRIu32 " %p",
                   x, ptr);
    }
    if (nx->visited) {
      TURBO_RAW_LOG(FATAL, "Did not clear visited marker on node %" PRIu32, x);
    }
    if (!ranks.insert(nx->rank)) {
      TURBO_RAW_LOG(FATAL, "Duplicate occurrence of rank %" PRId32, nx->rank);
    }
...
void GraphCycles::UpdateStackTrace(GraphId id, int priority,
  ...
  n->nstack = (*get_stack_trace)(n->stack, KUMO_ARRAYSIZE(n->stack));
  n->priority = priority;
}
