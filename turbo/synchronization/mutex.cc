...
using turbo::base_internal::PerThreadSynch;
using turbo::base_internal::SchedulingGuard;
using turbo::base_internal::ThreadIdentity;
using turbo::synchronization_internal::GetOrCreateCurrentThreadIdentity;
using turbo::synchronization_internal::GraphCycles;
using turbo::synchronization_internal::GraphId;
using turbo::synchronization_internal::InvalidGraphId;
using turbo::synchronization_internal::KernelTimeout;
using turbo::synchronization_internal::PerThreadSem;

extern "C" {
KUMO_ATTRIBUTE_WEAK void TURBO_INTERNAL_C_SYMBOL(TurboInternalMutexYield)() {
...
KUMO_CONST_INIT std::atomic<OnDeadlockCycle> synch_deadlock_detection(
    kDeadlockDetectionDefault);
KUMO_CONST_INIT std::atomic<bool> synch_check_invariants(false);

KUMO_INTERNAL_ATOMIC_HOOK_ATTRIBUTES
turbo::base_internal::AtomicHook<void (*)(int64_t wait_cycles)>
    submit_profile_data;
KUMO_INTERNAL_ATOMIC_HOOK_ATTRIBUTES turbo::base_internal::AtomicHook<void (*)(
    const char* msg, const void* obj, int64_t wait_cycles)>
    mutex_tracer;
KUMO_INTERNAL_ATOMIC_HOOK_ATTRIBUTES
turbo::base_internal::AtomicHook<void (*)(const char* msg, const void* cv)>
    cond_var_tracer;
...
struct KUMO_CACHELINE_ALIGNED MutexGlobals {
  ...
  turbo::Duration mutex_sleep_time;
};

KUMO_CONST_INIT static MutexGlobals globals;
...
const MutexGlobals& GetMutexGlobals() {
  turbo::base_internal::LowLevelCallOnce(&globals.once, [&]() {
...
int MutexDelay(int32_t c, int mode) {
  ...
  const turbo::Duration sleep_time = GetMutexGlobals().mutex_sleep_time;
  if (c < limit) {
    // Spin.
    c++;
  } else {
    SchedulingGuard::ScopedEnable enable_rescheduling;
    TURBO_TSAN_MUTEX_PRE_DIVERT(nullptr, 0);
    if (c == limit) {
      // Yield once.
      TURBO_INTERNAL_C_SYMBOL(TurboInternalMutexYield)();
      c++;
...
KUMO_CONST_INIT static turbo::base_internal::SpinLock deadlock_graph_mu(
    base_internal::SCHEDULE_KERNEL_ONLY);

// Graph used to detect deadlocks.
KUMO_CONST_INIT static GraphCycles* deadlock_graph
    TURBO_GUARDED_BY(deadlock_graph_mu) TURBO_PT_GUARDED_BY(deadlock_graph_mu);
...
KUMO_CONST_INIT static turbo::base_internal::SpinLock synch_event_mu(
    base_internal::SCHEDULE_KERNEL_ONLY);
...
static struct SynchEvent {  // this is a trivial hash table for the events
  ...
  int refcount TURBO_GUARDED_BY(synch_event_mu);

  // buckets have linear, 0-terminated  chains
  SynchEvent* next TURBO_GUARDED_BY(synch_event_mu);
...
}* synch_event[kNSynchEvent] TURBO_GUARDED_BY(synch_event_mu);
...
static SynchEvent* EnsureSynchEvent(std::atomic<intptr_t>* addr,
  ...
  static size_t synch_event_count TURBO_GUARDED_BY(synch_event_mu);
...
static void PostSynchEvent(void* obj, int ev) {
  ...
  if (e == nullptr || e->log) {
    void* pcs[40];
    int n = turbo::GetStackTrace(pcs, KUMO_ARRAYSIZE(pcs), 1);
    // A buffer with enough space for the ASCII for all the PCs, even on a
    // 64-bit machine.
    char buffer[KUMO_ARRAYSIZE(pcs) * 24];
    ...
    TURBO_RAW_LOG(INFO, "%s%p %s %s", event_properties[ev].msg, obj,
...
// allow them to use functions such a KLOG() within Condition functions.  The
...
static PerThreadSynch* Synch_GetPerThreadAnnotated(Mutex* mu) {
  if (mu) {
    TURBO_TSAN_MUTEX_PRE_DIVERT(mu, 0);
  }
  PerThreadSynch* w = Synch_GetPerThread();
  if (mu) {
    TURBO_TSAN_MUTEX_POST_DIVERT(mu, 0);
  }
  return w;
}
...
// prevent certain TURBO_RAW_CHECK() statements from being triggered when
// re-rentry is detected.  The TURBO_RAW_CHECK() statements are those in the
...
void Mutex::Dtor() {
  ...
  TURBO_TSAN_MUTEX_DESTROY(this, __tsan_mutex_not_static);
}
#endif
...
void Mutex::EnableDebugLog(const char* name) {
  ...
  e->log = true;
  UnrefSynchEvent(e);
  // This prevents "error: undefined symbol: turbo::Mutex::~Mutex()"
  ...
  KUMO_ATTRIBUTE_UNUSED volatile auto dtor = &Mutex::Dtor;
...
static PerThreadSynch* Enqueue(PerThreadSynch* head, SynchWaitParams* waitp,
...
#if KUMO_HAVE_PTHREAD_GETSCHEDPARAM
  if ((flags & kMuIsFer) == 0) {
    ...
    if (s->next_priority_read_cycles < now_cycles) {
      ...
      if (err != 0) {
        TURBO_RAW_LOG(ERROR, "pthread_getschedparam failed: %d", err);
      } else {
  ...
  if (head == nullptr) {         // s is the only waiter
    ...
    if (enqueue_after != nullptr) {
      ...
      // insertion points.
      TURBO_RAW_CHECK(enqueue_after->skip == nullptr ||
...
static PerThreadSynch* DequeueAllWakeable(PerThreadSynch* head,
  ...
  do {
    if (w->wake) {  // remove this element
      TURBO_RAW_CHECK(pw->skip == nullptr, "bad skip in DequeueAllWakeable");
...
void Mutex::Block(PerThreadSynch* s) {
  ...
  TURBO_RAW_CHECK(s->waitp != nullptr || s->suppress_fatal_errors,
...
static GraphId GetGraphIdLocked(Mutex* mu)
    TURBO_EXCLUSIVE_LOCKS_REQUIRED(deadlock_graph_mu) {
...
static GraphId GetGraphId(Mutex* mu) TURBO_LOCKS_EXCLUDED(deadlock_graph_mu) {
...
static void LockEnter(Mutex* mu, GraphId id, SynchLocksHeld* held_locks) {
  ...
  if (i == n) {
    if (n == KUMO_ARRAYSIZE(held_locks->locks)) {
...
static char* StackString(void** pcs, int n, char* buf, int maxlen,
  ...
  for (int i = 0; i != n; i++) {
    ...
    if (symbolize) {
      if (!turbo::Symbolize(pcs[i], sym, kSymLen)) {
        sym[0] = '\0';
      }
...
static char* CurrentStackString(char* buf, int maxlen, bool symbolize) {
  void* pcs[40];
  return StackString(pcs, turbo::GetStackTrace(pcs, KUMO_ARRAYSIZE(pcs), 2), buf,
                     maxlen, symbolize);
}
...
int GetStack(void** stack, int max_depth) {
  return turbo::GetStackTrace(stack, max_depth, 3);
}
}  // anonymous namespace
...
static GraphId DeadlockCheck(Mutex* mu) {
  ...
  turbo::base_internal::SpinLockHolder lock(deadlock_graph_mu);
  ...
  for (int i = 0; i != all_locks->n; i++) {
    ...
    if (!deadlock_graph->InsertEdge(other_node_id, mu_id)) {
      ...
      TURBO_RAW_LOG(ERROR, "Potential Mutex deadlock: %s",
      ...
      TURBO_RAW_LOG(ERROR,
                   "Acquiring turbo::Mutex %p while holding %s; a cycle in the "
                   "historical lock ordering graph has been observed",
                   static_cast<void*>(mu), b->buf);
      TURBO_RAW_LOG(ERROR, "Cycle: ");
      int path_len = deadlock_graph->FindPath(mu_id, other_node_id,
                                              KUMO_ARRAYSIZE(b->path), b->path);
      for (int j = 0; j != path_len && j != KUMO_ARRAYSIZE(b->path); j++) {
        GraphId id = b->path[j];
        ...
        StackString(stack, depth, b->buf + strlen(b->buf),
                    ...
                    symbolize);
        TURBO_RAW_LOG(ERROR, "%s", b->buf);
      }
      if (path_len > static_cast<int>(KUMO_ARRAYSIZE(b->path))) {
        TURBO_RAW_LOG(ERROR, "(long cycle; list truncated)");
      }
      if (synch_deadlock_detection.load(std::memory_order_acquire) ==
          OnDeadlockCycle::kAbort) {
        deadlock_graph_mu.unlock();  // avoid deadlock in fatal sighandler
        TURBO_RAW_LOG(FATAL, "dying due to potential deadlock");
        return mu_id;
      }
  ...
  if (kDebugMode &&
    ...
    for (int i = 0; i != locks->n; i++) {
      if (locks->locks[i].id == id) {
        ...
        TURBO_RAW_LOG(FATAL, "thread should not hold mutex %p %s",
...
void Mutex::lock() {
  ...
  if (KUMO_UNLIKELY((v & (kMuWriter | kMuReader | kMuEvent)) != 0) ||
      KUMO_UNLIKELY(!mu_.compare_exchange_strong(
          v, kMuWriter | v, std::memory_order_acquire,
          std::memory_order_relaxed))) {
    // try spin acquire, then slow loop
    if (KUMO_UNLIKELY(!TryAcquireWithSpinning(&this->mu_))) {
...
void Mutex::lock_shared() {
  ...
  DebugOnlyLockEnter(this, id);
  TURBO_TSAN_MUTEX_POST_LOCK(this, __tsan_mutex_read_lock, 0);
}
...
bool Mutex::LockWhenCommon(const Condition& cond,
  ...
  TURBO_TSAN_MUTEX_PRE_LOCK(this, TsanFlags(how));
  GraphId id = DebugOnlyDeadlockCheck(this);
  bool res = LockSlowWithDeadline(how, &cond, t, 0);
  DebugOnlyLockEnter(this, id);
  TURBO_TSAN_MUTEX_POST_LOCK(this, TsanFlags(how), 0);
  return res;
}
...
bool Mutex::try_lock() {
  ...
  intptr_t v = mu_.load(std::memory_order_relaxed);
  // Try fast acquire.
  if (KUMO_LIKELY((v & (kMuWriter | kMuReader | kMuEvent)) == 0)) {
    if (KUMO_LIKELY(mu_.compare_exchange_strong(
            v, kMuWriter | v, std::memory_order_acquire,
            std::memory_order_relaxed))) {
  ...
  } else if (KUMO_UNLIKELY((v & kMuEvent) != 0)) {
...
KUMO_ATTRIBUTE_NOINLINE bool Mutex::TryLockSlow() {
  intptr_t v = mu_.load(std::memory_order_relaxed);
...
bool Mutex::try_lock_shared() {
  TURBO_TSAN_MUTEX_PRE_LOCK(this,
...
KUMO_ATTRIBUTE_NOINLINE bool Mutex::ReaderTryLockSlow() {
  intptr_t v = mu_.load(std::memory_order_relaxed);
#if defined(__clang__)
#pragma nounroll
#endif
...
void Mutex::unlock() {
  ...
  if (kDebugMode && ((v & (kMuWriter | kMuReader)) != kMuWriter)) {
    TURBO_RAW_LOG(FATAL, "Mutex unlocked when destroyed or not locked: v=0x%x",
  ...
  if (kDebugMode && should_try_cas != should_try_cas_fast) {
    ...
    TURBO_RAW_LOG(FATAL, "internal logic error %llx %llx %llx\n",
...
void Mutex::unlock_shared() {
  TURBO_TSAN_MUTEX_PRE_UNLOCK(this, __tsan_mutex_read_lock);
  DebugOnlyLockLeave(this);
  intptr_t v = mu_.load(std::memory_order_relaxed);
  assert((v & (kMuWriter | kMuReader)) == kMuReader);
  for (;;) {
    if (KUMO_UNLIKELY((v & (kMuReader | kMuWait | kMuEvent)) !=
                           kMuReader)) {
      this->UnlockSlow(nullptr /*no waitp*/);  // take slow path
      break;
    }
  ...
  TURBO_TSAN_MUTEX_POST_UNLOCK(this, __tsan_mutex_read_lock);
}
...
KUMO_ATTRIBUTE_NOINLINE void Mutex::LockSlow(MuHow how, const Condition* cond,
                                             int flags) {
...
static inline bool EvalConditionAnnotated(const Condition* cond, Mutex* mu,
  ...
  if (locking) {
    ...
    // deadlock detector.
    TURBO_TSAN_MUTEX_POST_LOCK(mu, tryflags, 0);
    res = cond->Eval();
    // There is no "try" version of Unlock, so use flags instead of tryflags.
    TURBO_TSAN_MUTEX_PRE_UNLOCK(mu, flags);
    TURBO_TSAN_MUTEX_POST_UNLOCK(mu, flags);
    TURBO_TSAN_MUTEX_PRE_LOCK(mu, tryflags);
  } else {
    ...
    TURBO_TSAN_MUTEX_POST_UNLOCK(mu, flags);
    TURBO_TSAN_MUTEX_PRE_LOCK(mu, flags);
    TURBO_TSAN_MUTEX_POST_LOCK(mu, flags, 0);
    res = cond->Eval();
    TURBO_TSAN_MUTEX_PRE_UNLOCK(mu, flags);
  }
...
static void CheckForMutexCorruption(intptr_t v, const char* label) {
  ...
  if (KUMO_LIKELY((w & (w << 3) & (kMuWriter | kMuWrWait)) == 0)) return;
    ...
    if ((v & waitp->how->slow_need_zero) == 0) {
      ...
      if ((v & (kMuSpin | kMuWait)) == 0) {  // no waiters
        ...
        intptr_t nv =
            ...
            kMuWait;
        TURBO_RAW_CHECK(new_h != nullptr, "Enqueue to empty list failed");
        ...
        intptr_t wr_wait = 0;
        TURBO_RAW_CHECK(new_h != nullptr, "Enqueue to list failed");
        if (waitp->how == kExclusive && (v & kMuReader) != 0) {
...
KUMO_ATTRIBUTE_NOINLINE void Mutex::UnlockSlow(SynchWaitParams* waitp) {
  SchedulingGuard::ScopedDisable disable_rescheduling;
  intptr_t v = mu_.load(std::memory_order_relaxed);
  this->AssertReaderHeld();
  ...
  turbo::base_internal::ThreadIdentity* clear_waking_des_waker = nullptr;
  TURBO_RAW_CHECK(waitp == nullptr || waitp->thread->waitp == nullptr ||
  ...
  for (;;) {
    ...
    if ((v & kMuWriter) != 0 && (v & (kMuWait | kMuDesig)) != kMuWait &&
      ...
      TURBO_RAW_CHECK(old_h == nullptr || h->maybe_unlocking,
      ...
      if (old_h != nullptr &&
        ...
        TURBO_RAW_CHECK(old_h->skip == nullptr, "illegal skip from head");
      ...
      if (h->next->waitp->how == kExclusive &&
        ...
        TURBO_RAW_CHECK(h->skip == nullptr, "illegal skip from head");
      ...
      TURBO_RAW_CHECK(pw->next == w, "pw not w's predecessor");
      ...
      TURBO_RAW_CHECK(wake_list != kPerThreadSynchNull,
...
void Mutex::Fer(PerThreadSynch* w) {
  SchedulingGuard::ScopedDisable disable_rescheduling;
  int c = 0;
  TURBO_RAW_CHECK(w->waitp->cond == nullptr,
                 "Mutex::Fer while waiting on Condition");
  TURBO_RAW_CHECK(w->waitp->cv_word == nullptr,
...
void Mutex::AssertHeld() const {
  if ((mu_.load(std::memory_order_relaxed) & kMuWriter) == 0) {
    ...
    TURBO_RAW_LOG(FATAL, "thread should hold write lock on Mutex %p %s",
...
static void CondVarEnqueue(SynchWaitParams* waitp) {
  ...
  TURBO_RAW_CHECK(waitp->thread->waitp == nullptr, "waiting when shouldn't be");
...
bool CondVar::WaitCommon(Mutex* mutex, KernelTimeout t) {
  ...
  TURBO_TSAN_MUTEX_PRE_UNLOCK(mutex, TsanFlags(mutex_how));
  ...
  TURBO_RAW_CHECK(waitp.thread->waitp != nullptr, "not waiting when should be");
  ...
  TURBO_TSAN_MUTEX_POST_UNLOCK(mutex, TsanFlags(mutex_how));
  TURBO_TSAN_MUTEX_PRE_LOCK(mutex, TsanFlags(mutex_how));
  mutex->Trans(mutex_how);  // Reacquire mutex
  TURBO_TSAN_MUTEX_POST_LOCK(mutex, TsanFlags(mutex_how), 0);
  return rc;
}

void CondVar::Signal() {
  SchedulingGuard::ScopedDisable disable_rescheduling;
  TURBO_TSAN_MUTEX_PRE_SIGNAL(nullptr, 0);
  intptr_t v;
  int c = 0;
  for (v = cv_.load(std::memory_order_relaxed); v != 0;
    ...
    if ((v & kCvSpin) == 0 &&  // attempt to acquire spinlock
      ...
      TURBO_TSAN_MUTEX_POST_SIGNAL(nullptr, 0);
      return;
    } else {
      c = synchronization_internal::MutexDelay(c, GENTLE);
    }
...
void ReleasableMutexLock::Release() {
  TURBO_RAW_CHECK(this->mu_ != nullptr,
...
KUMO_CONST_INIT const Condition Condition::kTrue;
