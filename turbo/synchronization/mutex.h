...
#ifndef TURBO_SYNCHRONIZATION_MUTEX_H_
#define TURBO_SYNCHRONIZATION_MUTEX_H_
...
class TURBO_LOCKABLE KUMO_ATTRIBUTE_WARN_UNUSED Mutex {
 public:
  ...
  // `Mutex::Mutex(turbo::kConstInit)` below instead.
  Mutex();

  // Creates a mutex with static storage duration.  A global variable
  // constructed this way avoids the lifetime issues that can occur on program
  // startup and shutdown.  (See turbo/base/const_init.h.)
  //
  ...
  // Example usage:
  //   namespace foo {
  //   KUMO_CONST_INIT turbo::Mutex mu(turbo::kConstInit);
  //   }
  explicit constexpr Mutex(turbo::ConstInitType);
  ...
  void lock() TURBO_EXCLUSIVE_LOCK_FUNCTION();

  KUMO_DEPRECATE_AND_INLINE()
  inline void Lock() TURBO_EXCLUSIVE_LOCK_FUNCTION() { lock(); }

  // Mutex::unlock()
  //
  // Releases this `Mutex` and returns it from the exclusive/write state to the
  // free state. Calling thread must hold the `Mutex` exclusively.
  void unlock() TURBO_UNLOCK_FUNCTION();
  ...
  [[nodiscard]] bool try_lock() TURBO_EXCLUSIVE_TRYLOCK_FUNCTION(true);

  KUMO_DEPRECATE_AND_INLINE()
  [[nodiscard]] bool TryLock() TURBO_EXCLUSIVE_TRYLOCK_FUNCTION(true) {
    return try_lock();
  }
  ...
  void AssertHeld() const TURBO_ASSERT_EXCLUSIVE_LOCK();
  ...
  // lock on the mutex.
  void lock_shared() TURBO_SHARED_LOCK_FUNCTION();

  KUMO_DEPRECATE_AND_INLINE()
  void ReaderLock() TURBO_SHARED_LOCK_FUNCTION() { lock_shared(); }
  ...
  void unlock_shared() TURBO_UNLOCK_FUNCTION();

  KUMO_DEPRECATE_AND_INLINE()
  void ReaderUnlock() TURBO_UNLOCK_FUNCTION() { unlock_shared(); }
  ...
  [[nodiscard]] bool try_lock_shared() TURBO_SHARED_TRYLOCK_FUNCTION(true);

  KUMO_DEPRECATE_AND_INLINE()
  [[nodiscard]] bool ReaderTryLock() TURBO_SHARED_TRYLOCK_FUNCTION(true) {
  ...
  // This function is intended only as a tool to assist debugging; it doesn't
  // guarantee correctness.
  void AssertReaderHeld() const TURBO_ASSERT_SHARED_LOCK();
  ...
  KUMO_DEPRECATE_AND_INLINE()
  void WriterLock() TURBO_EXCLUSIVE_LOCK_FUNCTION() { lock(); }

  KUMO_DEPRECATE_AND_INLINE()
  void WriterUnlock() TURBO_UNLOCK_FUNCTION() { unlock(); }

  KUMO_DEPRECATE_AND_INLINE()
  [[nodiscard]] bool WriterTryLock() TURBO_EXCLUSIVE_TRYLOCK_FUNCTION(true) {
    return try_lock();
  }
  ...
  void LockWhen(const Condition& cond) TURBO_EXCLUSIVE_LOCK_FUNCTION() {
    LockWhenCommon(cond, synchronization_internal::KernelTimeout::Never(),
                   true);
  }

  void ReaderLockWhen(const Condition& cond) TURBO_SHARED_LOCK_FUNCTION() {
    LockWhenCommon(cond, synchronization_internal::KernelTimeout::Never(),
                   false);
  }

  void WriterLockWhen(const Condition& cond) TURBO_EXCLUSIVE_LOCK_FUNCTION() {
    this->LockWhen(cond);
  }
  ...
  bool AwaitWithTimeout(const Condition& cond, turbo::Duration timeout) {
    return AwaitCommon(cond, synchronization_internal::KernelTimeout{timeout});
  }

  bool AwaitWithDeadline(const Condition& cond, turbo::Time deadline) {
  ...
  //
  // Negative timeouts are equivalent to a zero timeout.
  bool LockWhenWithTimeout(const Condition& cond, turbo::Duration timeout)
      TURBO_EXCLUSIVE_LOCK_FUNCTION() {
    return LockWhenCommon(
        cond, synchronization_internal::KernelTimeout{timeout}, true);
  }
  bool ReaderLockWhenWithTimeout(const Condition& cond, turbo::Duration timeout)
      TURBO_SHARED_LOCK_FUNCTION() {
    return LockWhenCommon(
        cond, synchronization_internal::KernelTimeout{timeout}, false);
  }
  bool WriterLockWhenWithTimeout(const Condition& cond, turbo::Duration timeout)
      TURBO_EXCLUSIVE_LOCK_FUNCTION() {
  ...
  //
  // Deadlines in the past are equivalent to an immediate deadline.
  bool LockWhenWithDeadline(const Condition& cond, turbo::Time deadline)
      TURBO_EXCLUSIVE_LOCK_FUNCTION() {
    return LockWhenCommon(
        cond, synchronization_internal::KernelTimeout{deadline}, true);
  }
  bool ReaderLockWhenWithDeadline(const Condition& cond, turbo::Time deadline)
      TURBO_SHARED_LOCK_FUNCTION() {
    return LockWhenCommon(
        cond, synchronization_internal::KernelTimeout{deadline}, false);
  }
  bool WriterLockWhenWithDeadline(const Condition& cond, turbo::Time deadline)
      TURBO_EXCLUSIVE_LOCK_FUNCTION() {
  ...
  void EnableInvariantDebugging(
      void (*turbo_nullable invariant)(void* turbo_nullability_unknown),
      void* turbo_nullability_unknown arg);

  // Mutex::EnableDebugLog()
  //
  // Cause all subsequent uses of this `Mutex` to be logged via
  // `TURBO_RAW_LOG(INFO)`. Log entries are tagged with `name` if no previous
  // call to `EnableInvariantDebugging()` or `EnableDebugLog()` has been made.
  //
  // Note: This method substantially reduces `Mutex` performance.
  void EnableDebugLog(const char* turbo_nullable name);
 ...
 private:
  ...
  static void IncrementSynchSem(Mutex* turbo_nonnull mu,
                                base_internal::PerThreadSynch* turbo_nonnull w);
  static bool DecrementSynchSem(Mutex* turbo_nonnull mu,
                                base_internal::PerThreadSynch* turbo_nonnull w,
                                synchronization_internal::KernelTimeout t);

  // slow path acquire
  void LockSlowLoop(SynchWaitParams* turbo_nonnull waitp, int flags);
  // wrappers around LockSlowLoop()
  bool LockSlowWithDeadline(MuHow turbo_nonnull how,
                            const Condition* turbo_nullable cond,
                            synchronization_internal::KernelTimeout t,
                            int flags);
  void LockSlow(MuHow turbo_nonnull how, const Condition* turbo_nullable cond,
                int flags) KUMO_ATTRIBUTE_COLD;
  // slow path release
  void UnlockSlow(SynchWaitParams* turbo_nullable waitp) KUMO_ATTRIBUTE_COLD;
  // TryLock slow path.
  bool TryLockSlow();
  ...
  void TryRemove(base_internal::PerThreadSynch* turbo_nonnull s);
  // Block a thread on mutex.
  void Block(base_internal::PerThreadSynch* turbo_nonnull s);
  // Wake a thread; return successor.
  base_internal::PerThreadSynch* turbo_nullable Wakeup(
      base_internal::PerThreadSynch* turbo_nonnull w);
  void Dtor();

  friend class CondVar;                // for access to Trans()/Fer().
  void Trans(MuHow turbo_nonnull how);  // used for CondVar->Mutex transfer
  void Fer(base_internal::PerThreadSynch* turbo_nonnull
               w);  // used for CondVar->Mutex transfer

  // Catch the error of writing Mutex when intending MutexLock.
  explicit Mutex(const volatile Mutex* turbo_nullable /*ignored*/) {}
...
//   Mutex mu_;
// };
class TURBO_SCOPED_LOCKABLE MutexLock {
 public:
  // Constructors

  // Calls `mu.lock()` and returns when that call returns. That is, `mu` is
  // guaranteed to be locked when this object is constructed.
  explicit MutexLock(Mutex& mu KUMO_INTERNAL_ATTRIBUTE_CAPTURED_BY_THIS)
      TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : mu_(mu) {
    this->mu_.lock();
  }
  ...
  explicit MutexLock(Mutex* turbo_nonnull mu) TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : MutexLock(*mu) {}
  ...
  explicit MutexLock(Mutex& mu KUMO_INTERNAL_ATTRIBUTE_CAPTURED_BY_THIS,
                     const Condition& cond) TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : mu_(mu) {
  ...
  [[deprecated("Use the constructor that takes a reference instead")]]
  KUMO_REFACTOR_INLINE
  explicit MutexLock(Mutex* turbo_nonnull mu, const Condition& cond)
      TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : MutexLock(*mu, cond) {}
  ...
  ~MutexLock() TURBO_UNLOCK_FUNCTION() { this->mu_.unlock(); }
...
class TURBO_SCOPED_LOCKABLE ReaderMutexLock {
 public:
  explicit ReaderMutexLock(Mutex& mu KUMO_INTERNAL_ATTRIBUTE_CAPTURED_BY_THIS)
      TURBO_SHARED_LOCK_FUNCTION(mu)
      : mu_(mu) {
    mu.lock_shared();
  }
  ...
  [[deprecated("Use the constructor that takes a reference instead")]]
  KUMO_REFACTOR_INLINE
  explicit ReaderMutexLock(Mutex* turbo_nonnull mu) TURBO_SHARED_LOCK_FUNCTION(mu)
      : ReaderMutexLock(*mu) {}

  explicit ReaderMutexLock(Mutex& mu KUMO_INTERNAL_ATTRIBUTE_CAPTURED_BY_THIS,
                           const Condition& cond) TURBO_SHARED_LOCK_FUNCTION(mu)
      : mu_(mu) {
  ...
  [[deprecated("Use the constructor that takes a reference instead")]]
  KUMO_REFACTOR_INLINE
  explicit ReaderMutexLock(Mutex* turbo_nonnull mu, const Condition& cond)
      TURBO_SHARED_LOCK_FUNCTION(mu)
  ...
  ~ReaderMutexLock() TURBO_UNLOCK_FUNCTION() { this->mu_.unlock_shared(); }
...
class TURBO_SCOPED_LOCKABLE WriterMutexLock {
 public:
  explicit WriterMutexLock(Mutex& mu KUMO_INTERNAL_ATTRIBUTE_CAPTURED_BY_THIS)
      TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : mu_(mu) {
    mu.lock();
  }
  ...
  [[deprecated("Use the constructor that takes a reference instead")]]
  KUMO_REFACTOR_INLINE
  explicit WriterMutexLock(Mutex* turbo_nonnull mu)
      TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : WriterMutexLock(*mu) {}
  ...
  [[deprecated("Use the constructor that takes a reference instead")]]
  KUMO_REFACTOR_INLINE
  explicit WriterMutexLock(Mutex* turbo_nonnull mu, const Condition& cond)
      TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
  ...
  ~WriterMutexLock() TURBO_UNLOCK_FUNCTION() { this->mu_.unlock(); }
...
//   int count_ TURBO_GUARDED_BY(mu_);
...
class Condition {
 public:
  ...
  Condition(bool (*turbo_nonnull func)(void* turbo_nullability_unknown),
            void* turbo_nullability_unknown arg);
  ...
  template <typename T>
  Condition(bool (*turbo_nonnull func)(T* turbo_nullability_unknown),
            T* turbo_nullability_unknown arg);
  ...
  template <typename T, typename = void>
  Condition(
      bool (*turbo_nonnull func)(T* turbo_nullability_unknown),
      typename turbo::type_identity<T>::type* turbo_nullability_unknown
          arg);
  ...
  // `object->Method()`.
  //
  // Implementation Note: `turbo::type_identity` is used to allow
  // methods to come from base classes. A simpler signature like
  // `Condition(T*, bool (T::*)())` does not suffice.
  template <typename T>
  Condition(
      T* turbo_nonnull object,
      bool (turbo::type_identity<T>::type::* turbo_nonnull method)());

  // Same as above, for const members
  template <typename T>
  Condition(
      const T* turbo_nonnull object,
      bool (turbo::type_identity<T>::type::* turbo_nonnull method)()
          const);

  // A Condition that returns the value of `*cond`
  explicit Condition(const bool* turbo_nonnull cond);
  ...
  template <typename T,
            std::enable_if_t<
                ...
                int> = 0>
  explicit Condition(const T* turbo_nonnull obj)
  ...
  KUMO_CONST_INIT static const Condition kTrue;
  ...
  // condition.
  static bool GuaranteedEqual(const Condition* turbo_nullable a,
                              const Condition* turbo_nullable b);
 ...
 private:
  ...
  bool (*turbo_nullable eval_)(const Condition* turbo_nonnull) = nullptr;

  // Either an argument for a function call or an object for a method call.
  void* turbo_nullable arg_ = nullptr;

  // Various functions eval_ can point to:
  static bool CallVoidPtrFunction(const Condition* turbo_nonnull c);
  template <typename T>
  static bool CastAndCallFunction(const Condition* turbo_nonnull c);
  template <typename T, typename ConditionMethodPtr>
  static bool CastAndCallMethod(const Condition* turbo_nonnull c);

  template <typename T>
  static bool CallByRef(const T* turbo_nonnull self) {
    return (*self)();
  }
  ...
  template <typename T>
  inline void ReadCallback(T* turbo_nonnull callback) const {
    std::memcpy(callback, callback_, sizeof(*callback));
  }

  static bool AlwaysTrue(const Condition* turbo_nullable) { return true; }
...
class CondVar {
 public:
  ...
  void Wait(Mutex* turbo_nonnull mu) {
  ...
  //
  // Requires and ensures that the current thread holds the `Mutex`.
  bool WaitWithTimeout(Mutex* turbo_nonnull mu, turbo::Duration timeout) {
  ...
  //
  // Requires and ensures that the current thread holds the `Mutex`.
  bool WaitWithDeadline(Mutex* turbo_nonnull mu, turbo::Time deadline) {
  ...
  // `TURBO_RAW_LOG(INFO)`. Log entries are tagged with `name` if `name != 0`.
  // Note: this method substantially reduces `CondVar` performance.
  void EnableDebugLog(const char* turbo_nullable name);

 private:
  bool WaitCommon(Mutex* turbo_nonnull mutex,
                  synchronization_internal::KernelTimeout t);
  void Remove(base_internal::PerThreadSynch* turbo_nonnull s);
...
class TURBO_SCOPED_LOCKABLE MutexLockMaybe {
 public:
  explicit MutexLockMaybe(Mutex* turbo_nullable mu)
      TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : mu_(mu) {
    if (this->mu_ != nullptr) {
      this->mu_->lock();
  ...
  explicit MutexLockMaybe(Mutex* turbo_nullable mu, const Condition& cond)
      TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : mu_(mu) {
    if (this->mu_ != nullptr) {
      this->mu_->LockWhen(cond);
  ...
  ~MutexLockMaybe() TURBO_UNLOCK_FUNCTION() {
    if (this->mu_ != nullptr) {
      this->mu_->unlock();
    }
  }
 ...
 private:
  Mutex* turbo_nullable const mu_;
...
class TURBO_SCOPED_LOCKABLE ReleasableMutexLock {
 public:
  explicit ReleasableMutexLock(
      Mutex& mu KUMO_INTERNAL_ATTRIBUTE_CAPTURED_BY_THIS)
      TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : mu_(&mu) {
    this->mu_->lock();
  }
  ...
  [[deprecated("Use the constructor that takes a reference instead")]]
  KUMO_REFACTOR_INLINE
  explicit ReleasableMutexLock(Mutex* turbo_nonnull mu)
      TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
  ...
  [[deprecated("Use the constructor that takes a reference instead")]]
  KUMO_REFACTOR_INLINE
  explicit ReleasableMutexLock(Mutex* turbo_nonnull mu, const Condition& cond)
      TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : ReleasableMutexLock(*mu, cond) {}

  ~ReleasableMutexLock() TURBO_UNLOCK_FUNCTION() {
    if (this->mu_ != nullptr) {
      this->mu_->unlock();
    }
  }

  void Release() TURBO_UNLOCK_FUNCTION();

 private:
  Mutex* turbo_nullable mu_;
...
inline constexpr Mutex::Mutex(turbo::ConstInitType) : mu_(0) {}

#if !defined(__APPLE__) && !KUMO_BUILD_DLL
KUMO_ATTRIBUTE_ALWAYS_INLINE
inline Mutex::~Mutex() { Dtor(); }
#endif

#if defined(NDEBUG) && !KUMO_HAVE_THREAD_SANITIZER && \
    !KUMO_BUILD_DLL
...
// (KUMO_BUILD_DLL), we must provide an out-of-line definition. This ensures the
...
// static
template <typename T, typename ConditionMethodPtr>
bool Condition::CastAndCallMethod(const Condition* turbo_nonnull c) {
...
// static
template <typename T>
bool Condition::CastAndCallFunction(const Condition* turbo_nonnull c) {
  bool (*function)(T*);
...
// measured by //turbo/base/internal/cycleclock.h, and which may not
...
// mode.)
void RegisterMutexProfiler(void (*turbo_nonnull fn)(int64_t wait_cycles));
...
// //turbo/base/internal/cycleclock.h, and which may not be real
// "cycle" counts.)
//
...
void RegisterMutexTracer(void (*turbo_nonnull fn)(const char* turbo_nonnull msg,
                                                 const void* turbo_nonnull obj,
                                                 int64_t wait_cycles));
...
// This has the same ordering and single-use limitations as
// RegisterMutexProfiler() above.
void RegisterCondVarTracer(void (*turbo_nonnull fn)(
    const char* turbo_nonnull msg, const void* turbo_nonnull cv));
...
// check.
extern "C" {
void TURBO_INTERNAL_C_SYMBOL(TurboInternalMutexYield)();
}  // extern "C"

#endif  // TURBO_SYNCHRONIZATION_MUTEX_H_
