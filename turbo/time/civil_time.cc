...
namespace turbo {
    ...
    namespace {
        ...
        std::string FormatYearAnd(std::string_view fmt, CivilSecond cs) {
            ...
            return str_cat(cs.year(), format_time(fmt, FromCivil(ncs, utc), utc));
        }
        ...
        bool ParseYearAnd(std::string_view fmt, std::string_view s, CivilT* c) {
            ...
            if (endp == np || errno == ERANGE)
                return false;
            const civil_year_t normalized_year = NormalizeYear(y);
            const std::string norm = str_cat(normalized_year, endp);

            const TimeZone utc = UTCTimeZone();
            Time t;
            if (parse_time(str_cat("%Y", fmt), norm, utc, &t, nullptr)) {
                ...
                *c = CivilT(year, cs.month(), cs.day(), cs.hour(), cs.minute(), cs.second());
                return true;
            }
        ...
        bool ParseLenient(std::string_view s, CivilT* c) {
            ...
            if (ParseCivilTime(s, c))
                return true;
            // Try parsing as each of the 6 types, trying the most common types first
            // (based on csearch results).
            if (ParseAs<CivilDay>(s, c))
                return true;
            if (ParseAs<CivilSecond>(s, c))
                return true;
            if (ParseAs<CivilHour>(s, c))
                return true;
            if (ParseAs<CivilMonth>(s, c))
                return true;
            if (ParseAs<CivilMinute>(s, c))
                return true;
            if (ParseAs<CivilYear>(s, c))
                return true;
            return false;
        }
    } // namespace
