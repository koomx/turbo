...
#ifndef TURBO_USE_CYCLECLOCK_FOR_GET_CURRENT_TIME_NANOS
#define TURBO_USE_CYCLECLOCK_FOR_GET_CURRENT_TIME_NANOS 0
#endif
...
#define GET_CURRENT_TIME_NANOS_FROM_SYSTEM() \
    ::turbo::time_internal::GetCurrentTimeNanosFromSystem()
#endif
...
// value.
    ...
    static_assert(((kMinNSBetweenSamples << (kScale + 1)) >> (kScale + 1)) == kMinNSBetweenSamples,
    ...
    struct KUMO_CACHELINE_ALIGNED TimeState {
        ...
        uint64_t last_now_cycles TURBO_GUARDED_BY(lock) { 0 };
        ...
        turbo::base_internal::SpinLock lock { base_internal::SCHEDULE_KERNEL_ONLY };
    };
    KUMO_CONST_INIT static TimeState time_state;
    ...
    static int64_t GetCurrentTimeNanosFromKernel(uint64_t last_cycleclock,
        ...
        int loops = 0;
        do {
            before_cycles = static_cast<uint64_t>(GET_CURRENT_TIME_NANOS_CYCLECLOCK_NOW());
            current_time_nanos_from_system = GET_CURRENT_TIME_NANOS_FROM_SYSTEM();
            after_cycles = static_cast<uint64_t>(GET_CURRENT_TIME_NANOS_CYCLECLOCK_NOW());
            // elapsed_cycles is unsigned, so is large on overflow
            elapsed_cycles = after_cycles - before_cycles;
            if (elapsed_cycles >= local_approx_syscall_time_in_cycles && ++loops == 20) { // clock changed frequencies?  Back off.
                loops = 0;
                if (local_approx_syscall_time_in_cycles < 1000 * 1000) {
                    local_approx_syscall_time_in_cycles = (local_approx_syscall_time_in_cycles + 1) << 1;
                }
                time_state.approx_syscall_time_in_cycles.store(
                    local_approx_syscall_time_in_cycles, std::memory_order_relaxed);
            }
        } while (elapsed_cycles >= local_approx_syscall_time_in_cycles || last_cycleclock - after_cycles < (static_cast<uint64_t>(1) << 16));
        ...
        if ((local_approx_syscall_time_in_cycles >> 1) < elapsed_cycles) {
            ...
            const uint64_t new_approximation = local_approx_syscall_time_in_cycles - (local_approx_syscall_time_in_cycles >> 3);
    ...
    static int64_t GetCurrentTimeNanosSlowPath() KUMO_ATTRIBUTE_COLD;
    ...
    int64_t GetCurrentTimeNanos() {
        ...
        uint64_t now_cycles = static_cast<uint64_t>(GET_CURRENT_TIME_NANOS_CYCLECLOCK_NOW());
        ...
        base_cycles = time_state.last_sample.base_cycles.load(std::memory_order_acquire);
        nsscaled_per_cycle = time_state.last_sample.nsscaled_per_cycle.load(std::memory_order_acquire);
        ...
        if (seq_read0 == seq_read1 && (seq_read0 & 1) == 0 && (delta_cycles = now_cycles - base_cycles) < min_cycles_per_sample) {
    ...
    static uint64_t UpdateLastSample(
        ...
        const struct TimeSample* sample) KUMO_ATTRIBUTE_COLD;
    ...
    static int64_t GetCurrentTimeNanosSlowPath()
        ...
        if (delta_cycles < sample.min_cycles_per_sample) {
            ...
            estimated_base_ns = sample.base_ns + ((delta_cycles * sample.nsscaled_per_cycle) >> kScale);
            time_state.stats_fast_slow_paths++;
        } else {
            estimated_base_ns = UpdateLastSample(now_cycles, now_ns, delta_cycles, &sample);
        }
    ...
    static uint64_t UpdateLastSample(uint64_t now_cycles, uint64_t now_ns,
        ...
        uint64_t lock_value = SeqAcquire(&time_state.seq); // acquire seqlock to block readers
        ...
        if (sample->raw_ns == 0 || // no recent sample, or clock went backwards
            sample->raw_ns + static_cast<uint64_t>(5) * 1000 * 1000 * 1000 < now_ns || now_ns < sample->raw_ns || now_cycles < sample->base_cycles) {
        ...
        } else if (sample->raw_ns + 500 * 1000 * 1000 < now_ns && sample->base_cycles + 50 < now_cycles) {
            ...
            if (sample->nsscaled_per_cycle != 0) { // Have a cycle time estimate.
                ...
                } while (estimated_scaled_ns / sample->nsscaled_per_cycle != (delta_cycles >> s));
                estimated_base_ns = sample->base_ns + (estimated_scaled_ns >> (kScale - s));
            }
            ...
            uint64_t assumed_next_sample_delta_cycles = SafeDivideAndScale(kMinNSBetweenSamples, measured_nsscaled_per_cycle);
            ...
            //  (assumed_next_sample_delta_cycles * nsscaled_per_cycle) >> kScale
            ns = static_cast<uint64_t>(static_cast<int64_t>(kMinNSBetweenSamples) + diff_ns - (diff_ns / 16));
            uint64_t new_nsscaled_per_cycle = SafeDivideAndScale(ns, assumed_next_sample_delta_cycles);
            if (new_nsscaled_per_cycle != 0 && diff_ns < 100 * 1000 * 1000 && -diff_ns < 100 * 1000 * 1000) {
                ...
                uint64_t new_min_cycles_per_sample = SafeDivideAndScale(kMinNSBetweenSamples, new_nsscaled_per_cycle);
                time_state.last_sample.min_cycles_per_sample.store(
...
KUMO_ATTRIBUTE_WEAK void TURBO_INTERNAL_C_SYMBOL(TurboInternalSleepFor)(
