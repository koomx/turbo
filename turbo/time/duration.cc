...
namespace turbo {
    ...
    namespace {
        ...
        inline int64_t DecodeTwosComp(uint64_t v) {
            return turbo::bit_cast<int64_t>(v);
        }
        ...
        struct SafeMultiply {
            uint128 operator()(uint128 a, uint128 b) const {
                ...
                return b == 0 ? b : (a > Uint128Max() / b) ? Uint128Max()
                                                           : a * b;
            }
        };
    ...
    namespace {
        ...
        KUMO_FORCE_INLINE int64_t IDivDurationImpl(bool satq,
            const Duration num,
            const Duration den,
            Duration* rem) {
            int64_t q = 0;
    ...
    Duration& Duration::operator+=(Duration rhs) {
        ...
        rep_hi_ = DecodeTwosComp(EncodeTwosComp(rep_hi_.Get()) + EncodeTwosComp(rhs.rep_hi_.Get()));
        ...
        if (rhs.rep_hi_.Get() < 0 ? rep_hi_.Get() > orig_rep_hi
            ...
            return *this = rhs.rep_hi_.Get() < 0 ? -InfiniteDuration() : InfiniteDuration();
        }
        return *this;
    }
