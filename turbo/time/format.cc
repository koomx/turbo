...
namespace turbo {
    ...
    std::string format_time(std::string_view format, turbo::Time t,
        ...
        if (t == turbo::InfiniteFuture())
            return std::string(kInfiniteFutureStr);
        if (t == turbo::InfinitePast())
            return std::string(kInfinitePastStr);
    ...
    std::string format_time(turbo::Time t) {
        return turbo::format_time(RFC3339_full, t, turbo::LocalTimeZone());
    }

    bool parse_time(std::string_view format, std::string_view input,
        turbo::Time* time, std::string* err) {
        return turbo::parse_time(format, input, turbo::UTCTimeZone(), time, err);
    }
    ...
    bool parse_time(std::string_view format, std::string_view input,
        ...
        std::string error;
        cctz_parts parts;
        const bool b = cctz::detail::parse(std::string(format), std::string(input),
    ...
    bool turbo_parse_flag(std::string_view text, turbo::Time* t, std::string* error) {
        return turbo::parse_time(RFC3339_full, text, turbo::UTCTimeZone(), t, error);
    }

    std::string turbo_unparse_flag(turbo::Time t) {
        return turbo::format_time(RFC3339_full, t, turbo::UTCTimeZone());
    }
