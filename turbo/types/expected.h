...
#define STATUS_EXPECTED_EXCEPTIONS_ENABLED
#endif

#ifdef _MSVC_LANG
#define STATUS_CPLUSPLUS _MSVC_LANG
#else
#define STATUS_CPLUSPLUS __cplusplus
#endif

#if !defined(STATUS_ASSERT)
#include <cassert>
#define STATUS_ASSERT(x) assert(x)
#endif
...
#ifndef STATUS_MONOSTATE_INPLACE_MUTEX
#define STATUS_MONOSTATE_INPLACE_MUTEX
...
#define STATUS_EXPECTED_THROW_EXCEPTION(e) throw((e));
#else
#define STATUS_EXPECTED_THROW_EXCEPTION(e) std::terminate();
#endif

    namespace detail {
#ifndef STATUS_TRAITS_MUTEX
#define STATUS_TRAITS_MUTEX
...
#define STATUS_TRAITS_LIBCXX_MEM_FN_WORKAROUND
#endif

// In C++11 mode, there's an issue in libc++'s std::mem_fn
// which results in a hard-error when using it in a noexcept expression
// in some cases. This is a check to workaround the common failing case.
#ifdef STATUS_TRAITS_LIBCXX_MEM_FN_WORKAROUND
        template <class T>
...
// Trait for checking if a type is a turbo::expected
        template<class T>
...
// This specialization is for when T and E are trivially copy constructible
        template<class T, class E,
                bool = is_void_or<T, std::is_trivially_copy_constructible<T>>::
                       value && std::is_trivially_copy_constructible<E>::value,
...
// This class manages conditionally having a trivial copy assignment operator
        template<class T, class E,
                bool = is_void_or<
                        T, conjunction<std::is_trivially_copy_assignable<T>,
...
/// tracked by the expected object.
    ...
    public:
        ...
        constexpr const U &value() const &{
            if (!has_value())
                STATUS_EXPECTED_THROW_EXCEPTION(bad_expected_access<E>(err().value()));
            return val();
        }
        ...
        constexpr const U &&value() const &&{
            if (!has_value())
                STATUS_EXPECTED_THROW_EXCEPTION(bad_expected_access<E>(std::move(err()).value()));
            return std::move(val());
        }
