...
namespace turbo {
    ...
    class KUMO_ATTRIBUTE_WARN_UNUSED FixedArray {
        ...
        using EnableIfInputIterator = std::enable_if_t<meta_internal::IsAtLeastInputIterator<Iterator>::value>;
        static constexpr bool NoexceptCopyable() {
            return std::is_nothrow_copy_constructible_v<StorageElement> && turbo::allocator_is_nothrow<allocator_type>::value;
        }
        static constexpr bool NoexceptMovable() {
            return std::is_nothrow_move_constructible_v<StorageElement> && turbo::allocator_is_nothrow<allocator_type>::value;
        }
    ...
    public:
        ...
        static constexpr size_type inline_elements = (N == kFixedArrayUseDefault ? kInlineBytesDefault / sizeof(value_type)
        ...
        const_pointer data() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        // elements.
        pointer data() KUMO_ATTRIBUTE_LIFETIME_BOUND {
            return AsValueType(storage_.begin());
        }
        ...
        // REQUIRES: 0 <= i < size()
        reference operator[](size_type i) KUMO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        // REQUIRES: 0 <= i < size()
        const_reference operator[](size_type i) const KUMO_ATTRIBUTE_LIFETIME_BOUND {
            turbo::base_internal::HardeningAssertLT(i, size());
            return data()[i];
        }
        ...
        reference at(size_type i) KUMO_ATTRIBUTE_LIFETIME_BOUND {
            if (KUMO_UNLIKELY(i >= size())) {
        ...
        // of the fixed array.
        const_reference at(size_type i) const KUMO_ATTRIBUTE_LIFETIME_BOUND {
            if (KUMO_UNLIKELY(i >= size())) {
                ThrowStdOutOfRange("FixedArray::at failed bounds check");
            }
            return data()[i];
        ...
        reference front() KUMO_ATTRIBUTE_LIFETIME_BOUND {
            turbo::base_internal::HardeningAssertNonEmpty(*this);
            return data()[0];
        }

        // Overload of FixedArray::front() to return a reference to the first element
        // of a fixed array of const values.
        const_reference front() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
            turbo::base_internal::HardeningAssertNonEmpty(*this);
            return data()[0];
        }
        ...
        reference back() KUMO_ATTRIBUTE_LIFETIME_BOUND {
            turbo::base_internal::HardeningAssertNonEmpty(*this);
            return data()[size() - 1];
        }
        ...
        // of a fixed array of const values.
        const_reference back() const KUMO_ATTRIBUTE_LIFETIME_BOUND {
            turbo::base_internal::HardeningAssertNonEmpty(*this);
            return data()[size() - 1];
        }
        ...
        iterator begin() KUMO_ATTRIBUTE_LIFETIME_BOUND { return data(); }

        // Overload of FixedArray::begin() to return a const iterator to the
        // beginning of the fixed array.
        const_iterator begin() const KUMO_ATTRIBUTE_LIFETIME_BOUND { return data(); }
        ...
        iterator end() KUMO_ATTRIBUTE_LIFETIME_BOUND { return data() + size(); }
        ...
        reverse_iterator rend() KUMO_ATTRIBUTE_LIFETIME_BOUND {
    ...
    private:
        ...
        using StorageElement = std::conditional_t<std::is_array_v<value_type>,
        ...
        using InlinedStorage = std::conditional_t<inline_elements == 0, EmptyInlinedStorage,
        ...
        private:
            ...
            CompressedTuple<size_type, allocator_type> size_alloc_;
