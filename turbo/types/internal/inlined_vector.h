...
#pragma GCC diagnostic ignored "-Warray-bounds"
        ...
        template <typename A>
        using IsSwapOk = turbo::type_traits_internal::IsSwappable<ValueType<A>>;

        template <typename A, bool IsTriviallyDestructible = std::is_trivially_destructible_v<ValueType<A>> && std::is_same_v<A, std::allocator<ValueType<A>>>>
        ...
        template <typename A,
            bool IsOverAligned = (alignof(ValueType<A>) > KUMO_DEFAULT_NEW_ALIGNMENT)>
        struct MallocAdapter {
            static Allocation<A> Allocate(A& allocator, SizeType<A> requested_capacity) {
                return {
                    AllocatorTraits<A>::allocate(allocator, requested_capacity),
                    requested_capacity
                };
            }
        ...
        void ConstructElements(turbo::type_identity_t<A>& allocator,
        ...
        public:
            ...
            Pointer<A> Allocate(SizeType<A> requested_capacity) {
                Allocation<A> result = MallocAdapter<A>::Allocate(GetAllocator(), requested_capacity);
                GetData() = result.data;
        ...
        private:
            ...
            CompressedTuple<A, Pointer<A>> allocator_data_;
        ...
        public:
            ...
            using SwapInlinedElementsPolicy = std::conditional_t<
                ...
                std::conjunction_v<turbo::is_trivially_relocatable<ValueType<A>>,
                    std::is_same<A, std::allocator<ValueType<A>>>>,
                MemcpyPolicy,
            ...
            ~Storage() {
                ...
                // maybe not even that).
                if (std::is_trivially_destructible_v<ValueType<A>> && std::is_same_v<A, std::allocator<ValueType<A>>>) {
...
#if KUMO_INTERNAL_HAVE_MIN_GNUC_VERSION(12, 0)
#pragma GCC diagnostic push
...
#pragma GCC diagnostic pop
            ...
            // KUMO_ATTRIBUTE_NO_SANITIZE_CFI is used because the memory pointed to may be
            ...
            // as a bad cast.
            KUMO_ATTRIBUTE_NO_SANITIZE_CFI Pointer<A> GetInlinedData() {
                return reinterpret_cast<Pointer<A>>(data_.inlined.inlined_data);
            }

            KUMO_ATTRIBUTE_NO_SANITIZE_CFI ConstPointer<A> GetInlinedData() const {
            ...
            StorageView<A> MakeStorageView() {
                return GetIsAllocated()
                    ? StorageView<A> {
                          GetAllocatedData(), GetSize(),
                          GetAllocatedCapacity()
                      }
                    : StorageView<A> { GetInlinedData(), GetSize(), GetInlinedCapacity() };
            }
            ...
            KUMO_ATTRIBUTE_NOINLINE void InitFrom(const Storage& other);
            ...
            void SetSize(SizeType<A> size) {
                GetSizeAndIsAllocated() = (size << 1) | static_cast<SizeType<A>>(GetIsAllocated());
            }
            ...
            void SubtractSize(SizeType<A> count) {
                turbo::base_internal::HardeningAssertLE(count, GetSize());
            ...
            void MemcpyFrom(const Storage& other_storage) {
                ...
                {
                    using V = ValueType<A>;
                    KUMO_ASSERT(other_storage.GetIsAllocated() || (std::is_same_v<A, std::allocator<V>> && (
                                                                       // First case above
                                                                       turbo::is_trivially_relocatable<V>::value ||
                                                                       // Second case above
                                                                       (std::is_trivially_move_assignable_v<V> && std::is_trivially_destructible_v<V>) ||
                                                                       // Third case above
                                                                       (std::is_trivially_copy_constructible_v<V> || std::is_trivially_copy_assignable_v<V>))));
                }
        ...
        private:
            KUMO_ATTRIBUTE_NOINLINE void DestroyContents();

            using Metadata = CompressedTuple<A, SizeType<A>>;
            ...
            template <typename... Args>
            KUMO_ATTRIBUTE_NOINLINE Reference<A> EmplaceBackSlow(Args&&... args);
        ...
        void Storage<T, N, A>::InitFrom(const Storage& other) {
            ...
            KUMO_ASSERT(n > 0); // Empty sources handled in caller.
            ConstPointer<A> src;
            Pointer<A> dst;
            if (!other.GetIsAllocated()) {
                ...
                Allocation<A> allocation = MallocAdapter<A>::Allocate(GetAllocator(), requested_capacity);
            ...
            if (std::is_trivially_copy_constructible_v<ValueType<A>> && std::is_same_v<A, std::allocator<ValueType<A>>>) {
        ...
        auto Storage<T, N, A>::Initialize(ValueAdapter values,
            ...
            KUMO_ASSERT(!GetIsAllocated());
            KUMO_ASSERT(GetSize() == 0);
            ...
            if (new_size > GetInlinedCapacity()) {
                ...
                // safe to take on the allocation with size `0`. If `ConstructElements(...)`
                // throws, deallocation will be automatically handled by `~Storage()`.
                SizeType<A> requested_capacity = ComputeCapacity(GetInlinedCapacity(), new_size);
                Allocation<A> allocation = MallocAdapter<A>::Allocate(GetAllocator(), requested_capacity);
        ...
        auto Storage<T, N, A>::Assign(ValueAdapter values,
            ...
            if (new_size > storage_view.capacity) {
                SizeType<A> requested_capacity = ComputeCapacity(storage_view.capacity, new_size);
                construct_loop = { allocation_tx.Allocate(requested_capacity), new_size };
                destroy_loop = { storage_view.data, storage_view.size };
            } else if (new_size > storage_view.size) {
                assign_loop = { storage_view.data, storage_view.size };
                construct_loop = {
                    storage_view.data + storage_view.size,
        ...
        auto Storage<T, N, A>::Insert(ConstIterator<A> pos, ValueAdapter values,
            ...
            if (new_size > storage_view.capacity) {
                ...
                SizeType<A> move_construction_destination_index = (std::max)(insert_end_index, storage_view.size);
                ...
                turbo::Span<ValueType<A>> insert_assignment = {
        ...
        auto Storage<T, N, A>::EmplaceBack(Args&&... args) -> Reference<A> {
            ...
            if (KUMO_LIKELY(n != storage_view.capacity)) {
        ...
        auto Storage<T, N, A>::Erase(ConstIterator<A> from,
            ...
            if (turbo::is_trivially_relocatable<ValueType<A>>::value && std::is_nothrow_destructible_v<ValueType<A>> && std::is_same_v<A, std::allocator<ValueType<A>>>) {
        ...
        auto Storage<T, N, A>::Reserve(SizeType<A> requested_capacity) -> void {
            ...
            if (KUMO_UNLIKELY(requested_capacity <= storage_view.capacity))
                return;
            ...
            SizeType<A> new_requested_capacity = ComputeCapacity(storage_view.capacity, requested_capacity);
        ...
        auto Storage<T, N, A>::ShrinkToFit() -> void {
            ...
            StorageView<A> storage_view {
                GetAllocatedData(), GetSize(),
                GetAllocatedCapacity()
            };

            if (KUMO_UNLIKELY(storage_view.size == storage_view.capacity))
                return;
        ...
        auto Storage<T, N, A>::Swap(Storage* other_storage_ptr) -> void {
            using std::swap;
            KUMO_ASSERT(this != other_storage_ptr);
