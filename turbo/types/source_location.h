...
// -----------------------------------------------------------------------------
//
// turbo::SourceLocation provides source-code location info for C++17 and later.
//
...
// callsite, define it with a parameter of type `turbo::SourceLocation`. The
// caller can then invoke the function, passing
// `turbo::SourceLocation::current()` as the argument.
//
// If at all possible, make the `turbo::SourceLocation` parameter be the
...
#ifndef TURBO_TYPES_SOURCE_LOCATION_H_
#define TURBO_TYPES_SOURCE_LOCATION_H_
