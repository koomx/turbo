...
namespace turbo {
    ...
    namespace detail {
        ...
        [[gnu::noinline]] const char* find_first_symbols_neon_long(const char* pos, const char* const end) {
            ...
            /// the sparse case.
            if (maybe_negate<positive>(is_in<symbols...>(pos[0])))
                return pos;
            if (maybe_negate<positive>(is_in<symbols...>(pos[1])))
                return pos + 1;
            if (maybe_negate<positive>(is_in<symbols...>(pos[2])))
                return pos + 2;
            if (maybe_negate<positive>(is_in<symbols...>(pos[3])))
                return pos + 3;
            if (maybe_negate<positive>(is_in<symbols...>(pos[4])))
                return pos + 4;
            if (maybe_negate<positive>(is_in<symbols...>(pos[5])))
                return pos + 5;
            if (maybe_negate<positive>(is_in<symbols...>(pos[6])))
                return pos + 6;
            if (maybe_negate<positive>(is_in<symbols...>(pos[7])))
                return pos + 7;
            pos += 8;
        ...
        [[gnu::noinline]] const char* find_first_symbols_neon_long_rt(const char* pos, const char* const end, const char* symbols, size_t num_chars) {
            if (maybe_negate<positive>(is_in(pos[0], symbols, num_chars)))
                return pos;
            if (maybe_negate<positive>(is_in(pos[1], symbols, num_chars)))
                return pos + 1;
            if (maybe_negate<positive>(is_in(pos[2], symbols, num_chars)))
                return pos + 2;
            if (maybe_negate<positive>(is_in(pos[3], symbols, num_chars)))
                return pos + 3;
            if (maybe_negate<positive>(is_in(pos[4], symbols, num_chars)))
                return pos + 4;
            if (maybe_negate<positive>(is_in(pos[5], symbols, num_chars)))
                return pos + 5;
            if (maybe_negate<positive>(is_in(pos[6], symbols, num_chars)))
                return pos + 6;
            if (maybe_negate<positive>(is_in(pos[7], symbols, num_chars)))
                return pos + 7;
            pos += 8;
        ...
        [[gnu::noinline]] const char* find_last_symbols_neon_long(const char* const begin, const char* pos) {
            ...
            if (maybe_negate<positive>(is_in<symbols...>(pos[-7])))
                return pos - 7;
            if (maybe_negate<positive>(is_in<symbols...>(pos[-8])))
                return pos - 8;
            pos -= 8;
        ...
        [[gnu::noinline]] const char* find_last_symbols_neon_long_rt(const char* const begin, const char* pos, const char* symbols, size_t num_chars) {
            ...
            if (maybe_negate<positive>(is_in(pos[-7], symbols, num_chars)))
                return pos - 7;
            if (maybe_negate<positive>(is_in(pos[-8], symbols, num_chars)))
                return pos - 8;
            pos -= 8;
        ...
        {
            static_assert(0 <= sizeof...(symbols) && sizeof...(symbols) <= 16, "0 <= sizeof...(symbols) && sizeof...(symbols) <= 16");
#if defined(__SSE4_2__)
            if (sizeof...(symbols) >= 5)
