...
size_t encode_base64_impl(char* dst, const char* src, size_t srclen,
    ...
    const uint8x16x4_t table = vld1q_u8_x4((reinterpret_cast<const char*>(options & base64_url)
            ? tables::base64::base64_url::e1
            : tables::base64::base64_default::e1));
#else
    const uint8x16x4_t table = vld1q_u8_x4(reinterpret_cast<const unsigned char*>(
    ...
    for (; i + 16 * 3 <= srclen; i += 16 * 3) {
        ...
        result.val[1] = vandq_u8(vsliq_n_u8(vshrq_n_u8(in.val[1], 4), in.val[0], 4), v3f);
        result.val[2] = vandq_u8(vsliq_n_u8(vshrq_n_u8(in.val[2], 6), in.val[1], 2), v3f);
    ...
    if (i + 24 <= srclen) {
        ...
        result.val[1] = vand_u8(vsli_n_u8(vshr_n_u8(in.val[1], 4), in.val[0], 4), v3f_d);
        result.val[2] = vand_u8(vsli_n_u8(vshr_n_u8(in.val[2], 6), in.val[1], 2), v3f_d);
...
uint64_t to_base64_mask(block64* b, bool* error) {
    ...
    *error = checks == 0;
    if (checks <= 1) {
        ...
        uint8x16_t sum0 = vpaddq_u8(vandq_u8(test0, bit_mask), vandq_u8(test1, bit_mask));
        uint8x16_t sum1 = vpaddq_u8(vandq_u8(test2, bit_mask), vandq_u8(test3, bit_mask));
    ...
    if (default_or_url) {
        ...
        uint8x16_t delta_hash0 = vrhaddq_u8(vqtbl1q_u8(delta_asso, lo_nibbles0), hi_bits0);
        uint8x16_t delta_hash1 = vrhaddq_u8(vqtbl1q_u8(delta_asso, lo_nibbles1), hi_bits1);
        uint8x16_t delta_hash2 = vrhaddq_u8(vqtbl1q_u8(delta_asso, lo_nibbles2), hi_bits2);
        uint8x16_t delta_hash3 = vrhaddq_u8(vqtbl1q_u8(delta_asso, lo_nibbles3), hi_bits3);
...
static size_t compress_block_single(block64* b, uint64_t mask, char* output) {
    ...
    case 0b00: {
        ...
        const uint8x16_t v2 = vcgtq_s8(vreinterpretq_s8_u8(v1),
    ...
    case 0b01: {
        ...
        const uint8x16_t v0 = vmovq_n_u8((uint8_t)(pos - 1));
        const uint8x16_t v2 = vcgtq_s8(vreinterpretq_s8_u8(v1), vreinterpretq_s8_u8(v0));
