...
#define unicode_vec(x) vmovq_n_u16(static_cast<uint16_t>(x))
            ...
            const uint16x8_t s1 = vandq_u16(in, unicode_vec(0b0000111111000000));
            ...
            const uint16x8_t m0 = vbicq_u16(unicode_vec(0b0100000000000000), one_or_two_bytes_bytemask);
...
#endif
            const uint16x8_t combined = vorrq_u16(vandq_u16(one_byte_bytemask, onemask),
...
arm64_utf8_length_from_utf16_bytemask(const char16_t* in, size_t size) {
    ...
    for (; size - pos >= N; pos += N) {
        ...
        auto c0 = vminq_u8(vorrq_u8(vandq_u8(base_input.val[idx_lsb], vdupq_n_u8(0x80)),
                               base_input.val[idx]),
            one);
...
arm64_utf8_length_from_utf16_with_replacement(const char16_t* in, size_t size) {
    ...
    for (; size - pos >= N + 1; pos += N) {
        ...
        if (any_lane_set(vreinterpretq_u16_u8(is_surrogate)) || scalar::utf16::is_low_surrogate<big_endian>(in[pos + N])) {
            any_surrogates = true;
