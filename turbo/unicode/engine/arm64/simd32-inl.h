...
struct simd32<uint32_t> {
    ...
    KUMO_FORCE_INLINE simd32(const uint32x4_t v)
        : value(v) { }
    ...
    KUMO_FORCE_INLINE uint64_t sum() const { return vaddvq_u32(value); }
...
template <typename T>
KUMO_FORCE_INLINE simd32<T> operator|(const simd32<T> a,
    const simd32<T> b) {
    return vorrq_u32(a.value, b.value);
}

KUMO_FORCE_INLINE simd32<uint32_t> min(const simd32<uint32_t> a,
    const simd32<uint32_t> b) {
    return vminq_u32(a.value, b.value);
}

KUMO_FORCE_INLINE simd32<uint32_t> max(const simd32<uint32_t> a,
    const simd32<uint32_t> b) {
...
KUMO_FORCE_INLINE simd32<bool> operator!(const simd32<bool> v) {
...
KUMO_FORCE_INLINE simd32<uint32_t> select(const simd32<bool> cond,
