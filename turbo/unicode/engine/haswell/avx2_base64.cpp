...
public:
    ...
    KUMO_FORCE_INLINE block64(const char16_t* src) {
        ...
        const auto m4 = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(src + 48));
    ...
    KUMO_FORCE_INLINE uint32_t to_base64_mask(__m256i* src, uint32_t* error) {
        ...
        __m256i delta_hash = _mm256_avg_epu8(_mm256_shuffle_epi8(delta_asso, *src), shifted);
        if (default_or_url) {
            delta_hash = _mm256_and_si256(delta_hash, _mm256_set1_epi8(0xf));
        }
        const __m256i check_hash = _mm256_avg_epu8(_mm256_shuffle_epi8(check_asso, *src), shifted);
        const __m256i out = _mm256_adds_epi8(_mm256_shuffle_epi8(delta_values, delta_hash), *src);
        const __m256i chk = _mm256_adds_epi8(_mm256_shuffle_epi8(check_values, check_hash), *src);
        const int mask = _mm256_movemask_epi8(chk);
        if (!ignore_garbage && mask) {
            __m256i ascii_space = _mm256_cmpeq_epi8(_mm256_shuffle_epi8(ascii_space_tbl, *src), *src);
