...
avx2_convert_latin1_to_utf16(const char* latin1_input, size_t len,
    ...
    for (; i < rounded_len; i += 16) {
        ...
        const __m128i latin1 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(latin1_input + i));

        // Zero extend each byte in `in` to word
        __m256i utf16 = _mm256_cvtepu8_epi16(latin1);

        if (big_endian == Endian::big) {
            const __m128i swap128 = _mm_setr_epi8(1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10, 13, 12, 15, 14);
