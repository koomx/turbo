...
void utf16fix_block(char16_t* out, const char16_t* in) {
    ...
    lb_masked = _mm256_and_si256(lookback, _mm256_set1_epi16(swap_if_needed(0xfc00u)));
    block_masked = _mm256_and_si256(block, _mm256_set1_epi16(swap_if_needed(0xfc00u)));
    lb_is_high = _mm256_cmpeq_epi16(lb_masked, _mm256_set1_epi16(swap_if_needed(0xd800u)));
...
#if UNICODE_GCC9OROLDER
        // Old GCC versions are missing _mm256_zextsi128_si256, so we emulate it.
        __m128i tmp_legacygcc = _mm_bslli_si128(_mm256_extracti128_si256(lb_illseq, 1), 14);
        __m256i tmp_legacygcc256 = _mm256_set_m128i(_mm_setzero_si128(), tmp_legacygcc);
        lb_illseq_shifted = _mm256_or_si256(_mm256_bsrli_epi128(lb_illseq, 2), tmp_legacygcc256);
#else
        lb_illseq_shifted = _mm256_or_si256(_mm256_bsrli_epi128(lb_illseq, 2),
            _mm256_zextsi128_si256(_mm_bslli_si128(
                _mm256_extracti128_si256(lb_illseq, 1), 14)));
#endif // UNICODE_GCC9OROLDER
...
#if UNICODE_GCC10 || UNICODE_GCC9OROLDER
...
void utf16fix_block_sse(char16_t* out, const char16_t* in) {
    ...
    lb_is_high = _mm_cmpeq_epi16(lb_masked, _mm_set1_epi16(swap_if_needed(0xd800U)));
    block_is_low = _mm_cmpeq_epi16(block_masked, _mm_set1_epi16(swap_if_needed(0xdc00U)));
...
void utf16fix_sse(const char16_t* in, size_t n, char16_t* out) {
    ...
    out[0] = scalar::utf16::is_low_surrogate<big_endian>(in[0]) ? replacement : in[0];
