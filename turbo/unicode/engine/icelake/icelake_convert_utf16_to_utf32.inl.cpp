...
convert_utf16_to_utf32(const char16_t* buf, size_t len,
    ...
    while (end - buf >= 32) {
        ...
        if ((H | L)) {
            ...
            const __mmask32 V = (L ^ (carry | (H << 1))); // A high surrogate must be followed by low one
            ...
            if (V == 0) {
                ...
                */
                ...
                const __m512i added_second = _mm512_mask_add_epi32(aligned_second, (__mmask16)(H >> 16),
