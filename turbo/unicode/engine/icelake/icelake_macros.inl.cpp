...
/*
    This upcoming macro (UNICODE_ICELAKE_TRANSCODE16) takes 16 + 4 bytes (of a
...
#define UNICODE_ICELAKE_TRANSCODE16(LANE0, LANE1, MASKED)                                   \
    {                                                                                       \
        ...
        char_class = _mm512_ternarylogic_epi32(char_class, v_0000_000f, v_8080_8000, 0xea); \
                                                                                            \
        const int valid_count = static_cast<int>(popcount(leading_bytes));                \
...
#define UNICODE_ICELAKE_WRITE_UTF16_OR_UTF32(INPUT, VALID_COUNT, MASKED)           \
    {                                                                              \
        if (UTF32) {                                                               \
            if (MASKED) {                                                          \
...
#define UNICODE_ICELAKE_STORE_ASCII(UTF32, utf8, output)                  \
    if (UTF32) {                                                          \
