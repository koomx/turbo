...
KUMO_FORCE_INLINE size_t icelake_utf8_length_from_utf16(const char16_t* in,
    ...
    // N remaining.
    __mmask32 remaining_mask = 0xFFFFFFFFULL >> (32 - (size - pos)); // mask for the remaining char16 values
...
KUMO_FORCE_INLINE UnicodeResult icelake_utf8_length_from_utf16_with_replacement(
    ...
    constexpr uint32_t straddle_mask = match_system(big_endian) ? 0xfc00fc00 : 0x00fc00fc;
    constexpr uint32_t straddle_pair = match_system(big_endian) ? 0xdc00d800 : 0x00dc00d8;
    ...
    for (; pos < (size - 1) / (2 * N) * (2 * N); pos += 2 * N) {
        __m512i current1 = _mm512_loadu_si512(reinterpret_cast<const __m512i*>(in + pos));
        if constexpr (!match_system(big_endian)) {
            current1 = _mm512_shuffle_epi8(current1, byteflip);
        }
        __m512i current2 = _mm512_loadu_si512(reinterpret_cast<const __m512i*>(in + pos + N));
        if constexpr (!match_system(big_endian)) {
        ...
        __mmask32 is_surrogate2 = _mm512_cmpeq_epi16_mask(
            ...
            _mm512_set1_epi16(uint16_t(0xd800)));
        __mmask32 c01 = _mm512_test_epi16_mask(current1, _mm512_set1_epi16(uint16_t(0xff80)));
        ...
        if (_kor_mask32(is_surrogate1, is_surrogate2)) {
            ...
            __m512i lb_masked2 = _mm512_and_si512(current2, _mm512_set1_epi16(uint16_t(0xfc00)));
            ...
            matches += ((straddle1 & straddle_mask) == straddle_pair) + ((straddle2 & straddle_mask) == straddle_pair);
        }
    }
    if (pos + N + 1 <= size) {
        __m512i input = _mm512_loadu_si512(reinterpret_cast<const __m512i*>(in + pos));
    ...
    __m512i input = _mm512_maskz_loadu_epi16(remaining_mask, in + pos - overshoot);
    if constexpr (!match_system(big_endian)) {
        input = _mm512_shuffle_epi8(input, byteflip);
    }
