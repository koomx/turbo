...
size_t encode_base64(char* dst, const char* src, size_t srclen,
    ...
    for (; i + 100 <= srclen; i += 96) {
        __m128i in0_lo = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 0);
        __m128i in0_hi = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 1);
        __m128i in1_lo = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 2);
        __m128i in1_hi = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 3);
        __m128i in2_lo = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 4);
        __m128i in2_hi = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 5);
        __m128i in3_lo = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 6);
        __m128i in3_hi = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 7);
        ...
        __m256i input0_result = __lasx_xvbitsel_v(input0_shuf1, input0_shuf0, input0_mask);
        ...
        __m256i input1_result = __lasx_xvbitsel_v(input1_shuf1, input1_shuf0, input1_mask);
        ...
        __m256i input2_result = __lasx_xvbitsel_v(input2_shuf1, input2_shuf0, input2_mask);
        ...
        __m256i input3_result = __lasx_xvbitsel_v(input3_shuf1, input3_shuf0, input3_mask);
    ...
    for (; i + 28 <= srclen; i += 24) {
        ...
        __m128i in_hi = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 1);
        ...
        __m256i indices_result = __lasx_xvbitsel_v(indices_shuf1, indices_shuf0, indices_mask);
...
static inline uint32_t to_base64_mask(__m256i* src, bool* error) {
    __m256i ascii_space_tbl = ____m256i((__m128i)v16u8 { 0x20, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
        0x9, 0xa, 0x0, 0xc, 0xd, 0x0, 0x0 });
    // credit: aqrit
    __m256i delta_asso;
    if (default_or_url) {
        delta_asso = ____m256i((__m128i)v16u8 { 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x1, 0x0,
    ...
    if (mask) {
        ...
        ascii_space_mask = ascii_space_mask | (__lsx_vpickve2gr_hu(lasx_extracti128_hi(ascii_space_ltz), 0) << 16);
...
static inline void base64_decode(char* out, __m256i str) {
    ...
    __m256i t1 = __lasx_xvsrli_w(__lasx_xvand_v(str, lasx_splat_u32(0x003f0000)), 2);
...
compress_decode_base64(char* dst, const chartype* src, size_t srclen,
    ...
    char* end_of_safe_64byte_zone = (srclen + 3) / 4 * 3 >= 63 ? dst + (srclen + 3) / 4 * 3 - 63 : dst;
