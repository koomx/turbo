...
struct simd16<bool> : base16<bool> {
    ...
    template <unsigned N>
    KUMO_FORCE_INLINE simd16 byte_right_shift() const {
...
struct base16_numeric : base16<T> {
    ...
    static KUMO_FORCE_INLINE simd16<T> zero() { return __lasx_xvldi(0); }
    template <typename Pointer>
...
struct simd16<uint16_t> : base16_numeric<uint16_t> {
    ...
    KUMO_FORCE_INLINE uint64_t sum() const {
        ...
        return uint64_t(__lasx_xvpickve2gr_du(sum_u64, 0)) + uint64_t(__lasx_xvpickve2gr_du(sum_u64, 1)) + uint64_t(__lasx_xvpickve2gr_du(sum_u64, 2)) + uint64_t(__lasx_xvpickve2gr_du(sum_u64, 3));
    }
