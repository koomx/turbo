...
struct simd64<uint64_t> {
    ...
    // constructors
    simdutf_really_inline simd64(__m256i v)
        : value(v) { }
    ...
    simdutf_really_inline uint64_t sum() const {
        return uint64_t(__lasx_xvpickve2gr_du(value, 0)) + uint64_t(__lasx_xvpickve2gr_du(value, 1)) + uint64_t(__lasx_xvpickve2gr_du(value, 2)) + uint64_t(__lasx_xvpickve2gr_du(value, 3));
    }
