...
#include <turbo/unicode/engine/lsx/lsx_validate_utf16.cpp>
#include <turbo/unicode/engine/lsx/lsx_validate_utf32le.cpp>

#include <turbo/unicode/engine/lsx/lsx_convert_latin1_to_utf8.cpp>
#include <turbo/unicode/engine/lsx/lsx_convert_latin1_to_utf16.cpp>
#include <turbo/unicode/engine/lsx/lsx_convert_latin1_to_utf32.cpp>

#include <turbo/unicode/engine/lsx/lsx_convert_utf8_to_utf16.cpp>
#include <turbo/unicode/engine/lsx/lsx_convert_utf8_to_utf32.cpp>
#include <turbo/unicode/engine/lsx/lsx_convert_utf8_to_latin1.cpp>

#include <turbo/unicode/engine/lsx/lsx_convert_utf16_to_latin1.cpp>
#include <turbo/unicode/engine/lsx/lsx_convert_utf16_to_utf8.cpp>
#include <turbo/unicode/engine/lsx/lsx_convert_utf16_to_utf32.cpp>

#include <turbo/unicode/engine/lsx/lsx_convert_utf32_to_latin1.cpp>
#include <turbo/unicode/engine/lsx/lsx_convert_utf32_to_utf8.cpp>
#include <turbo/unicode/engine/lsx/lsx_convert_utf32_to_utf16.cpp>
#include <turbo/unicode/engine/lsx/lsx_base64.cpp>
...
namespace turbo {
    namespace UNICODE_IMPLEMENTATION {
         ...
         [[nodiscard]] int
        UnicodeImplementLsx::detect_encodings(const char* input,
         ...
         [[nodiscard]] bool
        UnicodeImplementLsx::validate_utf8(const char* buf, size_t len) const noexcept {
            return lsx::utf8_validation::generic_validate_utf8(buf, len);
        }

         [[nodiscard]] UnicodeResult UnicodeImplementLsx::validate_utf8_with_errors(
            const char* buf, size_t len) const noexcept {
            return lsx::utf8_validation::generic_validate_utf8_with_errors(buf, len);
        }

         [[nodiscard]] bool
        UnicodeImplementLsx::validate_ascii(const char* buf, size_t len) const noexcept {
            return lsx::ascii_validation::generic_validate_ascii(buf, len);
        }

         [[nodiscard]] UnicodeResult UnicodeImplementLsx::validate_ascii_with_errors(
            const char* buf, size_t len) const noexcept {
            return lsx::ascii_validation::generic_validate_ascii_with_errors(buf, len);
        }
         [[nodiscard]] bool
        UnicodeImplementLsx::validate_utf16le_as_ascii(const char16_t* buf,
         ...
         [[nodiscard]] bool
        UnicodeImplementLsx::validate_utf16be_as_ascii(const char16_t* buf,
            size_t len) const noexcept {
         ...
         [[nodiscard]] bool
        UnicodeImplementLsx::validate_utf16le(const char16_t* buf,
            size_t len) const noexcept {
         ...
         [[nodiscard]] bool
        UnicodeImplementLsx::validate_utf16be(const char16_t* buf,
            size_t len) const noexcept {
            if (KUMO_UNLIKELY(len == 0)) {
                // empty input is valid. protected the implementation from nullptr.
                return true;
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::validate_utf16le_with_errors(
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::validate_utf16be_with_errors(
            const char16_t* buf, size_t len) const noexcept {
            if (KUMO_UNLIKELY(len == 0)) {
                return UnicodeResult(UnicodeError::SUCCESS, 0);
            }
        ...
        void UnicodeImplementLsx::to_well_formed_utf16le(const char16_t* input, size_t len,
            char16_t* output) const noexcept {
            utf16::to_well_formed<Endian::little>(input, len, output);
        }

        void UnicodeImplementLsx::to_well_formed_utf16be(const char16_t* input, size_t len,
            char16_t* output) const noexcept {
            utf16::to_well_formed<Endian::big>(input, len, output);
        }

         [[nodiscard]] bool
        UnicodeImplementLsx::validate_utf32(const char32_t* buf, size_t len) const noexcept {
            if (KUMO_UNLIKELY(len == 0)) {
                // empty input is valid. protected the implementation from nullptr.
                return true;
            }
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::validate_utf32_with_errors(
            const char32_t* buf, size_t len) const noexcept {
            if (KUMO_UNLIKELY(len == 0)) {
                return UnicodeResult(UnicodeError::SUCCESS, 0);
            }
            UnicodeResult res = lsx_validate_utf32le_with_errors(buf, len);
            if (res.count != len) {
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_latin1_to_utf8(
            const char* buf, size_t len, char* utf8_output) const noexcept {
            std::pair<const char*, char*> ret = lsx_convert_latin1_to_utf8(buf, len, utf8_output);
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_latin1_to_utf16le(
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_latin1_to_utf16be(
            const char* buf, size_t len, char16_t* utf16_output) const noexcept {
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_latin1_to_utf32(
            const char* buf, size_t len, char32_t* utf32_output) const noexcept {
            std::pair<const char*, char32_t*> ret = lsx_convert_latin1_to_utf32(buf, len, utf32_output);
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf8_to_latin1(
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf8_to_latin1_with_errors(
            const char* buf, size_t len, char* latin1_output) const noexcept {
            utf8_to_latin1::validating_transcoder converter;
            return converter.convert_with_errors(buf, len, latin1_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf8_to_latin1(
            const char* buf, size_t len, char* latin1_output) const noexcept {
            return lsx::utf8_to_latin1::convert_valid(buf, len, latin1_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf8_to_utf16le(
            const char* buf, size_t len, char16_t* utf16_output) const noexcept {
            utf8_to_utf16::validating_transcoder converter;
            return converter.convert<Endian::little>(buf, len, utf16_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf8_to_utf16be(
            const char* buf, size_t len, char16_t* utf16_output) const noexcept {
            utf8_to_utf16::validating_transcoder converter;
            return converter.convert<Endian::big>(buf, len, utf16_output);
        }

         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf8_to_utf16le_with_errors(
            const char* buf, size_t len, char16_t* utf16_output) const noexcept {
            utf8_to_utf16::validating_transcoder converter;
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf8_to_utf16be_with_errors(
            const char* buf, size_t len, char16_t* utf16_output) const noexcept {
            utf8_to_utf16::validating_transcoder converter;
            return converter.convert_with_errors<Endian::big>(buf, len, utf16_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf8_to_utf16le(
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf8_to_utf16be(
            const char* input, size_t size, char16_t* utf16_output) const noexcept {
            return utf8_to_utf16::convert_valid<Endian::big>(input, size,
                utf16_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf8_to_utf32(
            const char* buf, size_t len, char32_t* utf32_output) const noexcept {
            utf8_to_utf32::validating_transcoder converter;
            return converter.convert(buf, len, utf32_output);
        }

         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf8_to_utf32_with_errors(
            const char* buf, size_t len, char32_t* utf32_output) const noexcept {
            utf8_to_utf32::validating_transcoder converter;
            return converter.convert_with_errors(buf, len, utf32_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf8_to_utf32(
            const char* input, size_t size, char32_t* utf32_output) const noexcept {
            return utf8_to_utf32::convert_valid(input, size, utf32_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf16le_to_latin1(
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf16be_to_latin1(
            const char16_t* buf, size_t len, char* latin1_output) const noexcept {
        ...
        UnicodeImplementLsx::convert_utf16le_to_latin1_with_errors(
            const char16_t* buf, size_t len, char* latin1_output) const noexcept {
         ...
         [[nodiscard]] UnicodeResult
        UnicodeImplementLsx::convert_utf16be_to_latin1_with_errors(
            const char16_t* buf, size_t len, char* latin1_output) const noexcept {
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf16be_to_latin1(
            const char16_t* buf, size_t len, char* latin1_output) const noexcept {
            // optimization opportunity: implement a custom function.
            return convert_utf16be_to_latin1(buf, len, latin1_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf16le_to_latin1(
            const char16_t* buf, size_t len, char* latin1_output) const noexcept {
            // optimization opportunity: implement a custom function.
            return convert_utf16le_to_latin1(buf, len, latin1_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf16le_to_utf8(
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf16be_to_utf8(
            const char16_t* buf, size_t len, char* utf8_output) const noexcept {
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf16le_to_utf8_with_errors(
            const char16_t* buf, size_t len, char* utf8_output) const noexcept {
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf16be_to_utf8_with_errors(
            const char16_t* buf, size_t len, char* utf8_output) const noexcept {
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf16le_to_utf8(
            const char16_t* buf, size_t len, char* utf8_output) const noexcept {
            return convert_utf16le_to_utf8(buf, len, utf8_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf16be_to_utf8(
            const char16_t* buf, size_t len, char* utf8_output) const noexcept {
            return convert_utf16be_to_utf8(buf, len, utf8_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf32_to_utf8(
            const char32_t* buf, size_t len, char* utf8_output) const noexcept {
            if (KUMO_UNLIKELY(len == 0)) {
                return 0;
            }
            std::pair<const char32_t*, char*> ret = lsx_convert_utf32_to_utf8(buf, len, utf8_output);
            if (ret.first == nullptr) {
                return 0;
            }
            size_t saved_bytes = ret.second - utf8_output;
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf32_to_utf8_with_errors(
            const char32_t* buf, size_t len, char* utf8_output) const noexcept {
            if (KUMO_UNLIKELY(len == 0)) {
                return UnicodeResult(UnicodeError::SUCCESS, 0);
            }
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
            std::pair<UnicodeResult, char*> ret = lsx_convert_utf32_to_utf8_with_errors(buf, len, utf8_output);
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf16le_to_utf32(
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf16be_to_utf32(
            const char16_t* buf, size_t len, char32_t* utf32_output) const noexcept {
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf16le_to_utf32_with_errors(
            const char16_t* buf, size_t len, char32_t* utf32_output) const noexcept {
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf16be_to_utf32_with_errors(
            const char16_t* buf, size_t len, char32_t* utf32_output) const noexcept {
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf32_to_latin1(
            const char32_t* buf, size_t len, char* latin1_output) const noexcept {
            std::pair<const char32_t*, char*> ret = lsx_convert_utf32_to_latin1(buf, len, latin1_output);
            if (ret.first == nullptr) {
                return 0;
            }
            size_t saved_bytes = ret.second - latin1_output;
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf32_to_latin1_with_errors(
            const char32_t* buf, size_t len, char* latin1_output) const noexcept {
            std::pair<UnicodeResult, char*> ret = lsx_convert_utf32_to_latin1_with_errors(buf, len, latin1_output);
            if (ret.first.error) {
                return ret.first;
            } // Can return directly since scalar fallback already found correct
              // ret.first.count
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf32_to_latin1(
            const char32_t* buf, size_t len, char* latin1_output) const noexcept {
            std::pair<const char32_t*, char*> ret = lsx_convert_utf32_to_latin1(buf, len, latin1_output);
            if (ret.first == nullptr) {
                return 0;
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf32_to_utf8(
            const char32_t* buf, size_t len, char* utf8_output) const noexcept {
            // optimization opportunity: implement a custom function.
            return convert_utf32_to_utf8(buf, len, utf8_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf32_to_utf16le(
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_utf32_to_utf16be(
            const char32_t* buf, size_t len, char16_t* utf16_output) const noexcept {
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf32_to_utf16le_with_errors(
            const char32_t* buf, size_t len, char16_t* utf16_output) const noexcept {
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementLsx::convert_utf32_to_utf16be_with_errors(
            const char32_t* buf, size_t len, char16_t* utf16_output) const noexcept {
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf32_to_utf16le(
            const char32_t* buf, size_t len, char16_t* utf16_output) const noexcept {
            return convert_utf32_to_utf16le(buf, len, utf16_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf32_to_utf16be(
            const char32_t* buf, size_t len, char16_t* utf16_output) const noexcept {
            return convert_utf32_to_utf16be(buf, len, utf16_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf16le_to_utf32(
            const char16_t* buf, size_t len, char32_t* utf32_output) const noexcept {
            return convert_utf16le_to_utf32(buf, len, utf32_output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::convert_valid_utf16be_to_utf32(
            const char16_t* buf, size_t len, char32_t* utf32_output) const noexcept {
            return convert_utf16be_to_utf32(buf, len, utf32_output);
        }

        void UnicodeImplementLsx::change_endianness_utf16(const char16_t* input,
            size_t length,
            char16_t* output) const noexcept {
            utf16::change_endianness_utf16(input, length, output);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::count_utf16le(
            const char16_t* input, size_t length) const noexcept {
            return utf16::count_code_points<Endian::little>(input, length);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::count_utf16be(
            const char16_t* input, size_t length) const noexcept {
            return utf16::count_code_points<Endian::big>(input, length);
        }

         [[nodiscard]] size_t
        UnicodeImplementLsx::count_utf8(const char* input, size_t length) const noexcept {
            return utf8::count_code_points(input, length);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::latin1_length_from_utf8(
            const char* buf, size_t len) const noexcept {
            return count_utf8(buf, len);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::utf8_length_from_latin1(
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::utf8_length_from_utf16le(
            const char16_t* input, size_t length) const noexcept {
            return utf16::utf8_length_from_utf16_bytemask<Endian::little>(input,
                length);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::utf8_length_from_utf16be(
            const char16_t* input, size_t length) const noexcept {
            return utf16::utf8_length_from_utf16_bytemask<Endian::big>(input, length);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::utf32_length_from_utf16le(
            const char16_t* input, size_t length) const noexcept {
            return utf16::utf32_length_from_utf16<Endian::little>(input, length);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::utf32_length_from_utf16be(
            const char16_t* input, size_t length) const noexcept {
            return utf16::utf32_length_from_utf16<Endian::big>(input, length);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::utf16_length_from_utf8(
            const char* input, size_t length) const noexcept {
            return utf8::utf16_length_from_utf8_bytemask(input, length);
        }
         [[nodiscard]] UnicodeResult
        UnicodeImplementLsx::utf8_length_from_utf16le_with_replacement(
            const char16_t* input, size_t length) const noexcept {
         ...
         [[nodiscard]] UnicodeResult
        UnicodeImplementLsx::utf8_length_from_utf16be_with_replacement(
            const char16_t* input, size_t length) const noexcept {
            return scalar::utf16::utf8_length_from_utf16_with_replacement<
         ...
         [[nodiscard]] size_t
        UnicodeImplementLsx::convert_utf16le_to_utf8_with_replacement(
         ...
         [[nodiscard]] size_t
        UnicodeImplementLsx::convert_utf16be_to_utf8_with_replacement(
            const char16_t* input, size_t length, char* utf8_buffer) const noexcept {
            return utf16_to_utf8::convert_with_replacement_via(
                [this](const char16_t* b, size_t l, char* o) {
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::utf8_length_from_utf32(
            const char32_t* input, size_t length) const noexcept {
            return utf32::utf8_length_from_utf32(input, length);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::utf16_length_from_utf32(
            const char32_t* input, size_t length) const noexcept {
         ...
         [[nodiscard]] size_t UnicodeImplementLsx::utf32_length_from_utf8(
            const char* input, size_t length) const noexcept {
            return utf8::count_code_points(input, length);
        }

         [[nodiscard]] UnicodeResult UnicodeImplementLsx::base64_to_binary(
         ...
         [[nodiscard]] full_result UnicodeImplementLsx::base64_to_binary_details(
            const char* input, size_t length, char* output, Base64Options options,
            last_chunk_handling_options last_chunk_options) const noexcept {
            if (options & base64_default_or_url) {
                if (options == Base64Options::base64_default_or_url_accept_garbage) {
        ...
        size_t UnicodeImplementLsx::binary_to_base64(const char* input, size_t length,
            char* output,
        ...
        size_t UnicodeImplementLsx::binary_to_base64_with_lines(
        ...
        const char* UnicodeImplementLsx::find(const char* start, const char* end,
            char character) const noexcept {
            return util_find(start, end, character);
        }

        const char16_t* UnicodeImplementLsx::find(const char16_t* start, const char16_t* end,
            char16_t character) const noexcept {
            return util_find(start, end, character);
        }

         [[nodiscard]] size_t UnicodeImplementLsx::binary_length_from_base64(
            const char* input, size_t length) const noexcept {
    ...
    static turbo::UnicodeImplement *get_lsx_instance() {
        static lsx::UnicodeImplementLsx ins;
        return &ins;
    }
} // namespace turbo
