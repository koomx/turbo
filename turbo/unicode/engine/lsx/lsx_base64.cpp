...
size_t encode_base64(char* dst, const char* src, size_t srclen,
    ...
    static const char* lookup_tbl = isbase64url
    ...
    for (; i + 52 <= srclen; i += 48) {
        __m128i in0 = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 0);
        __m128i in1 = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 1);
        __m128i in2 = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 2);
        __m128i in3 = __lsx_vld(reinterpret_cast<const __m128i*>(input + i), 4 * 3 * 3);
        ...
        __m128i input0_result = __lsx_vbitsel_v(input0_shuf1, input0_shuf0, input0_mask);
        ...
        __m128i input1_result = __lsx_vbitsel_v(input1_shuf1, input1_shuf0, input1_mask);
        ...
        __m128i input2_result = __lsx_vbitsel_v(input2_shuf1, input2_shuf0, input2_mask);
        ...
        __m128i input3_result = __lsx_vbitsel_v(input3_shuf1, input3_shuf0, input3_mask);
    ...
    for (; i + 16 <= srclen; i += 12) {
        ...
        __m128i indices_result = __lsx_vbitsel_v(indices_shuf1, indices_shuf0, indices_mask);

        __lsx_vst(indices_result, reinterpret_cast<__m128i*>(out), 0);
        out += 16;
    }

    return i / 3 * 4 + scalar::base64::tail_encode_base64((char*)out, src + i, srclen - i, options);
}
...
static inline void compress(__m128i data, uint16_t mask, char* output) {
    ...
    // at the end.
    __m128i compactmask = __lsx_vld(reinterpret_cast<const __m128i*>(
...
static inline uint16_t to_base64_mask(__m128i* src, bool* error) {
    ...
    */
    ...
    v16i8 delta_values;
    if (default_or_url) {
        delta_values = v16i8 { int8_t(0xBF), int8_t(0xE0), int8_t(0xB9), int8_t(0x13),
        ...
        delta_values = v16i8 { int8_t(0x00), int8_t(0x00), int8_t(0x00), int8_t(0x13),
            int8_t(0x04), int8_t(0xBF), int8_t(0xBF), int8_t(0xB9),
    ...
    v16i8 check_values;
    if (default_or_url) {
        check_values = v16i8 { int8_t(0x80), int8_t(0x80), int8_t(0x80), int8_t(0x80),
    ...
    const __m128i delta_hash = __lsx_vavgr_bu(__lsx_vshuf_b((__m128i)delta_asso, (__m128i)delta_asso,
                                                  (__m128i)asso_index),
        shifted);
    const __m128i check_hash = __lsx_vavgr_bu(__lsx_vshuf_b((__m128i)check_asso, (__m128i)check_asso,
                                                  (__m128i)asso_index),
        shifted);

    const __m128i out = __lsx_vsadd_b(__lsx_vshuf_b((__m128i)delta_values, (__m128i)delta_values,
                                          (__m128i)delta_hash),
        *src);
    const __m128i chk = __lsx_vsadd_b(__lsx_vshuf_b((__m128i)check_values, (__m128i)check_values,
                                          (__m128i)check_hash),
        *src);
    ...
    if (mask) {
        __m128i ascii_space = __lsx_vseq_b(__lsx_vshuf_b((__m128i)ascii_space_tbl,
                                               ...
                                               (__m128i)asso_index),
            *src);
        *error |= (mask != __lsx_vpickve2gr_hu(__lsx_vmskltz_b((__m128i)ascii_space), 0));
    }
...
static inline uint64_t to_base64_mask(block64* b, bool* error) {
    *error = 0;
    uint64_t m0 = to_base64_mask<base64_url, default_or_url>(&b->chunks[0], error);
    uint64_t m1 = to_base64_mask<base64_url, default_or_url>(&b->chunks[1], error);
    uint64_t m2 = to_base64_mask<base64_url, default_or_url>(&b->chunks[2], error);
    uint64_t m3 = to_base64_mask<base64_url, default_or_url>(&b->chunks[3], error);
...
static inline uint64_t compress_block(block64* b, uint64_t mask, char* output) {
    uint64_t nmask = ~mask;
    uint64_t count = __lsx_vpickve2gr_d(__lsx_vpcnt_h(__lsx_vreplgr2vr_d(nmask)), 0);
...
compress_decode_base64(char* dst, const char_type* src, size_t srclen,
    ...
    if (srclen >= 64) {
        ...
        while (src <= srcend64) {
            ...
            src += 64;
            bool error = false;
            uint64_t badcharmask = to_base64_mask<base64_url, default_or_url>(&b, &error);
            if (badcharmask) {
    ...
    if (src < srcend + equalsigns) {
        ...
        if (is_partial(last_chunk_options) && r.error == UnicodeError::SUCCESS && r.input_count < full_input_length) {
            ...
            if (r.input_count < full_input_length) {
                while (r.input_count > 0 && base64_ignorable(*(srcinit + r.input_count - 1), options)) {
                    r.input_count--;
                }
            }
        }
