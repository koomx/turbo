...
lsx_convert_utf16_to_utf8(const char16_t* buf, size_t len, char* utf8_out) {
    ...
    while (end - buf >= std::ptrdiff_t(16 + safety_margin)) {
        ...
        if (__lsx_bz_v(__lsx_vslt_hu(v_07ff, in))) {
            ...
            __m128i one_byte_bytemask = __lsx_vsle_hu(in, __lsx_vrepli_h(0x7F /*0x007F*/));
        ...
        if (__lsx_bz_v(surrogates_bytemask)) {
            ...
            __m128i one_or_two_bytes_bytemask_low = __lsx_vilvl_h(one_or_two_bytes_bytemask, zero);
            __m128i one_or_two_bytes_bytemask_high = __lsx_vilvh_h(one_or_two_bytes_bytemask, zero);

            __m128i one_byte_bytemask_low = __lsx_vilvl_h(one_byte_bytemask, one_byte_bytemask);
            __m128i one_byte_bytemask_high = __lsx_vilvh_h(one_byte_bytemask, one_byte_bytemask);
