...
size_t convert_masked_utf8_to_utf32(const char* input,
    ...
    if (idx < 64) {
        ...
        __m128i high = __lsx_vsrli_w(__lsx_vand_v(perm, __lsx_vldi(0xf)), 4); // 4 bits
        // Use 16 bit bic instead of and.
        // The top bits will be corrected later in the bsl
        // 00000000 10bbbbbb 00000000
        __m128i middle = __lsx_vand_v(perm, lsx_splat_u32(0x0000FF00)); // 5 or 6 bits
        ...
        if (input_utf8_end_of_code_point_mask == 0x888) {
            ...
            __m128i merge2 = __lsx_vbitsel_v(__lsx_vslli_w(merge1, 12), /* merge1 << 12 */
        ...
        __m128i corrected_srli2 = __lsx_vsrli_w(__lsx_vand_v(corrected, __lsx_vrepli_b(0x7)), 2);
        __m128i ab = __lsx_vbitsel_v(corrected_srli2, corrected, __lsx_vrepli_h(0x3f));
