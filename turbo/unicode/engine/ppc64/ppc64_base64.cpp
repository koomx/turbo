...
vector_u8 encoding_translate_6bit_values(const vector_u8 input) {
    ...
    const auto shift_LUT = base64_url ? vector_u8('a' - 26, '0' - 52, '0' - 52, '0' - 52, '0' - 52,
...
size_t encode_base64(char* dst, const char* src, size_t srclen,
    ...
    for (; i + 52 <= srclen; i += 48) {
        ...
        const auto base64_0 = encoding_translate_6bit_values<isbase64url>(expanded0);
        const auto base64_1 = encoding_translate_6bit_values<isbase64url>(expanded1);
        const auto base64_2 = encoding_translate_6bit_values<isbase64url>(expanded2);
        const auto base64_3 = encoding_translate_6bit_values<isbase64url>(expanded3);
...
static KUMO_FORCE_INLINE void compress(const vector_u8 data, uint16_t mask,
    ...
    // at the end.
    const auto compactmask = vector_u8::load(tables::base64::pshufb_combine_table + pop1 * 8);
...
static KUMO_FORCE_INLINE vector_u8 decoding_pack(vector_u8 input) {
    ...
    const auto shuffle = vector_u8(1, 2, 3, 5, 6, 7, 9, 10, 11, 13, 14, 15, 0, 0, 0, 0);
    ...
    const auto shuffle = vector_u8(2, 1, 0, 6, 5, 4, 10, 9, 8, 14, 13, 12, 0, 0, 0, 0);
...
public:
    ...
    static inline uint16_t to_base64_mask(vector_u8& src, uint16_t& error) {
        const auto ascii_space_tbl = vector_u8(0x20, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x9, 0xa, 0x0,
            0xc, 0xd, 0x0, 0x0);

        // credit: aqrit
        const auto delta_asso = default_or_url
        ...
        const auto delta_values = default_or_url
        ...
        const auto check_asso = default_or_url
        ...
        const auto check_values = default_or_url
