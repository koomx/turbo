...
namespace base64tests {
    const char* base64_std = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
base64_decoding_valid(const simdutf::implementation&) {
    ...
    for (uint8_t i = 0; i < 64; i++) {
        ...
        uint16_t error = 0;
        const auto mask = block64::to_base64_mask<with_base64_std, with_ignore_errors>(ascii,
            error);

        const auto err = (mask != 0) || (error != 0) || (ascii != vector_u8::splat(i)).any();
    ...
    for (uint8_t i = 0; i < 64; i++) {
        ...
        uint16_t error = 0;
        const auto mask = block64::to_base64_mask<with_base64_url, with_ignore_errors>(ascii,
            error);
...
static void unittest_decoding_invalid_ignore_errors(const char* base64) {
    ...
    for (size_t i = 0; i < 256; i++) {
        ...
        uint16_t error = 0;
        const auto mask = block64::to_base64_mask<base64_url, with_ignore_errors>(ascii, error);
...
static void unittest_decoding_invalid_strict_errors(const char* base64) {
    ...
    for (size_t i = 0; i < 256; i++) {
        ...
        auto ascii = vector_u8::splat(b);
        uint16_t error = 0;
        const auto mask = block64::to_base64_mask<base64_url, with_strict_checking>(ascii, error);
