...
KUMO_FORCE_INLINE void ppc64_convert_utf16_to_1_2_3_bytes_of_utf8(
...
#if KUMO_ENDIAN_BIG
    const auto dup_lsb = vector_u8(1, 1, 3, 3, 5, 5, 7, 7, 9, 9, 11, 11, 13, 13, 15, 15);
#else
    const auto dup_lsb = vector_u8(0, 0, 2, 2, 4, 4, 6, 6, 8, 8, 10, 10, 12, 12, 14, 14);
    ...
    */
    ...
    const auto m0 = ~as_vector_u16(one_or_two_bytes_bytemask) & uint16_t(0b0100000000000000);
    const auto s4 = s3 ^ m0;
    ...
    if (mask == 0) {
        ...
        const auto shuffle0 = vector_u8(1, 0, 16, 3, 2, 18, 5, 4, 20, 7, 6, 22, 9, 8, 24, 11);
        const auto shuffle1 = vector_u8(10, 26, 13, 12, 28, 15, 14, 30, -1, -1, -1,
            -1, -1, -1, -1, -1);
#else
        const auto shuffle0 = vector_u8(0, 1, 17, 2, 3, 19, 4, 5, 21, 6, 7, 23, 8, 9, 25, 10);
...
utf16_to_utf8_t ppc64_convert_utf16_to_utf8(const char16_t* buf, size_t len,
    ...
    while (end - buf >= std::ptrdiff_t(16 + safety_margin)) {
        ...
        const uint16_t one_or_two_bytes_bitmask = one_or_two_bytes_bytemask.to_bitmask();
