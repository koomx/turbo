...
namespace turbo {
    namespace UNICODE_IMPLEMENTATION {
        ...
        UnicodeImplementWestmere::validate_utf16le(const char16_t* buf,
            ...
            const auto res = westmere::utf16::validate_utf16_with_errors<Endian::little>(buf, len);
            if (res.is_err()) {
                return false;
            }
        ...
        UnicodeImplementWestmere::validate_utf16be(const char16_t* buf,
            ...
            const auto res = westmere::utf16::validate_utf16_with_errors<Endian::big>(buf, len);
            if (res.is_err()) {
                return false;
            }
        ...
        UnicodeImplementWestmere::convert_utf16be_to_latin1_with_errors(
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementWestmere::convert_utf16le_to_utf8_with_errors(
            ...
            std::pair<UnicodeResult, char*> ret = westmere::sse_convert_utf16_to_utf8_with_errors<Endian::little>(
                buf, len, utf8_output);
            if (ret.first.error) {
                return ret.first;
            } // Can return directly since scalar fallback already found correct
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementWestmere::convert_utf16be_to_utf8_with_errors(
            ...
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
            std::pair<UnicodeResult, char*> ret = westmere::sse_convert_utf16_to_utf8_with_errors<Endian::big>(
                buf, len, utf8_output);
            if (ret.first.error) {
                return ret.first;
            } // Can return directly since scalar fallback already found correct
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementWestmere::convert_utf16le_to_utf32_with_errors(
            ...
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
            std::pair<UnicodeResult, char32_t*> ret = westmere::sse_convert_utf16_to_utf32_with_errors<Endian::little>(
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementWestmere::convert_utf16be_to_utf32_with_errors(
            ...
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
            std::pair<UnicodeResult, char32_t*> ret = westmere::sse_convert_utf16_to_utf32_with_errors<Endian::big>(
                buf, len, utf32_output);
            if (ret.first.error) {
                return ret.first;
            } // Can return directly since scalar fallback already found correct
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementWestmere::convert_utf32_to_utf16le_with_errors(
            ...
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
            std::pair<UnicodeResult, char16_t*> ret = westmere::sse_convert_utf32_to_utf16_with_errors<Endian::little>(
         ...
         [[nodiscard]] UnicodeResult UnicodeImplementWestmere::convert_utf32_to_utf16be_with_errors(
            ...
            // ret.first.count is always the position in the buffer, not the number of
            // code units written even if finished
            std::pair<UnicodeResult, char16_t*> ret = westmere::sse_convert_utf32_to_utf16_with_errors<Endian::big>(
                buf, len, utf16_output);
            if (ret.first.count != len) {
        ...
        UnicodeImplementWestmere::convert_utf16be_to_utf8_with_replacement(
         ...
         [[nodiscard]] size_t UnicodeImplementWestmere::utf16_length_from_utf32(
            ...
            for (; pos + 4 <= length; pos += 4) {
                ...
                size_t surrogate_count = (16 - popcount(surrogate_bitmask)) / 4;
