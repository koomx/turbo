        ...
        inline void memcpy_atomic_read(char* dst, const char* src, size_t len) {
            ...
            auto bbb_memcpy_atomic_read = [](char* bytedst, const char* bytesrc,
                ...
                for (size_t j = 0; j < bytelen; ++j) {
                    bytedst[j] = std::atomic_ref<char>(mutable_src[j]).load(std::memory_order_relaxed);
                }
            };
            ...
            while (len >= alignment) {
                ...
                const auto dst_value = std::atomic_ref<uint64_t>(*src_aligned).load(std::memory_order_relaxed);
