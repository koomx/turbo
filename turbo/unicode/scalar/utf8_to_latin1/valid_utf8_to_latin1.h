#ifndef UNICODE_VALID_UTF8_TO_LATIN1_H
#define UNICODE_VALID_UTF8_TO_LATIN1_H
                    ...
                    while (pos < len) {
                        {
                            ...
                            if (pos + 16 <= len) { // if it is safe to read 16 more bytes, check that
                                ...
                                uint64_t v { v1 | v2 }; // We are only interested in these bits: 1000 1000 1000
                                                        // 1000, so it makes sense to concatenate everything
                                if ((v & 0x8080808080808080) == 0) { // if NONE of these are set, e.g. all of them are zero, then
                                                                     // everything is ASCII
                        ...
                        if (leading_byte < 0b10000000) {
                            ...
                            *latin_output++ = char(leading_byte);
                            ...
                            // range check -
                            uint32_t code_point = (leading_byte & 0b00011111) << 6 | (uint8_t(data[pos + 1]) & 0b00111111); // assembles the Unicode code point from the two bytes.
